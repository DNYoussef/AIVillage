...
def compute_bonferroni_correction(
    overall_error_rate: float, num_steps: int
) -> list[float]:
...
def compute_confidence_interval(
    observed_error: float, sample_size: int, confidence_level: float
) -> tuple:
    ...
    z_score = np.abs(
        np.percentile(
            np.random.standard_normal(10000), (1 - confidence_level) / 2 * 100
        )
    )
    margin_of_error = z_score * np.sqrt(
        (observed_error * (1 - observed_error)) / sample_size
    )
