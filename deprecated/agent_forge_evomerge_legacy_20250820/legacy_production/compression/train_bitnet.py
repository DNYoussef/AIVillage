...
class BitNetTrainer:
    ...
    def __init__(
        ...
        self.model = convert_to_bitnet(self.model, rmsnorm_post_attn=self.rmsnorm_post_attn)
    ...
    def train(
        ...
        callback = GradualBitnetCallback(total_steps=steps, warmup_ratio=self.lambda_warmup_frac)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train BitNet model with gradual λ schedule")
    parser.add_argument("--base_model", required=True, help="Path to base model directory")
