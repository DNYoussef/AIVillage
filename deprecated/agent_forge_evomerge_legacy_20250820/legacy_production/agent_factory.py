...
class AgentFactory:
                ...
                for template_file in search_dir.glob("*.json"):
                    ...
                    if agent_id not in loaded_templates:
                        try:
                            ...
                            msg = f"Loaded template {agent_id} from legacy location: " f"{template_file}"
                            print(msg)
    ...
    def _initialize_agent_classes(self) -> dict[str, type]:
        ...
        for agent_id, (module_path, class_name) in agent_specs.items():
            try:
                ...
                logger.warning("Falling back to GenericAgent for %s: %s", agent_id, e)
    ...
    def _validate_required_templates(self, templates: dict[str, dict[str, Any]]) -> None:
        ...
        msg = f"[PASS] All {len(required_types)} required agent templates " f"loaded successfully"
        print(msg)
    ...
    def create_agent(self, agent_spec: str | dict[str, Any], config: dict[str, Any] | None = None) -> BaseMetaAgent:
        ...
        if agent_type not in self.templates:
            msg = f"Unknown agent type: {agent_type}. Available: " f"{list(self.templates.keys())}"
        ...
        if config:
            ...
            if unknown:
                msg = f"Unknown config parameters for {agent_type}: " f"{sorted(unknown)}"
                raise ValueError(msg)
