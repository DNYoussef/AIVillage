...
class TestDatabaseLocking(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            from token_economy.credit_system import (
                EarningRule,
                SQLiteDatabase,
                VILLAGECreditSystem,
            )
    ...
    def test_sqlite_wal_mode_configured(self):
        ...
        try:
            ...
            self.assertGreaterEqual(
                timeout, 5000, "Busy timeout should be at least 5 seconds"
            )
    ...
    def test_concurrent_database_access(self):
        ...
        def worker_thread(thread_id: int, results: list):
            ...
            try:
                ...
                credit_system.earn_credits(
                    user_id, "TEST_ACTION", {"thread": str(thread_id)}
                )
                balance = credit_system.get_balance(user_id)

                results.append(
                    {"thread_id": thread_id, "balance": balance, "success": True}
                )
        ...
        try:
            ...
            if failed_threads:
                error_messages = [
                    r.get("error", "Unknown error") for r in failed_threads
                ]
    ...
    def test_database_recovery_from_busy_state(self):
        ...
        try:
            ...
            credit_system.earn_credits(
                "recovery_user", "RECOVERY_TEST", {"test": "recovery"}
            )
            end_time = time.time()
            ...
            self.assertLess(
                end_time - start_time, 10, "Recovery should not take too long"
            )
    ...
    def test_cross_platform_file_paths(self):
        ...
        for db_path in test_cases:
            try:
                ...
                credit_system.earn_credits(
                    "path_user", "CROSS_PLATFORM", {"path": db_path}
                )
    ...
    def test_portalocker_availability(self):
        ...
        try:
            ...
            try:
                ...
                self.assertTrue(
                    test_file.exists(), "Portalocker should create and lock file"
                )
            ...
            self.skipTest(
                "Portalocker not available - install with: pip install portalocker"
            )
            ...
            for credit_system, db_path in databases:
                ...
                if db_path.exists():
                    try:
                        ...
                        self.fail(
                            f"Database file {db_path} is still locked after close"
                        )
