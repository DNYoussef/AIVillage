...
bitnet = safe_import_and_test(
    "src.agent_forge.compression.bitnet", "BITNETCompressor", "BitNet"
)
if bitnet:
    try:
        ...
        packed_bytes = (
            len(compressed["packed_weights"]) if "packed_weights" in compressed else 0
        )
        ...
        if compressed_bytes > 0:
            ...
            print(
                f"[OK] BitNet compression: {original_bytes} -> {compressed_bytes} bytes ({ratio:.2f}x)"
            )
            results.append(
                ("BitNet", "tensor_50x50", original_bytes, compressed_bytes, ratio)
            )
...
seedlm = safe_import_and_test(
    "src.agent_forge.compression.seedlm", "SEEDLMCompressor", "SeedLM"
)
if seedlm:
    try:
        ...
        if compressed_bytes > 0:
            ratio = original_bytes / compressed_bytes
            print(
                f"[OK] SeedLM compression: {original_bytes} -> {compressed_bytes} bytes ({ratio:.2f}x)"
            )
            results.append(
                ("SeedLM", "tensor_24x24", original_bytes, compressed_bytes, ratio)
            )
...
vptq = safe_import_and_test(
    "src.agent_forge.compression.vptq", "VPTQCompressor", "VPTQ"
)
if vptq:
    try:
        ...
        if compressed_bytes > 0:
            ratio = original_bytes / compressed_bytes
            print(
                f"[OK] VPTQ compression: {original_bytes} -> {compressed_bytes} bytes ({ratio:.2f}x)"
            )
            results.append(
                ("VPTQ", "tensor_32x32", original_bytes, compressed_bytes, ratio)
            )
...
simple = safe_import_and_test(
    "src.core.compression.simple_quantizer", "SimpleQuantizer", "SimpleQuantizer"
)
if simple:
    try:
        ...
        if compressed_bytes > 0:
            ratio = original_bytes / compressed_bytes
            print(
                f"[OK] SimpleQuantizer: {original_bytes} -> {compressed_bytes} bytes ({ratio:.2f}x)"
            )
            results.append(
                (
                    "SimpleQuantizer",
...
if results:
    ...
    if bitnet_ratios:
        ...
        print(
            f"BitNet: {max_bitnet:.2f}x vs claimed ~16x - {'VALIDATED' if max_bitnet >= 10 else 'DISPUTED'}"
        )
    ...
    if seedlm_ratios:
        ...
        print(
            f"SeedLM: {max_seedlm:.2f}x vs claimed ~5x - {'VALIDATED' if max_seedlm >= 4 else 'DISPUTED'}"
        )
    ...
    if vptq_ratios:
        ...
        print(
            f"VPTQ: {max_vptq:.2f}x vs claimed 14-16x - {'VALIDATED' if max_vptq >= 10 else 'DISPUTED'}"
        )
    ...
    if simple_ratios:
        ...
        print(
            f"SimpleQuantizer: {max_simple:.2f}x vs claimed 4x - {'VALIDATED' if max_simple >= 3.5 else 'DISPUTED'}"
        )
