...
from .model_config import TaskType, MODEL_ROUTING_CONFIG, RATE_LIMITS
...
class OpenRouterClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        # Cost tracking
        self.total_cost = 0.0
        self.cost_by_task: Dict[TaskType, float] = {task: 0.0 for task in TaskType}
    ...
    async def complete(
        ...
        # All models failed
        raise Exception(f"All models failed for {task_type.value}. Last error: {last_error}")
    ...
    def _update_metrics(self, model: str, usage: Dict[str, int], cost: float, latency: float):
