...
from agent_forge.benchmark_suite import (
    BenchmarkConfig, ComprehensiveBenchmark, ComparisonReport
)
...
"""
    ...
    async def _generate_cross_stage_comparison(self, stage_results: Dict[str, Any]):
        ...
        successful_results = {
            stage: result for stage, result in stage_results.items()
...
"""
        ...
        for stage_name in successful_results.keys():
        ...
        for _ in successful_results.keys():
            report_content += "----------|"
        report_content += "\n"
        ...
        if len(successful_results) >= 2:
            ...
            for benchmark in benchmarks:
                ...
                if benchmark in successful_results[first_stage]['comparison_report'].target_results:
                    first_score = successful_results[first_stage]['comparison_report'].target_results[benchmark].overall_score

                if benchmark in successful_results[last_stage]['comparison_report'].target_results:
                    last_score = successful_results[last_stage]['comparison_report'].target_results[benchmark].overall_score

                if first_score is not None and last_score is not None:
                    improvement = ((last_score - first_score) / first_score) * 100
                    improvements.append(f"- **{benchmark}**: {improvement:+.1f}% ({first_score:.3f} → {last_score:.3f})")
