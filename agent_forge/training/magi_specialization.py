...
from ..geometry.snapshot import snapshot, GeomState
from .curriculum import CurriculumGenerator, CurriculumLevel, Question
from .sleep_and_dream import FrozenEncoder
...
class MagiConfig:
    ...
    specialization_areas: List[str] = field(default_factory=lambda: [
        "python_programming", "algorithm_design", "mathematical_proofs",
        "computational_complexity", "data_structures", "numerical_analysis"
    ])
    ...
    modification_safety_bounds: Dict[str, float] = field(default_factory=lambda: {
...
class GeometricSelfAwareness:
    ...
    def detect_grokking_signature(self, loss_history: List[float], 
                                 accuracy_history: List[float]) -> Optional[Dict[str, Any]]:
...
class MagiSpecializationPipeline:
    ...
    async def run_magi_specialization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.self_modification:
                modification_interface = self.self_modification.enable_self_modification(model)
            ...
            specialization_results = {
                "specialization_summary": {
                    ...
                    "self_modifications_applied": len(self.self_modification.modification_history) if self.self_modification else 0
                },
            ...
            logger.info("=" * 80)
            logger.info(f"Specialized Magi agent ready for deployment")
    ...
    async def execute_curriculum(self, model: nn.Module, tokenizer: Any, 
    ...
    async def execute_level(self, model: nn.Module, tokenizer: Any, 
                           questions: List[Question], level: int) -> Dict[str, Any]:
    ...
    async def final_evaluation(self, model: nn.Module, tokenizer: Any) -> Dict[str, Any]:
        ...
        for area in self.config.specialization_areas:
            ...
            test_questions = [
                self.question_generator._generate_single_question(area, 10)  # Max difficulty
                for _ in range(20)
            ]
        ...
        final_eval = {
            ...
            "specialization_level": "Expert" if overall_accuracy > 0.8 else "Advanced" if overall_accuracy > 0.6 else "Intermediate",
...
def main(config=None, output_dir='D:/AgentForge/magi_output', levels=10, 
         questions_per_level=1000, enable_self_mod=False):
