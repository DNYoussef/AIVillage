...
from .evaluation import evaluate_coding, evaluate_mathematics, evaluate_writing, evaluate_zero_shot_classification, evaluate_zero_shot_qa
...
def cross_domain_fine_tuning(model: torch.nn.Module, tokenizer: AutoTokenizer, target_domain: ModelDomain, train_data: List[str], num_epochs: int = 3):
    """
...
def cross_domain_merge_and_adapt(models: List[torch.nn.Module], tokenizers: List[AutoTokenizer], merge_settings: MergeSettings, target_domain: ModelDomain, train_data: List[str]) -> torch.nn.Module:
    # Detect domains of input models
    domains = [get_model_domain(model, tokenizer) for model, tokenizer in zip(models, tokenizers)]
