...
from .agent_evolution_engine import AgentEvolutionEngine, AgentKPIs, AgentGenome
from .evolution_dashboard import EvolutionDashboard, PerformanceAnalyzer
from .safe_code_modifier import SafeCodeModifier, CodeModification
...
class HealthMonitor:
    ...
    async def check_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_mods = len([mod for mod in self.orchestrator.code_modifier.modifications.values() 
                             if not mod.applied])
...
class EvolutionOrchestrator:
    ...
    async def apply_safe_modification(self, 
                                    agent_id: str,
        ...
        try:
            ...
            # Apply if safe
            if (modification.safety_score >= 0.8 and 
                test_results.get('success', False) and
                not self.config.safety_mode):
    ...
    async def _record_modification_outcome(self, 
    ...
    async def _cancel_pending_modifications(self):
        ...
        pending_mods = [mod for mod in self.code_modifier.modifications.values() 
                       if not mod.applied]
        ...
        async with orchestrator.orchestration_context():
            ...
            try:
                while True:
                    ...
                    logger.info(f"Status: Generation {status['orchestrator']['current_generation']}, "
