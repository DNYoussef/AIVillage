...
from agent_forge.benchmark_suite import BenchmarkConfig
...
class AgentForgeBenchmarkOrchestrator:
    ...
    async def run_comprehensive_evaluation(
        ...
        available_models = {
            name: path for name, path in self.model_paths.items()
    ...
    async def _generate_comprehensive_comparison(self, evaluation_results: Dict[str, Any]):
        ...
        for model_name in successful_results.keys():
            row = [model_name, model_averages[model_name], int(model_rankings[model_name])]
    ...
    async def _create_executive_summary(self, evaluation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'recommendations': [
                ...
                f"Average performance across pipeline: {sum(all_scores) / len(all_scores):.3f}" if all_scores else "No valid scores",
...
Examples:
  ...
  python run_agent_forge_benchmark.py --skip-frontier
    ...
    # Model validation
    parser.add_argument("--validate-only", action="store_true", help="Only validate models, don't run benchmarks")
    ...
    try:
        ...
        if results['status'] == 'completed':
            ...
            print(f"📊 W&B Dashboard: https://wandb.ai/agent-forge/agent-forge-comprehensive-benchmark")
