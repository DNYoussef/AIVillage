...
class LintingOrchestrator:
    ...
    def detect_available_tools(self) -> dict[str, bool]:
        ...
        # Test Ruff (CLI)
        try:
            result = subprocess.run(["ruff", "--version"], check=False, capture_output=True, text=True, timeout=10)
        ...
        try:
            result = subprocess.run([sys.executable, "-c", "import black; print('ok')"],
        ...
        # Test isort
        try:
            result = subprocess.run([sys.executable, "-c", "import isort; print('ok')"],
                                  check=False, capture_output=True, text=True, timeout=10)
        ...
        # Test mypy
        try:
            result = subprocess.run([sys.executable, "-c", "import mypy; print('ok')"],
                                  check=False, capture_output=True, text=True, timeout=10)
        ...
        # Test flake8
        try:
            result = subprocess.run([sys.executable, "-c", "import flake8; print('ok')"],
                                  check=False, capture_output=True, text=True, timeout=10)
    ...
    def run_ruff(self, files: list[Path] = None, fix: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            issues_count = len([line for line in result.stdout.splitlines() if line.strip() and not line.startswith("warning:")]) if result.stdout else 0
    ...
    def run_black(self, files: list[Path] = None, check_only: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "duration": time.time() - start_time,
                "files_processed": len(files) if files else "all",
                "issues_found": len([line for line in result.stdout.splitlines() if "would reformat" in line or "error:" in line.lower()])
            }
        except subprocess.TimeoutExpired:
            return {"status": "timeout", "duration": time.time() - start_time}
        except Exception as e:
...
def main():
    ...
    parser.add_argument("--output", choices=["json", "summary", "both"], default="summary",
                       help="Output format")
