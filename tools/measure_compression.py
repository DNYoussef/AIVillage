...
class CompressionMeasurer:
    ...
    def calculate_compression_ratio(
        ...
        if original_metrics["disk_size_mb"] > 0:
            ratios["disk_compression_ratio"] = (
                original_metrics["disk_size_mb"] / compressed_metrics["disk_size_mb"]
            )
        else:
        ...
        if original_metrics["memory_size_mb"] > 0:
            ratios["memory_compression_ratio"] = (
                original_metrics["memory_size_mb"]
                / compressed_metrics["memory_size_mb"]
            )
        else:
        ...
        if original_metrics["total_parameters"] > 0:
            ratios["parameter_compression_ratio"] = (
                original_metrics["total_parameters"]
                / compressed_metrics["total_parameters"]
            )
        else:
    ...
    def comprehensive_measurement(
        self, model_path: str, reference_path: str | None = None
    ) -> dict:
        ...
        if reference_path:
            ...
            comparison_metrics = self.calculate_compression_ratio(
                reference_path, model_path
            )
            print(
                f"Compression ratio: {comparison_metrics.get('disk_compression_ratio', 1.0):.2f}x"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Measure compression metrics for models"
    )
