...
class TLSFingerprint:
    ...
    def calculate_ja4(self) -> str:
        ...
        cipher_hash = hashlib.sha256(
            ",".join(str(c) for c in sorted(self.cipher_suites)).encode()
        ).hexdigest()[:12]

        ext_hash = hashlib.sha256(
            ",".join(str(e) for e in sorted(self.extensions)).encode()
        ).hexdigest()[:12]

        ja4_string = (
            f"t{tls_str}{sni}{num_ciphers}{num_ext}{alpn}_{cipher_hash}_{ext_hash}"
        )
        return ja4_string
...
class TLSCalibrator:
    ...
    async def compare_fingerprints(
        self, fp1: TLSFingerprint, fp2: TLSFingerprint
    ) -> dict:
        """Compare two fingerprints and calculate similarity."""
        comparison = {
            "ja3_match": fp1.ja3_hash == fp2.ja3_hash,
            "ja4_match": fp1.ja4_hash == fp2.ja4_hash,
            "cipher_similarity": self._calculate_similarity(
                fp1.cipher_suites, fp2.cipher_suites
            ),
            "extension_similarity": self._calculate_similarity(
                fp1.extensions, fp2.extensions
            ),
            "alpn_match": fp1.alpn_protocols == fp2.alpn_protocols,
            "differences": {
                "ciphers_added": list(set(fp2.cipher_suites) - set(fp1.cipher_suites)),
                "ciphers_removed": list(
                    set(fp1.cipher_suites) - set(fp2.cipher_suites)
                ),
