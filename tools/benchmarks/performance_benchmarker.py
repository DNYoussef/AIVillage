...
try:
    from packages.agents.core.base import BaseAgent, SimpleAgent, AgentMessage, AgentResponse
...
class RealAgentForgeBenchmark:
    ...
    async def benchmark_concurrent_agents(self, concurrent_agents: int = 10, messages_per_agent: int = 50) -> PerformanceMetrics:
...
class RealGatewayBenchmark:
    ...
    def benchmark_concurrent_requests(self, concurrent_users: int = 20, requests_per_user: int = 10) -> PerformanceMetrics:
        """Benchmark concurrent HTTP requests to gateway."""
        logger.info(f"Benchmarking concurrent gateway requests ({concurrent_users} users, {requests_per_user} requests each)")
