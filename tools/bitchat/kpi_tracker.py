...
class BitChatKPITracker:
    ...
    def calculate_network_density(self) -> float:
        ...
        if row and row["avg_messages"]:
            ...
            return (
                estimated_connections / max_connections if max_connections > 0 else 0.0
            )
        return 0.0
    ...
    def _generate_recommendations(self, snapshot: NetworkSnapshot) -> list[str]:
        ...
        if snapshot.delivery_rate < 0.9:
            recommendations.append(
                "Low delivery rate: Consider increasing TTL or improving store-and-forward"
            )

        if snapshot.avg_latency_ms > 150:
            recommendations.append(
                "High latency: Optimize routing algorithms or reduce hop counts"
            )

        if snapshot.active_peers < 10:
            recommendations.append(
                "Low peer count: Improve discovery mechanisms or incentivize participation"
            )

        if snapshot.network_density < 0.3:
            recommendations.append(
                "Low network density: Encourage more peer connections"
            )

        if snapshot.battery_impact and snapshot.battery_impact < 0.3:
            recommendations.append(
                "High battery drain: Optimize power management and duty cycling"
            )
...
def simulate_test_data(tracker: BitChatKPITracker):
    ...
    statuses = (
        ...
        + [MessageStatus.DROPPED] * 1
        + [MessageStatus.STORED] * 1
    )
    for _ in range(100):
