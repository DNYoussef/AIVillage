...
class DTNHandlerService(IDTNHandlerService):
    ...
    async def store_message(
        ...
        try:
            ...
            # Emit storage event
            self._emit_dtn_event("message_stored", {
                "message_id": message_id,
                "destination": destination,
    ...
    async def _free_storage_space(self, required_bytes: int) -> bool:
        ...
        sorted_messages = sorted(
            ...
            key=lambda m: (m.context.priority, m.created_at)
        )
    ...
    async def forward_stored_messages(self) -> Dict[str, int]:
        ...
        if forwarded > 0 or failed > 0:
            self._emit_dtn_event("forwarding_completed", {
                "forwarded": forwarded,
                "failed": failed,
                "skipped": skipped,
    ...
    async def _mark_message_delivered(self, message: StoredMessage) -> None:
        ...
        self.storage_metrics["avg_storage_time"] = (
            (current_avg * (delivered_count - 1) + storage_time) / delivered_count
        )
    ...
    async def _cleanup_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                expired_messages = [
                    msg_id for msg_id, msg in self.stored_messages.items()
                    if msg.is_expired()
                ]
    ...
    def _emit_dtn_event(self, event_type: str, data: Dict[str, Any]) -> None:
        ...
        event = RoutingEvent(
            ...
            timestamp=time.time(),
            source_service="DTNHandlerService",
            data=data
        )
        ...
        return {
            **self.storage_metrics,
            ...
            "recent_deliveries": len([
                ...
                if time.time() - d["delivered_at"] < 3600
            ])  # Last hour
        }
    ...
    def update_peer_connectivity(self, peer_id: str, connected: bool) -> None:
        ...
        self.peer_connectivity_history[peer_id] = [
            t for t in self.peer_connectivity_history[peer_id]
            if abs(t) > cutoff_time
        ]
    ...
    def set_forwarding_strategy(self, strategy: ForwardingStrategy) -> None:
        ...
        self._emit_dtn_event("forwarding_strategy_changed", {
