...
from ..path_policy import PathProtocol
...
class ProtocolManagerService(IProtocolManagerService):
    ...
    def __init__(self):
        ...
        self.connection_pools: Dict[PathProtocol, Set[str]] = {protocol: set() for protocol in PathProtocol}
    ...
    async def switch_protocol(self, from_protocol: PathProtocol, to_protocol: PathProtocol, destination: str) -> bool:
    ...
    async def _establish_new_connection(self, protocol: PathProtocol, destination: str) -> bool:
    ...
    async def _establish_protocol_connection(self, protocol: PathProtocol, destination: str) -> bool:
    ...
    async def manage_connections(self) -> Dict[str, Any]:
        ...
        connection_stats = {
            ...
            "connections_by_protocol": {
                protocol.value: len(connections) for protocol, connections in self.connection_pools.items()
            },
    ...
    async def handle_protocol_fallbacks(self, failed_protocol: PathProtocol, destination: str) -> PathProtocol:
    ...
    async def _can_establish_protocol(self, protocol: PathProtocol, destination: str) -> bool:
    ...
    async def _trigger_fallback(self, connection: ProtocolConnection) -> None:
        ...
        fallback_protocol = await self.handle_protocol_fallbacks(connection.protocol, connection.destination)
    ...
    def _track_switch_performance(
        self, from_protocol: PathProtocol, to_protocol: PathProtocol, switch_time_ms: float, success: bool
    ) -> None:
        ...
        self.switch_success_rates[switch_key] = alpha * (1.0 if success else 0.0) + (1 - alpha) * current_rate
    ...
    def _emit_protocol_event(self, event_type: str, data: Dict[str, Any]) -> None:
        ...
        event = RoutingEvent(
            event_type=event_type, timestamp=time.time(), source_service="ProtocolManagerService", data=data
        )
    ...
    def get_switch_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "switches_within_target": {
                ...
                for key, times in self.switch_times.items()
                if times
            },
        }
    ...
    def get_connection_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connection_states": self._get_connection_state_counts(),
            "active_destinations": len(set(conn.destination for conn in self.active_connections.values())),
        }
