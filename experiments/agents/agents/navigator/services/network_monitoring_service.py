...
class NetworkMonitoringService(INetworkMonitoringService):
    ...
    async def monitor_network_links(self) -> NetworkConditions:
        ...
        self.quality_metrics[PathProtocol.BETANET] = wifi_quality if self.current_conditions.wifi_connected else cellular_quality
            ...
            for server in ["8.8.8.8", "1.1.1.1"]:
                try:
                    _, writer = await asyncio.wait_for(
                        asyncio.open_connection(server, 53), timeout=2.0
                    )
                    writer.close()
    ...
    async def _measure_latency(self, target: str) -> float:
        ...
        try:
            start_time = time.time()
            _, writer = await asyncio.wait_for(
                asyncio.open_connection(target, 53), timeout=2.0
            )
    ...
    async def _measure_packet_loss(self, target: str) -> float:
        ...
        try:
            ...
            for _ in range(total_attempts):
                try:
                    _, writer = await asyncio.wait_for(
                        asyncio.open_connection(target, 53), timeout=1.0
                    )
                    writer.close()
                    await writer.wait_closed()
                    success_count += 1
    ...
    def _calculate_overall_reliability(
        ...
        if bluetooth_quality.availability > 0:
            bluetooth_score = (
                bluetooth_quality.availability * 0.6 +
                (1.0 - min(1.0, bluetooth_quality.packet_loss_rate)) * 0.4
            )
            reliability += weights["bluetooth"] * bluetooth_score
        
        return max(0.0, min(1.0, reliability))
    
    async def _detect_and_process_changes(
        self,
    ...
    async def _process_significant_change(self, change: LinkChangeEvent) -> None:
        ...
        # Emit change event
        self._emit_monitoring_event("significant_change_detected", {
    ...
    async def detect_link_changes(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        recent_changes = [
            ...
            if current_time - change.timestamp < 10.0  # Last 10 seconds
        ]
    ...
    def _emit_monitoring_event(self, event_type: str, data: Dict[str, Any]) -> None:
        ...
        event = RoutingEvent(
            ...
            timestamp=time.time(),
            source_service="NetworkMonitoringService",
            data=data
        )
