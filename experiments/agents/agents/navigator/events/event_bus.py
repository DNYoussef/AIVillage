...
class EventBusService:
    ...
    def unsubscribe(self, subscription_id: str) -> bool:
        ...
        logger.info(
            f"Service '{subscription.service_name}' unsubscribed from "
            f"'{subscription.event_type}' events"
        )
        return True
        ...
        for sub_id in subscription_ids:
            if sub_id in self.subscriptions and self.subscriptions[sub_id].active:
                task = asyncio.create_task(
                    self._deliver_to_subscriber(event, self.subscriptions[sub_id])
                )
    ...
    async def _deliver_to_subscriber(
        self, event: RoutingEvent, subscription: Subscription
    ) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to deliver event '{event.event_type}' to "
                f"service '{subscription.service_name}': {e}"
            )
