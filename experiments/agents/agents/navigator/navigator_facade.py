...
from .path_policy import EnergyMode, MessageContext, NetworkConditions, PathProtocol, PeerInfo, RoutingPriority, Receipt
...
class NavigatorFacade(INavigatorFacadeService):
    ...
    def _setup_event_subscriptions(self) -> None:
        ...
        self.event_bus.subscribe("route_selected", self._handle_route_selected, "NavigatorFacade")

        # Network change events
        self.event_bus.subscribe("significant_change_detected", self._handle_network_change, "NavigatorFacade")

        # Protocol events
        self.event_bus.subscribe("protocol_switched", self._handle_protocol_switch, "NavigatorFacade")

        # Energy events
        self.event_bus.subscribe("power_management_update", self._handle_power_update, "NavigatorFacade")

        # Security events
        self.event_bus.subscribe("circuit_created", self._handle_circuit_created, "NavigatorFacade")
    ...
    async def select_path(
        ...
        try:
            ...
            selected_protocol, path_scores = await self.route_selection.select_optimal_route(
                destination, message_context, available_protocols, network_conditions
            )

            # Step 4: Configure QoS parameters
            qos_config = await self.qos_manager.manage_qos_parameters(selected_protocol, message_context)

            # Step 5: Apply privacy/security measures if required
            privacy_config = {}
            if message_context.privacy_required:
                privacy_config = self.security_mixnode.ensure_routing_privacy(selected_protocol, message_context)

                # Get mixnodes if needed
                if privacy_config.get("mixnode_hops", 0) > 0:
                    mixnodes = await self.security_mixnode.select_privacy_mixnodes(
                        destination, 0.8  # Default privacy level
                    )
            ...
            if selected_protocol == PathProtocol.STORE_FORWARD:
                ...
                if not message_stored:
                    ...
                    fallback_protocol = await self.protocol_manager.handle_protocol_fallbacks(
                        selected_protocol, destination
                    )
                    selected_protocol = fallback_protocol

            # Step 7: Establish protocol connection if needed
            await self._ensure_protocol_connection(selected_protocol, destination)
            ...
            receipt = self._create_receipt(selected_protocol, execution_time_ms, path_scores, network_conditions)
    ...
    async def _ensure_protocol_connection(self, protocol: PathProtocol, destination: str) -> bool:
        ...
        try:
            ...
            self.protocol_manager.get_connection_status()
    ...
    def _generate_routing_metadata(
        ...
        if protocol == PathProtocol.BITCHAT:
            metadata.update({"max_hops": 7, "mesh_routing": True, "energy_efficient": True, "offline_capable": True})
            ...
            metadata.update({"multipath": True, "path_aware": True, "high_performance": True, "failover_support": True})
        elif protocol == PathProtocol.STORE_FORWARD:
            metadata.update({"delay_tolerant": True, "guaranteed_delivery": True, "energy_minimal": True})
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        system_status = {"facade": self.get_status(), "services": {}}
    ...
    def update_routing_success(self, protocol: str, destination: str, success: bool) -> None:
        ...
        self.route_selection.route_success_rates[f"{protocol}_{destination}"] = 0.9 if success else 0.1
