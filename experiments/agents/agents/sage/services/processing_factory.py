...
class ProcessingChainFactory(AbstractServiceBase, IProcessingService):
    ...
    def __init__(
        ...
        self._processing_semaphore = asyncio.Semaphore(config.max_concurrent_tasks if config.parallel_processing else 1)
    ...
    def query_processor(self) -> QueryProcessor:
        ...
        if self._query_processor is None and self.processing_config.query_processor_enabled:
            self._query_processor = QueryProcessor(self.rag_system, self.latent_space_activation, self.cognitive_nexus)
            ...
            try:
                ...
                if self.response_generator and "rag_result" in result:
                    response = await self.response_generator.generate_response(
                        query, result["rag_result"], result["interpreted_intent"]
                    )
    ...
    async def _pre_process_query(self, query: str, context: Optional[Dict[str, Any]]) -> str:
    ...
    async def _post_process_result(self, result: Dict[str, Any], original_query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.confidence_estimator and "rag_result" in result:
                confidence = await self.confidence_estimator.estimate(original_query, result["rag_result"])
    ...
    def _update_processing_stats(self, operation_type: str, processing_time: float, success: bool) -> None:
        ...
        total_ops = self._processing_stats["total_queries"] + self._processing_stats["total_tasks"]
