...
class SageAgentServiceLocator:
    ...
    def _initialize_locks(self) -> None:
        ...
        service_names = [
            "cognitive_composite", "processing_chain", "rag_system",
            "vector_store", "knowledge_tracker", "error_controller",
            "confidence_estimator", "collaboration_manager", 
            "research_capabilities", "user_intent_interpreter"
        ]
            ...
            if service_name in self._services:
                ...
                if service_type and not isinstance(service, service_type):
                    raise ServiceNotFoundError(
                        f"Service {service_name} exists but is not of type {service_type}"
                    )
