...
class SageAgent(UnifiedBaseAgent):
    ...
    def _setup_services(self, config: UnifiedConfig, knowledge_tracker: Optional[UnifiedKnowledgeTracker]) -> None:
        ...
        self.services.register_service_factory("rag_system", lambda: create_rag_system(config, knowledge_tracker))

        self.services.register_service_factory("cognitive_nexus", create_cognitive_nexus)

        self.services.register_service_factory("latent_space_activation", create_latent_space_activation)

        self.services.register_service_factory("error_controller", create_error_controller)

        self.services.register_service_factory("confidence_estimator", create_confidence_estimator)

        self.services.register_service_factory("user_intent_interpreter", create_user_intent_interpreter)

        # Register composite services
        self.services.register_service_factory(
            "cognitive_composite", lambda: create_cognitive_composite(self.vector_store, self.sage_config.cognitive)
        )
        ...
        self.services.register_service_factory("processing_chain", create_processing_chain_with_deps)

        # Collaboration manager
        self.services.register_service_factory(
            "collaboration_manager", lambda: create_collaboration_manager(self, self.sage_config.collaboration)
        )

        # Research capabilities
        self.services.register_service_factory(
            "research_capabilities", lambda: create_research_capabilities(self, self.sage_config.research)
        )
        ...
        self.services.register_service_factory("exploration_mode", create_exploration_mode)
        ...
        return {
            **base_info,
            ...
            "service_architecture": {
                ...
                "instantiated_services": len(
                    [s for s in self.services.get_registered_services() if self.services.is_service_instantiated(s)]
                ),
