...
class NetworkFaultInjector:
    ...
    def inject_delay(self, added_delay_ms: float, duration_seconds: float = 10.0):
        ...
        logger.info(
            f"Injected {added_delay_ms}ms additional delay for {duration_seconds}s"
        )
    ...
    async def _remove_delay_after(self, seconds: float):
        ...
        self.added_delay_ms = 0.0
        self.active_faults = [
            f for f in self.active_faults if not f.startswith("delay_")
        ]
    ...
    async def _remove_jitter_after(self, seconds: float):
        ...
        self.jitter_ms = 0.0
        self.active_faults = [
            f for f in self.active_faults if not f.startswith("jitter_")
        ]
    ...
    async def _remove_loss_after(self, seconds: float):
        ...
        self.packet_loss_rate = 0.0
        self.active_faults = [
            f for f in self.active_faults if not f.startswith("loss_")
        ]
        logger.info("Loss fault removed")

    def simulate_message_send(
        self, peer_id: str, message_id: str
    ) -> tuple[float, bool]:
        ...
        jitter = (
            random.uniform(-self.jitter_ms, self.jitter_ms) if self.jitter_ms > 0 else 0
        )
    ...
    def get_fault_summary(self) -> dict[str, any]:
        ...
        return {
            ...
            "fault_duration_seconds": time.time() - self.fault_start_time
...
class MockPingSender:
    ...
    async def send_ping(self, peer_id: str, payload: bytes) -> bool:
        ...
        self.call_count += 1
        _, success = self.fault_injector.simulate_message_send(
            peer_id, f"ping_{self.call_count}"
        )

        # Simulate network delay
        rtt_ms, _ = self.fault_injector.simulate_message_send(
            peer_id, f"ping_{self.call_count}"
        )
...
class TestNetworkFaultInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_delay_injection_triggers_protocol_switch(self):
        ...
        context = MessageContext(
            recipient=self.test_peer, payload_size=1024, priority=5
        )
        ...
        self.assertNotEqual(
            initial_protocol,
            new_protocol,
            "Protocol should have switched due to high delay",
        )
        self.assertIn(
            new_protocol,
            ["betanet", "bitchat"],
            "Should switch to more robust protocol for high delay",
        )

        logger.info(
            f"✅ Protocol switched {initial_protocol} → {new_protocol} in {decision_time_ms:.1f}ms"
        )
    ...
    async def test_packet_loss_triggers_reliability_preference(self):
        ...
        # Inject packet loss
        self.fault_injector.inject_packet_loss(
            loss_rate=0.15, duration_seconds=5.0
        )  # 15% loss
        ...
        self.assertIsNotNone(
            conditions.measured_loss_rate, "Should have loss rate measurement"
        )
        self.assertGreater(
            conditions.measured_loss_rate, 0.1, "Should measure significant loss"
        )
        self.assertTrue(
            conditions.needs_path_switch(), "Should trigger path switch due to loss"
        )
        self.assertIn(
            protocol,
            ["betanet", "bitchat"],
            "Should prefer reliable protocols under loss",
        )

        logger.info(
            f"✅ Selected {protocol} for {conditions.measured_loss_rate * 100:.1f}% loss rate"
        )
    ...
    async def test_jitter_affects_chunk_sizing(self):
        ...
        await self._establish_baseline_measurements()
        initial_chunk_size = self.navigator.get_adaptive_chunk_size(
            self.test_peer, 4096
        )
        ...
        self.assertGreater(
            conditions.measured_jitter_ms, 100, "Should measure high jitter"
        )
        self.assertLess(
            new_chunk_size,
            initial_chunk_size,
            "Chunk size should decrease with high jitter",
        )
        self.assertLessEqual(
            new_chunk_size, 1024, "High jitter should trigger small chunks"
        )

        logger.info(
            f"✅ Chunk size: {initial_chunk_size} → {new_chunk_size} bytes due to {conditions.measured_jitter_ms:.1f}ms jitter"
        )
    ...
    async def test_fault_recovery_restores_optimal_protocol(self):
        ...
        baseline_protocol, _ = await self.navigator.select_optimal_protocol(
            self.test_peer, context, available
        )
        ...
        fault_protocol, fault_meta = await self.navigator.select_optimal_protocol(
            self.test_peer, context, available
        )
        ...
        recovery_protocol, recovery_meta = await self.navigator.select_optimal_protocol(
            self.test_peer, context, available
        )

        # Assertions
        self.assertNotEqual(
            baseline_protocol, fault_protocol, "Should switch during faults"
        )
        self.assertEqual(
            baseline_protocol,
            recovery_protocol,
            "Should restore optimal protocol after recovery",
        )
        self.assertLess(
            recovery_meta["decision_time_ms"], 500, "Recovery decision should be fast"
        )

        logger.info(
            f"✅ Protocol recovery: {baseline_protocol} → {fault_protocol} → {recovery_protocol}"
        )
    ...
    async def test_control_ping_provides_measurements_under_faults(self):
        ...
        asyncio.create_task(
            self.metrics_collector.start_control_ping_loop(
                self.test_peer, self.ping_sender.send_ping
            )
        )

        # Let it establish baseline
        await asyncio.sleep(2.0)
        baseline_measurements = self.metrics_collector.peer_metrics.get(
            self.test_peer, None
        )
        ...
        # Check measurements
        fault_measurements = self.metrics_collector.peer_metrics.get(
            self.test_peer, None
        )
        ...
        self.assertGreater(
            fault_measurements.rtt_ewma_ms,
            baseline_measurements.rtt_ewma_ms,
            "RTT should increase under delay fault",
        )
        self.assertGreater(
            fault_measurements.jitter_ms, 50, "Should measure increased jitter"
        )
        self.assertGreater(
            self.ping_sender.call_count, 5, "Should have made multiple ping attempts"
        )

        logger.info(
            f"✅ Control ping measured RTT increase: "
    ...
    async def test_delivery_thresholds_under_faults(self):
        ...
        self.fault_injector.inject_packet_loss(
            loss_rate=0.1, duration_seconds=10.0
        )  # 10% loss
        ...
        for i in range(total_messages):
            ...
            rtt_ms, success = self.fault_injector.simulate_message_send(
                self.test_peer, f"delivery_test_{i}"
            )
        ...
        max_observed_delivery_ms = (
            max(delivery_times) if delivery_times else float("inf")
        )
        ...
        logger.info(
            f"✅ Delivery under faults: {success_rate:.1%} success rate, "
    ...
    async def _establish_baseline_measurements(self, count: int = 3):
        ...
        for i in range(count):
            seq_id = self.metrics_collector.record_message_sent(
                self.test_peer, f"baseline_{i}", 512
            )
    ...
    async def _simulate_measurements_with_faults(self, count: int = 5):
        ...
        for i in range(count):
            seq_id = self.metrics_collector.record_message_sent(
                self.test_peer, f"fault_{i}", 1024
            )

            # Use fault injector for realistic delay
            rtt_ms, success = self.fault_injector.simulate_message_send(
                self.test_peer, f"fault_{i}"
            )
            await asyncio.sleep(rtt_ms / 1000.0)
        ...
        logger.debug(
            f"Simulated {count} measurements with faults: {self.fault_injector.get_fault_summary()}"
        )
...
async def run_fault_injection_demo():
    ...
    baseline_protocol, _ = await navigator.select_optimal_protocol(
        test_peer, context, available
    )
    ...
    fault_protocol, fault_meta = await navigator.select_optimal_protocol(
        test_peer, context, available
    )

    print("\n🎯 Adaptive Response:")
    print(
        f"   RTT: {baseline_conditions.measured_rtt_ms:.1f} → {fault_conditions.measured_rtt_ms:.1f} ms"
    )
    print(
        f"   Jitter: {baseline_conditions.measured_jitter_ms:.1f} → {fault_conditions.measured_jitter_ms:.1f} ms"
    )
    print(
        f"   Loss: {baseline_conditions.measured_loss_rate:.1%} → {fault_conditions.measured_loss_rate:.1%}"
    )
    ...
    recovery_protocol, recovery_meta = await navigator.select_optimal_protocol(
        test_peer, context, available
    )
...
if __name__ == "__main__":
    ...
    if args.demo:
        ...
        suite = unittest.TestLoader().loadTestsFromName(
            f"TestNetworkFaultInjection.{args.test}", module=sys.modules[__name__]
        )
