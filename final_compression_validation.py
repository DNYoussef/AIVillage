        ...
        for comp_name, compressor in compressors.items():
            ...
            try:
                ...
                if comp_name == "BitNet":
                    ...
                    comp_size = (len(compressed['seeds']) * 2 + 
                               compressed['coefficients'].size + 
                               len(compressed['shared_exponents']) + 32)
                elif comp_name == "VPTQ":
                    comp_size = (compressed['codebook'].numel() * 4 + 
                               len(compressed['indices']) + 32)
...
def test_pipeline_integration():
    ...
    s2_size = (len(s2_compressed['seeds']) * 2 + 
              s2_compressed['coefficients'].size + 
              len(s2_compressed['shared_exponents']) + 32)
    ...
    s3_size = (s3_compressed['codebook'].numel() * 4 + 
              len(s3_compressed['indices']) + 32)
...
def demonstrate_mobile_scenarios():
    ...
    for device_name, device_info in devices.items():
        ...
        for model_name, model_size in model_sizes.items():
            ...
            print(f"{device_name:<25} {model_name:<15} {original_mb:>7.0f}MB "
    ...
    try:
        ...
        for model_name, model_results in compression_results.items():
            ...
            for method, results in model_results.items():
                if results["ratio"] > 0:
                    print(f"  {method:8}: {results['ratio']:5.1f}x compression, "
