...
def mock_base_agent():
    ...
    agent.info = {"name": "TestAgent", "description": "Mock agent for testing", "capabilities": ["test_capability"]}
    return agent
...
def mock_p2p_transport():
    ...
    class MockTransport:
        ...
        async def send_message(self, recipient_id, message_data):
            ...
            self.messages_sent.append({"recipient": recipient_id, "data": message_data, "timestamp": time.time()})
        ...
        def get_status(self):
            return {"name": self.name, "available": self.is_available, "messages_sent": len(self.messages_sent)}
...
def mock_error_handler():
    ...
    handler.get_error_stats = Mock(return_value={"total_errors": 0, "error_types": {}, "recent_errors": []})
    return handler
...
def integration_test_suite():
    ...
    class IntegrationTestSuite:
        ...
        def add_result(self, test_name, success, details=None):
            self.results[test_name] = {"success": success, "details": details or {}, "timestamp": time.time()}
