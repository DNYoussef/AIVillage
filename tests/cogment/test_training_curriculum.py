...
try:
    from core.agent_forge.models.cogment.training.curriculum import (
        FourStageCurriculum, CurriculumStage, StageConfig, CurriculumScheduler
    )
    from core.agent_forge.models.cogment.training.grokfast import (
        GrokFast, GrokFastConfig, GradientFilter
    )
    from core.agent_forge.models.cogment.training.loss_functions import (
        CogmentLoss, StageLoss, ACTLoss, MemoryLoss
    )
    from core.agent_forge.models.cogment.training.optimizer import (
        CogmentOptimizer, OptimizerConfig
    )
...
class TestFourStageCurriculum:
    ...
    def test_curriculum_stage_progression(self, curriculum):
        ...
        assert len(set(visited_stages) & set(expected_stages)) >= 4, \
            f"Should visit most stages: {visited_stages}"
    ...
    def test_curriculum_config_adaptation(self, curriculum):
        ...
        for i in range(len(configs) - 1):
            ...
            diff_found = (
                config1.learning_rate != config2.learning_rate or
                config1.sequence_length != config2.sequence_length or
                config1.loss_weights != config2.loss_weights
            )
...
class TestStageLossFunctions:
    ...
    def test_act_loss_computation(self, cogment_loss):
        ...
        act_outputs = type('ACTOutputs', (), {
            'halting_probability': halting_prob,
    ...
    def test_memory_loss_computation(self, cogment_loss):
        ...
        memory_outputs = type('MemoryOutputs', (), {
...
class TestCurriculumScheduler:
    ...
    def test_scheduler_training_step(self, scheduler):
        ...
        # Training step
        loss_info = scheduler.training_step(
            model=model,
            optimizer=optimizer,
            inputs=x,
            targets=targets
