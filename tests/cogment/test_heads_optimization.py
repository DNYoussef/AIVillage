...
try:
    ...
    from core.agent_forge.models.cogment.heads.vocabulary_optimization import (
        VocabularyOptimizer, VocabOptimConfig, TiedEmbedding
    )
            ...
            with torch.no_grad():
                ...
                if size != image_config.image_size:
                    arc_images = F.interpolate(
                        arc_images, 
                        size=(image_config.image_size, image_config.image_size),
                        mode='nearest'
                    )
...
class TestTextHead:
    ...
    def test_text_head_parameter_efficiency(self, text_head, text_config):
        ...
        if text_config.tie_embeddings:
            ...
            assert total_params <= expected_params * 1.1, \
...
class TestTaskAdapters:
    ...
    def test_task_adapter_parameter_efficiency(self, task_adapter, adapter_config):
        ...
        assert total_params <= max_expected, \
            f"Task adapters should be parameter efficient: {total_params:,} params"
...
class TestHeadsIntegration:
    ...
    def test_heads_parameter_budget(self):
        ...
        vocab_optimizer = VocabularyOptimizer(VocabOptimConfig(
        ...
        assert total_head_params <= max_head_budget, \
    ...
    def test_heads_memory_efficiency(self):
        ...
        assert memory_per_head <= max_memory_per_head, \
            f"Head memory usage too high: {memory_per_head:.1f}MB per head"
