...
try:
    from config.cogment.config_loader import load_cogment_config, CogmentConfigLoader
...
class TestCogmentConfig:
    ...
    def test_config_parameter_budget_calculation(self, cogment_config):
        ...
        assert min_acceptable <= estimated_params <= max_acceptable, \
    ...
    def test_config_option_a_compliance(self, cogment_config):
        ...
        assert cogment_config.vocab_size == 13000, f"Option A vocab size should be 13000, got {cogment_config.vocab_size}"
...
class TestCogmentConfigValidator:
    ...
    def test_architecture_validation(self, config_validator, valid_config):
        ...
        invalid_config = CogmentConfig(
            d_model=513,  # Not divisible by n_head
            n_head=8,
    ...
    def test_dependency_validation(self, config_validator, valid_config):
        ...
        invalid_config = CogmentConfig(
            d_model=512,
            n_head=7,  # d_model not divisible by n_head
            target_params=25000000
        )
...
class TestConfigurationIntegration:
    ...
    def test_parameter_budget_enforcement(self):
        ...
        assert abs(estimated_params - achieved_target) <= tolerance_range, \
    ...
    def test_configuration_vs_hrrm_baseline(self):
        ...
        assert reduction_factor >= 5.0, \
            f"Insufficient parameter reduction: {reduction_factor:.1f}x"
