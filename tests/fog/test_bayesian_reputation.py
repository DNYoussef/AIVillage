...
class TestReputationScore:
    ...
    def test_tier_assignment(self):
        ...
        new_node = ReputationScore(
            node_id="new_node",
            alpha=3.0,  # 75% success rate but only 2 samples
            beta=1.0,
            last_updated=time.time()
        )
...
class TestReputationEvent:
    ...
    def test_positive_event_detection(self):
        ...
        positive_event = ReputationEvent(
            node_id="test_node",
        ...
        negative_event = ReputationEvent(
            node_id="test_node",
        ...
        for node_id in nodes:
            ...
            for i in range(failure_count):
                event = ReputationEvent(
                    node_id=node_id,
                    event_type=EventType.TASK_FAILURE,
                    timestamp=time.time() + success_count + i
                )
                self.engine.record_event(event)
        
        # Recommend nodes with minimum trust threshold
        recommendations = self.engine.recommend_nodes_for_task(
            task_requirements={},
            min_trust=0.6,
            max_nodes=2
        )
...
class TestTrustComposition:
    ...
    def test_direct_trust(self):
        ...
        trust_graph = {
            "node_a": {"node_b": 0.8, "node_c": 0.6},
            "node_b": {"node_c": 0.9}
        }
        
        # Direct trust should return exact value
        trust = self.trust_composition.compute_transitive_trust(
            "node_a", "node_b", trust_graph
        )
        assert trust == 0.8
    ...
    def test_transitive_trust(self):
        ...
        trust = self.trust_composition.compute_transitive_trust(
            "node_a", "node_c", trust_graph, max_hops=3
        )
    ...
    def test_tier_trust_aggregation(self):
        ...
        tier_trust = self.trust_composition.aggregate_tier_trust(
            node_scores, tier_structure
        )
...
class TestIntegrationFunctions:
    ...
    def test_scheduler_integration(self):
        ...
        scheduler_config = {
            "available_nodes": ["high_rep", "med_rep", "low_rep"]
        }
