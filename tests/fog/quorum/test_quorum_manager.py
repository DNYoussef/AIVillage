...
class TestQuorumManager:
    ...
    @pytest.mark.asyncio
    async def test_validate_quorum_bronze_tier(
        self, quorum_manager, mock_classifier, device_candidates, sample_profiles
    ):
        ...
        result = await quorum_manager.validate_quorum(
            device_candidates=device_candidates, requirement=QuorumRequirement.NONE, target_size=3
        )
    ...
    @pytest.mark.asyncio
    async def test_validate_quorum_gold_tier_success(
        self, quorum_manager, mock_classifier, device_candidates, sample_profiles
    ):
        ...
        result = await quorum_manager.validate_quorum(
            device_candidates=device_candidates, requirement=QuorumRequirement.GOLD, target_size=3
        )
...
class TestConstraintValidation:
    ...
    def test_validate_constraints_asn_violations(self, quorum_manager, sample_profiles):
        ...
        constraints = DiversityConstraints(min_asn_diversity=3, max_devices_per_asn=1)
    ...
    def test_validate_constraints_tee_requirement(self, quorum_manager, sample_profiles):
        ...
        constraints = DiversityConstraints(require_tee_diversity=True)
    ...
    def test_validate_constraints_confidence_threshold(self, quorum_manager, sample_profiles):
        ...
        constraints = DiversityConstraints(min_confidence_score=0.8)
...
class TestRecommendations:
    ...
    def test_generate_recommendations_all_satisfied(self, quorum_manager, sample_profiles):
        ...
        constraints = DiversityConstraints()
        violations = []
        metrics = {"unique_asns": 3, "unique_tee_vendors": 2, "unique_power_regions": 3, "unique_countries": 1}

        recommendations = quorum_manager._generate_recommendations(sample_profiles, constraints, violations, metrics)
    ...
    def test_generate_recommendations_tee_diversity(self, quorum_manager, sample_profiles):
        ...
        metrics = {"unique_asns": 3, "unique_tee_vendors": 1, "unique_power_regions": 3, "unique_countries": 1}
    ...
    async def test_select_optimal_quorum_diversity_prioritization(self, quorum_manager, mock_classifier):
        ...
        profiles = [
            ...
            InfrastructureProfile(
                device_id="device3",
                asn=3,
                tee_vendor=TEEVendor.ARM_TRUSTZONE,
                power_region=PowerRegion.NERC_SERC,
                country_code="CA",
                region="ON",
                city="Toronto",
            ...
            InfrastructureProfile(
                device_id="device4",
                asn=1,
                tee_vendor=TEEVendor.AMD_SEV_SNP,  # Same ASN as device1
                power_region=PowerRegion.NERC_RFC,
                country_code="US",  # Same region
                region="VA",
                city="Norfolk",
                network_topology="residential",
                attestation_hash="4",
        ...
        constraints = DiversityConstraints(
            min_asn_diversity=3, min_tee_vendor_diversity=2, min_power_region_diversity=2
        )
