...
async def test_bayesian_graph_rag_system():
    ...
    test_queries = [
        ...
        },
        {
            ...
            "expected_concepts": ["deep learning", "computer vision", "neural networks"],
    ...
    for i, test_case in enumerate(test_queries, 1):
        ...
        if results:
            ...
            concepts_found = sum(1 for concept in expected_concepts 
                               if concept.lower() in result_text)
    ...
    if "graph_structure" in graph_stats:
        ...
        print(f"  Trust Range: {trust_metrics.get('min_trust_score', 0):.3f} - {trust_metrics.get('max_trust_score', 0):.3f}")
    ...
    # Evaluation criteria
    graph_working = (graph_integration.get('enabled', False) and 
                    graph_integration.get('chunks_in_graph', 0) > 0 and
                    graph_integration.get('relationships_detected', 0) > 0)
    
    trust_working = (graph_integration.get('trust_propagations', 0) > 0 and
                    trust_metrics.get('avg_trust_score', 0) > 0)
    ...
    if (graph_working and trust_working and retrieval_working and 
        performance_good and context_rich):
