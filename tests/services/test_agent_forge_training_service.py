...
class TestAgentForgeTrainingService:
    ...
    async def test_start_agent_training_session(self, training_service):
        ...
        training_parameters = {"focus": "coordination", "complexity": "intermediate"}

        session_info = await training_service.start_agent_training_session(
            task_id=task_id, training_parameters=training_parameters
        )
    ...
    async def test_execute_agent_training_pipeline(self, training_service):
        ...
        await training_service.start_agent_training_session(
            task_id=task_id, training_parameters={"focus": "multi_agent"}
        )
    ...
    async def test_agent_artifacts_creation(self, training_service):
        ...
        agent_spec = {"name": "test_coordinator", "architecture": "hierarchical_agent", "role": "coordination"}
        ...
        artifacts = await training_service._create_agent_artifacts(agent_spec, 0, training_stats, environment_results)
    ...
    async def test_different_training_modes(self, mock_dependencies, training_config):
        ...
        for mode in modes_to_test:
            ...
            session_info = await service.start_agent_training_session(
                task_id=task_id, training_parameters={"mode": mode.value}
            )
    ...
    async def test_session_management(self, training_service):
        ...
        await training_service.start_agent_training_session(
            task_id=task_id, training_parameters={"test": "session_mgmt"}
        )
...
async def test_integration_example():
    ...
    await service.start_agent_training_session(
        task_id=task_id, training_parameters={"integration": True}, agent_specifications=agent_specifications
    )
