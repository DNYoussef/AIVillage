...
class DocumentationFreshnessValidator:
    ...
    async def _analyze_documentation_file(self, doc_path: Path) -> DocumentationFile:
        ...
        return DocumentationFile(
            path=doc_path, last_modified=last_modified, content_hash=content_hash, links=links, mece_score=mece_score
        )
    ...
    async def validate_link_freshness(self, doc_files: List[DocumentationFile]):
        ...
        for doc_path, link in all_links:
            ...
            link_validation_results.append({"doc": str(doc_path), "link": link, "valid": is_valid})
...
async def main():
    ...
    # Overall result
    overall_success = (
        mece_result["target_met"] and link_validation["zero_dead_links"] and sync_validation["sync_target_met"]
    )
