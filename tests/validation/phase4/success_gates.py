...
class SuccessGateManager:
    ...
    async def _evaluate_coupling_gate(self, gate: SuccessGate, validation_result: Any) -> GateResult:
        ...
        return GateResult(gate_name=gate.name, status=status, score=score, details=details)
    ...
    async def _evaluate_performance_gate(self, gate: SuccessGate, validation_result: Any) -> GateResult:
        ...
        if "max_memory_increase" in criteria:
            ...
            details["memory_increase"] = {"max_allowed": max_allowed, "actual": actual_increase, "passed": passed}
        ...
        if "min_throughput_ratio" in criteria:
            ...
            details["throughput_ratio"] = {"min_required": min_required, "actual": actual_ratio, "passed": passed}
    ...
    async def _evaluate_quality_gate(self, gate: SuccessGate, validation_result: Any) -> GateResult:
        ...
        if "min_test_coverage" in criteria:
            ...
            details["test_coverage"] = {"min_required": min_required, "actual": actual_coverage, "passed": passed}
        ...
        if "max_magic_literals" in criteria:
            ...
            details["magic_literals"] = {"max_allowed": max_allowed, "actual": actual_count, "passed": passed}
        ...
        if "max_lines_per_class" in criteria:
            ...
            details["lines_per_class"] = {"max_allowed": max_allowed, "actual_max": actual_max, "passed": passed}
    ...
    async def _evaluate_compatibility_gate(self, gate: SuccessGate, validation_result: Any) -> GateResult:
        ...
        status = (
            GateStatus.PASSED
            if (score >= min_score and len(critical_failures) <= max_critical_failures)
    ...
    async def _evaluate_custom_script_gate(self, gate: SuccessGate, validation_result: Any) -> GateResult:
        ...
        try:
            ...
            result = subprocess.run(
                [script_path], cwd=self.project_root, capture_output=True, text=True, timeout=gate.timeout_seconds
            )
            ...
            return GateResult(
                gate_name=gate.name, status=GateStatus.FAILED, score=0.0, details={}, error_message=str(e)
            )
    ...
    def _calculate_overall_result(self) -> Dict[str, Any]:
            ...
            if result.status == GateStatus.PASSED:
                ...
                if gate and gate.critical:
                    critical_failures.append(
                        {"gate": result.gate_name, "error": result.error_message, "score": result.score}
                    )
        ...
        deployment_approved = (
            ...
            and gates_failed == 0  # No gates failed
            and overall_score >= 0.95  # At least 95% overall score
        )
    ...
    async def _rollback_git_reset(self, action: RollbackAction, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if create_backup_branch:
                ...
                subprocess.run(["git", "checkout", "-b", backup_branch], cwd=self.project_root, check=True)
                result["details"]["backup_branch"] = backup_branch

            # Reset to target commit
            subprocess.run(["git", "reset", "--hard", target_commit], cwd=self.project_root, check=True)
    ...
    async def _rollback_custom_script(self, action: RollbackAction, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            process_result = subprocess.run(
                command, cwd=self.project_root, capture_output=True, text=True, timeout=action.timeout_seconds
            )
    ...
    def _define_success_gates(self) -> List[SuccessGate]:
        ...
        return [
            ...
            SuccessGate(
                ...
                criteria={"minimum_score": 0.90},  # 90% integration test success rate
                weight=2.5,
                critical=True,
            ),
            SuccessGate(
                ...
                criteria={"script_path": str(self.project_root / "scripts/security_validation.py")},
                weight=2.0,
                critical=True,
