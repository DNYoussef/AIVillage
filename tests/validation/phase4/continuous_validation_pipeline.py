...
class ContinuousValidationPipeline:
    ...
    async def start_pipeline(self) -> None:
        ...
        await self.performance_monitor.start_monitoring(interval=monitor_config.get("interval", 1.0))
        ...
        file_triggers = [t for t in self.config.validation_triggers if t.trigger_type == "file_change" and t.enabled]
        if file_triggers:
            ...
            self.validation_history.append(
                {
                    ...
                    "summary": {"errors": len(validation_result.errors), "warnings": len(validation_result.warnings)},
                }
            )
    ...
    async def _check_success_gates(self, validation_result: ValidationResult) -> bool:
        ...
        if len(validation_result.errors) > max_errors:
            self.logger.warning(
                f"Success gate failed: Too many errors ({len(validation_result.errors)} > {max_errors})"
            )
            return False
        ...
        if validation_result.performance_results:
            ...
            if degradation > max_degradation:
                self.logger.warning(
                    f"Success gate failed: Performance degradation too high ({degradation}% > {max_degradation}%)"
                )
                return False
            ...
            for component, min_improvement in coupling_gates.items():
                if component in improvements:
                    ...
                    if actual_improvement < min_improvement:
                        self.logger.warning(
                            f"Success gate failed: {component} coupling improvement insufficient ({actual_improvement}% < {min_improvement}%)"
                        )
                        return False
    ...
    async def _handle_validation_failure(
        self, validation_result: ValidationResult, trigger: str, gates_passed: bool
    ) -> None:
    ...
    async def _perform_git_rollback(self) -> None:
        ...
        try:
            ...
            subprocess.run(["git", "checkout", "-b", rollback_branch], cwd=self.project_root, check=True)
    ...
    async def _schedule_validation_triggers(self) -> None:
        ...
        scheduled_triggers = [t for t in self.config.validation_triggers if t.trigger_type == "schedule" and t.enabled]
    ...
    async def _get_current_commit_hash(self) -> Optional[str]:
        ...
        try:
            result = subprocess.run(
                ["git", "rev-parse", "HEAD"], cwd=self.project_root, capture_output=True, text=True, check=True
            )
    ...
    def _update_pipeline_metrics(self, validation_result: ValidationResult, execution_time_ms: int) -> None:
        ...
        self.pipeline_metrics["average_execution_time_ms"] = (
            current_avg * (total_validations - 1) + execution_time_ms
        ) / total_validations
    ...
    def _load_config(self, config_path: Optional[Path]) -> PipelineConfig:
        ...
        triggers = [ValidationTrigger(**trigger_data) for trigger_data in config_data.get("validation_triggers", [])]
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "validation_triggers": [
                {
                    ...
                    "config": {"file_patterns": ["swarm/**/*.py", "tests/**/*.py", "requirements.txt"]},
                },
                {
            ...
            "monitoring_config": {"interval": 1.0},
            "success_gates": {
                "max_errors": 0,
                "max_performance_degradation": 10.0,
                "min_test_coverage": 80.0,
                "coupling_requirements": {"UnifiedManagement": 50.0, "SageAgent": 30.0},
