...
class QualityAnalyzer:
    ...
    async def analyze_quality_metrics(self) -> Dict[str, Any]:
        ...
        results = {
            "files": [],
            "summary": {"total_files": 0, "analyzed_files": 0, "failed_files": 0},
            "aggregated_metrics": {},
            "targets_met": {},
        }
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for file_path in self.target_files:
                ...
                if full_path.exists():
                    task = asyncio.get_event_loop().run_in_executor(executor, self._analyze_file_quality, full_path)
            ...
            for file_path, task in tasks:
                try:
                    ...
                    results["files"].append(
                        {"file_path": file_path, "metrics": quality_metrics.__dict__, "analysis_successful": True}
                    )
                    results["summary"]["analyzed_files"] += 1
                except Exception as e:
                    self.logger.error(f"Failed to analyze {file_path}: {e}")
                    results["files"].append({"file_path": file_path, "error": str(e), "analysis_successful": False})
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if end_line == start_line:
                    ...
                    for other_node in ast.walk(tree):
                        if (
                            isinstance(other_node, (ast.ClassDef, ast.FunctionDef))
                            and other_node != node
                            and other_node.lineno > start_line
                        ):
        ...
        class MagicLiteralVisitor(ast.NodeVisitor):
            def visit_Constant(self, node):
                if isinstance(node.value, (int, float)):
                    ...
                    if (
                        len(node.value) > 1
                        and node.value not in excluded_strings
                        and not node.value.isspace()
                        and not re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", node.value)
                    ):  # Not just identifier-like
            ...
            def visit_Str(self, node):
                if len(node.s) > 1 and node.s not in excluded_strings and not node.s.isspace():
    ...
    def _estimate_test_coverage(self, file_path: Path) -> float:
        ...
        test_patterns = [f"test_{file_path.stem}.py", f"{file_path.stem}_test.py", f"test_{file_path.stem}_*.py"]
    ...
    def _calculate_maintainability_index(self, content: str, cyclomatic_complexity: int) -> float:
        ...
        halstead_volume = max(
            1, (unique_operators + unique_operands) * math.log2(max(1, unique_operators + unique_operands))
        )
    ...
    def _check_quality_targets(self, aggregated_metrics: Dict[str, Any]) -> Dict[str, bool]:
        ...
        return {
            "lines_per_class_target_met": aggregated_metrics.get("max_lines_per_class", float("inf"))
            <= self.targets["max_lines_per_class"],
            "magic_literals_target_met": aggregated_metrics.get("magic_literals_count", float("inf"))
            <= self.targets["magic_literals_target"],
            "test_coverage_target_met": aggregated_metrics.get("avg_test_coverage", 0)
            >= self.targets["min_test_coverage"],
            "cyclomatic_complexity_target_met": aggregated_metrics.get("max_cyclomatic_complexity", float("inf"))
            <= self.targets["max_cyclomatic_complexity"],
            "maintainability_target_met": aggregated_metrics.get("avg_maintainability_index", 0)
            >= self.targets["min_maintainability_index"],
            "code_duplication_target_met": aggregated_metrics.get("max_code_duplication", float("inf"))
            <= self.targets["max_code_duplication"],
        }
    ...
    def _calculate_overall_quality_score(self, aggregated_metrics: Dict[str, Any]) -> float:
        ...
        complexity_score = (
            max(0, 100 - (max_complexity - self.targets["max_cyclomatic_complexity"]) * 10)
            if max_complexity > 0
            else 100
        )
        ...
        for tool_name, command in tools:
            try:
                ...
                full_command = command + [
                    str(self.project_root / fp) for fp in self.target_files if (self.project_root / fp).exists()
                ]

                result = subprocess.run(
                    full_command, capture_output=True, text=True, cwd=self.project_root, timeout=60  # 1 minute timeout
                )
                ...
                external_results[tool_name] = {"error": "Timeout", "success": False}
            except FileNotFoundError:
                external_results[tool_name] = {"error": f"{tool_name} not installed", "success": False}
            except Exception as e:
                external_results[tool_name] = {"error": str(e), "success": False}
