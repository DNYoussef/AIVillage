...
class BaseSecurityTest(BaseAIVillageTest):
    ...
    def setup_method(self, method):
        ...
        self.security_validator.get_security_report = Mock(
        ...
        self.threat_payloads = TestDataGenerator.generate_security_test_payloads(
            ["code_injection", "command_injection", "script_injection", "path_traversal", "sql_injection"]
        )
...
class BaseP2PTest(BaseAIVillageTest):
    ...
    def setup_method(self, method):
        ...
        self.mock_transport.send_message = AsyncMock(return_value={"status": "delivered", "message_id": "msg_123"})
        ...
        self.network_topology = TestDataGenerator.generate_p2p_network_topology(node_count=5, connectivity=0.4)

    async def simulate_message_delivery(
        self, sender: str, recipient: str, message: Dict[str, Any], success_rate: float = 0.95
    ) -> Dict[str, Any]:
...
class BasePerformanceTest(BaseAIVillageTest):
    ...
    def measure_function_performance(self, func, *args, **kwargs) -> Dict[str, Any]:
        ...
        result, execution_time = self.performance_tester.measure_execution_time(func, *args, **kwargs)
...
class BaseMLModelTest(BaseAIVillageTest):
    ...
    def benchmark_model_inference(
        self, model: torch.nn.Module, input_shape: tuple, iterations: int = 100
    ) -> Dict[str, float]:
        """Benchmark model inference performance."""
        return self.performance_tester.benchmark_model_inference(model, input_shape, iterations)
...
class BaseIntegrationTest(BaseAIVillageTest):
    ...
    async def run_integration_scenario(self, scenario_name: str, test_steps: List[callable]) -> Dict[str, Any]:
...
class BaseRAGTest(BaseAIVillageTest):
    ...
    def setup_method(self, method):
        ...
        self.mock_vector_store.query = AsyncMock(
            return_value=[
...
class TestSuiteBase:
    ...
    def get_test_classes(cls) -> List[Type[BaseAIVillageTest]]:
        ...
        for attr_name in dir(cls):
            ...
            if isinstance(attr, type) and issubclass(attr, BaseAIVillageTest) and attr != BaseAIVillageTest:
