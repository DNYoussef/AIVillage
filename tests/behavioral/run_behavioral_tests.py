...
class TestSuiteRunner:
    ...
    def run_pytest_command(self, 
                          ...
                          coverage: bool = False,
                          parallel: bool = False) -> Tuple[int, str, str]:
        ...
        for test in performance_tests:
            if test.get("outcome") == "failed":
                performance_summary["failed_benchmarks"].append({
                    ...
                    "failure_reason": test.get("call", {}).get("longrepr", "")
                })
    ...
    def run_category(self, category: str, **kwargs) -> bool:
        ...
        exit_code, stdout, stderr = self.run_pytest_command(
            test_files=cat_config["files"],
            markers=cat_config.get("markers"),
            **kwargs
        )
    ...
    def generate_console_report(self) -> str:
        ...
        report_lines = [
            "=" * 80,
            "🧪 AGENT FORGE BEHAVIORAL TEST SUITE RESULTS",
            "=" * 80,
            ""
        ]
        ...
        if self.results["coverage_percent"]:
            report_lines.extend([
                f"📈 TEST COVERAGE",
                f"   Overall coverage: {self.results['coverage_percent']:.1f}%",
                ""
            ])
        ...
        report_lines.extend([
            "=" * 80,
            f"🏁 Test suite completed at {datetime.now().isoformat()}",
            "=" * 80
        ])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Agent Forge Behavioral Test Suite Runner"
    )
    ...
    parser.add_argument(
        "--ci",
        action="store_true",
        help="CI/CD mode with structured output"
    )
