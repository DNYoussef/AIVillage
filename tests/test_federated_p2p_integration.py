...
try:
    from infrastructure.distributed_inference.federated_p2p_integration import (
        ...
        P2PTrainingMode,
        P2PParticipant,
        create_federated_p2p_coordinator
    )
    from infrastructure.distributed_inference.p2p_hierarchical_aggregator import (
        ...
        ClusterRole,
        AggregationTier,
        create_hierarchical_aggregator
    )
    ...
    from infrastructure.p2p import create_network, P2PNetwork, NetworkConfig
...
class TestFederatedP2PIntegration:
    ...
    async def test_p2p_coordinator_initialization(self, mock_p2p_network, mock_betanet_transport):
        ...
        coordinator = FederatedP2PCoordinator(
        ...
        with patch.object(coordinator, 'p2p_network', mock_p2p_network), \
             patch.object(coordinator, 'betanet_transport', mock_betanet_transport):
    ...
    async def test_participant_discovery_and_selection(self, mock_p2p_network, sample_participants):
        ...
        with patch.object(coordinator, 'p2p_network', mock_p2p_network):
            ...
            selected = await coordinator._select_p2p_participants(
                coordinator.active_training_jobs.get("test_job", Mock(
                    min_participants=3,
                    max_participants=10,
                    enable_betanet_privacy=True,
                    job_type=Mock()
    ...
    async def test_hierarchical_aggregation_integration(self, sample_participants):
        ...
        aggregator = P2PHierarchicalAggregator(
    ...
    async def test_mobile_optimization_integration(self):
        ...
        for device in mobile_devices:
            training_request = {
                "job_id": "test_mobile_job",
                "priority": "normal"
            }
            
            session = await optimizer.optimize_training_for_mobile(
                device.device_id, training_request
            )
            ...
            if device.battery_percent > device.battery_threshold_percent or device.is_charging:
                ...
                if device.is_charging:
                    assert session.training_mode in [MobileTrainingMode.AGGRESSIVE, MobileTrainingMode.CONSERVATIVE], \
                        "Charging device should allow training"
                        
                if not device.wifi_connected and device.cellular_connected:
                    assert session.training_mode == MobileTrainingMode.WIFI_ONLY or \
                           device.allow_cellular_training, "Should handle network constraints"
    ...
    async def test_secure_gradient_exchange(self, mock_betanet_transport, sample_participants):
        ...
        with patch.object(coordinator, 'betanet_transport', mock_betanet_transport):
            ...
            gradient_data = await coordinator._collect_gradients_via_betanet(participant, job_config)
    ...
    async def test_end_to_end_training_workflow(self, mock_p2p_network, mock_betanet_transport, sample_participants):
        ...
        coordinator = FederatedP2PCoordinator(
            coordinator_id="e2e_coordinator",
            p2p_config=config
        )
    ...
    async def test_system_resilience_and_recovery(self, mock_p2p_network, sample_participants):
        ...
        with patch.object(coordinator, 'p2p_network', mock_p2p_network):
            ...
            participant = P2PParticipant(
                participant_id="failing_participant",
                p2p_peer_id="failing_peer",
                transport_types=["libp2p"]
            )
            coordinator.p2p_participants["failing_participant"] = participant
            
            # Mock training failure
            with patch.object(coordinator, '_execute_participant_training_p2p', 
                            AsyncMock(side_effect=Exception("Training failure"))):
    ...
    async def test_performance_and_scalability(self, sample_participants):
        ...
        assert total_clusters >= expected_clusters * 0.8, \
        ...
        result = await aggregator.start_hierarchical_aggregation(
            "perf_test", gradients, {"byzantine_tolerance": 0.3}
        )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            ...
            with patch.object(coordinator, '_initialize_p2p_network', AsyncMock()), \
                 patch.object(coordinator, '_initialize_betanet_transport', AsyncMock()), \
                 patch.object(coordinator, '_initialize_base_coordinators', AsyncMock()), \
                 patch.object(coordinator, '_start_p2p_discovery', AsyncMock()), \
                 patch.object(coordinator, '_start_background_tasks', AsyncMock()):
