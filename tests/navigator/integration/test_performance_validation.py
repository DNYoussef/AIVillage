...
class TestPerformanceValidation:
    ...
    def performance_contexts(self):
        ...
        return [
            ...
            MessageContext(size_bytes=100, priority=9, requires_realtime=True, privacy_required=False),
            # Medium, standard message
            MessageContext(size_bytes=1024, priority=5, requires_realtime=False, privacy_required=False),
            # Large, bulk transfer
            MessageContext(size_bytes=10240, priority=2, requires_realtime=False, privacy_required=False),
            # Privacy-sensitive message
            MessageContext(size_bytes=512, priority=8, requires_realtime=False, privacy_required=True),
            # Real-time communication
            MessageContext(size_bytes=256, priority=10, requires_realtime=True, privacy_required=True),
        ]
    ...
    async def test_routing_decision_performance(self, navigator_facade, performance_contexts):
        ...
        for i, dest in enumerate(destinations):
            ...
            assert (
                decision_time_ms < TARGET_MAX_LATENCY_MS
    ...
    async def test_concurrent_routing_performance(self, navigator_facade, performance_contexts):
        ...
        assert (
            avg_concurrent_latency < TARGET_CONCURRENT_LATENCY_MS
        ), f"Average concurrent latency {avg_concurrent_latency:.2f}ms exceeds target"

        assert (
            max_concurrent_latency < TARGET_CONCURRENT_LATENCY_MS * 1.5
    ...
    async def test_memory_efficiency(self, navigator_facade, performance_contexts):
        ...
        assert (
            total_memory_mb < MAX_MEMORY_INCREASE_MB
    ...
    async def test_algorithm_performance_comparison(self, navigator_facade):
        ...
        network_conditions = [
            NetworkConditions(bluetooth_available=True, internet_available=False, bandwidth_mbps=0.1, latency_ms=200.0),
            NetworkConditions(bluetooth_available=False, internet_available=True, bandwidth_mbps=50.0, latency_ms=30.0),
            NetworkConditions(bluetooth_available=True, internet_available=True, bandwidth_mbps=25.0, latency_ms=60.0),
        ]
        ...
        for conditions in network_conditions:
            for context in message_contexts:
                ...
                protocol, scores = await route_service.select_optimal_route(
                    "algorithm_test_dest", context, ["bitchat", "betanet", "scion", "store_forward"], conditions
                )
        ...
        assert (
            avg_algorithm_time < TARGET_AVG_ALGORITHM_TIME
        ), f"Average algorithm time {avg_algorithm_time:.2f}ms too slow"

        assert max_algorithm_time < TARGET_MAX_ALGORITHM_TIME, f"Max algorithm time {max_algorithm_time:.2f}ms too slow"
    ...
    async def test_energy_efficiency_validation(self, navigator_facade):
        ...
        for scenario in battery_scenarios:
            ...
            if level < 20:  # Low battery
                ...
                assert (
                    top_choice in efficient_protocols
        ...
        for level, decision in energy_decisions.items():
            print(
                f"  Battery {level}%: {decision['optimized_protocols']} " f"({decision['optimization_time_ms']:.2f}ms)"
            )
    ...
    async def test_scalability_validation(self, navigator_facade):
        ...
        for size in message_sizes:
            ...
            for _ in range(10):  # Multiple samples per size
                start_time = time.perf_counter()
                protocol, metadata = await navigator_facade.select_path(f"size_test_{size}", context)
                end_time = time.perf_counter()
            ...
            assert avg_time < TARGET_MAX_TIME_MS, f"Size {size} bytes took {avg_time:.2f}ms (too slow)"
        ...
        for peer_count in peer_counts:
            ...
            for i in range(5):  # Fewer samples for higher peer counts
                start_time = time.perf_counter()
                protocol, metadata = await navigator_facade.select_path(f"scalability_dest_{i}", context)
                end_time = time.perf_counter()
            ...
            assert avg_time < TARGET_SCALABILITY_MS, f"Peer count {peer_count} took {avg_time:.2f}ms (poor scalability)"
    ...
    async def test_performance_regression_protection(self, navigator_facade, performance_contexts):
        ...
        assert (
            avg_decision_time < NEW_TARGETS["avg_decision_time_ms"]
        ), f"Average decision time {avg_decision_time:.2f}ms regression"

        assert (
            max_decision_time < NEW_TARGETS["max_decision_time_ms"]
        ), f"Max decision time {max_decision_time:.2f}ms regression"

        print("Performance Regression Test:")
        print(f"  Target avg: {NEW_TARGETS['avg_decision_time_ms']}ms, " f"Actual: {avg_decision_time:.2f}ms")
        print(f"  Target max: {NEW_TARGETS['max_decision_time_ms']}ms, " f"Actual: {max_decision_time:.2f}ms")
...
async def test_overall_architecture_performance():
    ...
    facade = create_navigator_facade(
        routing_priority=RoutingPriority.PERFORMANCE_FIRST, energy_mode=EnergyMode.BALANCED
    )
    ...
    try:
        ...
        print("  ✓ Original 1,438-line god class successfully separated")
