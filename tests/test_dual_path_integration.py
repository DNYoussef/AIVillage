...
try:
    from core.p2p.bitchat_transport import BitChatTransport, BitChatMessage
    from core.p2p.betanet_transport import BetanetTransport, BetanetMessage  
    from core.p2p.dual_path_transport import DualPathTransport, DualPathMessage
    from navigator.path_policy import NavigatorAgent, PathProtocol, MessageContext, EnergyMode, RoutingPriority
...
class TestDualPathIntegration:
    ...
    async def test_message_routing_via_dual_transport(self, dual_transport):
        ...
        with patch.object(dual_transport, '_send_via_bitchat', return_value=True) as mock_bitchat:
            # Send test message
...
class TestNavigatorPathSelection:
    ...
    async def test_data_cost_aware_routing(self, navigator):
        ...
        context = MessageContext(
            size_bytes=5000,  # Medium message
            priority=5
        )
...
class TestBitChatTransport:
    ...
    async def test_bitchat_ttl_enforcement(self, bitchat):
        ...
        with patch.object(bitchat, '_send_direct', return_value=True):
            ...
            assert success in [True, False]  # May succeed or fail depending on peer availability
