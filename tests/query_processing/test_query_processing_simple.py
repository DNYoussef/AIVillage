...
try:
    ...
    async def test_query_decomposition_only():
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            try:
                ...
                primary_intent, secondary_intents, confidence = query_processor._classify_intent(normalized)
        ...
        # Feature validation
        print(f"\nFeatures Tested:")
        print(f"  - Query normalization: Working")
        print(f"  - Key concept extraction: Working")
        print(f"  - Intent classification: Working")
        print(f"  - Context level detection: Working")
        print(f"  - Complexity assessment: Working")
        print(f"  - Temporal analysis: Working")
    ...
    def test_basic_patterns():
        ...
        for query in test_queries:
            ...
            factual_matches = sum(len(re.findall(pattern, query_lower, re.IGNORECASE)) 
                                for pattern in factual_patterns)
            
            # Test explanatory patterns  
            explanatory_matches = sum(len(re.findall(pattern, query_lower, re.IGNORECASE))
