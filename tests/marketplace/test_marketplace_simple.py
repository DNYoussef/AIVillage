...
async def test_marketplace():
    ...
    # Get price quote
    quote = await marketplace.get_price_quote(cpu_cores=2.0, memory_gb=4.0, duration_hours=1.0)
    ...
    bid_id = await marketplace.submit_bid(
        namespace="test-org/test-team", cpu_cores=2.0, memory_gb=4.0, max_price=0.25, estimated_duration_hours=1.0
    )
    ...
    print(
        f"[PASS] Usage report generated: {usage_report.usage_metrics.job_count} jobs, ${usage_report.cost_breakdown.total_cost:.4f} total cost"
    )
