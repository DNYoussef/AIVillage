...
async def test_edge_pricing():
    ...
    print(f'[PASS] Mobile device pricing calculated')
    ...
    print(f'[PASS] Generated marketplace listing')
    ...
    quote = await marketplace.get_price_quote(
        cpu_cores=0.5,  # Small job suitable for mobile
        memory_gb=1.0,
        duration_hours=0.5,
        min_trust_score=0.7
    )
    ...
    if quote['available']:
        print(f'[PASS] Price quote includes mobile device')
