...
try:
    from tests.marketplace.test_unified_federated_marketplace import (
        TestUnifiedFederatedMarketplace, UnifiedFederatedCoordinator
    )
    from tests.marketplace.test_user_tier_scenarios import TestUserTierScenarios
    from tests.marketplace.test_performance_benchmarks import TestPerformanceBenchmarks
    from tests.marketplace.test_budget_billing_integration import TestBudgetBillingIntegration
...
class ComprehensiveTestRunner:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        await self._run_test_suite(
            "unified_coordinator",
            "Core Unified Federated Coordinator Tests",
            self._test_unified_coordinator_core
        )
        
        # Test Suite 2: User Tier Scenarios
        await self._run_test_suite(
            "user_tier_scenarios", 
            "User Tier Integration Scenarios",
            self._test_user_tier_scenarios
        )
        ...
        await self._run_test_suite(
            "budget_billing",
            "Budget Management and Billing Integration",
            self._test_budget_billing
        )
    ...
    async def _test_unified_coordinator_core(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            training_capable = sum(1 for node in coordinator.shared_participants.values()
                                 if hasattr(node, 'supports_training'))
        ...
        try:
            ...
            tests.append({
                ...
                "metrics": {
                    "active_workloads": len(coordinator.active_workloads)
                }
            })
    ...
    async def _test_user_tier_scenarios(self) -> List[Dict[str, Any]]:
        ...
        for tier, config in tier_tests.items():
            try:
                ...
                tier_constraints_met = (
                    actual_budget_limit == config["expected_budget_limit"] and
                    actual_priority == config["expected_priority"]
                )
        ...
        try:
            ...
            marketplace_working = (
                "unified_coordinator" in stats and
                "gateway_marketplace" in stats and
                "inference_coordinator" in stats and
                "fog_marketplace" in stats
            )
            ...
            for test_case in scenario["test_cases"]:
                try:
                    ...
                    if scenario_name == "tier_budget_enforcement":
                        ...
                        scenario_type = test_case["scenario"]
