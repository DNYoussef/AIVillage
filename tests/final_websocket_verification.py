...
async def verify_connection_establishment(results: VerificationResults):
    ...
    try:
        ...
        if connection_success and server_sees_client and client_sees_server:
            results.add_test("connection_establishment", True, 
                           f"WebSocket connection established with proper handshake")
        else:
            results.add_test("connection_establishment", False,
        ...
        results.add_test("connection_establishment", False, f"Exception: {str(e)}")
    ...
    try:
        ...
        for i, msg in enumerate(messages_to_send):
            if i % 2 == 0:  # Client to server
                ...
                results.add_message("sent", msg.get('type') if isinstance(msg, dict) else msg.type.value, 
        ...
        if successful_sends == len(messages_to_send) and messages_received >= len(messages_to_send):
            results.add_test("bidirectional_messaging", True,
                           f"Successfully exchanged {messages_received} encrypted messages bidirectionally")
        else:
            results.add_test("bidirectional_messaging", False,
        ...
        results.add_test("bidirectional_messaging", False, f"Exception: {str(e)}")
...
async def verify_message_queuing(results: VerificationResults):
    ...
    try:
        ...
        results.add_test("message_queuing", False, f"Exception: {str(e)}")
...
async def verify_reconnection(results: VerificationResults):
    ...
    try:
        ...
        if not still_connected and reconnected:
            results.add_test("reconnection_logic", True,
                           "Client detected disconnect and automatically reconnected to server")
        else:
            results.add_test("reconnection_logic", False,
        ...
        results.add_test("reconnection_logic", False, f"Exception: {str(e)}")
...
async def verify_ssl_support(results: VerificationResults):
    ...
    try:
        ...
        server_ssl_support = (
            "ssl_context" in server_source and 
            "SSL_CERTFILE" in server_source and
            "SSL_KEYFILE" in server_source and
            "ssl.SSLContext" in server_source
        )
        
        client_ssl_support = (
            "ssl_context" in client_source and
            "wss" in client_source and
            "ssl.SSLContext" in client_source
        )
        
        if server_ssl_support and client_ssl_support:
            results.add_test("ssl_tls_support", True,
                           "SSL/TLS support fully implemented for both server and client connections")
        else:
            results.add_test("ssl_tls_support", False,
                           f"SSL support incomplete: server={server_ssl_support}, client={client_ssl_support}")
        
    except Exception as e:
        results.add_test("ssl_tls_support", False, f"Exception: {str(e)}")
