...
class TestCognativeNexusController:
    ...
    async def test_zero_nonetype_errors(self, controller, agent_types):
        ...
        for agent_type in all_agent_types[:15]:  # Test subset for thorough validation
            try:
                ...
                validation_results.append({"agent_type": agent_type, "success": True, "nonetype_errors": 0})

            except (AttributeError, TypeError) as e:
                if "NoneType" in str(e):
                    nonetype_errors.append({"agent_type": agent_type, "error": str(e)})
                validation_results.append({"agent_type": agent_type, "success": False, "error": str(e)})
            except Exception as e:
                validation_results.append({"agent_type": agent_type, "success": False, "error": str(e)})
    ...
    async def test_cognitive_reasoning_act_halting(self, controller):
        ...
        assert (
            result["final_confidence"] >= reasoning_request["confidence_threshold"]
        ...
        for category, types in agent_types.items():
            ...
            if test_type:
                try:
                    ...
                    category_test_results[category] = {"success": False, "agent_type": test_type, "error": str(e)}
    ...
    async def test_error_recovery_mechanisms(self, controller):
        ...
        for scenario in error_scenarios:
            try:
                result = await controller.create_agent(scenario["agent_type"], scenario.get("config"))

                # If creation succeeded unexpectedly, note it
                recovery_results.append(
                    {"scenario": scenario, "recovered_gracefully": True, "unexpected_success": True, "result": result}
                )
...
class TestCognativeNexusControllerBenchmarks:
    ...
    async def test_registry_throughput_benchmark(self, controller):
        ...
        for i in range(0, len(agent_types), batch_size):
            ...
            tasks = [controller.create_agent(f"{agent_type}_{i//batch_size}_{j}") for j, agent_type in enumerate(batch)]
