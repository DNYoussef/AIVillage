...
class TestUnifiedBaseAgentInitialization:
    ...
    async def test_successful_initialization(self, sample_agent_config, mock_communication_protocol):
    ...
    async def test_initialization_with_knowledge_tracker(self, sample_agent_config, mock_communication_protocol):
        """Test initialization with knowledge tracker."""
        mock_tracker = Mock()

        with patch.multiple("agents.unified_base_agent", EnhancedRAGPipeline=Mock(), OpenAIGPTConfig=Mock()):
            agent = UnifiedBaseAgent(sample_agent_config, mock_communication_protocol, knowledge_tracker=mock_tracker)

            assert agent is not None

    async def test_initialization_failure_handling(self, sample_agent_config, mock_communication_protocol):
        """Test initialization failure handling."""
        with patch("agents.unified_base_agent.EnhancedRAGPipeline", side_effect=Exception("RAG pipeline failed")):
...
class TestUnifiedBaseAgentTaskExecution:
    ...
    async def test_foundational_layer_failure(self, agent, sample_langroid_task):
        ...
        agent.quality_assurance_layer.check_task_safety = Mock(return_value=True)
        agent.foundational_layer.process_task = AsyncMock(side_effect=Exception("Foundation failed"))
    ...
    async def test_decision_making_failure(self, agent, sample_langroid_task):
        ...
        agent.decision_making_layer.make_decision = AsyncMock(side_effect=Exception("Decision failed"))
    ...
    async def test_continuous_learning_failure_graceful(self, agent, sample_langroid_task):
        ...
        agent.continuous_learning_layer.update = AsyncMock(side_effect=Exception("Learning failed"))
...
class TestUnifiedBaseAgentCommunication:
    ...
    async def test_message_handling_task_type(self, agent):
        ...
        message = Message(
            type=MessageType.TASK, sender="TestSender", receiver=agent.name, content={"content": "test task"}
        )
    ...
    async def test_message_handling_non_task_type(self, agent):
        ...
        message = Message(type=MessageType.QUERY, sender="TestSender", receiver=agent.name, content={"query": "test"})
...
class TestAgentFactory:
    ...
    async def test_create_agent_success(self, sample_agent_config, mock_communication_protocol):
        ...
        with patch.multiple("agents.unified_base_agent", EnhancedRAGPipeline=Mock(), OpenAIGPTConfig=Mock()):
            agent = create_agent("TestAgent", sample_agent_config, mock_communication_protocol)
    ...
    async def test_create_agent_failure(self, sample_agent_config, mock_communication_protocol):
        ...
        with patch("agents.unified_base_agent.UnifiedBaseAgent", side_effect=Exception("Creation failed")):

            with pytest.raises(AIVillageException) as exc_info:
                create_agent("TestAgent", sample_agent_config, mock_communication_protocol)
...
class TestUnifiedBaseAgentEdgeCases:
    ...
    async def test_memory_cleanup_on_failure(self, agent, isolation_manager):
        ...
        try:
            ...
            logging.debug(f"Expected failure during memory leak test: {e}")  # nosec S110 - intentional test exception
