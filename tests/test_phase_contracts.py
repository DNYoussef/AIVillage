...
class TestPhaseContracts(unittest.TestCase):
    ...
    def _mock_phase_dependencies(self, phase_name):
        ...
        if phase_name == 'evomerge':
            ...
            mock_pipeline = MagicMock()
            mock_pipeline.run_compression_pipeline = asyncio.coroutine(lambda: {
                'success': True,
            ...
            mock_loop = MagicMock()
            mock_loop.run_mastery_training = asyncio.coroutine(lambda: {
                'success': True,
                'levels_mastered': 1,
                'total_attempts': 10,
            ...
            mock_pipeline = MagicMock()
            mock_pipeline.run_prompt_baking_pipeline = asyncio.coroutine(lambda: {
                'success': True,
...
class TestPerformanceRegressionDetection(unittest.TestCase):
    ...
    def _test_regression_scenario(self, runner, scenario):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            ...
            mock_results = {
                'model_averages': {
                    'unified_pipeline': sum(scenario['scores'].values()) / len(scenario['scores'])
                },
