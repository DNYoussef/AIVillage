...
class TestP2PTransportLayer(BaseP2PTest):
    ...
    async def test_message_delivery_retry_on_failure(self):
        ...
        for attempt in range(max_attempts):
            try:
                result = await self.simulate_message_delivery(
                    sender, recipient, message, success_rate=0.3
                )
                # If successful, break
                assert result['status'] == 'delivered'
                break
...
class TestP2PMessageProtocol(BaseP2PTest):
    ...
    def test_message_priority_handling(self):
        ...
        sorted_messages = sorted(
            messages, 
            key=lambda m: priority_order.get(m['priority'], 0), 
            reverse=True
        )
