...
try:
    ...
    from infrastructure.distributed_inference.integration.unified_gateway_integration import (
        ...
        integrate_distributed_inference_with_gateway
    )
...
class TestNodeDiscoveryService:
    ...
    def discovery_service(self):
        ...
        return NodeDiscoveryService(
            discovery_methods=[DiscoveryMethod.STATIC_CONFIG],
    ...
    async def test_node_affinity_scoring(self, sample_capabilities):
        ...
        requirements2 = {
            'cpu_cores': 32,  # Exceeds node capability
            'memory_gb': 64.0,
            'gpu_count': 4
        }
...
class TestUnifiedGatewayIntegration:
    ...
    def mock_inference_manager(self):
        ...
        manager.execute_inference = AsyncMock(return_value={
            'request_id': 'test_request_123',
            'output': 'test output',
            'metadata': {'execution_time': 1.5},
            'nodes_used': ['node1', 'node2']
        })
        manager.execute_batch_inference = AsyncMock(return_value=[
            {
        ...
        manager.get_system_health = AsyncMock(return_value={
            'status': 'healthy',
            'total_requests': 100,
            'success_rate': 0.95,
    ...
    def mock_discovery_service(self):
        ...
        service.get_discovery_stats = Mock(return_value={
            'total_nodes': 2,
            'healthy_nodes': 2,
            'discovery_methods': 1,
    ...
    async def test_distributed_inference_endpoint(self, mock_inference_manager):
        ...
        with patch.object(integration.inference_manager, 'execute_inference', 
                         return_value={
                             'request_id': 'test_123',
...
class TestIntegrationScenarios:
    ...
    async def test_phase_2_completion_validation(self):
        ...
        try:
            from infrastructure.distributed_inference.core.distributed_inference_manager import DistributedInferenceManager
            from infrastructure.distributed_inference.utils.node_discovery import NodeDiscoveryService
            from infrastructure.distributed_inference.api.distributed_inference_endpoints import InferenceRequest
            from infrastructure.distributed_inference.integration.unified_gateway_integration import UnifiedGatewayIntegration
