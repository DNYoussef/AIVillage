...
class PerformanceBenchmarks(unittest.TestCase):
        ...
        for data_size in data_sizes:
            for metric_count in metric_counts:
                ...
                for report_format in formats:
                    ...
                    def generate_report():
                        return analytics.generate_analytics_report(report_format=report_format, include_trends=True)

                    perf = self.measure_performance(generate_report)
                    report = perf["result"]

                    print(
                        f"  {report_format:8s}: {perf['execution_time']:.4f}s, "
        ...
        for num_workers, records_per_worker in worker_configs:
            ...
            def run_concurrent_test():
                with ThreadPoolExecutor(max_workers=num_workers) as executor:
                    futures = [
                        executor.submit(record_metrics_worker, i, records_per_worker) for i in range(num_workers)
                    ]
    ...
    def test_memory_efficiency_with_retention(self):
        ...
        print(
            f"  Memory reduction: {((perf_no_retention['memory_delta'] - perf_with_retention['memory_delta']) / perf_no_retention['memory_delta'] * 100):.1f}%"
        )
    ...
    def test_time_window_analysis_performance(self):
        ...
        for window_name, window in time_windows:
            ...
            def analyze_time_window():
                return analytics.generate_analytics_report(time_window=window, include_trends=True)
    ...
    def test_stress_test_extreme_scenarios(self):
        ...
        extreme_values = [float("inf"), float("-inf"), 1e308, -1e308, 1e-308, -1e-308, 0.0, -0.0]
        ...
        def test_extreme_values():
            for i, value in enumerate(extreme_values * 1000):
                try:
                    ...
                    logging.debug(
                        f"Extreme value not recordable: {value}, error: {e}"
                    )  # nosec S110 - testing extreme values
    ...
    def test_file_system_stress(self):
        ...
        for format_type in formats:
            ...
            for cycle in range(20):
                file_path = (
                    Path(self.test_dir)
                    / f"stress_{format_type}_{cycle}.{format_type if format_type != 'sqlite' else 'db'}"
                )
