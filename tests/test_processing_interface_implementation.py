...
class TestEnhancedProcessingInterface:
    ...
    async def test_progress_tracking(self, initialized_processor):
        ...
        task = asyncio.create_task(
            initialized_processor.process("test progress", context)
        )
    ...
    async def test_task_cancellation(self, initialized_processor):
        ...
        # Start processing
        task = asyncio.create_task(
            initialized_processor.process("test cancel", context)
        )
    ...
    async def test_concurrent_processing(self, initialized_processor):
        ...
        contexts = [
            ProcessingContext(request_id=f"concurrent_{i}")
            for i in range(5)
        ]
        
        # Process multiple tasks concurrently
        tasks = [
            initialized_processor.process(f"test {i}", contexts[i])
            for i in range(5)
        ]
...
async def test_processing_interface_integration():
    ...
    try:
        ...
        contexts = [
            ProcessingContext(request_id=f"integration_{i}")
            for i in range(3)
        ]
        
        tasks = [
            processor.process(f"Integration test {i}", contexts[i])
            for i in range(3)
        ]
