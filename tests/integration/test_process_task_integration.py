...
class TestProcessTaskIntegration:
    ...
    async def real_agent(self):
        ...
        config.capabilities = ["text_generation", "question_answering", "data_analysis", "code_generation"]
        config.model = "gpt-4"
        ...
        agent.rag_pipeline.process_query = AsyncMock(
            return_value={"answer": "RAG answer", "sources": ["source1", "source2"], "confidence": 0.85}
        )
    ...
    async def test_end_to_end_text_generation(self, real_agent):
        ...
        task = IntegrationTestTask(
            content="Write a short story about AI agents collaborating", type="text_generation", id="e2e_text_gen"
        )
        ...
        assert set(metadata["steps_completed"]) == {"validation", "routing", "processing", "formatting"}
    ...
    async def test_end_to_end_question_answering(self, real_agent):
        ...
        task = IntegrationTestTask(
            content="What are the benefits of distributed AI systems?", type="question_answering", id="e2e_qa"
        )
    ...
    async def test_workflow_with_large_content(self, real_agent):
        ...
        task = IntegrationTestTask(content=large_content, type="summarization", id="large_content_test")
    ...
    async def test_workflow_error_recovery(self, real_agent):
        ...
        task = IntegrationTestTask(content="Test error recovery", type="text_generation")
    ...
    async def test_concurrent_task_processing_integration(self, real_agent):
        ...
        tasks = [
            IntegrationTestTask(content=f"Analyze data set {i}", type="data_analysis", id=f"concurrent_{i}")
            for i in range(3)
        ]

        # Process all tasks concurrently
        results = await asyncio.gather(*[real_agent._process_task(task) for task in tasks])
    ...
    async def test_performance_under_load(self, real_agent):
        ...
        avg_processing_time = sum(result["metadata"]["processing_time_ms"] for result in results) / len(results)
    ...
    async def test_memory_usage_stability(self, real_agent):
        ...
        initial_results = []
        for i in range(5):
            task = IntegrationTestTask(content=f"Memory test task {i}", type="general", id=f"memory_{i}")
            result = await real_agent._process_task(task)
...
class TestPerformanceBenchmarks:
    ...
    async def test_100ms_performance_target(self, benchmark_agent):
        ...
        simple_task = IntegrationTestTask(content="Hello", type="general", id="perf_test")
    ...
    async def test_throughput_benchmark(self, benchmark_agent):
        ...
        num_tasks = 20
        tasks = [
            IntegrationTestTask(content=f"Throughput test {i}", type="general", id=f"throughput_{i}")
            for i in range(num_tasks)
        ]

        start_time = time.perf_counter()
        results = await asyncio.gather(*[benchmark_agent._process_task(task) for task in tasks])
        end_time = time.perf_counter()
    ...
    async def test_memory_efficiency_benchmark(self, benchmark_agent):
        ...
        for i in range(10):
            task = IntegrationTestTask(content=f"Memory efficiency test {i}", type="general")
