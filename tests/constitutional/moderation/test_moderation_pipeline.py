...
from infrastructure.constitutional.moderation.pipeline import (
    ConstitutionalModerationPipeline, ModerationDecision, ContentAnalysis
)
from infrastructure.constitutional.moderation.policy_enforcement import PolicyDecision
from infrastructure.constitutional.moderation.response_actions import ActionType
...
class TestConstitutionalModerationPipeline:
    ...
    async def test_h0_constitutional_content_protection(self, pipeline):
        ...
        with patch.object(pipeline.harm_classifier, 'classify_content') as mock_classify:
            ...
            result = await pipeline.process_content(
                content="Political speech about democracy",
                content_type="text",
                user_tier="Gold"
            )
    ...
    async def test_h1_warning_with_constitutional_safeguards(self, pipeline):
        ...
        with patch.object(pipeline.harm_classifier, 'classify_content') as mock_classify:
            mock_classify.return_value = Mock(
                harm_level="H1",
                categories=["minor_concern"],
                confidence=0.7,
                metadata={}
            )
            
            result = await pipeline.process_content(
                content="Mildly concerning content",
                content_type="text",
                user_tier="Silver"
            )
    ...
    async def test_h2_restriction_with_due_process(self, pipeline):
        ...
        with patch.object(pipeline.harm_classifier, 'classify_content') as mock_classify:
            mock_classify.return_value = Mock(
                harm_level="H2",
                categories=["moderate_harm"],
                confidence=0.85,
                metadata={}
            )
            
            result = await pipeline.process_content(
                content="Moderately harmful content",
                content_type="text",
                user_tier="Gold"
            )
    ...
    async def test_h3_severe_harm_escalation_gold_tier(self, pipeline):
        ...
        with patch.object(pipeline.harm_classifier, 'classify_content') as mock_classify:
            mock_classify.return_value = Mock(
                harm_level="H3",
                categories=["severe_harm"],
                confidence=0.9,
                metadata={}
            )
            
            result = await pipeline.process_content(
                content="Severely harmful content",
                content_type="text",
                user_tier="Gold"
            )
    ...
    async def test_first_amendment_protected_speech_handling(self, pipeline):
        ...
        with patch.object(pipeline.harm_classifier, 'classify_content') as mock_classify:
            ...
            result = await pipeline.process_content(
                content="Protected political and religious speech",
                content_type="text",
                user_tier="Gold"
            )
    ...
    async def test_constitutional_error_handling(self, pipeline):
        ...
        with patch.object(pipeline.harm_classifier, 'classify_content') as mock_classify:
            ...
            result = await pipeline.process_content(
                content="Content during system error",
                content_type="text",
                user_tier="Gold"
            )
        ...
        for harm_level, decision, tier, should_be_eligible in test_cases:
            with patch.object(pipeline.harm_classifier, 'classify_content') as mock_classify:
                ...
                with patch.object(pipeline.policy_enforcement, 'evaluate_content') as mock_policy:
                    ...
                    result = await pipeline.process_content(
                        f"Test content for {harm_level} {decision}",
                        "text",
                        tier
                    )
                    
                    assert result.appeal_eligible == should_be_eligible, \
...
class TestConstitutionalComplianceValidation:
    ...
    async def test_first_amendment_compliance(self):
        ...
        first_amendment_scenarios = [
            ...
            {
                "content": "True threat of violence",
                "categories": ["true_threats"],
    ...
    async def test_due_process_compliance(self):
        ...
        with patch.object(pipeline.harm_classifier, 'classify_content') as mock_classify:
            ...
            result = await pipeline.process_content(
                "Content requiring due process protections",
                "text",
                "Gold"
            )
            ...
            # Appeal rights
            if result.decision in [ModerationDecision.RESTRICT, ModerationDecision.QUARANTINE, ModerationDecision.BLOCK]:
                assert result.appeal_eligible
