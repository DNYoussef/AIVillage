...
class ConstitutionalTestRunner:
    ...
    def _run_test_category(self, category: str, config: Dict, args: argparse.Namespace) -> Dict[str, Any]:
        ...
        # Add pytest options
        cmd.extend(["--verbose", "--tb=short", f'--timeout={config["timeout"]}', "--color=yes", "--disable-warnings"])
        ...
        try:
            ...
            if test_result["status"] == "passed":
                ...
                if "tests_failed" in test_result:
                    print(
                        f"   📊 {test_result['tests_failed']} tests failed, {test_result.get('tests_passed', 0)} passed"
                    )
    ...
    def _generate_final_report(self, test_results: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
        ...
        for category, result in test_results.items():
            total_tests += (
                result.get("tests_passed", 0) + result.get("tests_failed", 0) + result.get("tests_skipped", 0)
            )
        ...
        report = {
            "summary": {
                ...
                "overall_status": (
                    "PASSED" if categories_failed == 0 and total_errors == 0 and total_timeouts == 0 else "FAILED"
                ),
...
Examples:
  ...
  python run_tests.py --output-file results/test_report.json --junit-xml
    ...
    parser.add_argument("--safety-validation-only", action="store_true", help="Run only safety validation tests")
    parser.add_argument("--harm-classification-only", action="store_true", help="Run only harm classification tests")
    parser.add_argument(
        "--constitutional-compliance-only", action="store_true", help="Run only constitutional compliance tests"
    )
    parser.add_argument("--tier-enforcement-only", action="store_true", help="Run only tier enforcement tests")
    parser.add_argument("--e2e-integration-only", action="store_true", help="Run only end-to-end integration tests")
    parser.add_argument(
        "--performance-benchmarks-only", action="store_true", help="Run only performance benchmark tests"
    )
    parser.add_argument("--adversarial-testing-only", action="store_true", help="Run only adversarial testing tests")

    # Execution options
    parser.add_argument(
        "--parallel", action="store_true", default=True, help="Enable parallel test execution (default: True)"
    )
    ...
    parser.add_argument("--fail-fast", action="store_true", help="Stop on first test category failure")

    # Coverage options
    parser.add_argument("--coverage", action="store_true", help="Enable code coverage reporting")
    parser.add_argument(
    ...
    parser.add_argument("--verbose", action="store_true", help="Verbose output with detailed error information")

    # Performance options
    parser.add_argument("--profile", action="store_true", help="Enable performance profiling")
    parser.add_argument(
        "--parallel-performance", action="store_true", help="Enable parallel execution for performance tests"
    )

    # Advanced options
    parser.add_argument(
        "--ignore-markers", action="store_true", help="Ignore pytest markers and run all tests in files"
    )
    parser.add_argument(
        "--timeout-multiplier", type=float, default=1.0, help="Multiply all timeouts by this factor (default: 1.0)"
    )
