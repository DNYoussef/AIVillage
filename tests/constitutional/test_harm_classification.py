...
try:
    from core.constitutional.harm_classifier import (
        ConstitutionalHarmClassifier, 
        HarmLevel, 
        HarmCategory,
        ClassificationResult
    )
    from core.constitutional.ml_models import (
        HarmDetectionModel,
        BiasDetectionModel, 
        ConstitutionalPrincipleClassifier
    )
except ImportError:
...
class TestConstitutionalHarmClassification:
    ...
    async def test_harm_category_detection_accuracy(self, classifier_tester, test_dataset):
        ...
        if category_total > 0:
            ...
            assert category_accuracy >= 0.85, (
                f"Category detection accuracy {category_accuracy:.3f} below required 85%"
            )
    ...
    async def test_bias_detection_and_fairness(self, classifier_tester, bias_test_dataset):
        ...
        for bias_test in bias_test_dataset:
            ...
            for content_variant in bias_test.content_variants:
                ...
                result = await classifier_tester.harm_classifier.classify_harm(
                    content_variant, {"bias_test": True}
                )
            ...
            if bias_test.expected_consistent_classification:
                ...
                assert len(unique_classifications) == 1, (
                    f"Inconsistent classifications for {bias_test.bias_type} bias test: "
                    f"{unique_classifications}"
                )
        ...
        for test_case in protected_speech_cases:
            classifier_tester.constitutional_classifier.analyze_constitutional_protections = AsyncMock(
                return_value={
                    ...
                    'protection_strength': 'strong' if 'free_speech' in test_case.constitutional_considerations else 'moderate',
            ...
            assert result['protected'], (
                f"Protected speech not recognized: {test_case.content[:50]}"
            )
    ...
    async def test_classification_performance_benchmarks(self, classifier_tester):
        ...
        assert max_processing_time < 200, (
            f"Max processing time {max_processing_time:.2f}ms exceeds 200ms requirement"
        )
    ...
    def test_harm_category_completeness(self):
        ...
        expected_categories = [
            ...
            "harassment", "misinformation", "dangerous_activities",
            "illegal_activities", "privacy_violation", "copyright_violation",
            "spam", "deceptive_content", "political_manipulation",
            "financial_fraud", "medical_misinformation", "extremism",
            "cyberbullying", "child_safety", "terrorism", "drug_abuse",
            "gambling", "weapons", "graphic_content", "discrimination",
            "doxxing", "identity_theft", "phishing"
        ]
        ...
        assert len(missing_categories) == 0, (
            f"Missing harm categories: {missing_categories}"
        )
    ...
    async def test_human_review_triggering(self, classifier_tester, test_dataset):
        ...
        for test_case in human_review_cases:
            ...
            assert result['human_review_required'], (
                f"Human review not triggered for case: {test_case.content[:50]}"
            )
            ...
            assert result['review_priority'] == expected_priority, (
                f"Incorrect review priority. Expected: {expected_priority}, "
                f"Got: {result['review_priority']}"
            )
    ...
    async def test_cultural_sensitivity_handling(self, classifier_tester, test_dataset):
        ...
        culturally_sensitive_cases = [
            case for case in test_dataset if case.cultural_sensitivity_required
        ]
        ...
        for test_case in culturally_sensitive_cases:
            ...
            result = await classifier_tester.harm_classifier.classify_harm(
                test_case.content, 
                {**test_case.context, "cultural_analysis": True}
            )
            
            assert result['cultural_sensitivity_detected'], (
                f"Cultural sensitivity not detected: {test_case.content[:50]}"
            )
            assert result['cultural_context_considered'], (
                "Cultural context not considered in classification"
            )
...
class TestHarmClassificationStress:
    ...
    async def test_concurrent_classification_load(self):
        ...
        tasks = [
            classifier_tester.harm_classifier.classify_harm(content, {})
            for content in test_contents
        ]
