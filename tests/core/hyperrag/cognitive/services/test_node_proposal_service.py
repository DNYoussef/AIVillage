...
class TestNodeProposalService:
    ...
    async def test_assess_connectivity_potential(self, node_proposal_service):
        ...
        high_score = await node_proposal_service._assess_connectivity_potential(
            proposal, high_connectivity_gap
        )
        
        # Lower connectivity gap types  
        low_connectivity_gap = DetectedGap(gap_type=GapType.CONFLICTING_INFO)
        low_score = await node_proposal_service._assess_connectivity_potential(
            proposal, low_connectivity_gap
        )
    ...
    async def test_rank_proposals(self, node_proposal_service):
        ...
        scores = [
            p.utility_score * p.confidence * p.existence_probability 
            for p in ranked
        ]
