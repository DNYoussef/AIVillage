...
class TestGapDetectionService:
    ...
    async def test_deduplicate_gaps(self, gap_detection_service):
        ...
        gap1 = DetectedGap(gap_type=GapType.MISSING_NODE, source_nodes=["node1", "node2"], description="Test gap")
        gap2 = DetectedGap(
        ...
        gap3 = DetectedGap(
            gap_type=GapType.MISSING_RELATIONSHIP, source_nodes=["node1", "node2"], description="Different type"
        )
    ...
    @pytest.mark.parametrize(
        "method", ["structural_analysis", "semantic_clustering", "connectivity_analysis", "trust_inconsistency"]
    )
    async def test_detection_methods(self, gap_detection_service, method):
        """Test individual detection methods."""
        gaps = await gap_detection_service._run_detection_method(method, "test query", [], "test area")
