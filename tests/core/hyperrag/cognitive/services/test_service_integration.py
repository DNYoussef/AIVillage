...
class TestServiceIntegration:
    ...
    async def test_end_to_end_gap_detection_and_proposal(self, all_services):
        ...
        # Step 1: Detect gaps
        gaps = await gap_service.detect_gaps(query="artificial intelligence neural networks", focus_area="AI")
        ...
        for proposal in node_proposals + rel_proposals:
            ...
            if matching_gaps:
                ...
                confidence = await confidence_service.calculate_confidence(proposal, gap, evidence)
    ...
    async def test_confidence_calculation_integration(self, all_services):
        ...
        for proposal in node_proposals:
            gap = gaps[0]
            confidence = await confidence_service.calculate_confidence(proposal, gap, gap.evidence)
        ...
        for proposal in rel_proposals:
            gap = gaps[0]
            confidence = await confidence_service.calculate_confidence(proposal, gap, proposal.evidence_sources)
    ...
    async def test_service_statistics_aggregation(self, all_services):
        ...
        gaps = [
            DetectedGap(id=str(uuid.uuid4()), gap_type=GapType.MISSING_NODE, source_nodes=["ai_node"], confidence=0.8)
        ]
    ...
    async def test_error_propagation_and_recovery(self, integration_config):
        ...
        bad_config = ServiceConfig(
            trust_graph=None, vector_engine=None, min_confidence_threshold=0.3  # Missing required dependency
        )
    ...
    async def test_facade_service_coordination(self, integrated_facade):
        ...
        result = await facade.perform_comprehensive_analysis(query="artificial intelligence", focus_area="AI")
