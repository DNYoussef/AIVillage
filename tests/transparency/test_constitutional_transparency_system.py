...
from infrastructure.transparency.merkle_audit import ConstitutionalMerkleAudit
from infrastructure.transparency.constitutional_logging import ConstitutionalDecisionLogger
from infrastructure.transparency.privacy_preserving_audit import PrivacyPreservingAuditSystem
from infrastructure.transparency.governance_audit import GovernanceAuditTrail
from infrastructure.transparency.public_dashboard import PublicAccountabilityDashboard
from infrastructure.transparency.cryptographic_verification import (
    ConstitutionalCryptographicVerifier, 
    VerificationLevel
)
...
class TestTierBasedTransparency:
    ...
    async def test_bronze_tier_full_transparency(self, transparency_system, sample_decision_data):
        ...
        merkle_entry = next(
            (e for e in transparency_system.merkle_audit.audit_entries 
        ...
        public_summary = transparency_system.decision_logger.get_public_decision_summary(decision_ids['decision_log_id'])
...
class TestCryptographicVerification:
    ...
    async def test_integrity_proofs(self, transparency_system, sample_decision_data):
        ...
        for level in verification_levels:
            ...
            proof_id = await transparency_system.crypto_verifier.generate_integrity_proof(
                sample_decision_data,
                level
            )
    ...
    async def test_audit_chains(self, transparency_system, sample_decision_data):
        ...
        new_link_hash = await transparency_system.crypto_verifier.add_to_audit_chain(
            chain_id, 
            additional_data
        )
...
class TestDemocraticGovernance:
    ...
    async def test_participant_registration(self, transparency_system):
        ...
        for i, (tier, level) in enumerate(zip(tiers, participation_levels)):
            participant_id = await transparency_system.register_democratic_participant(
                f'participant_{i}',
                tier,
                level
            )
...
class TestRealTimeCompliance:
    ...
    async def test_system_health_snapshot(self, transparency_system, sample_decision_data):
        ...
        expected_components = ['merkle_audit', 'decision_logger', 'privacy_system', 'governance_audit', 'crypto_verifier']
