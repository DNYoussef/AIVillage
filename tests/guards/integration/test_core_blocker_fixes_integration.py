...
from src.core.blockers_fix import (
    CoreBlockerResolver,
    BlockerStatus,
    BlockerResult
)
...
'''
    ...
    def test_grokfast_dependency_resolution_integration(self, resolver, temp_project_root):
        ...
        assert result.status == BlockerStatus.RESOLVED, \
            f"GrokFast resolution failed: {result.error}"
    ...
    def test_rag_accuracy_resolution_integration(self, resolver, temp_project_root):
        ...
        assert result.status in [BlockerStatus.RESOLVED, BlockerStatus.FAILED], \
        ...
        if result.status == BlockerStatus.RESOLVED:
            ...
            created_structures = [
                vector_dir.exists(),
                kb_dir.exists(),
                (config_dir / "rag_config.json").exists()
            ]
    ...
    def test_p2p_protocol_mismatch_resolution_integration(self, resolver, temp_project_root):
        ...
        assert result.status == BlockerStatus.RESOLVED, \
            f"P2P resolution failed: {result.error}"
    ...
    def test_import_path_conflicts_resolution_integration(self, resolver, temp_project_root):
        ...
        (temp_project_root / "core" / "agent-forge").mkdir(parents=True, exist_ok=True)
    ...
    def test_complete_blocker_resolution_workflow(self, resolver):
        ...
        assert result_blockers == expected_blockers, \
            f"Missing blockers: {expected_blockers - result_blockers}"
    ...
    def test_summary_report_generation_integration(self, resolver):
        ...
        assert summary["resolved_count"] + summary["failed_count"] == summary["total_blockers"], \
            "Counts don't add up"
    ...
    def test_error_handling_in_resolution_workflow(self, temp_project_root):
        ...
        for result in results:
            ...
            assert result.blocker_name in ["grokfast_dependency", "rag_accuracy", 
                                          "p2p_protocol_mismatch", "import_path_conflicts"]
        ...
        for r1, r2 in zip(results1, results2):
            if r1.blocker_name == r2.blocker_name:
                ...
                if r1.status == BlockerStatus.RESOLVED:
                    assert r2.status == BlockerStatus.RESOLVED, \
                        f"Resolution regressed for {r1.blocker_name}"
...
class TestCoreBlockerBehavioralContracts:
    ...
    def test_resolution_completeness_contract(self, resolver):
        ...
        assert resolved_blockers == expected_blockers, \
            f"Missing blockers: {expected_blockers - resolved_blockers}"
