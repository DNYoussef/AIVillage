...
from src.security.boundaries.secure_boundary_contracts import SecurityBoundaryFactory
...
class TestLocalhostAdminSecurityRegression:
    ...
    def security_boundary_factory(self, mock_security_services):
        ...
        return SecurityBoundaryFactory(auth_service, authz_service, audit_service, threat_service)
    ...
    def test_server_never_binds_to_all_interfaces(self, mock_security_services):
        ...
        auth_service, authz_service, audit_service, threat_service = mock_security_services
        factory = SecurityBoundaryFactory(auth_service, authz_service, audit_service, threat_service)

        # Should raise exception for wildcard binding
        with pytest.raises(Exception) as exc_info:
            SecureAdminServer(
                security_boundary_factory=factory, bind_interface="0.0.0.0", port=3006  # Dangerous wildcard
            )

        assert "all interfaces" in str(exc_info.value).lower(), "Should prevent wildcard interface binding"
    ...
    def test_session_manager_enforces_security_constraints(self):
        ...
        session_id = session_manager.create_session(user_id="admin_user", user_roles={"admin"}, client_ip="127.0.0.1")
    ...
    def test_session_manager_limits_concurrent_sessions(self):
        ...
        for session_id in session_ids:
            assert session_manager.validate_session(session_id, client_ip) is not None, "Session should be valid"
    ...
    def test_rate_limiting_prevents_brute_force_attacks(self):
        ...
        now = datetime.utcnow()
        middleware.blocked_attempts[client_ip] = [now - timedelta(minutes=20)]  # Old attempt
...
class TestLocalhostAdminBehavioralContracts:
    ...
    def test_admin_server_never_accepts_external_connections(self, mock_security_services):
        ...
        for interface in valid_interfaces:
            server = SecureAdminServer(security_boundary_factory=factory, bind_interface=interface, port=3006)
            assert server.bind_interface in valid_interfaces, f"Invalid interface accepted: {server.bind_interface}"
        ...
        for interface in dangerous_interfaces:
            with pytest.raises(Exception):
                SecureAdminServer(security_boundary_factory=factory, bind_interface=interface, port=3006)
        ...
        for session_id, client_ip in error_conditions:
            try:
                ...
                for term in sensitive_terms:
                    assert term not in error_msg, f"Error message contains sensitive term '{term}': {error_msg}"
