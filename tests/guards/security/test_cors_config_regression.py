...
class TestCORSConfigurationRegression:
    ...
    def test_admin_cors_more_restrictive_than_api(self):
        ...
        for env in ["production", "staging", "development"]:
            ...
            assert len(admin_origins) <= len(secure_origins), f"Admin CORS more permissive than API CORS in {env}"
    ...
    def test_cors_config_constants_security(self):
        ...
        assert len(admin_config["allow_methods"]) <= len(
            config["allow_methods"]
        ), "Admin CORS should be more restrictive"
        assert admin_config["max_age"] <= config["max_age"], "Admin CORS should have shorter cache time"

    @pytest.mark.parametrize(
        "env,expected_protocol", [("production", "https"), ("staging", "https"), ("development", "http")]
    )
    def test_environment_specific_protocols(self, env: str, expected_protocol: str):
        ...
        if env == "production":
            ...
            for origin in localhost_origins:
                assert origin.startswith("http://"), f"Localhost should use HTTP in development: {origin}"
    ...
    def test_websocket_cors_consistency(self):
        ...
        for env in ["production", "staging", "development"]:
            ...
            assert secure_origins == websocket_origins, f"WebSocket CORS mismatch with secure CORS in {env}"
...
class TestCORSBehavioralContracts:
    ...
    def test_cors_origin_format_validation(self):
        ...
        localhost_pattern = re.compile(r"^https?://(localhost|127\.0\.0\.1)(:\d+)?/?$", re.IGNORECASE)

        for env in ["production", "staging", "development"]:
            os.environ["AIVILLAGE_ENV"] = env

            all_origins = get_secure_cors_origins() + get_admin_cors_origins() + get_websocket_cors_origins()

            for origin in all_origins:
                is_valid = url_pattern.match(origin) or localhost_pattern.match(origin)
