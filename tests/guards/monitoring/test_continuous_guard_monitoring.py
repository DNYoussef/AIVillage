...
class GuardTestMonitor:
    ...
    def _check_for_regressions(self, result: GuardTestResult):
        ...
        if result.status == GuardTestStatus.FAILED:
            ...
            recent_failures = self._count_recent_failures(result.test_name, timedelta(hours=1))
    ...
    def _check_sustained_failures(self):
        ...
        recent_results = [r for r in self.test_results if r.timestamp > datetime.now() - timedelta(hours=24)]

        if not recent_results:
            return

        failure_rate = len([r for r in recent_results if r.status == GuardTestStatus.FAILED]) / len(recent_results)
        ...
        if failure_rate > self.performance_baselines["max_failure_rate"]:
            alert = GuardAlert(
                ...
                details={
                    ...
                    "failed_tests": len([r for r in recent_results if r.status == GuardTestStatus.FAILED]),
                },
            )
            self._raise_alert(alert)
    ...
    def _check_coverage_regressions(self):
        ...
        recent_results = [
            r for r in self.test_results if r.timestamp > datetime.now() - timedelta(hours=24) and r.coverage_data
        ]
    ...
    def _count_recent_failures(self, test_name: str, time_window: timedelta) -> int:
        ...
        return len(
            [
                ...
                if r.test_name == test_name and r.timestamp > cutoff_time and r.status == GuardTestStatus.FAILED
            ]
        )
    ...
    def _calculate_average_coverage(self, results: List[GuardTestResult]) -> Dict[str, float]:
        ...
        return {
            metric: coverage_sums[metric] / coverage_counts[metric]
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_execution_time": (
                sum(r.execution_time for r in recent_results) / len(recent_results) if recent_results else 0
            ),
...
class TestContinuousGuardMonitoring:
    ...
    def test_test_result_recording(self, monitor):
        ...
        result = GuardTestResult(test_name="test_example", status=GuardTestStatus.PASSED, execution_time=1.5)
    ...
    def test_regression_detection(self, monitor):
        ...
        for i in range(3):
            result = GuardTestResult(
                test_name="test_failing", status=GuardTestStatus.FAILED, execution_time=1.0, error_message=f"Error {i}"
            )
            monitor.record_test_result(result)
    ...
    def test_performance_threshold_detection(self, monitor):
        ...
        result = GuardTestResult(
            test_name="test_slow", status=GuardTestStatus.PASSED, execution_time=35.0  # Exceeds 30s threshold
        )
    ...
    def test_alert_callback_system(self, monitor):
        ...
        # Trigger an alert
        result = GuardTestResult(test_name="test_callback", status=GuardTestStatus.FAILED, execution_time=1.0)
    ...
    def test_monitoring_status_report(self, monitor):
        ...
        for i in range(5):
            ...
            result = GuardTestResult(test_name=f"test_{i}", status=status, execution_time=1.0 + i * 0.5)
            monitor.record_test_result(result)
    ...
    def test_alert_acknowledgment(self, monitor):
        ...
        # Create an alert
        alert = GuardAlert(
            alert_id="test_alert_123", level=AlertLevel.HIGH, category="test", message="Test alert", details={}
        )
    ...
    def test_metrics_export(self, monitor):
        ...
        # Add some data
        result = GuardTestResult(test_name="test_export", status=GuardTestStatus.PASSED, execution_time=2.0)
        monitor.record_test_result(result)
