...
async def test_navigator_mobile_integration():
    ...
    print(
        f"    [DEBUG] Battery level: {constraints.battery_level}, Active constraints: {[c.value for c in constraints.active_constraints]}"
    )
    ...
    print(
        f"    [PASS] Primary: {decision.primary_transport.transport_type}, Confidence: {decision.confidence_score:.2f}"
    )
