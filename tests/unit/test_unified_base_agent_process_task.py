...
class TestUnifiedBaseAgentProcessTask:
    ...
    def sample_task(self):
        ...
        return MockTask(
            content="Generate a summary of machine learning algorithms",
    ...
    async def test_process_task_content_sanitization(self, mock_agent, sample_task):
        ...
        task = MockTask(
            content="<script>alert('xss')</script>Normal content here",
            type="text_generation"
        )
    ...
    async def test_progress_tracking_stages(self, mock_agent, sample_task):
        ...
        assert result["metadata"]["steps_completed"] == [
    ...
    async def test_task_routing_capability_matching(self, mock_agent):
        ...
        task = MockTask(
            ...
            type="custom_data_analysis"  # Not in handlers but matches capability
        )
    ...
    async def test_agent_communication_handler_with_recipient(self, mock_agent):
        ...
        task = MockTask(
            content="Send message to: OtherAgent\nHello there!",
    ...
    async def test_capabilities_used_detection(self, mock_agent):
        ...
        task = MockTask(
            content="Please do some text_generation and data_analysis",
            type="general"
        )
    ...
    async def test_no_specific_capabilities_used(self, mock_agent):
        ...
        task = MockTask(
            content="Generic task with no specific capability keywords",
            type="general"
        )
    ...
    async def test_concurrent_task_processing(self, mock_agent):
        ...
        tasks = [
            MockTask(content=f"Task {i}", id=f"task_{i}")
            for i in range(3)
        ]
        
        # Process tasks concurrently
        results = await asyncio.gather(*[
            mock_agent._process_task(task) for task in tasks
        ])
