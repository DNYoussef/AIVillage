...
from tests.security.unit.test_vulnerability_reporting import (
    VulnerabilityReportingWorkflow,
    VulnerabilityReport,
...
class SecurityWorkflowOrchestrator:
    ...
    def execute_vulnerability_to_resolution_workflow(self, vulnerability_report: VulnerabilityReport) -> Dict[str, Any]:
        ...
        try:
            ...
            workflow_result["stages_completed"].append(
                {"stage": "report_processing", "status": "completed", "result": report_result}
            )
            ...
            sbom_content = self.sbom_generator.generate_sbom(
                {"name": f"Security-Update-{workflow_id}", "vulnerability_context": True}
            )
            ...
            if vulnerability_report.severity == VulnerabilitySeverity.CRITICAL:
                ...
                workflow_result["stages_completed"].append(
                    {"stage": "security_review", "status": "completed", "result": review_result}
                )
            ...
            self._audit_log(
                "workflow_failed",
                {"workflow_id": workflow_id, "error": str(e), "stage": workflow_result["current_stage"]},
            )
    ...
    def _require_admin_approval(self, workflow_id: str, vulnerability_report: VulnerabilityReport) -> Dict[str, Any]:
    ...
    def execute_dependency_to_sbom_workflow(self, ecosystems: List[DependencyEcosystem]) -> Dict[str, Any]:
        ...
        try:
            ...
            for ecosystem in ecosystems:
                scan_result = self.dependency_scanner.scan_ecosystem(ecosystem, f"{ecosystem.value}_dependencies")
            ...
            self._audit_log(
                "dependency_sbom_workflow_completed",
                {
                    "workflow_id": workflow_id,
    ...
    def _convert_scan_to_components(
        self, scan_result: Dict[str, Any], ecosystem: DependencyEcosystem
    ) -> List[SBOMComponent]:
        ...
        mock_dependencies = {
            ...
            DependencyEcosystem.JAVASCRIPT: [("express", "4.18.1", "MIT"), ("lodash", "4.17.21", "MIT")],
        }
    ...
    def _audit_log(self, event_type: str, event_data: Dict[str, Any]):
        ...
        log_entry = {"timestamp": datetime.utcnow().isoformat(), "event_type": event_type, "data": event_data}
...
class SecurityWorkflowIntegrationTest(unittest.TestCase):
    ...
    def test_vulnerability_to_resolution_workflow_integration(self):
        ...
        self.assertEqual(
            workflow_result["overall_status"], "completed", "Critical vulnerability workflow must complete successfully"
        )
        self.assertIn("workflow_id", workflow_result, "Workflow must have unique identifier")
        ...
        for expected_stage in expected_stages:
            self.assertIn(expected_stage, completed_stage_names, f"Workflow must complete {expected_stage} stage")

        # Verify critical vulnerability requires security review
        security_review_stage = next(stage for stage in stages_completed if stage["stage"] == "security_review")
        self.assertEqual(
            security_review_stage["status"],
            "completed",
            "Security review must be completed for critical vulnerabilities",
        )

        # Verify SBOM generation integration
        sbom_stage = next(stage for stage in stages_completed if stage["stage"] == "sbom_generation")
        self.assertTrue(sbom_stage["result"]["sbom_signed"], "SBOM must be cryptographically signed")
    ...
    def test_dependency_scanning_triggers_sbom_update(self):
        ...
        self.assertEqual(
            workflow_result["overall_status"], "completed", "Dependency vulnerability workflow must complete"
        )

        stages = workflow_result["stages_completed"]
        stage_names = [stage["stage"] for stage in stages]

        self.assertIn("dependency_scanning", stage_names, "Dependency vulnerability must trigger scanning")
        self.assertIn("sbom_generation", stage_names, "Dependency scanning must trigger SBOM generation")

        # Verify dependency scan results are integrated
        dep_scan_stage = next(stage for stage in stages if stage["stage"] == "dependency_scanning")
        self.assertIn("vulnerabilities_found", dep_scan_stage["result"], "Dependency scan results must be captured")
        self.assertIn("risk_level", dep_scan_stage["result"], "Risk assessment must be included")
    ...
    def test_multi_ecosystem_dependency_to_sbom_workflow(self):
        ...
        self.assertEqual(
            workflow_result["overall_status"], "completed", "Multi-ecosystem workflow must complete successfully"
        )
        self.assertTrue(workflow_result["sbom_generated"], "Comprehensive SBOM must be generated")

        # Verify all ecosystems processed
        ecosystems_processed = workflow_result["ecosystems_processed"]
        self.assertEqual(len(ecosystems_processed), len(ecosystems), "All ecosystems must be processed")

        processed_ecosystem_names = [eco["ecosystem"] for eco in ecosystems_processed]
        for expected_ecosystem in ecosystems:
            self.assertIn(
                expected_ecosystem.value,
                processed_ecosystem_names,
                f"Ecosystem {expected_ecosystem.value} must be processed",
            )

        # Verify aggregated results
        self.assertGreater(workflow_result["total_dependencies"], 0, "Must aggregate dependencies across ecosystems")
        self.assertGreater(workflow_result["sbom_components"], 0, "SBOM must contain components from all ecosystems")
    ...
    def test_critical_vulnerability_admin_approval_integration(self):
        ...
        self.assertEqual(
            workflow_result["overall_status"], "completed", "Critical vulnerability with admin approval must complete"
        )
        ...
        self.assertIsNotNone(security_review_stage, "Critical vulnerability must have security review stage")

        # Verify admin approval details
        approval_result = security_review_stage["result"]
        self.assertTrue(approval_result["approval_required"], "Critical vulnerability must require approval")
        self.assertIn("approved_by", approval_result, "Approval must include approver identity")
        self.assertIn("approval_timestamp", approval_result, "Approval must be timestamped")
    ...
    def test_workflow_audit_logging_integration(self):
        ...
        self.workflow_orchestrator.execute_vulnerability_to_resolution_workflow(self.critical_vulnerability)

        self.workflow_orchestrator.execute_dependency_to_sbom_workflow([DependencyEcosystem.PYTHON])

        # Assert - Audit logging integration
        audit_log = self.workflow_orchestrator.integration_audit_log
        self.assertGreater(len(audit_log), 0, "Workflow operations must be audit logged")

        # Verify log entries for both workflows
        workflow_events = [log for log in audit_log if "workflow" in log["event_type"]]
        self.assertGreaterEqual(len(workflow_events), 2, "Both workflows must generate audit events")
    ...
    def test_workflow_state_tracking_and_retrieval(self):
        ...
        self.assertNotEqual(status.get("error"), "workflow_not_found", "Executed workflow must be trackable")
        self.assertEqual(status["workflow_id"], workflow_id, "Status must match workflow ID")
        self.assertIn("overall_status", status, "Status must include overall status")

        # Test non-existent workflow
        invalid_status = self.workflow_orchestrator.get_workflow_status("non-existent-id")
        self.assertEqual(invalid_status["error"], "workflow_not_found", "Non-existent workflow must return error")
    ...
    def test_workflow_error_handling_and_recovery(self):
        ...
        with patch.object(self.workflow_orchestrator.dependency_scanner, "scan_ecosystem") as mock_scan:
            ...
            self.assertEqual(
                workflow_result["overall_status"], "failed", "Workflow with errors must have failed status"
            )
            self.assertIn("error", workflow_result, "Failed workflow must include error information")
            self.assertIn("end_time", workflow_result, "Failed workflow must record completion time")

            # Verify error is audit logged
            audit_log = self.workflow_orchestrator.integration_audit_log
            error_events = [log for log in audit_log if "failed" in log["event_type"]]
            self.assertGreater(len(error_events), 0, "Workflow failures must be audit logged")
    ...
    def test_workflow_performance_and_scalability(self):
        ...
        self.assertLess(total_duration, 30.0, "5 workflow executions must complete within 30 seconds")

        # Verify all workflows completed successfully
        successful_workflows = [r for r in workflow_results if r["overall_status"] == "completed"]
        self.assertEqual(
            len(successful_workflows), len(test_vulnerabilities), "All test workflows must complete successfully"
        )

        # Verify average workflow time
        avg_time_per_workflow = total_duration / len(test_vulnerabilities)
        self.assertLess(avg_time_per_workflow, 10.0, "Average workflow execution time must be under 10 seconds")
