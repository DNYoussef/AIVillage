...
class TestMiddlewarePerformance:
    ...
    async def test_middleware_chain_execution_time(self):
        ...
        for _ in range(100):
            await self.server._security_middleware(request,
                lambda req: self.server._rate_limit_middleware(req,
                    lambda req: self.server._session_middleware(req,
                        lambda req: self.server._mfa_middleware(req, fast_handler))))
    ...
    def test_rate_limit_memory_efficiency(self):
        ...
        for key in list(self.server.rate_limits.keys()):
            self.server.rate_limits[key] = [
                timestamp for timestamp in self.server.rate_limits[key] 
                if now - timestamp < 60
            ]
...
class TestMemoryUsageOptimization:
    ...
    def test_rate_limit_memory_management(self):
        ...
        for i in range(1000):
            key = f"test_key_{i}"
            self.server.rate_limits[key] = [
                now - j for j in range(0, 120, 5)  # 24 timestamps per key
            ]
...
class TestConcurrentPerformance:
    ...
    async def test_concurrent_middleware_execution(self):
        ...
        for i in range(50):
            ...
            task = self.server._security_middleware(request,
                lambda req: self.server._rate_limit_middleware(req,
                    lambda req: self.server._session_middleware(req,
                        lambda req: self.server._mfa_middleware(req, mock_handler))))
    ...
    async def test_high_throughput_session_operations(self):
        ...
        self.server.session_manager.redis_client.hgetall.return_value = {
            "user_id": user_id,
            "is_active": "True"
        }
...
class TestPerformanceRegression:
    ...
    async def test_no_performance_regression_in_auth(self):
        ...
        max_token_validation_time = 0.005  # 5ms max per validation
