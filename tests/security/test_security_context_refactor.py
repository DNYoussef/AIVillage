...
from infrastructure.shared.security.redis_session_manager import DeviceInfo, SessionData
...
class TestSecurityContextExtraction:
    ...
    def test_suspicious_request_detection(self):
        ...
        for pattern in suspicious_patterns:
            ...
            is_suspicious = (
                not user_agent or  # Missing User-Agent
                "sqlmap" in user_agent.lower() or  # Security scanner
                "tor" in user_agent.lower() or  # Tor browser
                len(request.headers.get("X-Forwarded-For", "").split(",")) > 3  # Too many proxies
            )
...
class TestUserContextValidation:
    ...
    def test_mfa_verification_status(self):
        ...
        for scenario in test_scenarios:
            access_allowed = (
                not scenario["endpoint_requires_mfa"] or 
                scenario["mfa_verified"]
            )
...
class TestContextPropagation:
    ...
    async def test_context_propagation_through_middleware(self):
        ...
        # Chain execution
        await tracking_middleware_1(request, 
            lambda req: tracking_middleware_2(req, final_handler))
