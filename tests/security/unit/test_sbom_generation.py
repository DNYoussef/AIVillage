...
class SBOMGenerator:
    ...
    def __init__(self, format_type: SBOMFormat = SBOMFormat.SPDX, signer: CryptographicSigner = None):
    ...
    def sign_sbom(self, sbom_content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        signature_metadata = self.signer.sign_document(
            sbom_json, {"document_type": "SBOM", "format": self.format_type.value}
        )
...
class SBOMGenerationSecurityTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.signer = CryptographicSigner(
            private_key_path="/path/to/test/key.pem", algorithm="RSA-SHA256"  # pragma: allowlist secret
        )
        self.sbom_generator = SBOMGenerator(format_type=SBOMFormat.SPDX, signer=self.signer)
    ...
    def test_sbom_component_validation_and_integrity(self):
        ...
        self.assertEqual(len(self.sbom_generator.components), 1, "Valid component must be added successfully")

        added_component = self.sbom_generator.components[0]
        self.assertIsNotNone(added_component.component_id, "Component must have unique identifier")
        self.assertEqual(len(added_component.component_id), 16, "Component ID must be consistent length")

        # Test component validation
        with self.assertRaises(ValueError):
            invalid_component = SBOMComponent(
                name="", version="1.0.0", component_type=ComponentType.LIBRARY  # Invalid empty name
            )
    ...
    def test_sbom_generation_completeness_and_structure(self):
        ...
        for field in required_fields:
            self.assertIn(field, sbom_content, f"SBOM must include required field: {field}")

        # Test component completeness
        self.assertEqual(
            sbom_content["component_count"], len(self.test_components), "Component count must match added components"
        )
        self.assertEqual(
            len(sbom_content["components"]), len(self.test_components), "All components must be included in SBOM"
        )
    ...
    def test_cryptographic_signing_workflow(self):
        ...
        sbom_content = self.sbom_generator.generate_sbom({"name": "AIVillage-Signed-SBOM-Test"})

        # Act
        signed_sbom = self.sbom_generator.sign_sbom(sbom_content)

        # Assert - Test signing behavior
        self.assertTrue(signed_sbom["signed"], "SBOM must be marked as signed")
        self.assertIn("signature", signed_sbom, "Signed SBOM must include signature metadata")
        self.assertIn("sbom_content", signed_sbom, "Signed SBOM must preserve original content")
        ...
        for field in required_signature_fields:
            self.assertIn(field, signature, f"Signature must include field: {field}")

        # Test signature metadata
        self.assertEqual(signature["algorithm"], "RSA-SHA256", "Must use specified signature algorithm")
        self.assertIsNotNone(signature["content_hash"], "Must include content hash for integrity")
        self.assertEqual(signature["signer_identity"], "AIVillage-SBOM-Signer-v1.0", "Must identify signer")
    ...
    def test_signature_verification_integrity(self):
        ...
        self.assertTrue(verification_result["verified"], "Valid signature must verify successfully")
        self.assertIn("verification_timestamp", verification_result, "Must include verification timestamp")
        self.assertEqual(
            verification_result["signature_algorithm"], "RSA-SHA256", "Must preserve signature algorithm info"
        )
        self.assertTrue(verification_result["content_hash_match"], "Content hash must match")
        ...
        self.assertFalse(tampered_verification["verified"], "Tampered content must fail verification")
        self.assertIn("error", tampered_verification, "Failed verification must include error message")
    ...
    def test_component_relationship_generation(self):
        ...
        SBOMComponent(name="aivillage-app", version="1.0.0", component_type=ComponentType.APPLICATION)
        ...
        self.assertIsInstance(relationships, list, "Relationships must be provided as list")

        library_components = [c for c in self.test_components if c.component_type == ComponentType.LIBRARY]
        library_relationships = [r for r in relationships if r["relationship_type"] == "DEPENDS_ON"]

        self.assertGreaterEqual(
            len(library_relationships), len(library_components), "Must generate dependency relationships for libraries"
        )
        ...
        for relationship in library_relationships:
            ...
            for field in required_relationship_fields:
                self.assertIn(field, relationship, f"Relationship must include field: {field}")
    ...
    def test_vulnerability_reference_integration(self):
        ...
        self.assertIsInstance(vulnerability_refs, list, "Vulnerability references must be provided as list")
        ...
        if vulnerability_refs:
            ...
            for field in required_vuln_fields:
                self.assertIn(field, vuln_ref, f"Vulnerability reference must include field: {field}")
    ...
    def test_sbom_format_compliance(self):
        ...
        self.assertEqual(sbom_content["sbom_format"], "spdx", "Must specify SPDX format")
        self.assertEqual(sbom_content["spec_version"], "2.3", "Must specify supported spec version")
        ...
        for component_data in sbom_content["components"]:
            ...
            for field in required_component_fields:
                self.assertIn(field, component_data, f"Component must include SPDX field: {field}")

            # Test package URL format if present
            if "purl" in component_data and component_data["purl"]:
                self.assertTrue(component_data["purl"].startswith("pkg:"), "Package URL must follow PURL specification")
    ...
    def test_large_scale_sbom_generation(self):
        ...
        for i in range(100):
            component = SBOMComponent(
                name=f"test-component-{i}", version=f"1.0.{i}", component_type=ComponentType.LIBRARY, license_info="MIT"
            )
        ...
        self.assertEqual(sbom_content["component_count"], 100, "Must handle large component sets")
        self.assertLess(generation_time.total_seconds(), 5.0, "Generation must complete within reasonable time")

        # Test memory efficiency
        self.assertEqual(len(sbom_content["components"]), 100, "All components must be included")
        ...
        for algorithm in algorithms:
            with self.subTest(algorithm=algorithm):
                ...
                test_component = SBOMComponent(
                    name="test-component", version="1.0.0", component_type=ComponentType.LIBRARY
                )
                ...
                # Assert
                self.assertEqual(
                    signed_sbom["signature"]["algorithm"],
                    algorithm,
                    f"Must preserve {algorithm} algorithm specification",
                )

                verification_result = test_generator.verify_sbom_integrity(signed_sbom)
                self.assertTrue(verification_result["verified"], f"Must verify {algorithm} signatures correctly")
    ...
    def test_sbom_content_hash_consistency(self):
        ...
        components_set1 = [SBOMComponent("test-lib", "1.0.0", ComponentType.LIBRARY)]
        components_set2 = [SBOMComponent("test-lib", "1.0.0", ComponentType.LIBRARY)]
        ...
        self.assertEqual(hash1, hash2, "Identical SBOM content must produce identical hashes")

        signature1 = signed_sbom1["signature"]["signature"]
        signature2 = signed_sbom2["signature"]["signature"]

        self.assertEqual(signature1, signature2, "Identical content must produce identical signatures")
