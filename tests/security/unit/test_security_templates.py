...
class SecurityTemplateValidationTest(unittest.TestCase):
    ...
    def test_issue_template_security_integration_detection(self):
        ...
        self.assertTrue(
            result["has_security_integration"], "Template with security fields must be detected as integrated"
        )
        self.assertGreaterEqual(result["security_fields_count"], 2, "Must detect multiple security fields")
        ...
        self.assertGreaterEqual(
            result["compliance_score"], 70, "Well-integrated template must have high compliance score"
        )
    ...
    def test_issue_template_missing_security_integration(self):
        ...
        self.assertFalse(result["has_security_integration"], "Template without security fields must be flagged")
        self.assertEqual(result["security_fields_count"], 0, "Must detect zero security fields")
        self.assertLess(result["compliance_score"], 50, "Non-integrated template must have low compliance score")
    ...
    def test_pr_template_security_checklist_validation(self):
        ...
        self.assertTrue(result["has_security_integration"], "PR template must have security integration")
        self.assertEqual(result["security_checks_count"], 5, "Must detect all security checks")
        self.assertEqual(result["compliance_score"], 100, "Complete security checklist must score 100%")
        self.assertEqual(len(result["recommendations"]), 0, "Complete template should have no recommendations")
        ...
        for check in expected_checks:
            self.assertIn(check, result["security_checks"], f"Must detect security check: {check}")
    ...
    def test_pr_template_partial_security_integration(self):
        ...
        self.assertTrue(result["has_security_integration"], "Template with some security fields must be detected")
        self.assertEqual(result["security_checks_count"], 2, "Must detect existing security checks")
        self.assertLess(result["compliance_score"], 50, "Partial integration must have medium compliance score")
        self.assertGreater(len(result["recommendations"]), 0, "Must provide improvement recommendations")
    ...
    def test_security_automation_workflow_detection(self):
        ...
        self.assertEqual(len(result["active_automations"]), 6, "Must detect all security automations")
        self.assertEqual(result["automation_coverage"], 100, "Complete automation must have 100% coverage")
        self.assertEqual(len(result["missing_automations"]), 0, "Complete workflow should have no missing automations")
        ...
        for automation in expected_automations:
            self.assertIn(automation, result["active_automations"], f"Must detect automation: {automation}")
    ...
    def test_security_automation_gaps_identification(self):
        ...
        self.assertEqual(len(result["active_automations"]), 1, "Must detect only active automations")
        self.assertLess(result["automation_coverage"], 30, "Basic workflow must have low coverage")
        self.assertEqual(len(result["missing_automations"]), 5, "Must identify missing automations")

        expected_missing = ["dependency scanning", "sast", "dast", "container scanning", "vulnerability scanning"]
        for missing in expected_missing:
            self.assertIn(missing, result["missing_automations"], f"Must identify missing automation: {missing}")
    ...
    def test_security_review_requirement_detection(self):
        ...
        self.assertTrue(result["requires_security_review"], "Template indicating security review must be detected")
        self.assertGreaterEqual(result["compliance_score"], 90, "Security review template must have high compliance")
        self.assertIn("threat model", result["security_fields"])
        self.assertIn("security assessment needed", security_review_template.content.lower())
    ...
    def test_template_compliance_scoring_accuracy(self):
        ...
        test_cases = [
            ...
            (
               ...
               privacy impact analysis, requires security review""",
                (90, 100),
            ),
        ]
        ...
        for content, (min_score, max_score) in test_cases:
            with self.subTest(content=content[:50]):
                ...
                # Assert
                self.assertGreaterEqual(result["compliance_score"], min_score, f"Score must be >= {min_score}")
                self.assertLessEqual(result["compliance_score"], max_score, f"Score must be <= {max_score}")
    ...
    def test_security_template_metadata_validation(self):
        ...
        # Act
        self.integration.validate_issue_template(template_with_metadata)
