...
class TestFederatedAuthenticationSystem:
    ...
    @pytest.mark.asyncio
    async def test_register_node_coordinates_security_setup(self, auth_system, mock_bcrypt, mock_pyotp, mock_rsa):
    ...
    @pytest.mark.asyncio
    async def test_register_participant_node_uses_basic_auth(self, auth_system, mock_bcrypt, mock_pyotp, mock_rsa):
        """Test participant node registration uses appropriate security level."""
        # When registering a participant node
        identity = await auth_system.register_node(
            "participant_001", NodeRole.PARTICIPANT, "StrongPassword456!", enable_certificate_auth=False
        )
    ...
    def test_register_node_rejects_weak_passwords(self, auth_system):
        ...
        with pytest.raises(ValueError, match="Password does not meet security requirements"):
            asyncio.run(
                auth_system.register_node(
                    "weak_node", NodeRole.PARTICIPANT, "weak"  # Too short, no special chars, etc.
                )
            )
    ...
    def test_register_duplicate_node_raises_error(self, auth_system, mock_bcrypt, mock_rsa):
        ...
        asyncio.run(auth_system.register_node("existing_node", NodeRole.PARTICIPANT, "SecurePassword123!"))

        # When attempting to register same node ID again
        with pytest.raises(ValueError, match="Node existing_node already registered"):
            asyncio.run(auth_system.register_node("existing_node", NodeRole.COORDINATOR, "AnotherPassword456!"))

    @pytest.mark.asyncio
    async def test_authenticate_node_coordinates_multi_factor_verification(
        self, auth_system, mock_bcrypt, mock_pyotp, mock_rsa
    ):
        """Test node authentication coordinates multi-factor verification."""
        # Given a coordinator node with MFA enabled
        await auth_system.register_node("coordinator_001", NodeRole.COORDINATOR, "SecurePassword123!")
        ...
        mock_bcrypt.checkpw.assert_called_once_with("SecurePassword123!".encode(), mock_bcrypt.hashpw.return_value)
    ...
    @pytest.mark.asyncio
    async def test_authenticate_node_rejects_missing_mfa_for_coordinator(self, auth_system, mock_bcrypt, mock_rsa):
    ...
    async def test_authenticate_unknown_node_fails_gracefully(self, auth_system):
        ...
        success, session = await auth_system.authenticate_node("unknown_node", "AnyPassword123!")
    ...
    async def test_create_authentication_challenge_generates_proper_challenge(self, auth_system, mock_rsa):
        ...
        await auth_system.register_node("participant_001", NodeRole.PARTICIPANT, "SecurePassword123!")

        # When creating authentication challenge
        challenge = await auth_system.create_authentication_challenge(
            "participant_001", AuthenticationMethod.ZERO_KNOWLEDGE
        )
    ...
    async def test_validate_session_checks_expiration_and_activity(self, auth_system, mock_bcrypt, mock_rsa):
        ...
        success, session = await auth_system.authenticate_node("participant_001", "SecurePassword123!")
    ...
    async def test_update_node_reputation_adjusts_trust_level(self, auth_system, mock_bcrypt, mock_rsa):
        ...
        # Given a registered node
        identity = await auth_system.register_node("participant_001", NodeRole.PARTICIPANT, "SecurePassword123!")
    ...
    async def test_health_check_coordinates_session_cleanup(self, auth_system, mock_bcrypt, mock_rsa):
        ...
        success, active_session = await auth_system.authenticate_node("participant_001", "SecurePassword123!")

        # Create expired session
        success, expired_session = await auth_system.authenticate_node("participant_001", "SecurePassword123!")
