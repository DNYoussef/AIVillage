...
class ComplianceRule:
    ...
    def __init__(self, rule_id: str, framework: ComplianceFramework,
                 title: str, description: str, validation_function: callable = None):
    ...
    def validate(self, system_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.status = ComplianceStatus.COMPLIANT if compliance_result.get("compliant", False) else ComplianceStatus.NON_COMPLIANT
...
class GovernanceFrameworkValidator:
    ...
    def validate_framework_compliance(self, framework: ComplianceFramework,
        ...
        framework_result = {
            ...
            "overall_status": ComplianceStatus.COMPLIANT.value if compliant_rules == len(framework_rules) else ComplianceStatus.PARTIALLY_COMPLIANT.value,
    ...
    def validate_all_frameworks(self, system_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        compliant_frameworks = sum(
            ...
            if result.get("overall_status") == ComplianceStatus.COMPLIANT.value
        )
    ...
    def get_compliance_dashboard(self) -> Dict[str, Any]:
        ...
        if len(self.compliance_history) >= 2:
            ...
            trend = {
                ...
                "trend_direction": "improving" if recent.get("compliance_percentage", 0) > previous.get("compliance_percentage", 0) else "stable"
            }
...
class GovernanceFrameworkTest(unittest.TestCase):
    ...
    def test_gdpr_compliance_validation(self):
        ...
        self.assertEqual(compliant_result["overall_status"], ComplianceStatus.COMPLIANT.value,
                        "Compliant system must pass GDPR validation")
        self.assertGreaterEqual(compliant_result["compliance_percentage"], 90,
                               "GDPR compliance percentage should be high")
        ...
        self.assertEqual(non_compliant_result["overall_status"], ComplianceStatus.PARTIALLY_COMPLIANT.value,
        ...
        for rule_id in expected_gdpr_rules:
            self.assertIn(rule_id, gdpr_rules_tested,
                         f"GDPR rule {rule_id} must be tested")
    ...
    def test_coppa_compliance_validation(self):
        ...
        self.assertEqual(violation_result["overall_status"], ComplianceStatus.PARTIALLY_COMPLIANT.value,
        ...
        violations_found = any(
            "prohibited children data" in str(result.get("findings", []))
            for result in rule_results
        )
        self.assertTrue(violations_found,
                       "COPPA data collection violations should be detected")
    ...
    def test_owasp_top_10_compliance_validation(self):
        ...
        for control in expected_controls:
            self.assertIn(control, owasp_controls_tested,
                         f"OWASP control '{control}' must be tested")
        ...
        self.assertEqual(vulnerable_result["overall_status"], ComplianceStatus.PARTIALLY_COMPLIANT.value,
    ...
    def test_comprehensive_multi_framework_compliance(self):
        ...
        self.assertGreaterEqual(all_results["overall_compliance_percentage"], 80,
                               "Overall compliance percentage should be high")
        self.assertGreater(all_results["compliant_frameworks"], 0,
                          "Should have at least some compliant frameworks")
        ...
        for framework in expected_frameworks:
            self.assertIn(framework, framework_names,
                         f"Framework {framework} must be tested")
        
        # Test with non-compliant system
        non_compliant_results = self.governance_validator.validate_all_frameworks(
            self.non_compliant_system_config
        )
        
        self.assertLess(non_compliant_results["overall_compliance_percentage"], 50,
                       "Non-compliant system should have low compliance percentage")
        
        # Verify recommendations are generated
        self.assertGreater(len(all_results["recommendations"]), 0,
                          "Compliance recommendations should be generated")
    ...
    def test_compliance_rule_validation_accuracy(self):
        ...
        self.assertFalse(result["compliant"],
                        "Rule should detect excessive data collection")
        self.assertIn("excessive data fields", str(result["findings"]).lower(),
                     "Should identify specific excessive fields")
        ...
        self.assertTrue(result["compliant"],
                       "Rule should pass for minimal data collection")
    ...
    def test_compliance_recommendation_prioritization(self):
        ...
        self.assertGreater(len(recommendations), 0,
                          "Should generate compliance recommendations")
        
        # High priority recommendations should be marked
        high_priority_recommendations = [
            rec for rec in recommendations if "[HIGH PRIORITY]" in rec
        ]
        
        if len(high_priority_recommendations) > 0:
            self.assertGreater(len(high_priority_recommendations), 0,
    ...
    def test_compliance_historical_tracking(self):
        ...
        self.assertGreater(initial_percentage, degraded_percentage,
                          "Should track compliance degradation")
