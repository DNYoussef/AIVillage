...
class TestFullSecurityWorkflowIntegration:
    ...
    async def test_middleware_chain_integration(self):
        ...
        request.headers = {"Authorization": "Bearer valid_token", "User-Agent": "Test Browser", "X-MFA-Token": "123456"}
        ...
        response = await self.server._security_middleware(
            request,
            lambda req: self.server._rate_limit_middleware(
                req,
                lambda req: self.server._session_middleware(
                    req,
                    lambda req: self.server._mfa_middleware(
                        req, lambda req: self.server._auth_middleware(req, final_handler)
                    ),
                ),
            ),
        )
    ...
    async def test_session_token_lifecycle_integration(self):
        ...
        with patch("jwt.encode", return_value="mock_jwt_token"):
            tokens = await self.authenticator.create_session_tokens(
                user_id=user_id, device_info=device_info, roles=["user"], permissions=["read"], mfa_verified=True
            )
        ...
        # Mock token verification
        mock_payload = {"user_id": user_id, "session_id": tokens["session_id"], "jti": "mock_jti"}
...
class TestBackwardCompatibilityIntegration:
    ...
    async def test_legacy_endpoint_compatibility(self):
        ...
        legacy_endpoints = ["/health", "/auth/login", "/profiles/user123", "/api/data"]
    ...
    def test_legacy_response_format_compatibility(self):
        ...
        expected_legacy_format = {"status": "healthy", "timestamp": datetime.utcnow().isoformat(), "version": "2.0.0"}
    ...
    async def test_legacy_authentication_flow_compatibility(self):
        ...
        mock_user = {"user_id": "legacy_user", "password_hash": "mock_hash", "password_salt": "mock_salt"}
...
class TestConcurrentOperationsIntegration:
    ...
    async def test_concurrent_login_attempts(self):
        ...
        # Mock user data
        mock_user = {"user_id": user_id, "password_hash": "mock_hash", "password_salt": "mock_salt"}
        ...
        for i in range(5):
            ...
            self.server.authenticator.create_session_tokens = AsyncMock(
                return_value={"access_token": f"token_{i}", "session_id": f"session_{i}"}
            )
    ...
    async def test_concurrent_session_operations(self):
        ...
        operations = [
            self.server.session_manager.add_token_to_session(session_id, f"token_{i}", "access") for i in range(10)
        ]
...
class TestSecurityBreachSimulation:
    ...
    async def test_session_hijacking_protection(self):
        ...
        await self.server._session_middleware(hijack_request, mock_handler)
