...
async def test_connection_establishment(results: TestResults):
    ...
    try:
        ...
        if success and server.is_connected("client_agent"):
            ...
            results.add_result("connection_establishment", False, f"Connection failed: client_success={success}, server_connected={server.is_connected('client_agent')}")
            
    except Exception as e:
        results.add_result("connection_establishment", False, f"Exception: {str(e)}")
    finally:
        if client:
...
async def test_message_delivery(results: TestResults):
    ...
    try:
        ...
        client_to_server_ok = (
            client_send_success and 
            len(server_messages) > 0 and 
            server_messages[0].get("content", {}).get("data") == "Hello from client"
        )
        
        server_to_client_ok = (
            server_send_success and 
            len(client_messages) > 0 and 
            client_messages[0].get("content", {}).get("data") == "Hello from server"
        )
        
        if client_to_server_ok and server_to_client_ok:
            results.add_result("message_delivery", True, f"Bidirectional messaging works. Server got {len(server_messages)} msgs, client got {len(client_messages)} msgs")
        else:
            results.add_result("message_delivery", False, f"Message delivery failed. C->S: {client_to_server_ok}, S->C: {server_to_client_ok}")
            
    except Exception as e:
        results.add_result("message_delivery", False, f"Exception: {str(e)}")
    finally:
        if client:
            await client.stop_server()
        if server:
...
async def test_reconnection_logic(results: TestResults):
    ...
    try:
        ...
        if not is_still_connected and reconnected:
            ...
            results.add_result("reconnection_logic", False, f"Reconnection failed. Still connected after kill: {is_still_connected}, Reconnected: {reconnected}")
            
    except Exception as e:
        results.add_result("reconnection_logic", False, f"Exception: {str(e)}")
    finally:
        if client:
            await client.stop_server()
        if server:
...
async def test_message_queuing(results: TestResults):
    ...
    try:
        ...
        if delivered_count >= 5 and messages_in_order:
            results.add_result("message_queuing", True, f"All {delivered_count} queued messages delivered in correct order")
        else:
            results.add_result("message_queuing", False, f"Queue delivery failed. Expected 5, got {delivered_count}, in_order: {messages_in_order}")
            
    except Exception as e:
        results.add_result("message_queuing", False, f"Exception: {str(e)}")
    finally:
        if client:
            await client.stop_server()
        if server:
...
async def test_tls_ssl_support(results: TestResults):
    ...
    try:
        ...
        if has_ssl_context and has_ssl_client_support:
            ...
            results.add_result("tls_ssl_support", False, f"SSL support incomplete. Server: {has_ssl_context}, Client: {has_ssl_client_support}")
            
    except Exception as e:
        results.add_result("tls_ssl_support", False, f"Exception: {str(e)}")
...
async def test_message_encryption(results: TestResults):
    ...
    try:
        ...
        if keys_match and encryption_works:
            results.add_result("message_encryption", True, "Encryption keys match and message encryption/decryption works")
        else:
            results.add_result("message_encryption", False, f"Encryption failed. Keys match: {keys_match}, Encryption works: {encryption_works}")
            
    except Exception as e:
        results.add_result("message_encryption", False, f"Exception: {str(e)}")
...
async def test_message_types_and_priorities(results: TestResults):
    ...
    try:
        ...
        if all_sent and correct_count:
            results.add_result("message_types_priorities", True, f"Successfully sent and received {len(received_messages)} different message types")
        else:
            results.add_result("message_types_priorities", False, f"Message type test failed. All sent: {all_sent}, Correct count: {correct_count}")
            
    except Exception as e:
        results.add_result("message_types_priorities", False, f"Exception: {str(e)}")
    finally:
        if client:
            await client.stop_server()
        if server:
