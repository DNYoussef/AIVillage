...
def test_real_pipeline_simulation():
    ...
    print(f"Using REAL measured compression ratios:")
    ...
    if stage_results['seedlm'] > 0:
        ...
        estimated_ratio = (stage_results['bitnet'] * stage_results['seedlm'] * stage_results['vptq']) * pipeline_efficiency
    else:
        # Fallback if SeedLM failed
        estimated_ratio = stage_results['bitnet'] * stage_results['vptq'] * 0.5
    
    print(f"\nRealistic Pipeline Estimate:")
    print(f"  Pipeline efficiency factor: 30% (accounts for overhead)")
