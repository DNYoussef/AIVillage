...
def test_optimized_compression_simulation():
    ...
    for name, param in model.named_parameters():
        ...
        print(f"    Stage 4 (Hyper): {s3_size/s4_size:.1f}x ({s4_time:.3f}s) {'[SKIPPED]' if not hyper_effective else ''}")
...
def simulate_full_optimized_pipeline():
    ...
    original_total = sum(p.numel() * 4 for p in nn.Sequential(
        nn.Linear(1024, 512),
        nn.ReLU(),
        nn.Linear(512, 256),
        nn.ReLU(), 
