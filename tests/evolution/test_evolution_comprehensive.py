...
class TestModelMutation:
    ...
    def test_structural_mutation(self):
        ...
        base_model = nn.Sequential(nn.Linear(20, 15), nn.ReLU(), nn.Linear(15, 10))
...
class TestEvolutionIntegration:
    ...
    def test_multi_generation_evolution(self):
        ...
        for i in range(initial_population_size):
            model = nn.Sequential(nn.Linear(20, 15), nn.ReLU(), nn.Linear(15, 10))
            population.append(
                {
                    "model": model,
...
def evolution_population():
    ...
    population = []
    for i in range(10):
        model = nn.Sequential(nn.Linear(30, 20), nn.ReLU(), nn.Linear(20, 10))
        population.append(
            {
                "model": model,
