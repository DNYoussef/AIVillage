...
async def test_fog_bridge_basic():
    ...
    try:
        from core.rag.integration.fog_compute_bridge import FogComputeBridge, QueryType, QueryDistributionStrategy
...
async def test_api_gateway_basic():
    ...
    try:
        from infrastructure.fog.gateway.api.federated import FederatedAPIGateway, FederatedInferenceRequest, UserTier
        ...
        request = FederatedInferenceRequest(
            user_id="test_user", user_tier=UserTier.MEDIUM, query="Test API query", max_budget=30.0
        )
