...
def mock_vector_store():
    ...
    mock.similarity_search = AsyncMock(return_value=[{"content": "test content", "metadata": {"source": "test"}}])
    mock.similarity_search_with_score = AsyncMock(
        return_value=[({"content": "test content", "metadata": {"source": "test"}}, 0.95)]
    )
    return mock
...
def mock_rag_config():
    ...
    return UnifiedConfig(model_name="gpt-4", max_tokens=1000, temperature=0.1)
...
class MockProcessingInterface(ProcessingInterface[str, str]):
    ...
    async def process(self, input_data: str, **kwargs) -> Any:
        ...
        if input_data == "error_input":
            return ProcessResult(status=ProcessStatus.FAILED, error="Simulated processing error")
...
def chaos_scenarios():
    ...
    return {
        "network_partition": {
            ...
            "implementation": lambda: patch(
                "core.communication.StandardCommunicationProtocol.send_message",
                side_effect=ConnectionError("Network partition"),
            ),
        },
        "memory_pressure": {
            "description": "Simulate memory pressure",
            "implementation": lambda: patch("psutil.virtual_memory", return_value=Mock(percent=95)),
        },
        "llm_timeout": {
            "description": "Simulate LLM timeout",
            "implementation": lambda: patch(
                "agents.language_models.openai_gpt.OpenAIGPTConfig.create", side_effect=TimeoutError("LLM timeout")
            ),
        },
        "vector_store_failure": {
            "description": "Simulate vector store failure",
            "implementation": lambda: patch.object(
                VectorStore, "similarity_search", side_effect=ConnectionError("Vector store down")
            ),
        },
    }
        ...
        def cleanup(self):
            ...
            for file_path in self.created_files:
                try:
                    ...
                    )  # nosec S110 - test cleanup, logging expected
            ...
            for patch_obj in self.mock_patches:
                try:
                    ...
                    logging.debug(
                        f"Failed to stop patch {patch_obj}: {e}"
                    )  # nosec S110 - test cleanup, logging expected
...
def assert_performance_threshold(duration_ms: float, threshold_ms: float, operation: str):
    ...
    assert duration_ms <= threshold_ms, (
        f"{operation} took {duration_ms:.2f}ms, " f"exceeded threshold of {threshold_ms}ms"
    )
...
def assert_memory_usage(memory_mb: float, threshold_mb: float, operation: str):
    ...
    assert memory_mb <= threshold_mb, (
        f"{operation} used {memory_mb:.2f}MB memory, " f"exceeded threshold of {threshold_mb}MB"
    )
