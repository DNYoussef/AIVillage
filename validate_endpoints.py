...
def test_http_endpoint(url: str, timeout: float = 10.0, expected_codes: List[int] = None) -> Dict[str, Any]:
    ...
    if expected_codes is None:
        expected_codes = [200, 404]  # 404 is acceptable if service exists but endpoint not implemented
...
def validate_service_endpoints() -> Dict[str, Any]:
    ...
    required_ports = [
        {'service': 'LibP2P Main', 'host': '127.0.0.1', 'port': 4001, 'protocol': 'TCP/UDP', 'purpose': 'Primary P2P communication'},
        {'service': 'LibP2P WebSocket', 'host': '127.0.0.1', 'port': 4002, 'protocol': 'TCP', 'purpose': 'WebSocket transport'},
        {'service': 'Digital Twin API', 'host': '127.0.0.1', 'port': 8080, 'protocol': 'HTTP', 'purpose': 'REST API endpoints'},
        {'service': 'Evolution Metrics', 'host': '127.0.0.1', 'port': 8081, 'protocol': 'HTTP', 'purpose': 'Metrics collection API'},
        {'service': 'RAG Pipeline', 'host': '127.0.0.1', 'port': 8082, 'protocol': 'HTTP', 'purpose': 'Query processing API'},
        {'service': 'Redis (Optional)', 'host': '127.0.0.1', 'port': 6379, 'protocol': 'TCP', 'purpose': 'Caching and pub/sub'},
    ]
    
    # CODEX Health Check Endpoints (from CODEX_INTEGRATION_REQUIREMENTS.md)
    health_endpoints = [
        {'name': 'Evolution Metrics', 'url': 'http://127.0.0.1:8081/health/evolution', 'expected': [200, 404]},
        {'name': 'RAG Pipeline', 'url': 'http://127.0.0.1:8082/health/rag', 'expected': [200, 404]},
        {'name': 'P2P Network', 'url': 'http://127.0.0.1:4001/health/p2p', 'expected': [200, 404]},
        {'name': 'Digital Twin', 'url': 'http://127.0.0.1:8080/health/twin', 'expected': [200, 404]},
    ]
    
    # Additional endpoints to test
    api_endpoints = [
        {'name': 'Digital Twin Root', 'url': 'http://127.0.0.1:8080/', 'expected': [200, 404, 405]},
        {'name': 'Evolution Metrics Root', 'url': 'http://127.0.0.1:8081/', 'expected': [200, 404, 405]},
        {'name': 'RAG Pipeline Root', 'url': 'http://127.0.0.1:8082/', 'expected': [200, 404, 405]},
        {'name': 'P2P Network Root', 'url': 'http://127.0.0.1:4001/', 'expected': [200, 404, 405]},
    ]
    ...
    for port_info in required_ports:
        ...
        print(f"{status_icon:<15} {port_info['service']:<20} {port_info['host']}:{port_info['port']}{response_time}{error_msg}")
    ...
    print(f"Health Endpoints: {results['summary']['health_endpoints_ok']}/{results['summary']['health_endpoints_total']} responding")
    print(f"API Endpoints: {results['summary']['api_endpoints_ok']}/{results['summary']['api_endpoints_total']} responding")
    
    # Calculate overall health
    total_checks = results['summary']['ports_total'] + results['summary']['health_endpoints_total'] + results['summary']['api_endpoints_total']
    total_passing = results['summary']['ports_listening'] + results['summary']['health_endpoints_ok'] + results['summary']['api_endpoints_ok']
