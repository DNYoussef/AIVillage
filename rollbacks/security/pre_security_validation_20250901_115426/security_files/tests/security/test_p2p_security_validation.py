...
class TestSecurityScanningIntegration:
    ...
    def test_bandit_scan_passes(self):
        ...
        # Run Bandit scan
        result = subprocess.run(["bandit", "-r", str(p2p_path), "-ll", "-f", "json"], capture_output=True, text=True)
        ...
        critical_issues = [
            issue for issue in scan_results.get("results", []) if issue.get("issue_severity") in ["HIGH", "CRITICAL"]
        ]
    ...
    def test_secret_detection_passes(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["detect-secrets", "scan", str(p2p_path), "--baseline", baseline_path, "--force-use-all-plugins"],
                capture_output=True,
                text=True,
            )
