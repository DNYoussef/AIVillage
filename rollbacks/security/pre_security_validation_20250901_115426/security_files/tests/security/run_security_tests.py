...
class SecurityTestSuite:
    ...
    def __init__(self):
        self.test_categories = {
            ...
            "integration": {
                ...
                "modules": ["tests.security.integration.test_security_workflows"],
            },
            "performance": {
                "description": "Security overhead and performance tests",
                "modules": ["tests.security.performance.test_security_overhead"],
            },
            "compliance": {
                "description": "Governance framework compliance tests",
                "modules": ["tests.security.compliance.test_governance_framework"],
            },
            "negative": {
                "description": "Attack prevention and negative security tests",
                "modules": ["tests.security.negative.test_attack_prevention"],
            },
        }
    ...
    def run_security_tests(
        self, category: Optional[str] = None, verbose: bool = False, fast_mode: bool = False
    ) -> Dict[str, Any]:
        ...
        runner = unittest.TextTestRunner(verbosity=verbosity, buffer=True, failfast=fast_mode)
        ...
        try:
            ...
            results_summary = {
                ...
                "failures": [
                    {"test": str(test), "error": traceback_str} for test, traceback_str in test_result.failures
                ],
                "errors": [{"test": str(test), "error": traceback_str} for test, traceback_str in test_result.errors],
    ...
    def generate_report(self, format_type: str = "markdown", output_path: Optional[str] = None) -> str:
    ...
    def _generate_markdown_report(self, output_path: Optional[str] = None) -> str:
        ...
        for category, info in self.test_categories.items():
            report_lines.extend([f"### {category.title()} Tests", f"{info['description']}", ""])
        ...
        if results.get("failures"):
            report_lines.extend(["## Test Failures", "", "The following tests failed:", ""])
            for failure in results["failures"]:
                report_lines.extend([f"### {failure['test']}", "```", failure["error"], "```", ""])

        # Add errors if any
        if results.get("errors"):
            report_lines.extend(["## Test Errors", "", "The following tests encountered errors:", ""])
            for error in results["errors"]:
                report_lines.extend([f"### {error['test']}", "```", error["error"], "```", ""])
        ...
        report_lines.extend(
            [
                ...
                "- **Compliance Tests:** Regulatory and governance framework adherence",
                "- **Negative Tests:** Attack prevention and security boundary enforcement",
                "",
                "---",
                "",
                "*Report generated by AIVillage Security Test Suite v1.0*  ",
...
Examples:
    ...
    parser.add_argument("--fast", action="store_true", help="Run fast test subset only (fail fast mode)")
    ...
    try:
        # Run security tests
        results = security_suite.run_security_tests(category=args.category, verbose=args.verbose, fast_mode=args.fast)
        ...
        if test_results["failed_tests"] > 0 or test_results["error_tests"] > 0:
            print(
                f"⚠️  Security tests completed with {test_results['failed_tests']} failures and {test_results['error_tests']} errors"
            )
            sys.exit(1)
        else:
