...
class SecurityPerformanceBenchmark:
    ...
    def benchmark_authentication_performance(self, iterations: int = 1000) -> Dict[str, Any]:
        ...
        benchmark_result = {
            ...
            "meets_threshold": (statistics.mean(auth_times) if auth_times else float("inf"))
            <= self.performance_thresholds["authentication_latency_ms"],
    ...
    def benchmark_authorization_performance(self, iterations: int = 5000) -> Dict[str, Any]:
        ...
        from tests.security.unit.test_boundary_security import (
            SecurityBoundary,
        ...
        test_boundary = SecurityBoundary(
            "performance-test-boundary", SecurityBoundaryType.MODULE_BOUNDARY, SecurityLevel.INTERNAL
        )

        test_context = TestSecurityContext(SecurityLevel.INTERNAL, ["read", "write", "execute"])
    ...
    def benchmark_encryption_performance(self, data_size_mb: int = 10) -> Dict[str, Any]:
        ...
        for i in range(10):  # 10 iterations for encryption benchmark
            ...
            start_time = time.perf_counter()
            signer.verify_signature(test_data, signature_metadata)
        ...
        benchmark_result = {
            ...
            "verification": {
                ...
                "meets_threshold": verification_throughput_mbps
                >= self.performance_thresholds["encryption_throughput_mbps"],
            },
            "timestamp": datetime.utcnow().isoformat(),
        }
    ...
    def benchmark_sbom_generation_performance(self, component_count: int = 100) -> Dict[str, Any]:
        ...
        for i in range(component_count):
            component = SBOMComponent(
                name=f"test-component-{i}", version=f"1.0.{i}", component_type=ComponentType.LIBRARY
            )
    ...
    def benchmark_concurrent_security_operations(
        self, thread_count: int = 10, operations_per_thread: int = 100
    ) -> Dict[str, Any]:
        ...
        test_boundary = SecurityBoundary(
            "concurrent-test-boundary", SecurityBoundaryType.MODULE_BOUNDARY, SecurityLevel.INTERNAL
        )
...
class SecurityOverheadTest(unittest.TestCase):
    ...
    def test_authentication_performance_overhead(self):
        ...
        self.assertLess(result["average_latency_ms"], 150, "Average authentication latency should be reasonable")

        self.assertGreater(result["successful_authentications"], 0, "Some authentication attempts must succeed")

        # P95 should be acceptable
        if result["p95_latency_ms"] > 0:
            self.assertLess(result["p95_latency_ms"], 200, "P95 authentication latency should be acceptable")
    ...
    def test_authorization_performance_overhead(self):
        ...
        self.assertLess(result["average_latency_ms"], 10, "Authorization should be very fast for frequent checks")

        self.assertEqual(
            result["successful_authorizations"],
            result["iterations"],
        ...
        self.assertLess(latency_variance, 50, "Authorization latency should be consistent")
    ...
    def test_encryption_performance_throughput(self):
        ...
        self.assertLess(
            verification_result["average_time_ms"],
    ...
    def test_vulnerability_scanning_performance(self):
        ...
        self.assertGreater(result["dependencies_per_second"], 10, "Should scan at least 10 dependencies per second")

        self.assertGreater(result["total_dependencies"], 0, "Should scan some dependencies")

        # Total scan time should be reasonable for CI/CD
        self.assertLess(result["total_scan_time_s"], 60, "Total scan time should be acceptable for CI/CD")
    ...
    def test_sbom_generation_performance(self):
        ...
        self.assertTrue(result["signed_successfully"], "SBOM must be successfully signed")

        self.assertGreater(result["components_per_second"], 10, "Should process at least 10 components per second")

        # Generation should be faster than signing
        self.assertLess(result["generation_time_s"], result["total_time_s"], "Generation should be part of total time")
    ...
    def test_audit_logging_performance_overhead(self):
        ...
        self.assertLess(result["overhead_percent"], 10, "Audit logging overhead should be minimal")

        # Both baseline and logging operations should complete
        self.assertGreater(result["operations_per_second_baseline"], 0, "Baseline operations must execute")
        self.assertGreater(result["operations_per_second_logging"], 0, "Logging operations must execute")
        ...
        self.assertLess(slowdown_factor, 2.0, "Logging shouldn't cause >2x slowdown")
    ...
    def test_concurrent_security_operations_scalability(self):
        ...
        self.assertGreaterEqual(
            result["concurrent_efficiency"], 95, "Concurrent operations should have high success rate"
        )

        self.assertGreater(
            result["operations_per_second"], 100, "Should handle at least 100 operations per second concurrently"
        )

        self.assertLess(result["average_operation_time_ms"], 100, "Concurrent operations should maintain low latency")
    ...
    def test_security_performance_regression_detection(self):
        ...
        self.performance_benchmark.benchmark_authentication_performance(iterations=50)
        self.performance_benchmark.benchmark_authorization_performance(iterations=500)
        ...
        self.assertGreaterEqual(
            summary["overall_performance_score"], 70, "Overall security performance score should be acceptable"
        )

        self.assertEqual(summary["benchmarks_completed"], 2, "Should track completed benchmarks")

        self.assertIn("benchmark_results", summary, "Should provide detailed benchmark results")

        # Check for performance recommendations
        if summary["overall_performance_score"] < 90:
            self.assertGreater(
                len(summary["recommendations"]), 0, "Should provide performance recommendations when needed"
            )
    ...
    def test_memory_usage_security_operations(self):
        ...
        self.performance_benchmark.benchmark_authentication_performance(iterations=100)
        ...
        self.assertLess(memory_increase_mb, 50, "Security operations should not use excessive memory (>50MB)")
        ...
        if crypto_result:
            ...
            self.assertLess(memory_efficiency, 2.0, "Memory usage should be efficient relative to data processed")
    ...
    def test_security_performance_under_stress(self):
        ...
        self.assertGreaterEqual(
            stress_result["concurrent_efficiency"], 90, "Security should maintain high efficiency under stress"
        )

        self.assertLess(
            stress_result["average_operation_time_ms"], 200, "Operations should remain responsive under stress"
        )

        # System should handle the stress load
        total_ops = stress_result["total_operations"]
        self.assertEqual(
            stress_result["successful_operations"], total_ops, "All operations should succeed even under stress"
        )
        ...
        if hasattr(self, "_previous_concurrent_result"):
            ...
            self.assertLess(degradation, 50, "Performance degradation under stress should be <50%")
