...
try:
    from ..rag.cognitive_nexus import AnalysisType, CognitiveNexus, ReasoningStrategy, RetrievedInformation
except ImportError:
...
class AgentFactory:
    ...
    async def create_agent(self, agent_type: AgentType, agent_id: str, **kwargs) -> BaseAgent | None:
        ...
        try:
            ...
            instantiation_time = (time.perf_counter() - start_time) * 1000
            logger.error(
                f"Agent creation failed for {agent_type.value}:{agent_id} after {instantiation_time:.1f}ms: {e}"
            )
            return None
...
class CognativeNexusController:
    ...
    async def create_agent(self, agent_type: AgentType, agent_id: str | None = None, **kwargs) -> str | None:
        ...
        try:
            ...
            instantiation_time = (time.perf_counter() - start_time) * 1000
            logger.error(
                f"❌ Agent creation failed: {agent_type.value}:{agent_id} after {instantiation_time:.1f}ms: {e}"
            )
            return None
    ...
    async def process_task_with_act_halting(self, task: CognativeTask) -> dict[str, Any]:
        ...
        try:
            ...
            if not suitable_agents:
                return {"status": "failed", "error": "No suitable agents available", "task_id": task.task_id}
            ...
            for iteration in range(task.max_iterations):
                ...
                if self.cognitive_nexus and task.requires_reasoning:
                    confidence = await self._analyze_result_confidence(
                        task.description, iteration_result, task.reasoning_strategy
                    )
                else:
                ...
                if task.iterative_refinement and task.current_iteration < task.max_iterations:
                    task.description = (
                        f"Refine this response: {iteration_result[:500]}... Original task: {task.description}"
                    )
            ...
            logger.info(
                f"✅ Task {task.task_id} completed in {processing_time:.1f}ms with {task.results['iterations_used']} iterations"
            )
    ...
    async def _find_suitable_agents(self, task: CognativeTask) -> list[str]:
        ...
        active_agents = [agent_id for agent_id, reg in self.agents.items() if reg.status == AgentStatus.ACTIVE]
    ...
    async def _analyze_result_confidence(self, query: str, result: str, strategy: str | None) -> float:
        ...
        try:
            ...
            if analysis_results:
                ...
                relevance_score = (
                    analysis_results[1].result.get("average_relevance", 0.7) if len(analysis_results) > 1 else 0.7
                )
