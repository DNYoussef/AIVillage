...
async def health_check() -> HealthResponse:
    ...
    if config.environment == "test" or not hasattr(app.state, 'start_time'):
        ...
        service_statuses = [svc.get("status", "unhealthy") for svc in services.values() 
...
async def query_endpoint(
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=config.request_timeout) as client:
            ...
            try:
                response = await client.post(
                    f"{config.knowledge_system_url}/v1/query",
                    json=request.dict()
                )
                ...
                raise HTTPException(
                    ...
                    detail="Knowledge system temporarily unavailable"
                )
    ...
    async def upload_endpoint(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=config.request_timeout) as client:
                ...
                response = await client.post(
                    f"{config.knowledge_system_url}/v1/upload",
                    files=files
                )
                response.raise_for_status()
...
async def chat_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Twin service error: {e.response.status_code}"
        )
    except Exception as e:
