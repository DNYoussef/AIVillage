...
class GapDetectionService(IGapDetectionService, CacheableMixin, AsyncServiceMixin):
    ...
    async def detect_gaps(
        ...
        try:
            ...
            for method in self.detection_methods:
                method_gaps = await self._run_detection_method(
                    method, query, retrieved_info, focus_area
                )
            ...
            if total_nodes > 10:
                ...
                if total_edges < expected_edges * 0.5:
                    gap = DetectedGap(
                        ...
                        evidence=[
                            f"Nodes: {total_nodes}, Edges: {total_edges}, Expected: ~{expected_edges:.0f}"
                        ],
                        confidence=0.7,
                        priority=0.6,
    ...
    async def _are_nodes_connected(self, node_id1: str, node_id2: str) -> bool:
        ...
        return any(
            (edge.source_id == node_id1 and edge.target_id == node_id2) or
