...
class KnowledgeValidatorService(IKnowledgeValidatorService, CacheableMixin, AsyncServiceMixin):
    ...
    async def _check_no_self_references(
        self, 
        proposal: Union[ProposedNode, ProposedRelationship]
    ) -> bool:
    ...
    async def _check_no_circular_dependencies(
        self, 
        proposal: Union[ProposedNode, ProposedRelationship]
    ) -> bool:
    ...
    async def _would_create_cycle(self, proposal: ProposedRelationship) -> bool:
        ...
        visited = set()
        return not await self._has_path_dfs(
            proposal.target_id, proposal.source_id, visited
        )
    ...
    async def _check_trust_score_consistency(
        self, 
        proposal: Union[ProposedNode, ProposedRelationship]
    ) -> bool:
    ...
    async def _check_semantic_coherence(
        ...
        if isinstance(proposal, ProposedNode):
            return (proposal.content and 
                   proposal.concept and 
                   len(proposal.content.strip()) > 5)
        else:
            return (proposal.relation_type and 
                   len(proposal.relation_type.strip()) > 0)
    
    async def _check_structural_validity(
        self, 
        proposal: Union[ProposedNode, ProposedRelationship]
    ) -> bool:
        ...
        if isinstance(proposal, ProposedNode):
            return (0.0 <= proposal.existence_probability <= 1.0 and
                   0.0 <= proposal.utility_score <= 1.0 and
                   0.0 <= proposal.confidence <= 1.0)
        else:
            return (proposal.source_id and 
                   proposal.target_id and
                   0.0 <= proposal.existence_probability <= 1.0 and
                   0.0 <= proposal.utility_score <= 1.0 and
                   0.0 <= proposal.confidence <= 1.0)
    
    async def _validate_against_existing_knowledge(
        self, 
        proposal: Union[ProposedNode, ProposedRelationship]
    ) -> bool:
    ...
    async def _validate_relationship_against_graph(self, proposal: ProposedRelationship) -> bool:
        ...
        if (proposal.source_id not in self.config.trust_graph.nodes or
            proposal.target_id not in self.config.trust_graph.nodes):
            return False
        
        # Check if relationship already exists
        for edge in self.config.trust_graph.edges.values():
            if (edge.source_id == proposal.source_id and 
                edge.target_id == proposal.target_id):
    ...
    async def _extract_proposal_pattern(
        self, 
        proposal: Union[ProposedNode, ProposedRelationship]
    ) -> Dict[str, Any]:
    ...
    async def _prune_learning_data(self, max_age_days: int = 90) -> None:
        ...
        self.learning_data["successful_patterns"] = [
            pattern for pattern in self.learning_data["successful_patterns"]
            if pattern["timestamp"] > cutoff_date
        ]
        
        # Prune failed patterns
        self.learning_data["failed_patterns"] = [
            pattern for pattern in self.learning_data["failed_patterns"]
            if pattern["timestamp"] > cutoff_date
        ]
        
        # Prune validation history
        self.learning_data["validation_history"] = [
            entry for entry in self.learning_data["validation_history"]
