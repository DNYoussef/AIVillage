...
UPDATED FOR COGMENT INTEGRATION:
Adapted from 3-model HRRM approach to single unified Cogment model workflow.
Preserves ACT halting and LTM dynamics while achieving 6x parameter reduction.
...
- NEW: integration/cogment/ (Cogment model integration)

Implements sophisticated evolutionary optimization to create a strong foundation model
from multiple base models using various merging techniques, now optimized for
...
class EvoMergeConfig(PhaseConfig):
    ...
    # Cogment seed models for fast iteration (replaces HRRM 3-model approach)
    seed_models: list[str] = field(default_factory=lambda: [])  # Will be populated with Cogment exports
...
class EvoMergePhase:
    """
    Phase 1: Evolutionary Model Merging (COGMENT INTEGRATION)

    UPDATED FOR SINGLE COGMENT MODEL WORKFLOW:
    Replaces 3-model HRRM approach with unified Cogment model evolution while
    preserving ACT halting and LTM dynamics. Achieves 6x parameter reduction
    and faster evolutionary operations.
    ...
    - NEW: Cogment model compatibility and ACT/LTM preservation
    """
    ...
    def __init__(self, config: Any):
        ...
        if getattr(config, "use_cogment_adapter", False):
            try:
                ...
                self.logger.warning(f"Cogment adapter not available: {e}")
                self.cogment_adapter = None
    ...
    async def run(self, base_model_paths: list[str] | None = None) -> Any:
        """
        Run the complete EvoMerge phase with Cogment integration.

        COGMENT INTEGRATION: Now supports single unified model workflow
        replacing 3-model HRRM approach while preserving ACT and LTM dynamics.
        ...
        """
        self.logger.info("=" * 80)
        if self.cogment_adapter:
            self.logger.info("Starting COGMENT EvoMerge Phase - Single Model Evolutionary Optimization")
            self.logger.info("🔄 WORKFLOW: 3-model HRRM → 1 unified Cogment (6x parameter reduction)")
        else:
        ...
        # Load base models using Cogment adapter if available
        if self.cogment_adapter:
            self.logger.info("🧠 Loading Cogment models for single-model evolution...")
            base_models = self.cogment_adapter.load_cogment_base_models(base_model_paths)
        else:
            self.logger.info("📦 Loading standard models for traditional evolution...")
            base_models = await self._load_base_models(base_model_paths)

        # Load tokenizer - use Cogment tokenizer or fallback to simple one
        try:
            ...
            elif Path(hrrm_tokenizer_path).exists():
                tokenizer = AutoTokenizer.from_pretrained(hrrm_tokenizer_path, local_files_only=True)
                self.logger.info("✅ Using HRRM tokenizer (fallback)")
            else:
                raise Exception("No Cogment or HRRM tokenizer found")
        except Exception as e:
            self.logger.warning(f"Could not load specialized tokenizer, creating simple tokenizer: {e}")
            # Create a basic tokenizer for Cogment models
            from transformers import PreTrainedTokenizer

            class SimpleCogmentTokenizer(PreTrainedTokenizer):
            ...
            tokenizer = SimpleCogmentTokenizer()
