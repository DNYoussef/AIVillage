...
from config.cognate_config import CognateModelConfig, create_default_config
...
class TestAgentForgeIntegration(unittest.TestCase):
    ...
    def test_parameter_targeting_accuracy(self):
        ...
        self.assertLess(error_pct, 2.0, 
                       f"Parameter count {param_count:,} is {error_pct:.2f}% off target {target:,}")
    ...
    def test_model_metadata(self):
        ...
        expected_components = [
            ...
            "act_head", "memory_controllers", "total"
        ]
...
class TestPipelineCompatibility(unittest.TestCase):
    ...
    def setUp(self):
        self.model = create_cognate_model(
            d_model=128, n_layers=4, vocab_size=1000, mem_capacity=64
        )
    ...
    def test_checkpoint_resumption(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            new_model = create_cognate_model(
                d_model=128, n_layers=4, vocab_size=1000, mem_capacity=64
            )
...
class TestEvoMergeCompatibility(unittest.TestCase):
    ...
    def test_state_dict_compatibility(self):
        ...
        for i in range(1, len(keys_sets)):
            self.assertEqual(keys_sets[0], keys_sets[i], 
                           f"Model {i} has different keys than model 0")
        
        # All tensors should have the same shapes
        for key in keys_sets[0]:
            shapes = [sd[key].shape for sd in state_dicts]
            for i in range(1, len(shapes)):
                self.assertEqual(shapes[0], shapes[i],
                               f"Parameter {key} has different shapes across models")
