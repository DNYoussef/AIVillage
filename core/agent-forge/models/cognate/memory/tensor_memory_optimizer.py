...
class TensorRegistry:
    ...
    def register_tensor(
        ...
        with self._lock:
            ...
            self._tensor_registry[tensor_id] = weakref.ref(
                tensor, 
                lambda ref: self._on_tensor_finalized(tensor_id)
            )
...
class TensorMemoryOptimizer:
    ...
    def optimize_tensor_operation(
        ...
        operation: str,
        tensors: List[Tensor],
        tensor_ids: Optional[List[str]] = None,
        **kwargs
    ) -> Tuple[Any, List[str]]:
        ...
        for i, tensor in enumerate(tensors):
            ...
            registered_id = self.registry.register_tensor(
                tensor, 
                tensor_id,
                metadata={'operation': operation, 'input_index': i}
            )
        ...
        try:
            ...
            if isinstance(result, Tensor):
                result_id = self.registry.register_tensor(
                    result,
                    metadata={'operation': operation, 'result': True}
                )
                ...
                for i, res_tensor in enumerate(result):
                    result_id = self.registry.register_tensor(
                        res_tensor,
                        metadata={'operation': operation, 'result': True, 'result_index': i}
                    )
                    result_ids.append(result_id)
