...
class CognateLTMBank(nn.Module):
    ...
    def read(
        ...
        similarities = F.cosine_similarity(
            query.unsqueeze(0),  # (1, d_mem)
            active_keys,         # (current_size, d_mem)
            dim=1
        )  # (current_size,)
        ...
        self.memory_usage[top_indices] = torch.clamp(
            self.memory_usage[top_indices] * 1.1, max=2.0
        )
        ...
        return (
            retrieved_values,
            retrieved_keys, 
            top_indices.tolist()[:actual_topk],
            retrieved_sims
        )
    ...
    def compress_memories(self) -> int:
        ...
        for i, j in high_sim_pairs:
            ...
            self.memory_values[keep_idx] = (
                self.memory_values[keep_idx] + self.memory_values[remove_idx]
            ) / 2.0
            self.memory_keys[keep_idx] = (
                self.memory_keys[keep_idx] + self.memory_keys[remove_idx]
            ) / 2.0
