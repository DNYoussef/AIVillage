...
class GrokFastOptimizer(Optimizer):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized GrokFast optimizer: alpha={alpha}, lamb={lamb}, "
    ...
    def _update_adaptive_lambda(self):
        ...
        grad_norm_trend = recent_grad_norms[-1] - recent_grad_norms[0]
    ...
    def get_grokfast_stats(self) -> Dict[str, Any]:
        ...
        if self.loss_history:
            stats.update({
                ...
                'loss_trend_10': (
                    self.loss_history[-1] - self.loss_history[-10] 
                    if len(self.loss_history) >= 10 else 0.0
                ),
            })
...
if __name__ == "__main__":
    ...
    optimizer = create_grokfast_optimizer(
        model,
        learning_rate=0.01,
        grokfast_config=GrokFastConfig(alpha=0.95, lamb=1.5, adaptive_lambda=True)
    )
