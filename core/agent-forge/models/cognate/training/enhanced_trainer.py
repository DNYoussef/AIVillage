...
try:
    ...
    from grokfast_enhanced import (
        ...
        EnhancedGrokFastConfig,
        create_enhanced_grokfast_optimizer
    )
    from grokfast_config_manager import (
        GrokFastHyperparameters,
        GrokFastConfigOptimizer,
        GrokFastValidator,
...
class EnhancedCognateTrainer:
    ...
    def _setup_enhanced_training(self):
        ...
        if self.config.auto_optimize_hyperparams and EnhancedGrokFastOptimizer is not None:
            ...
            self.grokfast_config = GrokFastHyperparameters(
                ...
                target_acceleration=self.config.grokfast_target_acceleration
            ) if GrokFastHyperparameters else None
        
        # Setup enhanced optimizer
        if (self.config.enhanced_grokfast_enabled and 
            EnhancedGrokFastOptimizer is not None and 
            self.grokfast_config is not None):
    ...
    def _compile_final_results(self, total_loss: float, total_time: float) -> Dict[str, Any]:
        ...
        if isinstance(self.optimizer, EnhancedGrokFastOptimizer):
            ...
            results.update({
                ...
                'target_achieved': final_benchmark['acceleration_factor'] >= self.config.grokfast_target_acceleration * 0.1
            })
