...
from .cognate_production import (
    ...
    create_production_cognate_model,
...
def validate_agent_forge_compatibility(models: List[CognateProductionModel]) -> Dict[str, Any]:
    ...
    for i, count in enumerate(param_counts):
        ...
        if error_pct > 10.0:  # 10% is hard limit
            raise AgentForgeCompatibilityError(
                f"Model {i+1} parameter count {count:,} is {error_pct:.1f}% off target"
            )
...
def save_agent_forge_models(
    models: List[CognateProductionModel],
...
if __name__ == "__main__":
    ...
    # Create models
    result = create_agent_forge_cognate_models(
        output_dir="./test_agent_forge_models",
