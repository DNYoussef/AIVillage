...
class GatedLTMMemory(nn.Module):
    ...
    def read(
        ...
        memory_context, attention_weights, selected_indices = self.cross_attention(
            query_states=memory_queries,
            memory_keys=current_keys,
            memory_values=current_values,
            key_mask=key_mask
        )
    ...
    def write(
        ...
        if self.novelty_detector is not None and predictions is not None:
            ...
            novelty_scores = self.novelty_detector(
                query_states.mean(dim=1),  # [B, query_dim]
                current_context.mean(dim=1)  # [B, query_dim] 
            )
    ...
    def consolidate_memory(self, threshold: float = 0.01):
        ...
        with torch.no_grad():
            ...
            if reset_candidates.any():
                ...
                self.memory_keys.data[reset_candidates] = torch.randn(
                    n_reset, self.memory_dim, device=self.memory_keys.device
                ) * (2.0 / self.memory_dim) ** 0.5
                
                self.memory_values.data[reset_candidates] = torch.randn(
                    n_reset, self.memory_dim, device=self.memory_values.device
                ) * 0.1
    ...
    def reset_memory(self, keep_fraction: float = 0.0):
        ...
        with torch.no_grad():
            if keep_fraction > 0:
                ...
                if n_reset > 0:
                    self.memory_keys.data[reset_mask] = torch.randn(
                        n_reset, self.memory_dim, device=self.memory_keys.device
                    ) * (2.0 / self.memory_dim) ** 0.5
                    
                    self.memory_values.data[reset_mask] = torch.randn(
                        n_reset, self.memory_dim, device=self.memory_values.device
                    ) * 0.1
    ...
    def count_parameters(self) -> int:
        ...
        return {
            ...
            'total': trainable_params + memory_slots
        }