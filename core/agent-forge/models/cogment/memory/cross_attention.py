...
from .memory_utils import normalize_memory_slots, topk_memory_selection
...
class CrossAttentionReader(nn.Module):
    ...
    def forward(
        ...
        memory_context, attention_weights = self._cross_attention(
            query_states, selected_keys, selected_values
        )
        ...
        for b in range(B):
            for n in range(N):
                ...
                similarities = F.cosine_similarity(
                    query_pos.unsqueeze(0), memory_keys, dim=-1
                )  # [M]
    ...
    def _mean_query_selection(
        ...
        similarities = torch.einsum('bd,md->bm', 
                                  F.normalize(query_for_sim, dim=-1),
                                  F.normalize(memory_keys, dim=-1))
