...
from .memory_utils import surprisal_from_loss, normalize_memory_slots
    ...
    def forward(
        ...
        if self.competitive_updates:
            ...
            updated_keys, updated_values = self._competitive_update(
                ...
                active_queries, active_values, active_gates, active_indices
            )
        else:
...
class NoveltyDetector(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,     # [B, N, hidden_dim]
        memory_context: torch.Tensor     # [B, N, hidden_dim]
    ) -> torch.Tensor:
        """
    ...
    def enhanced_surprisal(
        self,
        prediction_surprisal: torch.Tensor,  # [B, N]
        novelty_scores: torch.Tensor         # [B, N]
    ) -> torch.Tensor:
        """
