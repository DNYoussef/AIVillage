    ...
    def forward(
        ...
        if self.divergence_type == 'kl':
            ...
            consistency_loss = F.kl_div(
                F.log_softmax(augmented_logits / self.temperature, dim=-1),
                original_probs,
                reduction='batchmean'
            )
...
class PonderLoss(nn.Module):
    ...
    def forward(
        self,
        ponder_costs: torch.Tensor,  # [B] or [B, N] expected computation steps
...
class CogmentLoss(nn.Module):
    ...
    def __init__(
        ...
        self.deep_supervision = DeepSupervisionLoss(
            base_weight=1.0,
            decay_factor=deep_decay_factor,
            min_weight=deep_min_weight
        )
        ...
        self.ponder = PonderLoss(
            initial_ponder_cost=initial_ponder_cost,
            final_ponder_cost=final_ponder_cost,
            schedule_type=ponder_schedule
        )
        ...
        self.stage_weights = stage_weights or {
            1: {'deep_supervision': 1.0, 'improvement': 0.5, 'consistency': 0.1, 'ponder': 0.5},  # Stage 1: Focus on basic supervision
            2: {'deep_supervision': 1.0, 'improvement': 1.0, 'consistency': 0.5, 'ponder': 1.0},  # Stage 2: Add improvement
            3: {'deep_supervision': 0.8, 'improvement': 1.0, 'consistency': 1.0, 'ponder': 1.0},  # Stage 3: Add consistency
            4: {'deep_supervision': 0.6, 'improvement': 1.0, 'consistency': 1.0, 'ponder': 1.2}   # Stage 4: Emphasize efficiency
        }
    ...
    def forward(
        ...
        if step_logits:
            ...
            scaled_deep_loss = (
                self.weights['deep_supervision'] * 
                stage_scaling['deep_supervision'] * 
                deep_loss
            )
        ...
        if len(step_logits) > 1:
            ...
            scaled_improvement_loss = (
                self.weights['improvement'] * 
                stage_scaling['improvement'] * 
                improvement_loss
            )
        ...
        if augmented_logits is not None and step_logits:
            ...
            scaled_consistency_loss = (
                self.weights['consistency'] * 
                stage_scaling['consistency'] * 
                consistency_loss
            )
