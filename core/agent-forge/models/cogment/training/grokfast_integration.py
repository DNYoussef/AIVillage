...
from experiments.training.grokfast import GrokFastOptimizer, create_grokfast_adamw
...
class SelectiveGrokFastManager:
        ...
        for component, is_grokking in grokking_status.items():
            if is_grokking:
                ...
                slow_ratio = component_stats.get("gradient_stats", {}).get("slow_count", 0) / max(
                    ...
                    component_stats.get("gradient_stats", {}).get("fast_count", 0), 1
                )
    ...
    def get_grokking_summary(self) -> Dict:
        ...
        return {
            ...
            "grokking_timeline": [(h["step"], sum(h["grokking_detected"].values())) for h in self.grokking_history[-10:]]
        }
