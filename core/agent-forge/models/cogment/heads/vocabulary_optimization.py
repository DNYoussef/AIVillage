...
class FactorizedVocabularyHeads(VocabularyHeadBase):
    ...
    def parameter_breakdown(self) -> dict:
        ...
        factorized_params = (
            2 * (self.d_model * self.bottleneck_dim + self.bottleneck_dim * self.vocab_size)
        )
...
class OptimizedVocabularyHeads(nn.Module):
    ...
    def __init__(
        ...
        if strategy == "tied":
            ...
            self.vocab_heads = FactorizedVocabularyHeads(
                d_model, vocab_size, bottleneck_dim, delta_scale
            )
...
def replace_vocabulary_heads_in_refinement_core(refinement_core, strategy: str = "tied"):
    ...
    original_forward = refinement_core.forward
