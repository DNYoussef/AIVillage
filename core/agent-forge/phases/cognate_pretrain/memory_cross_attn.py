    ...
    def __init__(self, 
        ...
        if memory_gate:
            self.gate_proj = nn.Sequential(
                nn.Linear(d_model + d_mem, d_model // 2),
                nn.SiLU(),
...
class MemoryScheduler:
    ...
    def should_write(self, 
        ...
        if self.write_policy == "always":
            ...
            should_write = (
                surprise is not None and surprise > self.surprise_threshold and
                novelty is not None and novelty > self.novelty_threshold
            )
...
if __name__ == "__main__":
    ...
    mem_cross_attn = create_memory_cross_attention(
        d_model=384,
        d_mem=256,
        n_heads=12,
        dropout=0.1
