...
try:
    ...
    from .unified_refiner.ltm_bank import (
...
class CognateRefiner(nn.Module):
    ...
    def __init__(self, config: CognateConfig):
        ...
        self.read_controller, self.write_controller = create_ltm_controllers(
            d_model=config.d_model,
            d_mem=config.d_mem
        )
    ...
    def from_pretrained(cls, load_directory: str, **kwargs):
        ...
        config = CognateConfig(**{k: v for k, v in config_dict.items() 
                                 if k in CognateConfig.__dataclass_fields__})
