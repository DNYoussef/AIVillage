...
def run_50_generation_cycle():
    ...
    existing_results_path = Path("core/agent-forge/phases/cognate_evomerge_output/evomerge_results.json")
    ...
    for generation in range(1, 51):  # 50 generations
        ...
        if generation % 10 == 0:  # Every 10 generations
            elite_models.extend([{
                ...
                "fitness": model["fitness"],
                "scores": model["scores"]
            } for model in elite[:2]])  # Top 2 each milestone
    ...
    # Save results
    output_path = Path("core/agent-forge/phases/evomerge_50gen_final_results.json")
