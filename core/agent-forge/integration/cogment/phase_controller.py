...
from core.agent_forge.models.cogment.training.curriculum import FourStageCurriculum, CurriculumStage
...
class CogmentPhaseController(PhaseController):
        ...
        try:
            ...
            if cogment_model is None:
                return self.create_failure_result(
                    model, "Failed to initialize Cogment model", time.time() - start_time
                )
            ...
            if final_model is None:
                return self.create_failure_result(
                    cogment_model, "4-stage curriculum failed", time.time() - start_time
                )
            ...
            return self.create_success_result(
                final_model, final_metrics, artifacts, duration
            )
            
        except Exception as e:
            logger.exception("Cogment phase controller failed")
            return self.create_failure_result(
                model, f"Unexpected error: {str(e)}", time.time() - start_time
            )
        ...
        try:
            if input_model is None:
                ...
                if issues:
                    ...
                    cogment_model = self.compatibility_validator.resolve_model_issues(
                        cogment_model, issues
                    )
    ...
    async def _save_stage_checkpoint(
        self, 
        model: Cogment, 
        stage: CurriculumStage, 
        ...
        try:
            ...
            training_summary = {
                ...
                'stage_results_summary': [
                    {
                        'stage': result.get('stage', {}).name if hasattr(result.get('stage', {}), 'name') else 'unknown',
