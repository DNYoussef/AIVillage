    ...
    def send_message(self, target: int, message: str, ttl: int = 7) -> MessageResult:
        ...
        for neighbor in neighbors:
            if random.random() < 0.7:  # 70% chance to try this route
                ...
                hop_result = MockP2PNode(neighbor, self.topology).send_message(target, message, ttl - 1)
                if hop_result.success:
                    return MessageResult(True, hop_result.hops + 1, hop_result.latency_ms + 5, "bitchat")
...
class P2PReliabilityTest:
    ...
    def test_topology_reliability(self, num_topologies: int = 100) -> bool:
        ...
        success_rate = successful_transmissions / total_transmissions if total_transmissions > 0 else 0.0
        self.results["overall_success_rate"] = success_rate

        # Require ≥ 90% success rate
        return success_rate >= 0.90

    def _generate_random_topology(self, num_nodes: int, packet_loss: float) -> NetworkTopology:
    ...
    def test_store_and_forward(self) -> bool:
        ...
        return (
            store_forward_tests["offline_queue"]["success"] and store_forward_tests["connectivity_restored"]["success"]
        )
    ...
    def run_all_tests(self) -> bool:
        ...
        print(f"     Topology tests: {'PASS' if topology_success else 'FAIL'} (Success rate: {success_rate:.1%})")
        ...
        overall_success = import_success and policy_success and topology_success and store_forward_success
...
def main():
    ...
    try:
        ...
        # Save results
        results_file = Path(__file__).parent.parent / "artifacts" / "p2p_reliability.json"
