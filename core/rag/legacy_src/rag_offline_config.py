...
class OfflineRAGConfig:
    ...
    def adapt_for_mobile(self) -> "OfflineRAGConfig":
        ...
        base_dir = Path(self.embedding.model_path).parent if self.embedding.model_path else None
        ...
        mobile_config = OfflineRAGConfig(
            ...
            embedding=EmbeddingConfig(
                ...
                model_path=str(resolve_model_path("all-MiniLM-L6-v2.onnx", device="mobile", base_dir=base_dir)),
                dimensions=384,
        ...
        if self.embedding.provider == EmbeddingProvider.LOCAL_ONNX:
            if not self.embedding.model_path:
                ...
                if not model_path.exists():
                    errors.append(f"Local ONNX model path not found: {self.embedding.model_path}")
...
class OfflineRAGConfigRegistry:
    ...
    def get_registry_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "config_modes": list({config.mode.value for config in self.validated_configs.values()}),
            "embedding_providers": list(
                {config.embedding.provider.value for config in self.validated_configs.values()}
            ),
            "vector_store_types": list({config.vector_store_type.value for config in self.validated_configs.values()}),
            "retriever_types": list({config.retriever_type.value for config in self.validated_configs.values()}),
        }
