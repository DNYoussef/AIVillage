...
try:
    ...
    from training_pipeline import CognateDataset, CognateTrainingPipeline, TrainingConfig

    IMPORTS_SUCCESS = True
    logger.info("Successfully imported all Cognate components")
except ImportError as e:
    logger.error(f"Import error: {e}")
    # Create fallback mock classes for demonstration
...
def pretrain_single_model(
    model: Enhanced25MCognate, train_config: TrainingConfig, output_dir: str, model_name: str
) -> dict[str, Any]:
    ...
    for i, model in enumerate(models):
        ...
        try:
            stats = pretrain_single_model(
                model=model, train_config=train_config, output_dir=str(output_dir), model_name=model_name
            )
