...
class AgentMetricsCollector:
    ...
    def fail_task(
        ...
        self.recent_errors.append(
            {"timestamp": datetime.now(), "task_id": task_id, "error": error_message, "category": error_category}
        )
    ...
    def get_task_statistics(self, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
        ...
        if time_window:
            ...
            relevant_tasks = [task for task in self.completed_tasks if task.start_time >= cutoff_time]
        else:
    ...
    def export_metrics(self, include_task_history: bool = False) -> Dict[str, Any]:
        ...
        export_data = {
            ...
            "memory_usage_history": [
                {"timestamp": ts.isoformat(), "memory_mb": mem} for ts, mem in list(self.memory_usage_history)
            ],
            "custom_metrics": self.custom_metrics,
        }

        if include_task_history:
            export_data["task_history"] = [asdict(task) for task in list(self.completed_tasks)]
...
class MultiAgentMetricsManager:
    ...
    def get_performance_insights(self) -> Dict[str, Any]:
        ...
        for agent_id, collector in self.agent_collectors.items():
            health = collector.get_health_metrics()
            collector.get_task_statistics(timedelta(hours=1))  # Last hour
        ...
        problematic = [
            agent for agent in agent_performances if agent["health_score"] < 60 or agent["success_rate"] < 0.8
        ]
        ...
        if self.system_metrics["system_error_rate"] > 0.1:
            insights["recommendations"].append("System error rate is above 10% - investigate common failure patterns")
        ...
        while self.is_running:
            try:
                ...
                export_data = {
                    ...
                    "detailed_metrics": {
                        agent_id: collector.export_metrics() for agent_id, collector in self.agent_collectors.items()
                    },
                }
    ...
    async def demo():
        ...
        if insights["problematic_agents"]:
            ...
            for agent in insights["problematic_agents"]:
                print(
                    f"  - {agent['agent_id']}: Health {agent['health_score']:.1f}, Success {agent['success_rate']:.1%}"
                )
