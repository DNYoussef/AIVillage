...
try:
    from ...infrastructure.p2p.bitchat.ble_transport import BitChatTransport, BitChatMessage
...
class UnifiedDecentralizedSystem:
    ...
    def __init__(self, 
        ...
        logger.info(f"Enabled transports: BitChat={enable_bitchat}, BetaNet={enable_betanet}, Mobile={enable_mobile_bridge}")
            ...
            if TRANSPORTS_AVAILABLE and self.transports:
                try:
                    from ...infrastructure.p2p.core.transport_manager import (
                        TransportManager, TransportPriority, TransportCapabilities
                    )
    ...
    async def send_message(self, 
                          ...
                          priority: MessagePriority = MessagePriority.NORMAL,
                          transport_preference: Optional[DecentralizedTransportType] = None,
                          requires_privacy: bool = False) -> bool:
    ...
    async def _send_via_transport_manager(self, message: DecentralizedMessage) -> bool:
        ...
        try:
            ...
            from ...infrastructure.p2p.core.message_types import UnifiedMessage, MessageType
            ...
            unified_message = UnifiedMessage(
                message_type=msg_type_map.get(message.message_type, MessageType.DATA),
    ...
    async def _process_received_message(self, message: DecentralizedMessage, transport_type: DecentralizedTransportType):
    ...
    async def _message_cleanup_loop(self):
        ...
        while self._running:
            try:
                ...
                expired_acks = [
                    msg_id for msg_id, message in self.pending_acks.items()
                ...
                expired_peers = [
                    peer_id for peer_id, timestamp in self.peer_discovery_cache.items()
                ...
                if expired_messages or expired_acks or expired_peers:
                    logger.debug(f"Cleaned up {len(expired_messages)} messages, {len(expired_acks)} acks, {len(expired_peers)} peers")
    ...
    async def _send_acknowledgment(self, original_message: DecentralizedMessage, transport_type: DecentralizedTransportType):
