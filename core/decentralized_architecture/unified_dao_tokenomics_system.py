...
class UnifiedDAOTokenomicsSystem:
    ...
    def create_proposal(
        ...
        if voting_power < self.config.min_proposal_power:
            self.logger.warning(f"Insufficient voting power for proposal: {voting_power} < {self.config.min_proposal_power}")
            return None
    ...
    def cast_vote(
        ...
        self.award_tokens(
            voter_id,
            TokenAction.GOVERNANCE_PARTICIPATION,
            proposal_id=proposal_id,
            choice=choice.value
        )
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "earning_rules": {
                action.value: rule.base_amount 
                for action, rule in self.earning_rules.items()
            }
        }
