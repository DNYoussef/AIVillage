...
class CognateEvoConfig:
    ...
    fitness_weights: dict[str, float] = field(
        default_factory=lambda: {"code": 0.25, "math": 0.25, "reasoning": 0.25, "language": 0.25}
    )
...
class CognateEvoMerge:
            ...
            try:
                ...
                if recipe["primary"] == "linear":
                    ...
                    merged = self.merge_ops.slerp_merge(
                        parent_models[model_indices[0]], parent_models[model_indices[1]], recipe["t"]
                    )
                elif recipe["primary"] == "ties":
                    merged = self.merge_ops.ties_merge(parent_models, recipe["threshold"])
                elif recipe["primary"] == "dare":
                    merged = self.merge_ops.dare_merge(parent_models, recipe["threshold"], recipe["amplification"])
    ...
    def create_next_generation(
        self, population: list[CognateCandidate], parent_models: list[UnifiedRefiner]
    ...
    def run_evolution(self) -> dict[str, Any]:
        ...
        for gen in range(self.config.generations):
            ...
            if self.plateau_counter >= self.convergence_patience:
                logger.info(
                    f"🎯 Early convergence after {gen + 1} generations (no improvement for {self.convergence_patience} generations)"
                )
                break
        ...
        total_improvement = (
            ((peak_fitness - self.fitness_history[0]) / self.fitness_history[0] * 100) if self.fitness_history else 0
        )
        ...
        results = {
            "evolution_summary": {
                ...
                "convergence_reason": (
                    "early_stopping" if self.plateau_counter >= self.convergence_patience else "completed"
                ),
