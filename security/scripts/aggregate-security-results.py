...
class SecurityResultsAggregator:
    ...
    def _generate_recommendations(self):
        ...
        if critical_count > 0:
            recommendations.append(f"🚨 CRITICAL: {critical_count} critical vulnerabilities require immediate attention")
        ...
        if high_count > 0:
            recommendations.append(f"⚠️  HIGH: {high_count} high-severity vulnerabilities should be addressed within 7 days")
            
        # Ecosystem-specific recommendations
        for ecosystem, counts in self.report.ecosystems.items():
            total = sum(counts.values())
            if total > 0:
                recommendations.append(f"📦 {ecosystem.upper()}: {total} vulnerabilities found - consider dependency updates")
