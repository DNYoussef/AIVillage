...
class HybridRetriever:
    def __init__(self, config: RAGConfig, agent: AgentInterface):
        self.config = config
        self.agent = agent
        self.llm = agent.llm
    ...
    async def plan_retrieval(self, query: str) -> List[Dict[str, Any]]:
        """
        Generate a retrieval plan using the LLM.

        :param query: The user's query string.
        :return: A list of steps with 'type' and 'k' keys.
        """
        plan_prompt = f"Generate a retrieval plan for the query: {query}. Provide steps as a JSON list with 'type' ('vector' or 'graph') and 'k' values."
    ...
    def _parse_plan(self, plan_text: str) -> List[Dict[str, Any]]:
        """
        Parse the LLM-generated plan into structured steps.

        :param plan_text: The raw plan text generated by the LLM.
