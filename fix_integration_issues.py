...
def verify_fixes():
    ...
    databases = [
        ("evolution_metrics.db", ["evolution_rounds", "fitness_metrics", "resource_metrics", 
                                  "selection_outcomes", "kpi_tracking"]),
        ("digital_twin.db", ["learning_profiles", "learning_sessions", "knowledge_states", 
                            "privacy_settings"]),
        ("rag_index.db", ["documents", "chunks", "embeddings_metadata", "search_cache"])
    ]
    ...
    for db_file, expected_tables in databases:
        ...
        try:
            ...
            tables = [row[0] for row in cursor.fetchall() if row[0] != 'sqlite_sequence' and row[0] != 'schema_version']
