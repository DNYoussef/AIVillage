...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('fog_integration_tests.log')
    ]
)
        ...
        if test_suite.overall_metrics:
            ...
            if category_breakdown:
                ...
                for category, stats in category_breakdown.items():
                    ...
                    category_emoji = "[PASS]" if category_success >= 80 else "[WARN]" if category_success >= 50 else "[FAIL]"
                    print(f"   {category_emoji} {category}: {stats['passed']}/{stats['total']} ({category_success:.1f}%)")
