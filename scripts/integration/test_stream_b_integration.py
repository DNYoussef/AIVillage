...
def test_tokenomics(tokenomics_system):
    ...
    try:
        ...
        tokenomics_system.create_account("test_account_2", "0xaccount2", 5000)
...
def test_integration(dao_system, tokenomics_system, compliance_system):
    ...
    try:
        ...
        # Add to DAO
        dao_system.add_member(member_id=user_id, address="0xintegration123", role=MemberRole.MEMBER, voting_power=8000)

        # Create tokenomics account
        tokenomics_system.create_account(account_id=user_id, address="0xintegration123", initial_balance=15000)

        # Register with compliance
        compliance_system.register_entity(entity_id=user_id, entity_type="individual", jurisdiction="US")
        ...
        tokenomics_system.distribute_governance_rewards(user_id, 1, 1)

        # Monitor governance action for compliance
        compliance_system.monitor_governance_action(
            {"action_id": proposal_id, "action_type": "proposal_creation", "actor_id": user_id, "budget_request": 25000}
        )
