...
def generate_report():
    ...
    print(f"Memory: {sys_info.get('available_memory_gb', 0):.1f}GB available / {sys_info.get('total_memory_gb', 0):.1f}GB total")
    ...
    if comp.get("status") == "completed":
        ...
        # Calculate averages
        successful_ratios = [d.get("compression_ratio", 0) for d in compressions.values() if d.get("status") == "success"]
