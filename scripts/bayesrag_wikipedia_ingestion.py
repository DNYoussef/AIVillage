...
class BayesRAGWikipediaIngestion:
    ...
    def _extract_geographic_context(self, content: str) -> Optional[str]:
        ...
        geo_patterns = ['Germany', 'France', 'England', 'Italy', 'Spain', 'Europe', 'Asia', 'Africa']
    ...
    async def ingest_article_with_bayesrag(self, title: str) -> Tuple[GlobalContext, List[LocalContext]]:
        ...
        try:
            ...
            wiki_wiki = wikipedia.Wikipedia(
                language='en',
                user_agent='BayesRAG/1.0 (AIVillage Research Project)'
            )
    ...
    async def _store_contexts(self, global_context: GlobalContext, local_contexts: List[LocalContext]):
