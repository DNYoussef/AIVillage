...
def create_evolutionary_tree():
    ...
    for gen_idx, gen_data in enumerate(generations):
        ...
        for i, individual in enumerate(population):
            ...
            if fitness == max(ind['fitness'] for ind in population):
                ax1.scatter(x, y, s=size+50, c='gold', marker='*', 
            ...
            # Add fitness label
            ax1.annotate(f'{fitness:.2f}', (x, y), xytext=(5, 5), 
    ...
    ax2.plot(gen_numbers, best_fitness, 'o-', color='#6BCF7F', 
             linewidth=3, markersize=8, label='Best Fitness')
    ax2.plot(gen_numbers, avg_fitness, 's-', color='#FFD93D', 
    ...
    wedges, texts, autotexts = ax3.pie(counts, labels=methods, colors=pie_colors, 
    ...
    for i, (metric, scores) in enumerate(benchmark_data.items()):
        ...
        ax4.plot(gen_numbers, scores, 'o-', color=color, 
    ...
    plt.tight_layout()
    plt.suptitle('🧬 Agent Forge Evolution Merge - Evolutionary Analysis', 
