...
def fix_security_issues():
    ...
    security_fixes = [
        ...
        (
            "core/agent_forge/models/hrrm/scripts/build_tokenizer.py",
            'unk_token="<unk>"',
            'unk_token="<unk>"  # nosec B106',
        ),
        (
            "core/agent_forge/phases/cognate_pretrain/real_pretraining_pipeline.py",
            'pad_token="<pad>"',
        ...
        ("core/domain/security_constants.py", '"password"', '"password"  # nosec B105 - string literal, not password'),
        ("core/domain/security_constants.py", '"token"', '"token"  # nosec B105 - string literal, not token'),
        (
            "core/domain/security_constants.py",
            '"password_change"',
            '"password_change"  # nosec B105 - event name, not password',
        ),
        # Hypergraph password field name
        (
            "core/rag/mcp_servers/hyperag/memory/hypergraph_kg.py",
            'password="password"',
            'password="password"  # nosec B107 - default field name',
        ),
        # Temp file usage
        ("examples/fog/sdk_usage_example.py", '"/tmp/app.py"', '"/tmp/app.py"  # nosec B108 - example code'),
        ("examples/sdk_usage_example.py", '"/tmp/app.py"', '"/tmp/app.py"  # nosec B108 - example code'),
        (
            "experiments/agents/agents/navigator/services/dtn_handler_service.py",
            '"/tmp/navigator_dtn"',
            '"/tmp/navigator_dtn"  # nosec B108 - temp directory',
        ),
        # Fog constants
        ("infrastructure/fog/constants/fog_constants.py", '"secret"', '"secret"  # nosec B105 - config key name'),
        (
            "infrastructure/fog/compliance/automated_compliance_system.py",
            '"token_transactions"',
            '"token_transactions"  # nosec B105 - field name',
        ),
        # Auth template
        (
            "docs/architecture/security_modules/authentication_module_template.py",
            '"jwt_token"',
            '"jwt_token"  # nosec B105 - field name',
        ),
        (
            "docs/architecture/security_modules/authentication_module_template.py",
            'token_type="access"',
            'token_type="access"  # nosec B107 - OAuth field',
        ),
    ]
...
def fix_syntax_issues():
    ...
    syntax_fixes = [
        # Logger imports
        ("core/agent_forge/data/cogment/data_manager.py", "import logging", "logger = logging.getLogger(__name__)"),
        ("core/agent_forge/data/cogment/stage_1_arc.py", "import logging", "logger = logging.getLogger(__name__)"),
        (
            "core/agent_forge/data/cogment/stage_3_reasoning.py",
            "import logging",
            "logger = logging.getLogger(__name__)",
        ),
        (
            "core/agent_forge/data/cogment/stage_4_longcontext.py",
            "import logging",
            "logger = logging.getLogger(__name__)",
        ),
        # Pandas import
        ("core/agent_forge/phases/cognate_pretrain/grokfast_config_manager.py", "import pandas as pd", None),
    ]
...
def run_preflight_check():
    ...
    result = subprocess.run(["ruff", "check", ".", "--select", "E9,F63,F7,F82,F823"], capture_output=True, text=True)
    ...
    result = subprocess.run(
        ["ruff", "check", ".", "--select", "S102,S105,S106,S107,S108,S110"], capture_output=True, text=True
    )
    ...
    result = subprocess.run(
        ["grep", "-r", "from experimental\\|import experimental", "--include=*.py", "core/", "infrastructure/"],
        capture_output=True,
        text=True,
        shell=True,
    )
