...
class ProcessTaskBenchmarker:
    ...
    async def create_benchmark_agent(self):
        ...
        config.capabilities = [
            "general", "text_generation", "data_analysis", 
    ...
    def create_benchmark_task(self, task_id: str, task_type: str = "general", 
                            content: str = None, size: str = "small"):
    ...
    async def run_throughput_benchmark(self, agent, num_tasks: int = 50, 
        ...
        tasks = [
            self.create_benchmark_task(f"throughput_{i}", "general", size="small")
            for i in range(num_tasks)
        ]
        ...
        for i in range(0, num_tasks, batch_size):
            ...
            batch_results = await asyncio.gather(*[
                self._process_task_with_timing(agent, task) 
    ...
    def analyze_performance(self, results: List[BenchmarkResult]):
        ...
        if target_meeting_results:
            avg_target_meeting = sum(r.meets_100ms_target_percentage for r in target_meeting_results) / len(target_meeting_results)
