...
class ArchitecturalQualityGate:
            ...
            if result.returncode == 0:
                ...
                if not passed:
                    self.violations.append(
                        f"Coupling score {avg_coupling:.1f} exceeds threshold {threshold}"
                    )
                    self.recommendations.append(
                        "Refactor high-coupling modules using dependency injection"
                    )
    ...
    def _evaluate_complexity(self, threshold: int) -> QualityMetric:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if not passed:
                    self.violations.append(
                        f"Maximum complexity {max_complexity} exceeds threshold {threshold}"
                    )
                    self.recommendations.append(
    ...
    def _evaluate_magic_literals(self, threshold: float) -> QualityMetric:
        ...
        if not passed:
            self.violations.append(
                f"Magic literal density {magic_density:.1f}% exceeds threshold {threshold}%"
            )
            self.recommendations.append(
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if not passed:
                    ...
                    self.recommendations.append(
                        "Refactor strong connascence to weaker forms"
                    )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if not passed:
                    ...
                    self.recommendations.append(
                        "Eliminate anti-patterns through systematic refactoring"
                    )
    ...
    def _get_coupling_level(self, coupling: float) -> QualityLevel:
        ...
        if coupling <= 5: return QualityLevel.EXCELLENT
        elif coupling <= 8: return QualityLevel.GOOD
        elif coupling <= 12: return QualityLevel.ACCEPTABLE
        elif coupling <= 20: return QualityLevel.POOR
    ...
    def _get_complexity_level(self, complexity: int) -> QualityLevel:
        ...
        if complexity <= 5: return QualityLevel.EXCELLENT
        elif complexity <= 10: return QualityLevel.GOOD
        elif complexity <= 15: return QualityLevel.ACCEPTABLE
        elif complexity <= 25: return QualityLevel.POOR
        else: return QualityLevel.CRITICAL
    
    def _get_magic_literal_level(self, density: float) -> QualityLevel:
        """Determine quality level based on magic literal density."""
        if density <= 5: return QualityLevel.EXCELLENT
        elif density <= 10: return QualityLevel.GOOD
        elif density <= 20: return QualityLevel.ACCEPTABLE
        elif density <= 35: return QualityLevel.POOR
        else: return QualityLevel.CRITICAL
...
def main():
    ...
    parser.add_argument("--coupling-threshold", type=float, default=12.0,
                       help="Maximum allowed coupling score")
    ...
    parser.add_argument("--god-object-threshold", type=int, default=500,
                       help="Maximum lines of code per class")
    parser.add_argument("--magic-literal-threshold", type=float, default=20.0,
                       help="Maximum magic literal density percentage")
    parser.add_argument("--output", type=str, default="quality_gate_result.json",
