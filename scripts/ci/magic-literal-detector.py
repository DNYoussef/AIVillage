...
class MagicLiteralDetector:
    ...
    def __init__(self, threshold_percentage: float = 20.0):
        ...
        self.safe_contexts = {
            'test_', 'assert', 'raise', 'logging', 'print',
            '__str__', '__repr__', '__name__', '__main__',
    ...
    def format_violations(self, results: List[Dict], show_details: bool = True) -> str:
        ...
        violations.sort(
            key=lambda x: (severity_order.get(x['severity'], 0), x['density']),
            reverse=True
        )
        ...
        for result in violations:
            ...
            output.append(f"   Density: {result['density']:.1f}% ({result['magic_count']}/{result['total_literals']} literals)")
...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=20.0,
                       help="Magic literal density threshold percentage")
    ...
    filtered_results = [
        r for r in results
        if r.get('threshold_exceeded', False) and
           severity_levels.get(r.get('severity', 'low'), 1) >= min_level
    ]
