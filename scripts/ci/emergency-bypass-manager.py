...
class EmergencyBypassManager:
    ...
    def request_emergency_bypass(
        self, scenario: str, requestor: str, justification: str, commit_sha: str, token: str | None = None
    ) -> tuple[bool, str]:
    ...
    def check_bypass_status(self, commit_sha: str) -> tuple[bool, BypassRequest | None]:
        """Check if a commit has an active bypass."""
        try:
            if not self.bypass_log.exists():
                return False, None

            with open(self.bypass_log) as f:
    ...
    def _retrieve_token(self, token: str) -> BypassToken | None:
