...
class QualityHistoryManager:
    ...
    def update_quality_metrics(self, commit_sha: str, quality_score: float, reports_dir: Path, branch: str = "main"):
        ...
        metrics = self._extract_metrics(quality_gate_data, coupling_data, antipatterns_data)
    ...
    def _extract_metrics(self, quality_gate_data: dict, coupling_data: dict, antipatterns_data: dict) -> dict:
    ...
    def _check_quality_regression(
        self, cursor: sqlite3.Cursor, commit_sha: str, current_score: float, current_metrics: dict
    ):
    ...
    def _create_alert(self, cursor: sqlite3.Cursor, commit_sha: str, alert_type: str, severity: str, message: str):
    ...
    def get_quality_trends(self, days: int = 30) -> list[dict]:
        ...
        return [
            {"date": row[0], "avg_score": row[1], "trend": row[2], "debt_score": row[3], "violations": row[4]}
            for row in results
        ]
...
def main():
    ...
    parser.add_argument("--quality-score", type=float, required=True, help="Overall quality score")
    parser.add_argument("--reports-dir", type=str, default=".", help="Directory containing quality reports")
    parser.add_argument("--branch", type=str, default="main", help="Git branch name")
    parser.add_argument("--db-path", type=str, default="quality_history.db", help="Path to quality history database")
    ...
    history_manager.update_quality_metrics(
        commit_sha=args.commit_sha, quality_score=args.quality_score, reports_dir=reports_dir, branch=args.branch
    )
    ...
    if alerts:
        ...
        for alert in alerts:
            severity_emoji = {"low": "📝", "medium": "⚠️", "high": "🚨", "critical": "🔥"}.get(alert["severity"], "📝")
