...
class LintingFixTask(BaseCleanupTask):
    ...
    def analyze(self, project_root: Path) -> dict[str, Any]:
        ...
        python_files = [f for f in python_files if not any(
            part in str(f).lower() for part in ['deprecated', 'archive', 'experimental', '__pycache__']
        )]
    ...
    def execute(self, project_root: Path, analysis_result: dict[str, Any]) -> CleanupResult:
        ...
        try:
            ...
            if not self.config.dry_run:
                ...
                subprocess.run(['ruff', 'check', '--select=W291,W292,W293', '--fix', str(project_root)], 
                              capture_output=True)
...
class ImportFixTask(BaseCleanupTask):
    ...
    def analyze(self, project_root: Path) -> dict[str, Any]:
        python_files = list(project_root.rglob("*.py"))
        python_files = [f for f in python_files if not any(
            part in str(f).lower() for part in ['deprecated', 'archive', '__pycache__']
        )]
...
class UnifiedCleanupManager:
    ...
    def __init__(self, config: CleanupConfig, project_root: Path | None = None):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('unified_cleanup.log'),
