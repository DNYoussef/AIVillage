...
class LintingAnalyzer:
    ...
    def _fix_too_many_blank_lines(self, lines: List[str], issue: LintingIssue) -> List[str]:
        ...
        while (line_idx > 0 and 
               line_idx < len(lines) and 
               lines[line_idx].strip() == "" and 
    ...
    def generate_report(self, report: LintingReport, fixes_applied: Dict[str, int] = None) -> str:
        ...
        report_lines = [
            ...
            f"**Generated**: {str(Path.cwd())} at {subprocess.check_output(['date'], text=True).strip()}",
            "",
            "## Summary",
        ...
        if report.critical_issues:
            report_lines.append(f"1. **Fix critical issues**: {len(report.critical_issues)} critical issues need immediate attention")
...
def main():
    ...
    parser.add_argument("--source-dirs", nargs="+", default=["src", "tests", "scripts"],
                      help="Source directories to analyze")
    parser.add_argument("--fix", action="store_true", help="Apply automatic fixes")
    parser.add_argument("--report-file", default="linting_analysis_report.md",
