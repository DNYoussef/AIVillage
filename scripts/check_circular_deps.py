...
class CircularDependencyDetector:
    ...
    def _is_internal_module(self, module_name: str) -> bool:
        ...
        return (
            (module_path.with_suffix(".py")).exists() or (module_path / "__init__.py").exists() or module_path.is_dir()
        )
    ...
    def find_dependency_path(self, from_module: str, to_module: str) -> list[str] | None:
    ...
    def format_results(self, show_details: bool = True) -> str:
        ...
        sorted_deps = sorted(
            self.circular_deps, key=lambda x: (severity_order.get(x["severity"], 0), x["cycle_length"]), reverse=True
        )

        severity_emojis = {"high": "🚨", "medium": "⚠️", "low": "📝"}
...
def main():
    ...
    parser.add_argument("--project-root", type=str, default=".", help="Root directory of the project to analyze")
    parser.add_argument("--json", action="store_true", help="Output results in JSON format")
    parser.add_argument("--count", action="store_true", help="Output only the count of circular dependencies")
    parser.add_argument("--details", action="store_true", default=True, help="Show detailed information about cycles")
    parser.add_argument("--find-path", nargs=2, metavar=("FROM", "TO"), help="Find dependency path between two modules")
    ...
    if args.count:
        ...
        results = {"circular_dependencies": circular_deps, "statistics": detector.get_dependency_statistics()}
