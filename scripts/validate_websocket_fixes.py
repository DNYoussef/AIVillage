...
def validate_file_security(file_path):
    ...
    if re.search(eval_pattern, content):
        print(f"  CRITICAL: Still using eval(data) - RCE vulnerability present!")
        return False
    
    # Check that json.loads is used instead
    json_pattern = r'json\.loads\s*\(\s*data\s*\)'
    if re.search(json_pattern, content):
        print(f"  PASS: Using json.loads(data) instead of eval()")
    else:
        print(f"  WARNING: No json.loads(data) found")
    ...
    if not validation_found:
        print(f"  WARNING: No input validation patterns found")
