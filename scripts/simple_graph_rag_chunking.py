...
class SimpleGraphRAGChunker:
    ...
    async def _build_chunk_relationships(self) -> None:
        ...
        for i in range(0, len(chunk_list), batch_size):
            ...
            for _j, chunk1 in enumerate(batch):
                ...
                for chunk2 in chunk_list:
                    ...
                    if relationship:
                        ...
                        if self.graph:
                            self.graph.add_edge(chunk1.chunk_id, chunk2.chunk_id, **relationship.to_dict())
    ...
    async def _apply_basic_repairs(self) -> None:
        ...
        for isolated_chunk in isolated_chunks[:10]:  # Limit to avoid performance issues
            ...
            for other_chunk in self.chunks.values():
                if (
                    isolated_chunk.chunk_id == other_chunk.chunk_id
                    or isolated_chunk.embedding is None
                    or other_chunk.embedding is None
                ):
                    continue
        ...
        for chunks in topic_coverage.values():
    ...
    def _classify_relationship_type(self, chunk1: SimpleChunk, chunk2: SimpleChunk, similarity: float) -> str:
        ...
        if chunk1.document_id == chunk2.document_id:
            if abs(chunk1.position - chunk2.position) == 1:
                return "continues"
            if abs(chunk1.position - chunk2.position) <= 3:
                return "elaborates"
            return "references"
