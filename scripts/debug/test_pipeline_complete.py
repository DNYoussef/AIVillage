...
def test_unified_pipeline_import():
    ...
    try:
        ...
        pipeline_path = project_root / "core" / "agent-forge" / "core" / "unified_pipeline.py"
...
def main():
    ...
    overall_success = (
        ...
        len(working_phases) >= 6 and  # At least 6 out of 7 phases working
        execution_ready
    )
    ...
    if overall_success:
        ...
        print(f"- Unified pipeline can be imported and initialized")
        print(f"- {len(working_phases)} out of 7 phases are working ({len(working_phases)/7*100:.1f}%)")
        print(f"- Pipeline is ready for execution")
        ...
        if not execution_ready:
            print(f"- Pipeline execution environment not ready")
