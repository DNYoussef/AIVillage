...
class AutoTestFixer:
    ...
    def fix_failures(
        self, failure_type: str, failure_data: str, pr_number: Optional[int] = None, auto_commit: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if fix_result.get("success"):
                ...
                if applied:
                    ...
                    return {"fixed": False, "fix_applied": False, "error": "Failed to apply fix", "failure": failure}
            else:
                return {
                    "fixed": False,
                    "fix_applied": False,
            ...
            return {"fixed": False, "fix_applied": False, "error": str(e), "failure": failure}
    ...
    def _generate_fix_with_claude(self, context: Dict[str, Any], failure_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {"success": True, "fix_code": fix_code, "explanation": result.stdout}
            else:
                return {"success": False, "error": f"Claude Flow failed: {result.stderr}"}
...
Co-authored-by: Claude <claude@anthropic.com>"""
            ...
            result = subprocess.run(
                ["git", "commit", "-m", commit_msg], capture_output=True, text=True, cwd=self.project_root
            )
...
def main():
    ...
    parser.add_argument("--failure-data", required=True, help="JSON string or file path containing failure data")
    parser.add_argument("--pr-number", type=int, help="Pull request number")
    parser.add_argument("--auto-commit", action="store_true", help="Automatically commit successful fixes")
