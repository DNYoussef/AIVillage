                ...
                for file_path in search_dir.rglob("*.json"):
                    try:
                        with open(file_path) as f:
                            content = f.read()
                            if "sha256" in content.lower() and ("embedding" in content.lower() or "vector" in content.lower()):
        ...
        for file_path in legacy_files["sha256_files"]:
            try:
                ...
                if isinstance(data, dict):
                    ...
                    docs = data if data and isinstance(data[0], dict) else [{"content": str(item), "id": f"doc_{i}"} for i, item in enumerate(data)]
                else:
                    docs = [{"content": str(data), "id": file_path.stem}]
        ...
        for doc in documents:
            try:
                ...
                for chunk_idx, chunk_text in enumerate(chunks):
                    ...
                    index.add_with_ids(
                        np.array([embedding]),
                        np.array([chunk_hash], dtype="int64")
                    )
            ...
            try:
                ...
                for score, chunk_id in zip(scores[0], ids[0], strict=False):
                    if chunk_id != -1:  # Valid result
                        cursor.execute("SELECT chunk_id, content FROM chunks WHERE ? IN (SELECT faiss_index_id FROM embeddings_metadata WHERE chunk_id = chunks.chunk_id)", (int(chunk_id),))
...
def main():
    ...
    print(f"Successful retrievals: {report['accuracy_test']['successful_retrievals']}/{report['accuracy_test']['queries_tested']}")
