...
from ..tee.tee_runtime_manager import TEERuntimeManager, TEERuntime
from ..proofs.proof_generator import ProofGenerator
from ..proofs.proof_verifier import ProofVerifier  
from ..market.pricing_manager import DynamicPricingManager, ResourceLane, MarketCondition
from ..scheduler.placement import FogScheduler, PlacementStrategy
...
class ComponentHealth:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            self.health in [SystemHealth.HEALTHY, SystemHealth.DEGRADED] and
            self.last_heartbeat and
            (datetime.now(UTC) - self.last_heartbeat) < timedelta(minutes=2)
        )
...
class FogSystemManager:
    ...
    def _init_component_health(self):
        ...
        components = [
            "tee_runtime", "proof_generator", "proof_verifier", 
            "pricing_manager", "scheduler", "quorum_manager",
            "onion_router", "reputation_engine", "vrf_selector"
        ]
    ...
    async def _collect_system_metrics(self):
        ...
        try:
            ...
            if self.component_health["pricing_manager"].is_healthy():
                ...
                volatilities = [
                    lane["volatility_24h"] 
                    for lane in pricing_analytics["lane_analytics"].values()
                ]
    ...
    def _needs_recovery(self, health: ComponentHealth) -> bool:
        ...
        return (
            health.status == ComponentStatus.ERROR or
            health.health == SystemHealth.CRITICAL or
            health.error_count >= self.config["max_error_count"] or
            (health.last_heartbeat and 
             (datetime.now(UTC) - health.last_heartbeat) > timedelta(seconds=self.config["component_timeout_seconds"]))
        )
