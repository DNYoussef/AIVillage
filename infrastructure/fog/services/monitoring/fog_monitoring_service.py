...
class FogMonitoringService(BaseFogService):
    ...
    async def health_check(self) -> ServiceHealthCheck:
        ...
        try:
            ...
            recent_critical = sum(1 for alert in self.alert_history[-10:] if alert.get("severity") == "critical")
    ...
    async def _handle_service_stopped(self, event):
        ...
        if event.data.get("unexpected", False):
            await self.create_alert(
                "service_stopped", "warning", f"Service {service_name} stopped unexpectedly", service_name
            )
    ...
    async def _system_monitoring_task(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if cpu_usage > self.alert_thresholds["cpu_usage_percent"]:
                    await self.create_alert("high_cpu_usage", "warning", f"High CPU usage: {cpu_usage:.1f}%")

                # Monitor memory usage
                memory = psutil.virtual_memory()
                if memory.percent > self.alert_thresholds["memory_usage_percent"]:
                    await self.create_alert("high_memory_usage", "warning", f"High memory usage: {memory.percent:.1f}%")

                # Monitor disk usage
                disk = psutil.disk_usage("/")
                if disk.percent > self.alert_thresholds["disk_usage_percent"]:
                    await self.create_alert("high_disk_usage", "critical", f"High disk usage: {disk.percent:.1f}%")
            ...
            try:
                ...
                if len(self.alert_history) > 10:
                    ...
                    if recent_alert_count > 5:
                        await self.create_alert(
                            "alert_storm", "critical", f"Alert storm detected: {recent_alert_count} alerts in 5 minutes"
                        )
    ...
    async def _metrics_collection_task(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                await self.publish_event(
                    "monitoring_metrics_update",
                    {"metrics": stats["monitoring_metrics"], "timestamp": datetime.now(UTC).isoformat()},
                )
