...
class FogHarvestingService(BaseFogService):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.resource_manager = MobileResourceManager(
                ...
                token_rewards_enabled=self.config.get("enable_tokens", True)
            )
    ...
    async def health_check(self) -> ServiceHealthCheck:
        ...
        try:
            ...
            error_rate = (self.metrics["failed_harvests"] / 
                         max(1, self.metrics["successful_harvests"] + self.metrics["failed_harvests"]))
    ...
    async def assign_compute_task(self, request_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if assigned_device:
                ...
                await self.publish_event("task_assigned", {
