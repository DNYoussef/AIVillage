...
class FogConfigurationService(BaseFogService):
    ...
    async def update_configuration(
        ...
        try:
            ...
            await self.publish_event("configuration_updated", {
                "key_path": key_path,
                "value": value,
    ...
    async def reload_configuration(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                await self.publish_event("configuration_reloaded", {
                    "timestamp": datetime.now(UTC).isoformat()
                })
    ...
    async def get_configuration_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats.update({
                ...
                "watchers_by_path": {
                    path: len(watchers) for path, watchers in self.config_watchers.items()
                }
            })
    ...
    async def _handle_config_update(self, event):
        ...
        await self.publish_event("config_update_response", {
    ...
    async def _handle_config_reload(self, event):
        ...
        await self.publish_event("config_reload_response", {
            "request_id": event.data.get("request_id"),
            "success": success
        })
    ...
    async def _handle_service_config_request(self, event):
        ...
        await self.publish_event("service_config_response", {
            "request_id": event.data.get("request_id"),
    ...
    async def _config_validation_task(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if not is_valid:
                    await self.publish_event("configuration_invalid", {
                        "timestamp": datetime.now(UTC).isoformat()
                    })
