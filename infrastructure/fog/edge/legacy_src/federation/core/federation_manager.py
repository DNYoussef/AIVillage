...
class FederationManager:
    ...
    def __init__(
        ...
        self.privacy_circuits: dict[str, list[dict[str, str]]] = {}  # destination -> path
    ...
    async def _wait_for_correlated_response(self, request_id: str, timeout: float = 5.0) -> dict[str, Any] | None:
    ...
    async def _handle_device_announcement(self, message_data: dict, sender: str):
        ...
        try:
            ...
            identity = DeviceIdentity(
                ...
                public_key=(bytes.fromhex(device_info.get("public_key", "")) if device_info.get("public_key") else b""),
                signing_key_public=(
            ...
            if reputation is not None:
                ...
                if not 0.0 <= reputation <= 1.0:
                    logger.warning(f"Invalid reputation score from {sender}: {reputation}")
                    return
            ...
            if signature and identity.signing_key_public:
                try:
                    ...
                    if not identity.verify_signature(message_bytes, sig_bytes):
                        logger.warning(f"Invalid signature in device announcement from {sender}")
                        return
                except Exception:
                    logger.warning(f"Error verifying signature in device announcement from {sender}")
                    return
            ...
            logger.info(f"Device announcement from {sender}: role={profile.role.value}")
            ...
            if agent_type and hasattr(self, "agent_registry"):
                try:
                    ...
                    if agent:
                        ...
                        if prompt:
                            processed_by = (
                                self.local_profile.identity.device_id if self.local_profile else self.device_id
                            )
                            result = {
    ...
    async def _handle_ai_service_response(self, message_data: dict, sender: str):
        ...
        if future and not future.done():
            ...
            logger.debug(f"No pending request for AI service response {request_id} from {sender}")
    ...
    async def _build_privacy_circuit(self, destination: str, min_hops: int = 3) -> list[dict[str, str]] | None:
        ...
        selected_relays = random.sample(relay_nodes, min(min_hops - 1, len(relay_nodes)))
    ...
    async def _build_paranoid_circuit(self, destination: str) -> list[dict[str, str]] | None:
    ...
    async def _send_via_privacy_circuit(self, destination: str, message: dict) -> bool:
        ...
        for hop in circuit_path:
            success = await self._send_via_protocol(hop["node"], current_layer, hop.get("protocol", "bitchat"))
            if not success:
                return False
    ...
    async def _send_via_protocol(self, recipient: str, payload: Any, protocol: str) -> bool:
    ...
    async def _start_tor_transport(self):
        ...
        try:
            ...
            with Controller.from_port(port=9051) as controller:
                ...
                response = controller.create_ephemeral_hidden_service({8080: 8080}, await_publication=True)
                self.tor_onion_address = f"{response.service_id}.onion"

                logger.info(f"Tor hidden service available at: {self.tor_onion_address}")
                self.tor_enabled = True

        except ImportError:
            logger.warning("Tor dependencies not available (stem, PySocks). Install with: pip install stem PySocks")
            self.tor_enabled = False
        except Exception as e:
    ...
    async def _start_i2p_transport(self):
        ...
        try:
            ...
            if b"HELLO REPLY RESULT=OK" in response:
                ...
                if b"DEST REPLY" in dest_response:
                    ...
                    for part in dest_parts:
                        if "=" in part and len(part) > 50:  # I2P destinations are long base64 strings
                            self.i2p_destination = part.split("=")[1]
                            break

                    logger.info(f"I2P destination created: {self.i2p_destination[:32]}...")
            ...
            logger.warning("I2P dependencies not available. Ensure aiohttp is installed")
            self.i2p_enabled = False
