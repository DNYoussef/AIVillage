...
class TransparencyLogger:
    ...
    async def generate_transparency_report(self, days_back: int = 30) -> dict[str, Any]:
        ...
        report = {
            ...
            "safety_metrics": {
                "high_risk_workloads_processed": len(
                    [e for e in recent_entries if e.risk_assessment_summary.get("max_allowed_risk", 0) > 0.7]
                ),
...
class ConstitutionalWorkloadRouter:
    ...
    async def route_workload(
        ...
        if self.governance_engine and content_data:
            policy_decision = await self.governance_engine.evaluate_workload(workload_id, user_id, content_data)
        ...
        selected_nodes, fallback_nodes = await self._select_optimal_nodes(candidate_nodes, requirements)
        ...
        routing_decision.sla_commitments = await self._create_sla_commitments(requirements, selected_nodes)
    ...
    async def _find_candidate_nodes(self, requirements: WorkloadRequirements) -> list[NodeCapabilities]:
        ...
        for node_id, node in self.available_nodes.items():
            ...
            if (
                ...
                or node.available_memory_gb < requirements.memory_gb
                or node.available_storage_gb < requirements.storage_gb
            ):
                continue
            ...
            if requirements.classification in [
                WorkloadClassification.FEDERATED_INFERENCE,
                WorkloadClassification.FEDERATED_TRAINING,
            ]:
    ...
    async def _calculate_node_score(self, node: NodeCapabilities, requirements: WorkloadRequirements) -> Decimal:
        ...
        load_factor = Decimal("1") - (
            Decimal(str(node.current_workloads)) / Decimal(str(node.max_concurrent_workloads))
        )
        ...
        if node.viewpoint_firewall_available and requirements.constitutional_tier in [
        ...
        commitments.update(
            {
                ...
                "harm_detection_threshold": float(
                    tier_capabilities.constitutional_constraints.harm_detection_threshold
                ),
            }
        )
            ...
            commitments.update(
                {
                    ...
                    "secure_multi_party_computation": any(
                        node.secure_multi_party_computation for node in selected_nodes
                    ),
    ...
    async def get_routing_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "routing_performance": {
                ...
                "average_nodes_per_route": sum(len(d.selected_nodes) for d in all_decisions)
                / max(len(all_decisions), 1),
            },
        }
