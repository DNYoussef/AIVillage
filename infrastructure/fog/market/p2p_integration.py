...
class P2PMarketplaceIntegration:
    ...
    async def start(self):
        ...
        if self.p2p_network:
            await self.p2p_network.register_message_handler(
                "marketplace", self._handle_p2p_message
            )
    ...
    async def discover_federated_resources(
        ...
        for peer_id, peer_info in self.network_peers.items():
            if (peer_info.available_resources and 
                NetworkRole.RESOURCE_PROVIDER in peer_info.roles and
                peer_info.trust_score >= 0.5):
    ...
    async def submit_federated_bid(
        ...
        if self.p2p_network:
            await self.p2p_network.send_message(
                coordinator_peer_id, "marketplace", bid_msg.payload
            )
    ...
    async def allocate_federated_resources(
        ...
        if self.p2p_network:
            for proposal in proposals:
                ...
                await self.p2p_network.send_message(
                    peer_id, "marketplace", proposal.payload
                )
        ...
        logger.info(
            f"Federated allocation {allocation_id}: {len(accepted_peers)}/{len(target_peers)} peers accepted"
        )
    ...
    async def _handle_auction_announcement(self, sender_id: str, message_data: Dict[str, Any]):
        ...
        if (sender_id in self.network_peers and
            self.network_peers[sender_id].trust_score >= self.config["auction_participation_threshold"]):
            
            # Check if we have suitable resources
            requirements = auction_info["requirements"]
            
            if (self.resource_allocator and 
                len(self.resource_allocator.available_nodes) > 0):
    ...
    async def _handle_bid_submission(self, sender_id: str, message_data: Dict[str, Any]):
        ...
        if auction_id in self.active_auctions and self.auction_engine:
            ...
            bid_id = await self.auction_engine.submit_bid(
                ...
                reputation_score=Decimal(str(self.network_peers.get(sender_id, P2PPeerInfo(peer_id=sender_id)).reputation_score)),
    ...
    async def _handle_allocation_proposal(self, sender_id: str, message_data: Dict[str, Any]):
        ...
        accept_proposal = (
            sender_id in self.network_peers and
            self.network_peers[sender_id].trust_score >= 0.6 and
            compensation.get("amount", 0) >= 5.0  # Minimum compensation
        )
    ...
    async def _handle_reputation_update(self, sender_id: str, message_data: Dict[str, Any]):
        ...
        if (sender_id in self.network_peers and
            self.network_peers[sender_id].trust_score >= 0.7 and
            target_peer_id in self.network_peers):
            ...
            try:
                ...
                if self.resource_allocator and self.p2p_network:
                    ...
                    if total_nodes > 0:
                        ...
                        avg_trust = sum(node.trust_score for node in self.resource_allocator.available_nodes.values()) / total_nodes
    ...
    async def _peer_discovery_loop(self):
        ...
        while True:
            try:
                ...
                stale_peers = [
                    peer_id for peer_id, peer_info in self.network_peers.items()
                    if peer_info.last_seen < stale_cutoff
                ]
    ...
    **marketplace_components
    ...
    if _p2p_integration is None:
        _p2p_integration = P2PMarketplaceIntegration(
            peer_id=peer_id or f"marketplace_{uuid.uuid4().hex[:8]}",
