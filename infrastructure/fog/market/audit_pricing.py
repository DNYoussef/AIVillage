...
class ConstitutionalComplianceReport:
    ...
    def calculate_overall_score(self):
        ...
        scores = [self.transparency_score, self.privacy_score, self.governance_score]
...
class AuditTrailManager:
    ...
    def log_calculation(
        ...
        record = AuditRecord(
            ...
            event_data={"calculation_type": calculation_type, "calculation_details": calculation_data},
            actor_id=actor_id,
            device_id=device_id,
        ...
        for i, record in enumerate(self.audit_records):
            ...
            if not record.verify_integrity(previous_record):
                ...
                integrity_results["integrity_failures"].append(
                    {"record_id": record.record_id, "position": i, "failure_type": "integrity_check_failed"}
                )
            else:
    ...
    def generate_constitutional_compliance_report(self) -> ConstitutionalComplianceReport:
        ...
        governance_records = [r for r in self.audit_records if r.event_type == AuditEventType.GOVERNANCE_VOTE]
        report.governance_participation_enabled = len(governance_records) > 0

        # Check privacy protections
        privacy_violations = len([r for r in self.audit_records if not r.privacy_preserved])
    ...
    def get_transparency_report(self, include_privacy: bool = True) -> dict[str, Any]:
        ...
        recent_records = [r for r in self.audit_records if r.timestamp > last_24h]
        ...
        return {
            ...
            "compliance_status": {
                "latest_compliance_score": (
                    float(latest_compliance.overall_compliance_score) if latest_compliance else 0.0
                ),
