...
class DistributedKeyGenerator:
    ...
    async def _generate_distributed_key(self, ceremony_id: str):
        ...
        shares = await self._generate_key_shares(private_pem, participants, threshold)
    ...
    async def _generate_key_shares(
        ...
        for i, participant in enumerate(participants, 1):
            ...
            share_value = sum(coeff * pow(i, j, prime) for j, coeff in enumerate(coefficients)) % prime
            ...
            verification_hash = hashlib.sha256(share_bytes + participant.encode()).hexdigest()
...
class KeyRotationManager:
    ...
    async def rotate_key(
        self, key_id: str, trigger: RotationTrigger = RotationTrigger.MANUAL, initiated_by: str = "system"
    ) -> str:
    ...
    async def _generate_rotated_key(self, old_key: CryptographicKey, new_key_id: str) -> CryptographicKey:
...
class KeyBackupManager:
    ...
    async def create_backup(
        ...
        if method == "shamir_shares":
            backup = await self._create_shamir_backup(key, backup_id, backup_locations, recovery_threshold)
        elif method == "encrypted_copies":
            backup = await self._create_encrypted_backup(key, backup_id, backup_locations)
        else:
    ...
    async def _create_shamir_backup(
        ...
        backup = KeyBackup(
            ...
            metadata={"threshold": threshold, "total_shares": len(locations), "backup_locations": locations},
        )
    ...
    async def _create_encrypted_backup(self, key: CryptographicKey, backup_id: str, locations: list[str]) -> KeyBackup:
        ...
        backup = KeyBackup(
            ...
            recovery_shares=[],
            location="multiple",
            metadata={"backup_key": backup_key.decode(), "backup_locations": locations},
        )
...
class SecureKeyManager:
    ...
    async def initialize(self):
        ...
        self.backup_manager.add_backup_location(
            "local_encrypted", "local_storage", {"path": "./backups", "encryption_key": Fernet.generate_key().decode()}
        )
    ...
    async def generate_key(
        ...
        if auto_backup:
            try:
                await self.backup_manager.create_backup(key_id, ["local_encrypted"], recovery_threshold=1)
    ...
    async def rotate_key(self, key_id: str, reason: str = "manual") -> str:
        ...
        try:
            await self.backup_manager.create_backup(new_key_id, ["local_encrypted"], recovery_threshold=1)
            self.stats["keys_backed_up"] += 1
        except Exception as e:
...
def create_secure_key_manager(
    ...
    if config:
        ...
        for location_id, location_config in backup_locations.items():
            manager.backup_manager.add_backup_location(location_id, location_config["type"], location_config["config"])
