...
from ..proof.betanet_anchor import BetanetAnchorService, AnchorStatus
    ...
    async def search_events(
        ...
        async with self.lock:
            ...
            for event in self.buffer:
                ...
                # Node filter
                if nodes and not (event.source_node in nodes or 
                                (event.target_node and event.target_node in nodes)):
                    continue
            ...
            "rule_analysis": [
                {
                    ...
                    "compliance_rate": 1.0 - (violations_by_rule.get(rule.rule_id, 0) / total_events) if total_events > 0 else 1.0
                }
                for rule in framework_rules
            ],
            "violations_detail": [
                v for v in self.rule_violations
                if v.get("framework") == framework.value and
                time_range[0] <= v.get("timestamp", 0) <= time_range[1]
            ],
    ...
    async def _apply_detection_rule(
        ...
        if "threshold_count" in rule:
            if event_count >= rule["threshold_count"]:
                return await self._create_anomaly_pattern(
                    rule_id, rule, filtered_events, "threshold_exceeded"
                )
        ...
        if "threshold_multiplier" in rule and target_event_type:
            ...
            if baseline:
                ...
                if event_count > threshold:
                    return await self._create_anomaly_pattern(
                        rule_id, rule, filtered_events, "statistical_anomaly"
                    )
...
class ForensicAnalysisEngine:
    ...
    async def _filter_events_for_query(
        self,
        events: List[AuditEvent],
        query: ForensicQuery
    ) -> List[AuditEvent]:
        ...
        for event in events:
            ...
            if query.nodes_of_interest:
                if not (event.source_node in query.nodes_of_interest or
                       (event.target_node and event.target_node in query.nodes_of_interest)):
                    continue
        ...
        for actor, actor_event_list in actor_events.items():
            ...
            for i in range(len(actor_event_list) - 1):
                ...
                if next_event.timestamp - current.timestamp <= 3600:
                    ...
                    if ("admin" in next_event.resource.lower() and 
                        "user" in current.resource.lower()):
    ...
    async def _detect_persistence_mechanisms(self, events: List[AuditEvent]) -> List[Dict[str, Any]]:
        ...
        for event in config_events:
            ...
            if any(keyword in event.action.lower() for keyword in 
                   ["schedule", "startup", "service", "daemon", "cron"]):
    ...
    async def _find_actor_correlations(self, events: List[AuditEvent]) -> Dict[str, Any]:
        ...
        for event in events:
            if event.target_node:
                ...
                related_events = [
                    e for e in events
                    if (e.source_node == event.target_node and
                        abs(e.timestamp - event.timestamp) <= time_window and
        ...
        return {
            ...
            "strong_correlations": [
                ...
                for pair, count in strong_correlations.items()
            ]
        }
        ...
        for actor, actor_events in actor_sessions.items():
            ...
            for event in actor_events:
                ...
                if (not current_session or 
                    event.timestamp - current_session[-1].timestamp > session_window):
...
class AuditTrailSystem:
    ...
    async def log_event(
        ...
        if (self.config["blockchain_anchoring"] and 
    ...
    async def _anchor_event_to_blockchain(self, event: AuditEvent):
        ...
        try:
            ...
            anchor_id = await self.betanet_anchor.anchor_proof(
                event_hash, merkle_root, priority="high"
            )
    ...
    async def query_events(
        ...
        # Search in buffer
        buffer_events = await self.event_buffer.search_events(
            time_range, event_types, severity, nodes
        )
    ...
    async def generate_compliance_report(
        ...
        report = await self.compliance_engine.generate_compliance_report(
            framework, time_range, all_events
        )
    ...
    async def _archival_loop(self):
        ...
        while True:
            try:
                ...
                events_to_archive = await self.event_buffer.search_events(
...
def create_audit_trail_system(
    node_id: str,
    betanet_anchor: Optional[BetanetAnchorService] = None,
