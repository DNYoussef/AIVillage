...
class ReputationIntegrationManager:
    ...
    def __init__(
        self,
        reputation_engine: Optional[BayesianReputationEngine] = None,
    ...
    async def record_task_result(self, task_result: TaskResult):
        ...
        if task_result.success:
            event_type = EventType.QUALITY_HIGH if task_result.quality_score and task_result.quality_score > 0.8 else EventType.TASK_SUCCESS
        else:
    ...
    async def recommend_trusted_nodes(
        ...
        trusted_nodes = [
            node_id for node_id, trust_score in trust_scores.items()
    ...
    async def calculate_trust_based_reserve_prices(
        self,
        base_reserve_prices: Dict[str, float],
        node_ids: List[str]
    ) -> Dict[str, float]:
    ...
    async def get_system_reputation_health(self) -> Dict[str, Any]:
        ...
        # Get basic metrics
        metrics = self.reputation_engine.export_state()
        tier_distribution = self.reputation_engine.get_tier_distribution()
    ...
    async def _batch_update_loop(self):
        ...
        while True:
            try:
                ...
                if time_since_last.total_seconds() >= 60 or len(self._pending_events) >= self.integration_config["batch_update_size"]:
                    await self._flush_pending_events()
