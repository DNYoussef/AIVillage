...
class TopologyManager:
    ...
    async def assess_topology_health(self) -> TopologyHealthStatus:
        ...
        healthy_count = sum([spectral_gap_healthy, expansion_healthy, connectivity_healthy, k_core_healthy])
    ...
    async def _heal_k_core(self, action: TopologyHealingAction) -> bool:
        ...
        try:
            ...
            if self.vrf_selector.node_id in weak_nodes:
                ...
                self.vrf_selector.target_degree = min(original_target + 2, self.vrf_selector.max_degree)
        ...
        while True:
            try:
                ...
                if health in [TopologyHealthStatus.DEGRADED, TopologyHealthStatus.CRITICAL]:
                    ...
                    self.logger.warning(
                        f"Topology health degraded ({health.value}), planned {len(self.healing_actions)} healing actions"
                    )
