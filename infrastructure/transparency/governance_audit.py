...
class GovernanceAuditTrail:
    ...
    def _load_existing_governance_data(self):
        ...
        try:
            ...
            if proposals_file.exists():
                with open(proposals_file, 'r') as f:
                    ...
                    for proposal_data in data.get('proposals', []):
                        ...
                        if (proposal.current_status == "voting" and 
                            time.time() < proposal.voting_period_end):
            ...
            self.logger.info(f"Loaded {len(self.participants)} participants, "
    ...
    def _calculate_participation_metrics(self):
        ...
        if self.participation_metrics['total_participants'] > 0:
            engagement_score = min(100, 
                (self.participation_metrics['active_voters'] / 
                 self.participation_metrics['total_participants'] * 100) + 
        ...
        if total_proposals > 0:
            quorum_achieved = len([p for p in self.proposals.values() 
                                 if p.current_status in ["approved", "rejected"] and 
                                    (p.votes_for + p.votes_against + p.votes_abstain) >= p.quorum_requirement])
        ...
        while True:
            try:
                ...
                if len(self.recent_democratic_activity) > 0:
                    recent_activity = len([e for e in self.recent_democratic_activity 
    ...
    async def register_democratic_participant(self,
                                            participant_id: str,
                                            tier: str,
                                            initial_participation_level: ParticipationLevel = ParticipationLevel.VOTER) -> str:
    ...
    async def submit_governance_proposal(self,
                                       ...
                                       approval_threshold: float = 0.5) -> str:
        ...
        if participant.participation_level not in [ParticipationLevel.PROPOSER, 
    ...
    async def cast_democratic_vote(self,
                                 voter_id: str,
                                 proposal_id: str,
                                 vote_choice: str,
                                 rationale: Optional[str] = None,
                                 public_comment: Optional[str] = None) -> str:
        ...
        existing_vote = next((v for v in self.votes.values() 
                            if v.voter_id_hash == voter_id and v.proposal_id == proposal_id), None)
        if existing_vote:
    ...
    async def _log_democratic_event(self,
                                  event_type: DemocraticAction,
                                  participant_id: str,
                                  related_proposal_id: Optional[str],
                                  event_details: Dict[str, Any],
        ...
        audit_trail_hash = hashlib.sha256(
            json.dumps(audit_data, sort_keys=True).encode('utf-8')
        ).hexdigest()
    ...
    async def _persist_votes(self):
        ...
        votes_file = self.storage_path / "votes.json"
        data = {
            'votes': [asdict(v) for v in self.votes.values()],
            'last_updated': time.time()
        }
    ...
    async def _persist_democratic_events(self):
        ...
        events_file = self.storage_path / "democratic_events.json"
        data = {
            'events': [asdict(e) for e in self.democratic_events],
            'last_updated': time.time()
        }
    ...
    def get_democratic_participation_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'active_governance': {
                ...
                'recent_democratic_activity': len([e for e in self.recent_democratic_activity 
                                                 if time.time() - e.timestamp < 3600]),
                'constitutional_amendments_pending': len([a for a in self.amendments.values() 
                                                        if a.amendment_status == "voting"])
            },
    ...
    def _get_participation_by_activity(self) -> Dict[str, int]:
        ...
        for participant in self.participants.values():
            recent_votes = len([v for v in self.votes.values() 
    ...
    def _calculate_voter_turnout_trend(self) -> str:
        ...
        for proposal in recent_proposals:
            if proposal.current_status in ["approved", "rejected", "failed_quorum"]:
                total_votes = proposal.votes_for + proposal.votes_against + proposal.votes_abstain
                eligible_voters = len([p for p in self.participants.values() 
                                     if p.participation_level != ParticipationLevel.OBSERVER])
    ...
    def _calculate_proposal_success_rate(self) -> float:
        ...
        completed_proposals = [p for p in self.proposals.values() 
    ...
    def _calculate_constitutional_engagement(self) -> float:
        ...
        constitutional_events = [e for e in self.democratic_events 
                               if e.constitutional_significance in ["major", "constitutional"]]
    ...
    def _calculate_democratic_legitimacy(self) -> float:
        ...
        # Weighted average
        weights = {'participation_rate': 0.3, 'quorum_achievement': 0.25, 
                  'constitutional_engagement': 0.2, 'transparency_level': 0.15, 
                  'diversity_score': 0.1}
    ...
    def generate_governance_transparency_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'governance_activity_analysis': {
                ...
                'constitutional_amendments_considered': len([a for a in self.amendments.values() if a.timestamp >= recent_cutoff])
            },
...
if __name__ == "__main__":
    ...
    async def test_governance_audit():
        ...
        # Cast votes
        await governance_audit.cast_democratic_vote(bob, proposal_id, "approve", 
                                                  "I support increased transparency")
        await governance_audit.cast_democratic_vote(charlie, proposal_id, "approve", 
                                                  "This will improve trust in the system")
        ...
        print(f"Governance transparency report generated for {report['democratic_participation_summary']['total_registered_participants']} participants")
