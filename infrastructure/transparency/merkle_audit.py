...
class MerkleNode:
    ...
    def __init__(
        self,
        data: Union[str, "MerkleNode", "MerkleNode"] = None,
...
class ConstitutionalMerkleAudit:
    ...
    async def log_constitutional_decision(
        ...
        privacy_data = await self._create_privacy_preserving_data(decision_data, audit_level, user_tier)

        # Generate public summary based on transparency level
        public_summary = self._generate_public_summary(decision_data, audit_level, violation_type)
    ...
    async def _create_privacy_preserving_data(
        ...
        if audit_level == AuditLevel.BRONZE:
            ...
            commitment = hashlib.sha256(json.dumps(decision_data, sort_keys=True).encode("utf-8")).hexdigest()
    ...
    async def _generate_zk_proof(self, decision_data: Dict[str, Any], user_tier: str) -> str:
        ...
        proof_commitment = hashlib.sha256(json.dumps(proof_data, sort_keys=True).encode("utf-8")).hexdigest()
    ...
    def _generate_public_summary(
        ...
        if audit_level == AuditLevel.BRONZE:
            ...
            return (
                f"Constitutional decision: {decision_data.get('action', 'unknown')} - "
            ...
            return (
                f"Constitutional action taken. Type: {violation_type.value if violation_type else 'compliance_check'}. "
            ...
            return (
                "Constitutional process completed with privacy preservation. "
                "Compliance verified through zero-knowledge proof."
            )
    ...
    async def _update_daily_tree(self, date: str, entry: ConstitutionalAuditEntry):
        ...
        date_entries = [e for e in self.audit_entries if time.strftime("%Y-%m-%d", time.localtime(e.timestamp)) == date]
    ...
    async def _persist_audit_entry(self, entry: ConstitutionalAuditEntry):
        ...
        if date in self.merkle_trees and self.merkle_trees[date]:
            existing_data["tree_info"] = {
                ...
                "timestamp": time.time(),
                "entry_count": len(
                    [e for e in self.audit_entries if time.strftime("%Y-%m-%d", time.localtime(e.timestamp)) == date]
                ),
            }
    ...
    def get_constitutional_compliance_report(self) -> Dict[str, Any]:
        ...
        recent_compliance_rate = (
            ((len(recent_entries) - recent_violations) / len(recent_entries) * 100) if recent_entries else 100
        )
            ...
            if include_proofs:
                ...
                if tree:
                    ...
                    export_data["merkle_proofs"].append(
                        {"entry_id": entry.entry_id, "merkle_proof": proof, "root_hash": tree.hash}
                    )
...
if __name__ == "__main__":
    ...
    async def test_constitutional_audit():
        ...
        bronze_entry = await audit.log_constitutional_decision(
            decision_data, AuditLevel.BRONZE, "bronze", ConstitutionalViolationType.HARM_CLASSIFICATION
        )

        await audit.log_constitutional_decision(
            decision_data, AuditLevel.SILVER, "silver", ConstitutionalViolationType.TIER_VIOLATION
        )

        await audit.log_constitutional_decision(decision_data, AuditLevel.GOLD, "gold")
