...
class PublicAccountabilityDashboard:
    ...
    async def _collect_constitutional_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics.update({
                'compliance_rate': compliance_report['constitutional_compliance_summary']['overall_compliance_rate'],
                'total_decisions': compliance_report['constitutional_compliance_summary']['total_constitutional_decisions'],
                'violations_detected': compliance_report['constitutional_compliance_summary']['total_violations_detected'],
                'appeals_processed': compliance_report['constitutional_compliance_summary']['appeals_processed'],
                'governance_votes': compliance_report['constitutional_compliance_summary']['governance_votes_recorded'],
                'privacy_preserved_decisions': compliance_report['constitutional_compliance_summary']['privacy_preserved_decisions'],
            ...
            recent_decisions = [d for d in self.decision_logger.decision_logs 
                             if d.timestamp >= time.time() - (24 * 3600)]
            ...
            community_decisions = len([d for d in recent_decisions 
                                     if d.governance_level in [GovernanceLevel.COMMUNITY, GovernanceLevel.CONSTITUTIONAL]])
            democratic_participation_rate = (community_decisions / len(recent_decisions) * 100) if recent_decisions else 0
    ...
    async def _calculate_widget_data(self, widget: DashboardWidget, snapshot: ConstitutionalMetricSnapshot) -> Dict[str, Any]:
        ...
        if widget.metric_type == DashboardMetric.CONSTITUTIONAL_COMPLIANCE_RATE:
            return {
                ...
                'status': 'good' if snapshot.compliance_rate >= 95 else 'warning' if snapshot.compliance_rate >= 90 else 'critical'
            }
            ...
            return {
                ...
                'status': 'good' if avg_resolution_time <= 48 else 'warning' if avg_resolution_time <= 72 else 'critical'
            }
        
        elif widget.metric_type == DashboardMetric.PRIVACY_PRESERVATION_RATE:
            return {
                'value': (snapshot.privacy_preserved_decisions / snapshot.total_decisions * 100) if snapshot.total_decisions > 0 else 0,
                'unit': '%',
            ...
            return {
                ...
                'status': 'operational' if integrity_score >= 90 else 'degraded' if integrity_score >= 70 else 'critical'
            }
        
        elif widget.metric_type == DashboardMetric.CONSTITUTIONAL_VIOLATIONS:
            return {
                'total_violations': snapshot.violations_detected,
                'violation_rate': (snapshot.violations_detected / snapshot.total_decisions * 100) if snapshot.total_decisions > 0 else 0,
    ...
    def _get_historical_data(self, widget_id: str, time_range: TimeRange) -> List[Dict[str, Any]]:
        ...
        for snapshot in historical_snapshots:
            ...
            if widget_id == "democratic_participation":
                ...
                data_point['value'] = (snapshot.privacy_preserved_decisions / snapshot.total_decisions * 100) if snapshot.total_decisions > 0 else 0
    ...
    async def _calculate_average_appeal_resolution_time(self) -> float:
        ...
        resolved_appeals = [
            d for d in self.decision_logger.decision_logs 
    ...
    async def _get_recent_violations(self) -> List[Dict[str, Any]]:
        ...
        for entry in self.merkle_audit.audit_entries:
            if (entry.timestamp >= recent_cutoff and 
                entry.violation_type and 
                entry.audit_level in [AuditLevel.BRONZE, AuditLevel.SILVER]):
                
                violation = {
                    'timestamp': entry.timestamp,
                    'type': entry.violation_type.value,
                    'tier': entry.user_tier,
                    'summary': entry.public_summary[:100] + "..." if len(entry.public_summary) > 100 else entry.public_summary
                }
    ...
    def _clear_expired_cache(self):
        ...
        expired_keys = [
            key for key, (timestamp, _) in self.public_api_cache.items()
    ...
    async def get_constitutional_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'summary_metadata': {
                ...
                'public_accountability_score': min(100, 
                    (latest_snapshot.compliance_rate + 
                     latest_snapshot.democratic_participation_rate + 
                     (90 if latest_snapshot.merkle_trees_verified > 0 else 0)) / 3
                )
            }
        }
    ...
    def generate_public_transparency_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'constitutional_performance': {
                ...
                'violation_resolution_rate': round((1 - (total_violations / total_decisions)) * 100, 2) if total_decisions > 0 else 100
            },
            ...
            'privacy_and_transparency_balance': {
                'privacy_preserving_decisions_percentage': round(
                    sum(s.privacy_preserved_decisions for s in recent_snapshots) / total_decisions * 100, 2
...
if __name__ == "__main__":
    ...
    async def test_public_dashboard():
        ...
        # Create dashboard
        dashboard = PublicAccountabilityDashboard(
            merkle_audit, decision_logger, privacy_system
        )
        ...
        print(f"Constitutional compliance rate: {metrics_summary.get('constitutional_compliance', {}).get('overall_rate', 'N/A')}%")
        
        # Generate transparency report
        transparency_report = dashboard.generate_public_transparency_report()
        print(f"Transparency report generated for period: {transparency_report.get('transparency_report', {}).get('report_period', 'N/A')}")
