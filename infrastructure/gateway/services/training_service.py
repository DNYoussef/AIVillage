...
class ModelTrainer(ABC):
    ...
    async def train_model(
        ...
        progress_callback: Callable[[int, int, float, float], None]
    ) -> Dict[str, Any]:
...
class TrainingService:
    ...
    async def start_training_session(
        ...
        await self.progress_emitter.emit_progress(TrainingProgress(
            progress=0.0,
            message=f"🔥 Initializing training session with GrokFast optimization",
    ...
    async def _prepare_datasets(self, task_id: str) -> Dict[str, bool]:
        ...
        await self.progress_emitter.emit_progress(TrainingProgress(
            progress=0.1,
            message="📥 Downloading and preparing real datasets",
        ...
        try:
            mixed_dataset_path = await self.dataset_loader.create_mixed_training_data()
            await self.progress_emitter.emit_progress(TrainingProgress(
                progress=0.2,
    ...
    async def _train_models(self, task_id: str) -> List[ModelArtifacts]:
        ...
        for i, model_name in enumerate(model_names):
            ...
            try:
                ...
                # Train the model
                training_stats = await self.model_trainer.train_model(
                    model_name, self.config, model_progress_callback
                )
    ...
    def _calculate_parameter_count(self) -> int:
        ...
        if self.config.ltm_enabled:
            # Add LTM parameters
            layer_params += self.config.n_layers * (
                self.config.d_model * self.config.d_mem +
                self.config.mem_capacity * self.config.d_mem
            )
