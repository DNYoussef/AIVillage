...
class ThreatDetectionSystem:
    ...
    async def detect_threats(
        self,
        analysis_window: Optional[float] = None
    ) -> List[ThreatEvent]:
            ...
            try:
                if self.detection_config["real_time_correlation_enabled"]:
                    ...
                    recent_threats = [
                        ...
                        if time.time() - threat.detected_at < self.event_correlation_window
        ...
        while True:
            try:
                ...
                self.detected_threats = [
                    threat for threat in self.detected_threats
                    if threat.detected_at > retention_cutoff
                ]
        ...
        for metric_name, value in metrics.items():
            if metric_name in profile.baseline_metrics:
                profile.baseline_metrics[metric_name] = (
                    (1 - alpha) * profile.baseline_metrics[metric_name] +
                    alpha * value
                )
            else:
    ...
    async def _process_detected_threat(self, threat: ThreatEvent) -> None:
        ...
        if (self.detection_config["auto_mitigation_enabled"] and 
    ...
    def _initialize_mitigation_strategies(self) -> None:
        ...
        self.mitigation_strategies = {
            "block_malicious_node": MitigationStrategy(
                ...
                threat_categories={
                    ThreatCategory.AUTHENTICATION_ATTACK,
                    ThreatCategory.MALICIOUS_BEHAVIOR
                },
                attack_vectors={
            ...
            "enhance_monitoring": MitigationStrategy(
                ...
                threat_categories={
                    ThreatCategory.DATA_PRIVACY_VIOLATION,
    ...
    def get_detection_stats(self) -> Dict[str, Any]:
        ...
        return {
            **dict(self.detection_stats),
            ...
            "recent_threats": len([
                threat for threat in self.detected_threats
                if time.time() - threat.detected_at < 3600  # Last hour
            ])
        }
    ...
    def get_threat_summary(self) -> Dict[str, Any]:
        ...
        recent_threats = [
            threat for threat in self.detected_threats
            if current_time - threat.detected_at < 86400
        ]
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        unmitigated_critical = [
            threat for threat in self.detected_threats
            if (threat.threat_level == ThreatLevel.CRITICAL and
                not threat.mitigated and
                current_time - threat.detected_at < 3600)
        ]
