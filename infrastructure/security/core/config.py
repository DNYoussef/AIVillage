    ...
    def _load_component_configs(self):
        ...
        self.encryption = EncryptionConfig(
            ...
            key_rotation_interval=int(os.getenv("KEY_ROTATION_INTERVAL", 
                                              encryption_config.get("key_rotation_interval", 3600))),
            max_key_age=int(os.getenv("MAX_KEY_AGE", encryption_config.get("max_key_age", 7200))),
            enable_perfect_forward_secrecy=self._get_bool_config("ENABLE_PFS", 
                                                               encryption_config.get("enable_perfect_forward_secrecy", True))
        )
        
        # Authentication
        auth_config = self.config_dict.get("authentication", {})
        self.authentication = AuthenticationConfig(
            token_expiry_hours=int(os.getenv("TOKEN_EXPIRY_HOURS", 
                                           auth_config.get("token_expiry_hours", 24))),
            refresh_token_expiry_days=int(os.getenv("REFRESH_TOKEN_EXPIRY_DAYS", 
                                                   auth_config.get("refresh_token_expiry_days", 30))),
            max_failed_attempts=int(os.getenv("MAX_FAILED_ATTEMPTS", 
                                            auth_config.get("max_failed_attempts", 5))),
            lockout_duration_minutes=int(os.getenv("LOCKOUT_DURATION_MINUTES", 
                                                 auth_config.get("lockout_duration_minutes", 30))),
            ...
            session_timeout_minutes=int(os.getenv("SESSION_TIMEOUT_MINUTES", 
                                                auth_config.get("session_timeout_minutes", 60)))
        )
        ...
        self.authorization = AuthorizationConfig(
            ...
            permission_cache_ttl=int(os.getenv("PERMISSION_CACHE_TTL", 
                                             authz_config.get("permission_cache_ttl", 300))),
            enable_resource_isolation=self._get_bool_config("ENABLE_RESOURCE_ISOLATION", 
                                                          authz_config.get("enable_resource_isolation", True)),
            tenant_isolation=self._get_bool_config("TENANT_ISOLATION", 
                                                 authz_config.get("tenant_isolation", True))
        )
        
        # Threat Detection
        threat_config = self.config_dict.get("threat_detection", {})
        self.threat_detection = ThreatDetectionConfig(
            enable_anomaly_detection=self._get_bool_config("ENABLE_ANOMALY_DETECTION", 
                                                         threat_config.get("enable_anomaly_detection", True)),
            enable_rate_limiting=self._get_bool_config("ENABLE_RATE_LIMITING", 
                                                     threat_config.get("enable_rate_limiting", True)),
            max_requests_per_minute=int(os.getenv("MAX_REQUESTS_PER_MINUTE", 
                                                threat_config.get("max_requests_per_minute", 60))),
            ban_duration_minutes=int(os.getenv("BAN_DURATION_MINUTES", 
                                             threat_config.get("ban_duration_minutes", 60))),
            trust_threshold=float(os.getenv("TRUST_THRESHOLD", 
                                          threat_config.get("trust_threshold", 0.3)))
        )
        
        # Audit
        audit_config = self.config_dict.get("audit", {})
        self.audit = AuditConfig(
            enable_audit_logging=self._get_bool_config("ENABLE_AUDIT_LOGGING", 
                                                     audit_config.get("enable_audit_logging", True)),
            log_all_requests=self._get_bool_config("LOG_ALL_REQUESTS", 
                                                 audit_config.get("log_all_requests", False)),
            log_failed_attempts=self._get_bool_config("LOG_FAILED_ATTEMPTS", 
                                                    audit_config.get("log_failed_attempts", True)),
            retention_days=int(os.getenv("AUDIT_RETENTION_DAYS", 
