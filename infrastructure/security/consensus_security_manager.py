...
class ConsensusSecurityManager:
    ...
    async def initialize_distributed_keys(
        self,
        ...
        try:
            ...
            for i, participant_id in enumerate(participant_nodes):
                ...
                if CRYPTOGRAPHY_AVAILABLE:
                    ...
                    private_key_bytes = hashlib.sha256(
                        str(private_share).encode() + participant_id.encode()
                    ).digest()
            ...
            logger.info(f"Distributed key generation completed successfully")
            return True
    ...
    async def create_threshold_signature(
        self,
        message: bytes,
        signing_nodes: List[str]
    ) -> Tuple[bool, Optional[bytes]]:
        """Create a threshold signature using multiple nodes."""
        
        if len(signing_nodes) < self.security_params["signature_threshold"]:
            logger.error(f"Insufficient signing nodes: {len(signing_nodes)} < {self.security_params['signature_threshold']}")
            return False, None
    ...
    async def verify_threshold_signature(
        ...
        message: bytes,
        signature: bytes,
        expected_signers: Optional[List[str]] = None
    ) -> bool:
        ...
        try:
            ...
            if CRYPTOGRAPHY_AVAILABLE:
                ...
                verification_result = self._verify_rsa_signature(
                    message, signature, self.master_public_key
                )
            else:
                # Fallback hash-based verification
                expected_sig = hashlib.sha256(
                    message + self.master_public_key
                ).digest()
    ...
    async def verify_zero_knowledge_proof(
        ...
        try:
            ...
            response = bytes.fromhex(proof_data["response"])
            ...
            for attack in detected_attacks:
                ...
                for node_id in attack.suspected_nodes:
                    ...
                    self.node_reputations[node_id] = max(
                        self.security_params["min_reputation"],
    ...
    async def validate_consensus_message(
        ...
        expected_view: Optional[int] = None
    ) -> Tuple[bool, List[str]]:
    ...
    async def _create_partial_signature(
        self,
        message: bytes,
        threshold_key: ThresholdKey
    ) -> Optional[bytes]:
        ...
        try:
            if CRYPTOGRAPHY_AVAILABLE and len(threshold_key.private_key_share) > 32:
                ...
                private_key = serialization.load_pem_private_key(
                    threshold_key.private_key_share,
                    password=None,
                    backend=default_backend()
                )
    ...
    async def _combine_partial_signatures(
        self,
        message: bytes,
        partial_signatures: List[Dict[str, Any]]
    ) -> bytes:
        """Combine partial signatures using Lagrange interpolation."""
    ...
    async def _detect_contradictory_messages(
        self,
        consensus_round: ConsensusRound
                ...
                if len(type_payload_pairs) != len(unique_pairs):
                    contradictions[sender] = {
                        ...
                        "conflicting_messages": [
                            {"type": msg.message_type, "payload": msg.payload}
                            for msg in messages
                        ]
                    }
    ...
    async def _detect_fork_attacks(self, consensus_round: ConsensusRound) -> List[AttackEvidence]:
        ...
        if consensus_round.proposal:
            ...
            proposal_hash = hashlib.sha256(
                json.dumps(consensus_round.proposal, sort_keys=True).encode()
            ).hexdigest()
    ...
    def get_security_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.security_stats,
            ...
            "byzantine_nodes": len([
                node_id for node_id, info in self.participant_nodes.items()
                if info["role"] == NodeRole.OBSERVER.value
            ]),
            "average_reputation": np.mean(list(self.node_reputations.values())) if self.node_reputations else 0.0,
            "recent_attacks": len([
                attack for attack in self.attack_evidence
                if time.time() - attack.detected_at < 3600  # Last hour
            ]),
    ...
    def get_attack_summary(self) -> Dict[str, Any]:
        ...
        recent_attacks = [
            attack for attack in self.attack_evidence
            if time.time() - attack.detected_at < 86400  # Last 24 hours
        ]
        ...
        return {
            ...
            "high_confidence_attacks": len([
                a for a in recent_attacks if a.confidence_score > 0.8
            ]),
            "suspected_byzantine_nodes": len([
                node_id for node_id, info in self.participant_nodes.items()
                if info.get("byzantine_score", 0) > 0.5
            ])
        }
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        recent_byzantine = len([
            attack for attack in self.attack_evidence
            if (attack.attack_type == AttackType.BYZANTINE_NODE and
                time.time() - attack.detected_at < 3600 and
                not attack.mitigated)
        ])
