...
class SecurityIntegrationManager:
    ...
    def __init__(
        ...
        self.auth_system = FederatedAuthenticationSystem(
            enable_mfa=self.config.enable_mfa
        )
        
        self.secure_aggregation = SecureAggregationProtocol(
            default_method=self.config.default_aggregation_method,
        ...
        self.consensus_security = ConsensusSecurityManager(
            node_id,
            self.config.consensus_protocol,
    ...
    async def register_federated_node(
        ...
        try:
            ...
            identity = await self.auth_system.register_node(
                node_id, role, password, capabilities
            )
            
            # Initialize trust profile
            trust_score = initial_trust or self.config.initial_trust_score
            await self.trust_system.initialize_node_trust(
                node_id, bootstrap_method="neutral"
            )
            
            # Set up secure aggregation for the node
            await self.secure_aggregation.setup_participant(
                node_id, capabilities or {}, {"epsilon_limit": 10.0}
            )
        ...
        try:
            ...
            if not trust_decision:
                ...
                await self._emit_security_event(
                    ...
                    {"node_id": node_id, "reason": trust_reason}
                )
                
                return False, None, {"reason": trust_reason, "trust_score": trust_score}
            
            # Perform authentication
            auth_success, session = await self.auth_system.authenticate_node(
                node_id, password, mfa_token
            )
            
            if auth_success and session:
                # Record successful interaction
                await self.trust_system.record_interaction(
                    node_id, "authentication", "success"
                )
                ...
                await self.trust_system.record_interaction(
                    node_id, "authentication", "failure"
                )
                
                await self._emit_security_event(
                    SecurityEvent.AUTHENTICATION_FAILURE,
                    {"node_id": node_id, "reason": "invalid_credentials"}
                )
    ...
    async def secure_federated_aggregation(
        ...
        try:
            ...
            for gradient_data in participant_gradients:
                ...
                session_valid, session = await self.auth_system.validate_session(
                    gradient_data.get("session_id", "")
                )
                ...
                # Check trust level
                trust_decision, _, _ = await self.trust_system.get_trust_decision(
                    participant_id, TrustLevel.MEDIUM
                )
            ...
            for gradient_data in validated_participants:
                secure_gradient = await self.secure_aggregation.create_secure_gradient(
                    ...
                    gradient_data.get("privacy_params")
                )
    ...
    async def secure_consensus_round(
        ...
        try:
            ...
            if not self.consensus_security.threshold_keys:
                success = await self.consensus_security.initialize_distributed_keys(
                    trusted_participants
                )
                if not success:
                    return False, None
            ...
            if detected_attacks:
                # Mitigate attacks
                mitigated = await self.consensus_security.mitigate_attacks(detected_attacks)
                ...
                for attack in detected_attacks:
                    for node_id in attack.suspected_nodes:
                        await self.trust_system.report_byzantine_behavior(
                            ...
                            attack.evidence_data,
                            attack.confidence_score
                        )
                
                await self._emit_security_event(
                    SecurityEvent.BYZANTINE_BEHAVIOR_DETECTED,
                    {"round_id": round_id, "attacks": len(detected_attacks)}
                )
    ...
    async def handle_security_incident(
        ...
        try:
            ...
            await self.threat_detection.ingest_security_event(
                incident_type,
                source_node,
                self.node_id,
                incident_data
                ...
                for threat in detected_threats:
                    ...
                    if threat.source_nodes:
                        for node_id in threat.source_nodes:
                            await self.trust_system.record_interaction(
                                node_id, "security_incident", "threat_detected"
                            )
    ...
    async def _trust_maintenance_monitor(self) -> None:
        ...
        while self.started:
            try:
                ...
                low_trust_nodes = self.trust_system.get_trusted_nodes(TrustLevel.UNTRUSTED)
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "authentication_stats": self.auth_system.get_auth_stats() if hasattr(self.auth_system, 'get_auth_stats') else {},
            "trust_stats": self.trust_system.get_trust_statistics(),
    ...
    def get_node_security_status(self, node_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "authentication": {
                ...
                "active_sessions": len([
                    s for s in self.auth_system.active_sessions.values()
                    if s.node_id == node_id and s.is_active
                ])
            },
            "trust": self.trust_system.get_node_trust_details(node_id),
            "secure_channels": len([
                ch for ch in self.transport_security.active_channels.values()
                if ch.remote_node_id == node_id and ch.is_active
            ]) if hasattr(self.transport_security, 'active_channels') else 0
        }
