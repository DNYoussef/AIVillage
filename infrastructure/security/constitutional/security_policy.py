...
class ConstitutionalPolicy:
    ...
    # Core principles
    principles: List[ConstitutionalPrinciple] = field(default_factory=lambda: [
    ...
    def __init__(self):
        ...
        self.keyword_classifiers = {
            HarmCategory.VIOLENCE: [
                "kill", "murder", "assault", "attack", "violence", "hurt", "harm",
                "fight", "weapon", "gun", "knife", "bomb", "explosive"
            ],
            HarmCategory.HATE_SPEECH: [
                "hate", "racist", "sexist", "homophobic", "slur", "discriminat",
                "bigot", "supremac", "nazi", "fascist"
            ],
            HarmCategory.HARASSMENT: [
                "harass", "bully", "stalk", "threaten", "intimidat", "abuse",
                "torment", "persecute", "target"
            ],
            HarmCategory.PRIVACY_VIOLATIONS: [
                "social security", "ssn", "credit card", "password", "private",
                "confidential", "personal information", "dox", "leak"
            ],
            HarmCategory.MISINFORMATION: [
                "false", "lie", "hoax", "fake", "conspiracy", "debunked",
                "misinform", "propaganda", "mislead"
            ],
            HarmCategory.ILLEGAL_ACTIVITIES: [
                "illegal", "crime", "criminal", "fraud", "scam", "theft",
                "drugs", "cocaine", "heroin", "meth", "laundering"
            ]
        }
    ...
    async def analyze_behavioral_patterns(
        ...
        if len(recent_interactions) >= 3:
            ...
            if len(recent_harm_levels) >= 2:
                increasing = all(
                    recent_harm_levels[i] <= recent_harm_levels[i+1] 
                    for i in range(len(recent_harm_levels)-1)
                )
...
class ConstitutionalPolicyEngine:
    ...
    async def evaluate_content(
        ...
        evaluation_details = {
            ...
            "violations": [
                {
                    "category": v.harm_category.value,
    ...
    async def validate_workload_deployment(
        ...
        for category in workload_harm_categories:
            if not any(
                HarmCategory(category) in constraint.harm_categories 
                for constraint in policy.constraints
            ):
    ...
    def get_policy_violations_summary(
        ...
        for violation in relevant_violations:
            ...
            summary["most_violated_constraints"][constraint_name] = summary["most_violated_constraints"].get(constraint_name, 0) + 1
        ...
        if total_violations > 0:
            ...
            report["detailed_analysis"] = {
                ...
                "most_problematic_categories": sorted(
                    violations_summary["violations_by_category"].items(),
                    key=lambda x: x[1],
                    reverse=True
                )[:5],
    ...
    def _create_default_policy(self) -> ConstitutionalPolicy:
        ...
        privacy_constraint = SafetyConstraint(
            ...
            harm_categories=[HarmCategory.PRIVACY_VIOLATIONS, HarmCategory.PERSONAL_INFO_EXPOSURE, HarmCategory.DOXXING],
            max_risk_score=0.1,
            violation_response=ResponseAction.BLOCK
        )
        ...
        misinformation_constraint = SafetyConstraint(
            ...
            harm_categories=[HarmCategory.MISINFORMATION, HarmCategory.DISINFORMATION, HarmCategory.CONSPIRACY_THEORIES],
            max_risk_score=0.4,
    ...
    async def _process_violation(self, violation: PolicyViolation, policy: ConstitutionalPolicy):
        ...
        constraint = next(
            (c for c in policy.constraints if c.constraint_id == violation.constraint_id),
            None
        )
