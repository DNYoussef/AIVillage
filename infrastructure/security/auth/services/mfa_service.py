...
class MFAService(IMFAService):
    ...
    def setup_totp(self, user_id: str, email: str) -> MFASetupResult:
        ...
        try:
            if not TOTP_AVAILABLE:
                return MFASetupResult(success=False, error_message="TOTP libraries not available")
            ...
            provisioning_uri = totp.provisioning_uri(name=email, issuer_name="AIVillage")
            ...
            return MFASetupResult(success=True, qr_code=qr_code_data, backup_codes=backup_codes, secret=secret)
    ...
    def generate_backup_codes(self, user_id: str, count: int = 10) -> List[str]:
        ...
        try:
            ...
            for _ in range(count):
                ...
                code = "".join(
                    secrets.choice("ABCDEFGHJKMNPQRSTUVWXYZ23456789") for _ in range(self.backup_code_length)
                )
                codes.append(code)
    ...
    def is_mfa_required(self, user_id: str) -> bool:
        ...
        return status.totp_enabled or status.sms_enabled or status.email_enabled or status.backup_codes_available > 0
