...
class AuthenticationService(IAuthenticationService):
    ...
    async def authenticate_user(self, credentials: AuthCredentials) -> AuthResult:
        ...
        try:
            if not self.rbac_system:
                return AuthResult(success=False, error_message="RBAC system not configured")
            ...
            if mfa_required:
                if not credentials.mfa_token or not credentials.mfa_method:
                    return AuthResult(success=False, error_message="MFA verification required")
                ...
                mfa_verified = self.mfa_service.verify_mfa(
                    ...
                    secret=(
                        self.mfa_service.get_user_totp_secret(user_id)
                        if credentials.mfa_method == MFAMethodType.TOTP
                        else None
                    ),
                )
    ...
    async def validate_token(self, token: str, token_type: str = "access_token") -> TokenValidationResult:
        ...
        try:
            if not JWT_AVAILABLE:
                return TokenValidationResult(valid=False, error_message="JWT library not available")

            payload = jwt.decode(token, self.secret_key, algorithms=[self.algorithm])

            if payload.get("type") != token_type:
                return TokenValidationResult(valid=False, error_message=f"Invalid token type: expected {token_type}")

            jti = payload.get("jti")
            if not jti:
                return TokenValidationResult(valid=False, error_message="Token missing JTI")
            ...
            if session_id:
                ...
                if not session_data or not session_data.is_active:
                    return TokenValidationResult(valid=False, error_message="Session is no longer active")
            ...
            return TokenValidationResult(valid=False, error_message=f"Invalid token: {e}")
        except Exception as e:
            logger.error(f"Token validation failed: {e}")
            return TokenValidationResult(valid=False, error_message="Token validation failed")
    ...
    async def create_session_tokens(
        ...
        try:
            ...
            session_id = await self.session_manager.create_session(
                user_id=user_id, device_info=device_info, roles=roles or [], permissions=permissions or []
            )
