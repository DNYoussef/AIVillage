...
class SecurityPolicy:
    ...
    allowed_channel_types: Set[ChannelType] = field(default_factory=lambda: {
        ChannelType.HTTP3_COVERT,
        ChannelType.WEBSOCKET_COVERT,
        ChannelType.MIXNET_ROUTED
    })
...
class BetaNetSecurityManager:
    ...
    async def establish_secure_channel(
        ...
        try:
            ...
            success, encryption_key, auth_key = await self._perform_key_exchange(
                remote_node_id, security_level
            )
    ...
    async def encrypt_message(
        ...
        try:
            ...
            encrypted_message = await self._aead_encrypt(
                message,
            ...
            packet = {
                ...
                "auth_tag": self._create_auth_tag(
                    header_bytes + encrypted_message,
        ...
        try:
            ...
            if sequence <= channel.sequence_number:
                ...
                await self._record_threat_event(
                    ThreatType.REPLAY_ATTACK,
                    sender_id,
                    f"Invalid sequence: {sequence}"
                )
                return False, None, None
            
            # Verify authentication tag
            header_bytes = json.dumps(header, sort_keys=True).encode()
            expected_auth_tag = self._create_auth_tag(
            ...
            # Decrypt message
            decrypted_message = await self._aead_decrypt(
                encrypted_payload,
                channel.encryption_key,
                header_bytes
            )
        ...
        try:
            ...
            try:
                ...
                await self._record_threat_event(
                    ...
                    node_id,
                    "Invalid signature during identity verification"
                )
                self.security_stats["authentication_failures"] += 1
                return False
                ...
                for rule in pattern_rules:
                    ...
                    for match in matches:
                        ...
                        logger.warning(
                            ...
                            f"from {threat_event.source_node}"
                        )
    ...
    async def rotate_keys(self, channel_id: str) -> bool:
        ...
        try:
            ...
            success = await self._exchange_rotated_keys(
    ...
    async def _perform_key_exchange(
        ...
        try:
            ...
            if self.security_policy.enable_forward_secrecy:
                ...
                ephemeral_public = self.node_credentials.public_key[:32]
            
            # Derive shared secrets (simplified ECDH-like)
            shared_secret = hashlib.sha256(
                ephemeral_private + 
                remote_node_id.encode() + 
                self.node_id.encode()
            ).digest()
            ...
            if CRYPTOGRAPHY_AVAILABLE:
                ...
                derived_keys = hashlib.sha256(
                    shared_secret + b"betanet_key_derivation"
                ).digest() + hashlib.sha256(
                    shared_secret + b"betanet_auth_derivation"
                ).digest()
    ...
    async def _initialize_threat_patterns(self) -> None:
        ...
        self.threat_patterns = {
            "replay_attack": [{
                ...
                "severity": "high",
                "pattern": {
                    "duplicate_timestamps": {"threshold": 2},
                    "sequence_anomaly": {"threshold": 3}
                }
            }],
            ...
            "traffic_analysis": [{
                ...
                "severity": "medium",
                "pattern": {
                    "timing_correlation": {"threshold": 0.8},
                    "size_correlation": {"threshold": 0.9}
                }
            }],
            ...
            "identity_spoofing": [{
                ...
                "severity": "critical",
                "pattern": {
                    "auth_failures": {"threshold": 5, "window": 300},
                    "cert_mismatch": {"threshold": 1}
                }
            }]
        }
    ...
    async def _analyze_pattern(
        self,
        traffic_sample: List[Dict[str, Any]],
        rule: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for source, events in events_by_source.items():
                ...
                if "request_rate" in pattern:
                    ...
                    recent_events = [
                        e for e in events 
                        if time.time() - e.get("timestamp", 0) < window
                    ]
                ...
                if "auth_failures" in pattern:
                    ...
                    auth_failures = [
                        e for e in events
                        if e.get("type") == "auth_failure" and
                        time.time() - e.get("timestamp", 0) < window
                    ]
    ...
    async def _exchange_rotated_keys(
        self,
        remote_node_id: str,
    ...
    def get_security_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.security_stats,
            ...
            "recent_threats": len([
                event for event in self.threat_events[-100:]
                if time.time() - event.detected_at < 3600  # Last hour
            ])
        }
    ...
    def get_threat_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical_threats": len([
                e for e in recent_threats if e.severity == "critical"
            ])
        }
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        recent_critical_threats = len([
            event for event in self.threat_events
            if (time.time() - event.detected_at < 3600 and 
                event.severity == "critical" and not event.mitigated)
        ])
