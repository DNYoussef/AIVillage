...
class SecureAggregationProtocol:
    ...
    async def secure_aggregate(
        ...
        try:
            ...
            if method == AggregationMethod.HOMOMORPHIC:
                aggregated_result = await self._homomorphic_aggregation(
                    aggregation_id, valid_gradients
                )
                self.aggregation_stats["homomorphic_operations"] += 1
                
            elif method == AggregationMethod.SECRET_SHARING:
                aggregated_result = await self._secret_sharing_aggregation(
                    aggregation_id, valid_gradients
                )
                ...
                aggregated_result = await self._byzantine_robust_aggregation(
                    aggregation_id, valid_gradients
                )
                
            else:
                # Fallback to weighted average
                aggregated_result = await self._weighted_average_aggregation(
                    aggregation_id, valid_gradients
                )
            ...
            verification_result = await self._verify_aggregation(
                aggregation_id, valid_gradients, aggregated_result
            )
    ...
    async def create_secure_gradient(
        ...
        for name, gradient in raw_gradients.items():
            ...
            if participant_id in self.homomorphic_keys:
                ...
                encrypted_gradients[name] = await self._homomorphic_encrypt(
                    gradient_bytes, participant_id
                )
            else:
        ...
        verification_proof = self._create_gradient_proof(
            participant_id, gradient_id, encrypted_gradients
        )
    ...
    async def _homomorphic_aggregation(
        ...
        for param_name in first_gradient.encrypted_gradients.keys():
            ...
            if encrypted_values:
                ...
                aggregated_decrypted = await self._homomorphic_decrypt(
                    aggregated_encrypted, len(encrypted_values)
                )
    ...
    async def _secret_sharing_aggregation(
        ...
        for gradient in gradients:
            for param_name, encrypted_gradient in gradient.encrypted_gradients.items():
                ...
                shares = self._create_secret_shares(
                    encrypted_gradient, threshold, len(gradients)
                )
    ...
    def _create_secret_shares(
        self, 
        secret: bytes, 
        threshold: int, 
        total_shares: int
    ) -> List[AggregationShare]:
    ...
    def _detect_byzantine_gradients(
        self, 
    ...
    async def _check_privacy_budgets(self, gradients: List[SecureGradient]) -> bool:
        ...
        for gradient in gradients:
            ...
            if (budget.epsilon_used + epsilon_needed > budget.epsilon_limit or
                budget.delta_used + delta_needed > budget.delta_limit):
                return False
        
        return True

    async def _update_privacy_budgets(
        self, 
        gradients: List[SecureGradient], 
        privacy_level: PrivacyLevel
    ) -> None:
    ...
    def get_aggregation_stats(self) -> Dict[str, Any]:
        ...
        success_rate = (
            ...
            max(1, self.aggregation_stats["total_aggregations"])
        )
        ...
        return {
            **self.aggregation_stats,
            ...
            "privacy_budgets_active": sum(
                ...
                if budget.epsilon_used < budget.epsilon_limit
            )
        }
    ...
    def get_privacy_budget_status(self, participant_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "budget_exhausted": (
                budget.epsilon_used >= budget.epsilon_limit or
                budget.delta_used >= budget.delta_limit
            )
        }
