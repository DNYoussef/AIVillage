...
from .attestation import (
    TEEType, ConstitutionalTier, AttestationResult, TEEAttestationManager,
...
class EnclaveConfiguration:
    ...
    harm_categories_monitored: List[str] = field(default_factory=lambda: [
        "violence", "hate_speech", "harassment", "privacy_violations"
    ])
...
class IntelSGXEnclaveManager:
    ...
    async def create_enclave(self, config: EnclaveConfiguration) -> EnclaveInstance:
        ...
        instance = EnclaveInstance(
            ...
            measurement_hash=self._calculate_enclave_measurement(config)
        )
    ...
    def _validate_workload_compatibility(
        ...
        if (config.allowed_workload_types and 
            manifest.workload_type not in config.allowed_workload_types):
            return False
...
class AMDSEVEnclaveManager:
    ...
    async def create_enclave(self, config: EnclaveConfiguration) -> EnclaveInstance:
        ...
        instance = EnclaveInstance(
            ...
            measurement_hash=self._calculate_vm_measurement(config)
        )
...
class SoftwareEnclaveManager:
    ...
    async def create_enclave(self, config: EnclaveConfiguration) -> EnclaveInstance:
        ...
        instance = EnclaveInstance(
            ...
            measurement_hash=self._calculate_process_measurement(config)
        )
...
async def execute_constitutional_inference(
    ...
    return await manager.execute_constitutional_workload(
        node_id, manifest, input_data, constitutional_tier
    )


async def create_constitutional_training_enclave(
    node_id: str,
