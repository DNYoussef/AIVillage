...
class ValidationOptimizer:
    ...
    def record_routing_outcome(
        ...
        self.logger.info(
            f"Recorded routing outcome: {routing_decision.decision_id} "
            f"(accuracy: {prediction_accuracy:.3f})"
        )
    ...
    def calculate_performance_metrics(self, window_days: int = 7) -> Dict:
        ...
        mttr_accuracy = 1 - np.mean(np.abs(np.array(predicted_mttrs) - np.array(actual_mttrs)) / np.array(predicted_mttrs))
    ...
    def optimize_routing_parameters(self, target_metric: str = "success_rate") -> OptimizationResult:
        ...
        self.logger.info(
            f"Optimization completed: {improvement*100:.1f}% improvement "
            f"in {target_metric}"
        )
    ...
    def _calculate_validation_score(self, parameter_updates: Dict) -> float:
        ...
        aggressive_changes = sum(
            1 for adjustment in parameter_updates.values()
    ...
    def validate_routing_decision(self, routing_decision: RoutingDecision) -> Dict:
        ...
        if historical_success < 0.8:
            validation_result['warnings'].append(
                f"Strategy has low historical success rate: {historical_success:.3f}"
            )
    ...
    def _check_historical_performance(self, strategy_name: str) -> float:
        ...
        strategy_metrics = [
            m for m in self.validation_metrics[-200:]  # Last 200 decisions
    ...
    def generate_optimization_report(self) -> Dict:
        ...
        report = {
            ...
            'target_compliance': {
                ...
                'success_rate_gap': self.target_success_rate - current_metrics.get('success_rate', {}).get('actual_avg', 0),
    ...
    def _generate_recommendations(self, current_metrics: Dict) -> List[str]:
        ...
        if escalation.get('false_negative_rate', 0) > 0.05:
            recommendations.append(
                "High escalation false negative rate. "
                "Consider lowering escalation thresholds to catch more issues."
            )
