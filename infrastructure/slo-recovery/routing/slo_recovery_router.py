...
class SLORecoveryRouter:
    ...
    def _estimate_success_probability(
        ...
        severity_penalties = {"critical": 0.05, "high": 0.03, "medium": 0.01, "low": 0.0}
        severity_penalty = severity_penalties.get(breach_classification.severity.value, 0.02)

        success_probability = base_success_rate + confidence_adjustment + classification_adjustment - severity_penalty
    ...
    def _update_performance_metrics(self, routing_decision: RoutingDecision):
        ...
        self.success_metrics["success_rate"] = (
            self.success_metrics["successful_routes"] / self.success_metrics["total_routes"]
        )
        ...
        self.success_metrics["average_recovery_time"] = (current_avg * (total_routes - 1) + new_time) / total_routes
    ...
    def _analyze_routing_patterns(self) -> Dict:
        ...
        patterns["most_common_categories"] = dict(sorted(category_counts.items(), key=lambda x: x[1], reverse=True)[:5])

        patterns["average_confidence_by_category"] = {
            cat: sum(confs) / len(confs) for cat, confs in category_confidences.items()
        }
        ...
        patterns["most_common_strategies"] = dict(sorted(strategy_counts.items(), key=lambda x: x[1], reverse=True)[:5])
    ...
    def _identify_improvement_opportunities(self) -> List[str]:
        ...
        if len(self.routing_history) > 10:
            low_confidence_routes = [
                r for r in self.routing_history[-50:] if r.breach_classification.confidence_score < 0.7
            ]
        ...
        escalation_count = sum(1 for r in self.routing_history[-50:] if len(r.escalation_events) > 0)
