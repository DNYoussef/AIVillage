...
class GitHubOrchestratorAdapter(DataFeedAdapter):
    ...
    async def poll_data(self) -> Optional[GitHubOrchestratorData]:
        ...
        try:
            ...
            async with self.session.get(
                f"{self.webhook_url}/workflow-runs",
                params=params
            ) as response:
...
class IntegrationCoordinator:
    ...
    async def _execute_coordination_plan(self, routing_decision: RoutingDecision):
        ...
        try:
            ...
            execution_results = self.slo_router.parallel_coordinator.execute_coordination_plan(
                coordination_plan
            )
            
            # Log execution results
            success_rate = sum(
                1 for result in execution_results["agent_results"].values()
                if result.get("success", False)
            ) / len(execution_results["agent_results"]) if execution_results["agent_results"] else 0
            
            self.logger.info(
                f"Coordination plan executed: {coordination_plan.plan_id} "
...
def create_integration_coordinator(
    ...
    if flake_detector_config:
        flake_adapter = FlakeDetectorAdapter(
            api_endpoint=flake_detector_config['api_endpoint'],
            api_key=flake_detector_config['api_key']
        )
    ...
    if github_orchestrator_config:
        github_adapter = GitHubOrchestratorAdapter(
            webhook_url=github_orchestrator_config['webhook_url'],
            api_token=github_orchestrator_config['api_token']
        )
