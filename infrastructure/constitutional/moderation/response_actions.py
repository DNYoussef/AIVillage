...
class ResponseActions:
    ...
    def _load_action_templates(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            "h0_allow": {
                "actions": [
                    {
                        "type": ActionType.TRANSPARENCY_LOG,
                        "priority": 1,
                        "constitutional_note": "Promoting viewpoint diversity",
                    },
    ...
    async def determine_actions(self, enforcement_result: Any, harm_analysis: Any, user_tier: str) -> ActionPlan:
        """
    ...
    async def _apply_tier_modifications(
        ...
        if tier_config.get("viewpoint_firewall") and enforcement_result.constitutional_analysis.get(
            "viewpoint_neutrality", {}
        ).get("discrimination_risk"):
            modified_actions.append(
                {"type": ActionType.MONITOR, "priority": 10, "viewpoint_bias_monitoring": True, "duration_hours": 168}
            )
    ...
    async def _create_response_action(
        ...
        description = await self._generate_action_description(action_config, harm_analysis, user_tier)
    ...
    async def execute_actions(
        ...
        try:
            ...
            logger.info(
                f"Action execution completed for {content_id}: "
    ...
    async def _execute_restriction(self, action: ResponseAction, harm_analysis: Any, user_tier: str) -> dict[str, Any]:
        ...
        return {
            ...
            "constitutional_justification_documented": action.parameters.get(
                "constitutional_justification_required", False
            ),
        }
