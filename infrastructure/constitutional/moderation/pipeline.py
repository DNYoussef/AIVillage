...
class ConstitutionalModerationPipeline:
    ...
    async def process_content(
        ...
        try:
            ...
            harm_analysis = await self._analyze_constitutional_harm(content_id, content, content_type, context or {})

            # Step 2: Policy enforcement decision
            enforcement_result = await self.policy_enforcement.evaluate_content(harm_analysis, user_tier, context)
            ...
            requires_escalation = await self._requires_escalation(harm_analysis, enforcement_result, user_tier)
            ...
            result = ModerationResult(
                ...
                audit_trail=self._build_audit_trail(content_id, harm_analysis, enforcement_result, processing_time_ms),
                transparency_score=self._calculate_transparency_score(harm_analysis, enforcement_result),
            )
    ...
    async def _analyze_constitutional_harm(
        ...
        try:
            ...
            classification_result = await self.harm_classifier.classify_content(content, content_type, context)

            # Extract constitutional concerns
            constitutional_concerns = await self._extract_constitutional_concerns(content, classification_result)

            # Calculate viewpoint bias score
            viewpoint_bias_score = await self._calculate_viewpoint_bias(content, classification_result)
    ...
    async def _extract_constitutional_concerns(self, content: str, classification_result: Any) -> dict[str, Any]:
        ...
        if any(
            cat in ["political_speech", "religious_expression", "artistic_expression"]
    ...
    async def _calculate_viewpoint_bias(self, content: str, classification_result: Any) -> float:
        ...
        if any(cat in ["cultural_expression", "religious_content"] for cat in classification_result.categories):
    ...
    async def _requires_escalation(
        self, harm_analysis: ContentAnalysis, enforcement_result: Any, user_tier: str
    ) -> bool:
        """Determine if content requires human escalation"""

        # Gold tier: Escalate high-confidence severe cases
        if user_tier == "Gold":
            if harm_analysis.harm_level == "H3" and harm_analysis.confidence_score > 0.8:
                return True
        ...
        # Silver tier: Limited escalation for unclear cases
            ...
            if harm_analysis.harm_level in ["H2", "H3"] and harm_analysis.confidence_score < 0.6:
                return True
    ...
    def _is_appeal_eligible(self, enforcement_result: Any, user_tier: str) -> bool:
        ...
        if user_tier == "Gold" and enforcement_result.decision == ModerationDecision.ALLOW_WITH_WARNING:
            return True
    ...
    def _calculate_transparency_score(self, harm_analysis: ContentAnalysis, enforcement_result: Any) -> float:
    ...
    async def _execute_response_actions(self, result: ModerationResult):
        ...
        try:
            await self.response_actions.execute_actions(
                result.content_id, result.response_actions, result.harm_analysis, result.tier_level
            )
        except Exception as e:
    ...
    async def _update_statistics(self, result: ModerationResult):
        ...
        if result.requires_escalation:
            escalations = sum(1 for d in self.processing_stats["decisions_by_type"].values() if d == "escalate")
    ...
    async def _create_error_result(self, content_id: str, error_message: str, user_tier: str) -> ModerationResult:
