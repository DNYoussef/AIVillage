...
class AppealsManager:
    ...
    def __init__(self):
        ...
        self.appeal_slas = {
            "Bronze": {"standard": timedelta(days=5), "expedited": timedelta(days=2)},
            "Silver": {"standard": timedelta(days=3), "expedited": timedelta(days=1)},
            "Gold": {"standard": timedelta(days=2), "expedited": timedelta(hours=12)},
        }
    ...
    async def _determine_appeal_type(self, appeal_reason: str, context: dict[str, Any]) -> AppealType:
        ...
        if any(
            ...
            for keyword in ["free speech", "first amendment", "protected speech", "political", "religious", "artistic"]
        ):
            return AppealType.FIRST_AMENDMENT

        # Viewpoint discrimination appeals
        if any(
            keyword in reason_lower for keyword in ["bias", "discrimination", "viewpoint", "political bias", "unfair"]
        ):
            return AppealType.VIEWPOINT_DISCRIMINATION

        # Due process appeals
        if any(keyword in reason_lower for keyword in ["due process", "unfair process", "no notice", "no explanation"]):
            return AppealType.DUE_PROCESS

        # Prior restraint appeals
        if any(
            keyword in reason_lower for keyword in ["prior restraint", "censorship", "blocked unfairly", "suppressed"]
        ):
            return AppealType.PRIOR_RESTRAINT

        # Procedural error appeals
        if any(keyword in reason_lower for keyword in ["error", "mistake", "wrong", "misclassified"]):
            return AppealType.PROCEDURAL_ERROR

        # New evidence appeals
        if any(keyword in reason_lower for keyword in ["new evidence", "additional context", "misunderstood"]):
    ...
    async def _assign_constitutional_expert(self, appeal_case: AppealCase):
        ...
        for expert_id, expert_data in self.constitutional_experts.items():
            ...
            score = (
                (specialty_match / len(required_specialties))
                * expert_data["expertise_level"]
                * (1 - expert_data["bias_score"])
            )
    ...
    async def _conduct_constitutional_review(self, appeal_case: AppealCase, expert_id: str):
        ...
        constitutional_analysis = await self._perform_constitutional_analysis(appeal_case, expert_data["specialties"])
    ...
    async def _perform_constitutional_analysis(
        ...
        if appeal_type == AppealType.FIRST_AMENDMENT or "first_amendment" in expert_specialties:
            ...
            if (
                first_amendment_analysis["protected_speech_category"]
                and not first_amendment_analysis["narrow_tailoring"]
            ):
    ...
    async def _conduct_community_review(self, appeal_case: AppealCase, reviewer_ids: list[str]):
        ...
        logger.info(
            f"Community review completed for appeal {appeal_case.appeal_id}: "
    ...
    async def _generate_constitutional_summary(self, appeal_case: AppealCase) -> dict[str, Any]:
        ...
        if appeal_case.constitutional_review_result:
            ...
            summary["constitutional_principles_considered"] = constitutional_analysis.get(
                "constitutional_principles_analyzed", []
            )
    ...
    async def get_appeals_metrics(self) -> dict[str, Any]:
        ...
        for status in AppealStatus:
            status_distribution[status.value] = len([appeal for appeal in all_appeals if appeal.status == status])

        # Appeal type distribution
        type_distribution = {}
        for appeal_type in AppealType:
            type_distribution[appeal_type.value] = len(
                [appeal for appeal in all_appeals if appeal.submission.appeal_type == appeal_type]
            )
        ...
        constitutional_reviews = len([appeal for appeal in all_appeals if appeal.constitutional_review_result])

        community_reviews = len([appeal for appeal in all_appeals if appeal.community_review_result])
    ...
    async def shutdown(self):
        ...
        pending_appeals = len(
            [
                appeal
                for appeal in self.active_appeals.values()
