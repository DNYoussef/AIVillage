...
class PolicyEnforcement:
        ...
        return {
            ConstitutionalPrinciple.FIRST_AMENDMENT: {
                ...
                "unprotected_categories": [
                    "true_threats", "incitement_to_violence", "defamation",
                    "fraud", "child_exploitation", "copyright_infringement"
                ],
            ...
            ConstitutionalPrinciple.VIEWPOINT_NEUTRALITY: {
                "prohibited_discrimination": [
                    "political_ideology", "religious_beliefs", "social_viewpoints"
                ],
    ...
    async def evaluate_content(
        ...
        try:
            ...
            harm_decision = await self._apply_harm_level_policy(
                harm_analysis, constitutional_analysis, user_tier
            )
            
            # Step 3: Apply tier-specific modifications
            tier_modifications = await self._apply_tier_modifications(
                harm_decision, constitutional_analysis, user_tier
            )
            ...
            monitoring = await self._determine_monitoring_requirements(
                final_decision, harm_analysis, user_tier
            )
    ...
    async def _analyze_first_amendment_implications(self, harm_analysis: Any) -> Dict[str, Any]:
        ...
        protected_detected = any(
            category in guidelines["protected_categories"] 
            for category in harm_analysis.harm_categories
        )
        
        # Check for unprotected speech categories
        unprotected_detected = any(
            category in guidelines["unprotected_categories"]
            for category in harm_analysis.harm_categories
        )
        
        # Determine if heightened scrutiny is required
        heightened_scrutiny = (
            protected_detected and not unprotected_detected and 
            harm_analysis.confidence_score < 0.9
        )
    ...
    async def _analyze_prior_restraint_implications(self, harm_analysis: Any) -> Dict[str, Any]:
        ...
        narrowly_tailored = (
            harm_analysis.confidence_score > 0.8 and
            len(harm_analysis.harm_categories) <= 2
        )
    ...
    async def _generate_policy_rationale(
        ...
        rationale_parts.append(f"Content classified as {harm_analysis.harm_level} "
                              f"with {harm_analysis.confidence_score:.2f} confidence")
        
        # Constitutional considerations
        if constitutional_analysis.get("first_amendment", {}).get("protected_speech_detected"):
            rationale_parts.append("First Amendment protected speech detected - heightened constitutional scrutiny applied")
            
        if constitutional_analysis.get("viewpoint_neutrality", {}).get("discrimination_risk"):
            rationale_parts.append(f"Viewpoint neutrality concern (bias score: {harm_analysis.viewpoint_bias_score:.2f})")
        
        # Tier-specific rationale
        tier_policy = self.tier_policies[user_tier]
        rationale_parts.append(f"{user_tier} tier policy applied with {tier_policy['constitutional_protection']} constitutional protection")
        ...
        if decision == PolicyDecision.ALLOW:
            ...
            rationale_parts.append("Content quarantined pending review - preserves evidence and constitutional protections")
