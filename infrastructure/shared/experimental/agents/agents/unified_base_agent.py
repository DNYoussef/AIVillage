...
class UnifiedBaseAgent:
    ...
    async def _enrich_task_context(self, task: LangroidTask) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning("Failed to enrich task context", extra={"error": str(e), "task": task.content[:50]})
            return {}
    ...
    async def _validate_and_enhance_result(self, task: LangroidTask, result: Any) -> Any:
        ...
        if result is None:
            ...
            return {"type": "empty_result", "message": "No result generated", "task": task.content}

        # Ensure result is serializable
        if not isinstance(result, (dict, list, str, int, float, bool)):
            result = {"type": "converted_result", "original_type": type(result).__name__, "data": str(result)}
    ...
    async def _update_processing_metrics(self, task_id: str, processing_time: float, success: bool) -> None:
        ...
        try:
            ...
            self.logger.debug("Updated processing metrics", extra=metrics)
