...
from .task_constants import TaskConstants, TaskDefaults
from .project_constants import ProjectConstants, ProjectDefaults
from .timing_constants import TimingConstants, BatchProcessingDefaults
from .performance_constants import PerformanceConstants, IncentiveDefaults
from .message_constants import MessageConstants, MessageDefaults
...
class ConfigManager:
    ...
    def get_learning_rate(self) -> float:
        ...
        if self._environment_config and self._environment_config.learning_rate is not None:
            return self._validate_range(
                self._environment_config.learning_rate,
                0.001,
                1.0,
                "learning_rate"
            )
    ...
    def get_min_performance_multiplier(self) -> float:
        ...
        if self._environment_config and self._environment_config.min_performance is not None:
            return self._validate_range(
                self._environment_config.min_performance,
                0.1,
                1.0,
                "min_performance"
            )
    ...
    @staticmethod
    def _validate_range(value: Union[int, float], min_val: Union[int, float], 
                       max_val: Union[int, float], param_name: str) -> Union[int, float]:
        """Validate that a value is within the specified range."""
        if not min_val <= value <= max_val:
            raise ValueError(
                f"Configuration parameter '{param_name}' must be between {min_val} and {max_val}, "
                f"but got {value}"
            )
        return value
