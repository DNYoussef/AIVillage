...
class ComponentProfile:
    ...
    def add_metric(self, name: str, value: float, unit: str, **labels):
        ...
        metric = PerformanceMetric(
            name=name, value=value, unit=unit, timestamp=datetime.now(), component=self.component_name, labels=labels
        )
...
class PerformanceMonitor:
    ...
    def profile_component(self, component_name: str):
        ...
        profile = ComponentProfile(component_name=component_name, start_time=datetime.now())
    ...
    async def record_metric(self, name: str, value: float, unit: str, component: str = "", **labels):
        """Record a performance metric."""
        if not self.config.enabled:
            return

        metric = PerformanceMetric(
            name=name, value=value, unit=unit, timestamp=datetime.now(), component=component, labels=labels
        )
    ...
    async def generate_alert(self, severity: AlertSeverity, component: str, message: str, **metadata):
                ...
                for component in self._get_monitored_components():
                    ...
                    if health["status"] == "critical":
                        await self.generate_alert(
                            AlertSeverity.CRITICAL, component, f"Component health critical: {health['issues']}"
                        )
                    elif health["status"] == "unhealthy":
                        await self.generate_alert(
                            AlertSeverity.WARNING, component, f"Component health degraded: {health['issues']}"
                        )
        ...
        while True:
            try:
                ...
                self.metrics_buffer = deque(
                    [m for m in self.metrics_buffer if m.timestamp >= cutoff_time], maxlen=self.config.max_samples
                )

                # Clean resolved alerts older than retention period
                self.alerts = deque(
                    [a for a in self.alerts if not a.resolved or a.resolved_at >= cutoff_time], maxlen=1000
                )
    ...
    def _analyze_memory_profile(self, snapshot_start, snapshot_end) -> Dict[str, Any]:
        ...
        for stat in top_stats[:10]:  # Top 10 allocations
            memory_analysis["top_allocations"].append(
                {"size_diff": stat.size_diff, "count_diff": stat.count_diff, "size": stat.size, "count": stat.count}
            )
    ...
    async def _check_metric_alerts(self, metric: PerformanceMetric):
        ...
        if "cpu" in metric.name.lower() and metric.unit == "percent":
            if metric.value > 90:
                await self.generate_alert(
                    AlertSeverity.CRITICAL, metric.component, f"High CPU usage: {metric.value:.1f}%"
                )
            elif metric.value > 80:
                await self.generate_alert(
                    AlertSeverity.WARNING, metric.component, f"Elevated CPU usage: {metric.value:.1f}%"
                )
        ...
        if "memory" in metric.name.lower() and metric.unit == "percent":
            if metric.value > 95:
                await self.generate_alert(
                    AlertSeverity.CRITICAL, metric.component, f"Critical memory usage: {metric.value:.1f}%"
                )
            elif metric.value > 85:
                await self.generate_alert(
                    AlertSeverity.WARNING, metric.component, f"High memory usage: {metric.value:.1f}%"
                )
        ...
        if "error" in metric.name.lower():
            if metric.value > 10:
                await self.generate_alert(
                    AlertSeverity.CRITICAL, metric.component, f"High error rate: {metric.value} errors"
                )
            elif metric.value > 5:
                await self.generate_alert(
