        ...
        async with self.lock:
            ...
            try:
                ...
                self.connection = await asyncio.wait_for(
                    self.connection_factory(), timeout=self.config.connection_timeout
                )
        ...
        async with self.lock:
            ...
            try:
                ...
                if self.metrics.average_response_time == 0:
                    ...
                    self.metrics.average_response_time = self.metrics.average_response_time * 0.9 + execution_time * 0.1
        ...
        if self.metrics.total_requests > 0:
            ...
            if failure_rate > self.config.emergency_threshold:
                if not self.emergency_state:
                    logger.critical(
                        f"EMERGENCY: Connection {self.connection_id} failure rate {failure_rate:.2%} exceeds threshold"
                    )
...
class ConnectionPoolManager:
    ...
    async def initialize(self):
        ...
        async with self.lock:
            logger.info(
                f"Initializing connection pool with {self.config.min_connections}-{self.config.max_connections} connections"
            )
...
class EmergencyTriageSystem:
    ...
    async def _trigger_emergency_response(self, pool: ConnectionPoolManager, failure_rate: float):
        ...
        if self.last_emergency and current_time - self.last_emergency < self.config.emergency_response_time:
            return
    ...
    async def _perform_emergency_actions(self, pool: ConnectionPoolManager):
        ...
        failed_connections = [
            conn_id for conn_id, conn in pool.connections.items() if conn.metrics.state == ConnectionState.FAILED
        ]
