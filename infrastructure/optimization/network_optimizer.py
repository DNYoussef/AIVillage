...
try:
    ...
    StreamPriority = type(
        "StreamPriority", (), {"LOW": "low", "NORMAL": "normal", "HIGH": "high", "CRITICAL": "critical"}
    )
    StreamType = type("StreamType", (), {"DATA": "data", "CONTROL": "control", "MEDIA": "media"})

# Security enhancement imports for ECH + Noise Protocol
try:
    from ..p2p.betanet.noise_protocol import CRYPTO_AVAILABLE as NOISE_CRYPTO_AVAILABLE
...
class ProtocolSelector:
    ...
    async def select_optimal_protocol(
        ...
        try:
            ...
            if self.nat_optimizer:
                ...
                nat_info = {"nat_type": nat_discovery.nat_type, "confidence": nat_discovery.confidence}

            # Select protocol based on multiple factors
            protocol = await self._evaluate_protocol_options(destination, qos_requirement, network_condition, nat_info)
    ...
    async def _evaluate_protocol_options(
        self,
        destination: str,
    ...
    async def record_performance(self, protocol: NetworkProtocol, destination: str, metrics: NetworkMetrics):
...
class BandwidthManager:
    ...
    async def allocate_bandwidth(
        self, connection_id: str, requested_bandwidth: int, priority: StreamPriority, qos_requirement: QualityOfService
    ) -> int:
...
class LatencyOptimizer:
    ...
    async def optimize_latency(self, source: str, destination: str, current_latency: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if optimization_result["optimized_latency"] < current_latency:
                optimization_result["improvement_factor"] = current_latency / optimization_result["optimized_latency"]

            # Cache optimization result
            self.optimization_cache[route_key] = optimization_result

            logger.info(
                f"Latency optimization for {route_key}: "
    ...
    async def _discover_alternative_routes(self, source: str, destination: str) -> List[Dict[str, Any]]:
        ...
        alternatives.append(
            {"type": "geographic", "hops": ["relay1.example.com", "relay2.example.com"], "estimated_distance": 1000}
        )

        # CDN-based routing
        alternatives.append({"type": "cdn", "hops": ["cdn-edge.example.com"], "estimated_distance": 500})

        # P2P relay routing
        alternatives.append({"type": "p2p_relay", "hops": ["peer-relay-1", "peer-relay-2"], "estimated_distance": 800})
...
class QosManager:
    ...
    async def apply_qos_policy(
        self, connection_id: str, qos_requirement: QualityOfService, protocol: NetworkProtocol
    ) -> bool:
    ...
    async def monitor_qos_compliance(self, connection_id: str, metrics: NetworkMetrics) -> Dict[str, Any]:
        ...
        return {"compliant": compliant, "violations": violations, "policy": policy, "metrics": metrics}
...
class NetworkOptimizer:
    ...
    async def optimize_connection(
        ...
        try:
            ...
            qos_applied = await self.qos_manager.apply_qos_policy(connection_id, qos_requirement, optimal_protocol)
            if qos_applied:
            ...
            recommendations = await self._generate_optimization_recommendations(connection_id, optimization_result)
...
class SecurityContext:
    ...
    def __post_init__(self):
        ...
        if self.require_sni_protection and self.protocol not in [
            SecurityProtocol.ECH_TLS,
            SecurityProtocol.HYBRID_ECH_NOISE,
        ]:
            logger.warning("SNI protection requires ECH-capable protocol")
            self.protocol = SecurityProtocol.ECH_TLS

        if self.require_traffic_analysis_resistance and self.protocol not in [
            SecurityProtocol.NOISE_XK,
            SecurityProtocol.HYBRID_ECH_NOISE,
        ]:
...
class SecurityEnhancedNetworkOptimizer(NetworkOptimizer):
    ...
    async def optimize_secure_connection(
        ...
        try:
            ...
            return {"connection_id": connection_id, "error": str(e), "security_error": True, "fallback_applied": True}
    ...
    def get_security_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "security_protocols_used": list(
                set(
                    (
                        opt.get("selected_protocol", {}).value
                        if hasattr(opt.get("selected_protocol", {}), "value")
