        ...
        try:
            with self.lock:
                ...
                if size_bytes > available_memory:
                    ...
                    if size_bytes > available_memory:
                        logger.warning(f"Memory allocation failed: {size_bytes} bytes requested, {available_memory} available")
                        return False
    ...
    async def cleanup_tensors(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if cleanup_stats['cleaned'] > 0:
                logger.info(f"Cleaned up {cleanup_stats['cleaned']} tensor references, "
...
class CPUManager:
    ...
    async def allocate_cpu(self,
                          allocation_id: str,
                          cpu_percentage: float,
                          owner_id: str,
                          priority: int = 1,
                          preferred_cores: Optional[List[int]] = None) -> bool:
        ...
        try:
            with self.lock:
                ...
                if cpu_percentage > available_cpu:
                    ...
                    if cpu_percentage > available_cpu:
                        logger.warning(f"CPU allocation failed: {cpu_percentage}% requested, {available_cpu}% available")
                        return False
    ...
    async def _throttle_low_priority_processes(self):
        ...
        low_priority_allocations = [
            (alloc_id, alloc) for alloc_id, alloc in self.cpu_allocations.items()
...
class NetworkResourceManager:
        ...
        try:
            with self.lock:
                if self.connection_count >= self.config.network_connection_limit:
                    logger.warning(f"Connection limit reached: {self.connection_count}/{self.config.network_connection_limit}")
                    return False
    ...
    async def _throttle_low_priority_connections(self):
        ...
        low_priority = [
            (alloc_id, alloc) for alloc_id, alloc in self.bandwidth_allocations.items()
...
class ResourceLimiter:
    ...
    def get_limit_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'emergency_actions': {
                rt.value: len(actions) for rt, actions in self.emergency_actions_taken.items()
            }
        }
...
class ResourceManager:
    ...
    async def allocate_resources(self,
                               ...
                               owner_id: str,
                               priority: int = 1) -> Dict[ResourceType, bool]:
    ...
    async def deallocate_resources(self,
                                 allocation_id: str,
                                 resource_types: List[ResourceType]) -> Dict[ResourceType, bool]:
        ...
        try:
            for resource_type in resource_types:
                ...
                if resource_type == ResourceType.MEMORY:
                    success = await self.memory_manager.deallocate_memory(
                        f"{allocation_id}_{resource_type.value}"
                    )
                elif resource_type == ResourceType.CPU:
                    success = await self.cpu_manager.deallocate_cpu(
                        f"{allocation_id}_{resource_type.value}"
                    )
                elif resource_type == ResourceType.NETWORK:
                    success = await self.network_manager.deallocate_bandwidth(
                        f"{allocation_id}_{resource_type.value}"
                    )
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if self.system_metrics:
                    limit_status = await self.resource_limiter.check_resource_limits(
                        self.system_metrics
                    )
                ...
                if self.system_metrics:
                    for resource_type, metrics in self.system_metrics.items():
                        if metrics.utilization_percent > 0.95:  # 95% emergency threshold
                            logger.critical(f"Emergency condition detected: {resource_type.value} at {metrics.utilization_percent:.1f}%")
    ...
    async def _collect_system_metrics(self) -> Dict[ResourceType, ResourceMetrics]:
        ...
        try:
            ...
            # Network metrics
            network_util = (self.network_manager.total_allocated_bandwidth / 
                           self.config.network_bandwidth_limit * 100)
