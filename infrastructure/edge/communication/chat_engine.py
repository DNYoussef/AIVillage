"""
Resilient Chat Engine for Edge Computing
...
- LocalChatProcessor: Offline conversation handling
...
try:
    from infrastructure.twin.resilience.error_handling import (
    ...
    # Graceful degradation for missing resilience components
...
class ChatEngine:
    ...
    def __init__(self) -> None:
        ...
        if self._resilience_manager:
            ...
            # Graceful degradation without resilience manager
    ...
    def _update_service_status(self) -> ServiceStatus:
        ...
        if current_time - self._last_health_check > SERVICE_HEALTH_CHECK_INTERVAL:
            ...
            if self._mode == ChatMode.LOCAL:
                ...
                if self._circuit_breaker and self._circuit_breaker.state.value == "closed":
                    if self.health_check_twin_service():
                        self._current_status = ServiceStatus.HEALTHY
                    else:
                        self._current_status = ServiceStatus.DEGRADED
                elif self._circuit_breaker and self._circuit_breaker.state.value == "half_open":
                    self._current_status = ServiceStatus.DEGRADED
                else:  # open or no circuit breaker
                    self._current_status = ServiceStatus.OFFLINE
    ...
    def _call_twin_service(self, message: str, conversation_id: str) -> dict[str, Any]:
        ...
        # Use circuit breaker and retry logic if available
        if self._circuit_breaker and self._retry_handler:
            try:
            ...
            # Direct call without resilience features
            return twin_request()
