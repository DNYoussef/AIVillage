...
class EdgeDevice:
        ...
        if metrics.temperature_celsius:
            ...
            if metrics.temperature_celsius > thermal_threshold:
                if not self.thermal_throttling:
                    logger.warning(f"Device {self.device_id} entering thermal throttling at {metrics.temperature_celsius}°C")
    ...
    def can_accept_task(self, task: EdgeTask) -> bool:
        ...
        if self.device_type == DeviceType.MOBILE and self.current_metrics:
            if (self.current_metrics.battery_percent and 
                self.current_metrics.battery_percent < 20 and 
                not self.current_metrics.battery_charging):
                return False
...
class EdgeTaskScheduler:
    ...
    def _select_best_device(self, task: EdgeTask, devices: list[EdgeDevice]) -> EdgeDevice | None:
        ...
        scored_devices = [
            (device, device.get_capability_score(task))
...
class EdgeDeviceRegistry:
    ...
    def get_available_devices(self) -> list[EdgeDevice]:
        ...
        return [
            ...
            if d.status not in [DeviceStatus.OFFLINE, DeviceStatus.MAINTENANCE]
        ]
...
async def demo_unified_edge_system():
    ...
    try:
        ...
        system.register_device(
            "mobile-001",
            DeviceType.MOBILE,
            {"max_concurrent_tasks": 2, "cpu_cores": 8, "ram_gb": 6}
        )
        
        system.register_device(
            "desktop-001", 
            DeviceType.DESKTOP,
            {"max_concurrent_tasks": 8, "cpu_cores": 16, "ram_gb": 32}
        )
