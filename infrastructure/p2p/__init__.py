    ...
    async def connect(self, peer_address: str) -> Optional[str]:
        ...
        for transport in self.config.transport_priority:
            try:
                ...
                if peer_id:
                    self.peers[peer_id] = PeerInfo(peer_id=peer_id, addresses=[peer_address], protocols=[transport])
                    return peer_id
            except Exception as e:
    ...
    async def _discover_websocket_peers(self) -> None:
        ...
        try:
            ...
            results = await asyncio.gather(
                *[bounded_scan(task) for task in discovery_tasks[:100]], return_exceptions=True  # Limit to 100 scans
            )
