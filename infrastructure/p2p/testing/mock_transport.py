...
try:
    from ..base import (
        BaseTransport, BaseProtocol, BaseMessage, BaseNode, BaseDiscovery, BaseMetrics,
        TransportStatus, MessageType, MessagePriority, ProtocolCapability,
        NodeRole, DiscoveryMethod, MetricType, ConnectionInfo, PeerCapabilities,
        MessageMetadata, DiscoveryResult, MetricSample
    )
except ImportError:
...
class MockTransport(BaseTransport):
    ...
    async def list_peers(self) -> List[str]:
        ...
        return [pid for pid, data in self.mock_peers.items() 
...
class MockMetrics(BaseMetrics):
    ...
    async def record_metric(self, name: str, value, metric_type, 
    ...
    async def clear_metrics(self, older_than: Optional[datetime] = None) -> None:
        ...
        if older_than:
            self.recorded_metrics = [m for m in self.recorded_metrics 
