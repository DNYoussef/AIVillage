...
try:
    from infrastructure.p2p.common.configuration import ConfigManager
    from infrastructure.p2p.common.logging import P2PLogger, get_logger
    from infrastructure.p2p.common.monitoring import MetricsCollector, StandardMetrics
except ImportError:
...
class P2PTestCase(unittest.TestCase):
    ...
    def assert_eventually(
        self, condition: Callable[[], bool], timeout: float = 5.0, interval: float = 0.1, message: str = None
    ):
    ...
    def assert_metrics_recorded(self, metric_name: str):
        ...
        assert len(metrics.get(metric_name, [])) > 0, f"No samples recorded for metric {metric_name}"
...
class AsyncP2PTestCase(P2PTestCase):
    ...
    def tearDown(self):
        ...
        if self.async_cleanup_callbacks and self.loop and not self.loop.is_closed():
    ...
    async def assert_eventually_async(
        self, condition: Callable[[], Any], timeout: float = 5.0, interval: float = 0.1, message: str = None
    ):
    ...
    def assert_all_connected(self):
        ...
        for name, component in self.components.items():
            if hasattr(component, "is_connected"):
                assert component.is_connected(), f"Component {name} not connected"
    ...
    def assert_performance(self, benchmark_name: str, threshold_key: str):
        ...
        if threshold_key.startswith("max_"):
            ...
            if metric_name in result:
                assert result[metric_name] <= threshold, f"Performance threshold exceeded: {metric_name}"
        elif threshold_key.startswith("min_"):
            metric_name = threshold_key[4:]  # Remove "min_" prefix
            if metric_name in result:
                assert result[metric_name] >= threshold, f"Performance threshold not met: {metric_name}"
