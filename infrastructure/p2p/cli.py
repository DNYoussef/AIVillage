...
try:
    from . import NetworkConfig, P2PNetwork, create_network, discover_peers
    from . import __version__ as p2p_version
except ImportError:
    # Fallback for development
    sys.path.insert(0, str(Path(__file__).parent))
    from __init__ import NetworkConfig, P2PNetwork, create_network, discover_peers
...
class P2PCLI:
    ...
    async def show_status(self, args) -> None:
        ...
        try:
            ...
            if hasattr(self.network, "transport_manager") and self.network.transport_manager:
                ...
                print("     Active Transports: Available")
    ...
    async def run_benchmark(self, args) -> None:
        ...
        results = {"start_time": time.time(), "connections": 0, "messages_sent": 0, "errors": 0}
...
""",
    ...
    start_parser.add_argument("--mode", choices=["hybrid", "mesh", "anonymous", "direct"], help="Network mode")
    ...
    start_parser.add_argument("--transport-priority", type=str, help="Comma-separated transport priority list")
    ...
    discover_parser.add_argument("--timeout", "-t", type=int, default=30, help="Discovery timeout in seconds")

    # Benchmark command
    benchmark_parser = subparsers.add_parser("benchmark", help="Run benchmark")
    benchmark_parser.add_argument("--duration", "-d", type=int, default=60, help="Benchmark duration in seconds")
    benchmark_parser.add_argument("--messages", "-m", type=int, default=100, help="Number of messages to send")

    # Config command
    config_parser = subparsers.add_parser("config", help="Generate sample configuration")
    config_parser.add_argument("--output", "-o", default="p2p-config.yaml", help="Output configuration file")
    config_parser.add_argument("--format", choices=["yaml", "json"], default="yaml", help="Configuration format")
