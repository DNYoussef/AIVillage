...
class CommunicationsProtocol:
    ...
    def __init__(
        ...
        *,
        ...
        self.allow_self_signed = (
            ...
            else os.getenv("COMM_ALLOW_SELF_SIGNED", "false").lower() in {"1", "true", "yes"}
        )

    async def connect(self, target_agent_id: str, target_url: str | None = None) -> bool:
        ...
        try:
            ...
            logger.exception(f"SSL certificate verification failed for {target_agent_id}: {e}")
            raise
    ...
    async def rpc(self, agent_id: str, payload: dict, timeout: float = 5.0) -> dict | None:
    ...
    async def send_rpc_response(self, agent_id: str, correlation_id: str, payload: dict) -> bool:
    ...
    async def start_server(self) -> None:
        ...
        service_directory.register(self.agent_id, f"ws://localhost:{self.port}/ws")
