...
class CreditSystemBenchmark:
    ...
    def setup_test_data(self, num_users: int = 1000) -> List[str]:
        ...
        for i in range(num_users):
            ...
            try:
                self.ledger.create_user(username, f"node_{i}")
    ...
    def benchmark_bulk_balance_queries(self, usernames: List[str], batch_size: int = 100) -> BenchmarkResult:
        ...
        for i in range(0, len(usernames), batch_size):
            ...
            start = time.time()
            try:
                self.ledger.bulk_get_balances(batch)
                elapsed = time.time() - start
    ...
    def benchmark_transaction_queries(self, usernames: List[str], limit: int = 100) -> BenchmarkResult:
        ...
        for username in test_usernames:
            start = time.time()
            try:
                self.ledger.get_transactions(username, limit)
                elapsed = time.time() - start
                times.append(elapsed)
            except Exception as e:
                errors += 1
    ...
    def benchmark_bulk_transfers(self, usernames: List[str], num_transfers: int = 100) -> BenchmarkResult:
        ...
        for i in range(num_transfers):
            ...
            transfers.append({"from_username": usernames[from_idx], "to_username": usernames[to_idx], "amount": 10})

        start_total = time.time()

        try:
            start = time.time()
            self.ledger.bulk_transfer(transfers)
            elapsed = time.time() - start
    ...
    def calculate_performance_improvement(self, before: BenchmarkResult, after: BenchmarkResult) -> Dict[str, float]:
        ...
        time_improvement = (
            (before.avg_time_per_operation - after.avg_time_per_operation) / before.avg_time_per_operation
        ) * 100
        throughput_improvement = (
            (after.operations_per_second - before.operations_per_second) / before.operations_per_second
        ) * 100
    ...
    def print_benchmark_report(self, results: Dict[str, BenchmarkResult]):
        ...
        if "single_balance" in results and "bulk_balance" in results:
            improvement = self.calculate_performance_improvement(results["single_balance"], results["bulk_balance"])
