...
class CreditsLedger:
    ...
    def get_transactions(self, username: str, limit: int = 100) -> list[TransactionResponse]:
        ...
        with self.get_session() as session:
            ...
            all_txs_query = (
                ...
                .filter((Transaction.from_user_id == user.id) | (Transaction.to_user_id == user.id))
    ...
    def get_total_supply(self) -> int:
        ...
        with self.get_session() as session:
            ...
            result = session.execute(text("SELECT SUM(balance) FROM wallets")).scalar()
            return result or 0
    ...
    def bulk_get_balances(self, usernames: List[str]) -> Dict[str, int]:
        ...
        with self.get_session() as session:
            ...
            users = session.query(User).options(joinedload(User.wallet)).filter(User.username.in_(usernames)).all()

            return {user.username: user.wallet.balance if user.wallet else 0 for user in users}
    ...
    def bulk_transfer(self, transfers: List[Dict]) -> List[TransactionResponse]:
        ...
        with self.get_session() as session:
            ...
            for i, tx in enumerate(transactions_to_add):
                responses[i] = responses[i].__class__(**{**responses[i].__dict__, "id": tx.id})
