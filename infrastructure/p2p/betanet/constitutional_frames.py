...
from .htx_transport import HtxFrame, HtxFrameType, MAX_FRAME_SIZE
from ...constitutional.moderation.pipeline import ConstitutionalModerationPipeline, ModerationResult
from ...security.tee.integration import TEESecurityIntegrationManager
...
class ConstitutionalProof:
    ...
    def from_bytes(cls, data: bytes) -> "ConstitutionalProof":
        ...
        proof.enclave_signature = bytes.fromhex(parsed["enclave_signature"]) if parsed.get("enclave_signature") else None
...
class ConstitutionalFrameProcessor:
    ...
    async def initialize(self):
        ...
        try:
            from ...security.tee.integration import get_integration_manager
    ...
    async def _process_verification_request(
        ...
        if is_compliant and proof:
            ...
            response_frame = ConstitutionalFrame(
                frame_type=ConstitutionalFrameType.CONSTITUTIONAL_PROOF,
    ...
    async def _gold_tier_verification(
        ...
        proof = ConstitutionalProof(
            ...
            harm_level=moderation_result.harm_analysis.harm_level if moderation_result.harm_analysis.harm_level == "H3" else "PRIVATE",
            compliance_score=moderation_result.transparency_score,
            constitutional_flags=[] if moderation_result.harm_analysis.harm_level != "H3" else list(moderation_result.harm_analysis.constitutional_concerns.keys()),
            verification_metadata={
        ...
        is_compliant = (moderation_result.harm_analysis.harm_level != "H3") or (moderation_result.decision.value in ["allow", "allow_with_warning"])
    ...
    async def _platinum_tier_verification(
        ...
        if self.tee_integration:
            ...
            moderation_result = await self.moderation_pipeline.process_content(
                content=content,
                content_type="text",
                user_tier="Gold",  # Use Gold as fallback
                context=context
            )
    ...
    async def _process_zk_proof_request(
        ...
        response_frame = ConstitutionalFrame(
            frame_type=ConstitutionalFrameType.ZK_PROOF_RESPONSE,
            stream_id=frame.stream_id,
            payload=proof_data
        )
