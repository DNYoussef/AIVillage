    ...
    def validate(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.field_type and not isinstance(value, self.field_type):
                logger.error(
                    f"Field {self.field_path} has wrong type: expected {self.field_type.__name__}, got {type(value).__name__}"
                )
                return False
...
class ConfigManager:
    ...
    def add_file_source(self, file_path: Union[str, Path], format: Optional[ConfigFormat] = None, priority: int = 0):
    ...
    return {
        "transport": {
            ...
            "libp2p": {
                ...
                "port": 0,  # Auto-assign
                "nat_traversal": True,
                "discovery": {"mdns": True, "dht": True, "bootstrap_peers": []},
            },
            "bitchat": {
                "enabled": True,
            ...
            "websocket": {"enabled": True, "port": 8080, "ssl_enabled": False},
        },
        "security": {
            "encryption": {"default_algorithm": "aes_256_gcm", "key_rotation_interval": 3600},  # 1 hour
            "authentication": {"required": True, "timeout": 30},
        },
        "monitoring": {"enabled": True, "metrics_interval": 60, "prometheus_enabled": False, "log_level": "INFO"},
        "performance": {"max_connections": 100, "message_timeout": 30, "retry_attempts": 3, "connection_pool_size": 10},
    }
...
def get_p2p_validation_rules() -> List[ConfigValidationRule]:
    ...
    return [
        ...
        # Security settings
        ConfigValidationRule(
            "security.encryption.default_algorithm",
            field_type=str,
        ...
        ConfigValidationRule(
            "security.authentication.timeout", field_type=int, min_value=1, max_value=300, default_value=30
        ),
        # Performance settings
        ConfigValidationRule(
            "performance.max_connections", field_type=int, min_value=1, max_value=1000, default_value=100
        ),
        ConfigValidationRule(
            "performance.message_timeout", field_type=int, min_value=1, max_value=300, default_value=30
        ),
        ConfigValidationRule("performance.retry_attempts", field_type=int, min_value=1, max_value=10, default_value=3),
        # Monitoring settings
        ConfigValidationRule(
...
def create_p2p_config_manager(
    component_name: str, config_files: Optional[List[Union[str, Path]]] = None
) -> ConfigManager:
