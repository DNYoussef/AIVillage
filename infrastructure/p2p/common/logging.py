...
class P2PLoggerAdapter(logging.LoggerAdapter):
    ...
    def __init__(
        ...
        extra = {"component": component_name, "peer_id": peer_id, "session_id": session_id}
...
class P2PLogger:
    ...
    def with_correlation(self, correlation_id: str) -> P2PLoggerAdapter:
        ...
        return P2PLoggerAdapter(self.logger, self.component_name, extra=extra)
...
def log_performance(logger: P2PLogger, operation_name: str, **extra_fields):
    ...
    try:
        ...
        logger.info(f"Completed {operation_name}", duration_seconds=duration, **extra_fields)
    except Exception as e:
        duration = time.time() - start_time
        logger.error(
            f"Failed {operation_name}: {e}", duration_seconds=duration, error_type=e.__class__.__name__, **extra_fields
        )
        raise


def setup_logging(root_level: LogLevel = LogLevel.INFO, structured: bool = True, log_dir: Optional[str] = None) -> None:
    ...
    if log_dir:
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            log_path / "p2p.log", maxBytes=50 * 1024 * 1024, backupCount=10  # 50MB
        )
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    duration = time.time() - start_time
                    logger.error(
                        f"Failed {operation_name}: {e}", duration_seconds=duration, error_type=e.__class__.__name__
                    )
                    raise
