...
class AESEncryption(EncryptionManager):
    ...
    def encrypt(self, plaintext: bytes, key: bytes, **kwargs) -> EncryptedData:
        ...
        return EncryptedData(ciphertext=ciphertext, algorithm=self.algorithm, nonce=nonce, tag=encryptor.tag)
...
class ChaCha20Encryption(EncryptionManager):
    ...
    def encrypt(self, plaintext: bytes, key: bytes, **kwargs) -> EncryptedData:
        ...
        return EncryptedData(ciphertext=ciphertext, algorithm=self.algorithm, nonce=nonce)
...
def encrypt_data(
    plaintext: bytes, key: bytes, algorithm: EncryptionAlgorithm = EncryptionAlgorithm.AES_256_GCM
) -> EncryptedData:
...
def generate_keypair(algorithm: EncryptionAlgorithm = EncryptionAlgorithm.AES_256_GCM) -> KeyPair:
    ...
    if algorithm == EncryptionAlgorithm.RSA_OAEP and HAS_CRYPTOGRAPHY:
        ...
        return KeyPair(private_key=private_pem, public_key=public_pem, algorithm=EncryptionAlgorithm.RSA_OAEP)
        ...
        return KeyPair(private_key=key, public_key=key, algorithm=algorithm)  # Same key for symmetric encryption
