    ...
    def deserialize(cls, data: bytes) -> "ECHConfig":
        ...
        try:
            ...
            config = cls(
                ...
                extensions={k: bytes.fromhex(v) for k, v in config_data.get("extensions", {}).items()},
...
class ECHConfigManager:
    ...
    def cleanup_expired(self, max_age_seconds: int = 86400) -> int:
        ...
        expired_ids = [config_id for config_id, config in self._configs.items() if config.is_expired(max_age_seconds)]
...
def create_ech_config(
    public_key: bytes, cipher_suites: Optional[List[ECHCipherSuite]] = None, public_name: str = "p2p.aivillage.internal"
) -> ECHConfig:
    ...
    return ECHConfig(public_key=public_key, cipher_suites=cipher_suites, public_name=public_name)
