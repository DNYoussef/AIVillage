...
class SecurityConfig:
    ...
    def __post_init__(self):
        if self.allowed_ciphers is None:
            self.allowed_ciphers = ["AES256-GCM", "CHACHA20-POLY1305", "AES128-GCM"]
    ...
    def get_safe_host(service_name: str) -> str:
        ...
        if host == "0.0.0.0":  # nosec B104
            ...
            if security_level == "production":
                raise ValueError(
                    f"Binding to {host} not allowed in production. " f"Set {env_var} to specific interface."
                )
...
class SecureSerializer:
    ...
    def is_safe_format(data: bytes) -> bool:
        ...
        return all(not data.startswith(sig) for sig in pickle_signatures)
...
def init_security_config(security_level: SecurityLevel = SecurityLevel.DEVELOPMENT, **kwargs) -> SecurityConfig:
...
def validate_host_binding(host: str) -> str:
    ...
    if host == "0.0.0.0" and not config.allow_all_interfaces:  # nosec B104
        security_level = os.getenv("SECURITY_LEVEL", "development")
        if security_level == "production":
            raise ValueError("Binding to all interfaces is not allowed in production")
