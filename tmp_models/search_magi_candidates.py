    ...
    try:
        ...
        if config.get("model_type") == "qwen2":
            ...
            if hidden_size and num_layers and vocab_size:
                ...
                params = (
                    hidden_size * num_layers * 12 + vocab_size * hidden_size * 2
                ) / 1e9
                return params
            ...
            if hidden_size and num_layers and vocab_size:
                ...
                params = (
                    hidden_size * intermediate_size * num_layers * 3
    ...
    try:
        ...
        for model in models:
            try:
                ...
                if any(
                    size in model_name_lower
                    for size in ["1.5b", "1.6b", "1.7b", "1.8b", "1.3b", "1.4b"]
                ):
                    # Get model info
                    try:
...
def categorize_models(candidates: list[ModelInfo]) -> dict:
    ...
    for model in candidates:
        ...
        # Coding indicators
        if any(
            term in model_name_lower or term in tags_str
            for term in ["code", "coder", "coding", "python"]
        ):
        ...
        # Math indicators
            ...
            term in model_name_lower or term in tags_str
            for term in ["math", "mathematics", "gsm", "reasoning"]
        ):
        ...
        # Tool/instruct indicators
            ...
            term in model_name_lower or term in tags_str
            for term in ["instruct", "function", "tool", "chat"]
        ):
...
def main():
    ...
    for category, models in categorized.items():
        ...
        for model in models[:10]:  # Limit to top 10 per category
            ...
            print(
                f"  {details['repo_id']} - {details['downloads']} downloads - {details['license']}"
            )
