...
class EpisodicDocument(Document):
    ...
    def __init__(self, content: str, doc_type: str, user_id: str | None = None, **kwargs):
        super().__init__(
            id=str(uuid.uuid4()),
            content=content,
            doc_type=doc_type,
...
class HippoIndex(MemoryBackend):
    ...
    def __init__(self,
        ...
        logger.info(f"HippoIndex initialized with db_path={db_path}")
    ...
    async def store_node(self, node: Node, generate_embedding: bool = True) -> bool:
        ...
        try:
            ...
            logger.debug(f"Stored episodic node {node.id}")
            return True
    ...
    async def store_edge(self, edge: Edge) -> bool:
        ...
        try:
            ...
            logger.debug(f"Stored episodic edge {edge.id}")
            return True
    ...
    async def query_nodes(self,
                         ...
                         max_age_hours: int | None = None) -> QueryResult:
...
def create_episodic_document(content: str,
                           ...
                           user_id: str | None = None) -> EpisodicDocument:
