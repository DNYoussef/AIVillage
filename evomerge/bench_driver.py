...
def suite_for(model_dir: Path) -> str:
    ...
    for e in entries:
        ...
        if model_dir.name.startswith(rid):
            ...
            return {"coding": "coding", "math": "math", "logic": "logic"}.get(
                t, "general"
            )
    return "general"
...
def main():
    ...
    out = Path(
        os.environ.get("AIV_BENCHMARKS_DIR", str(aiv_root / "benchmarks" / "results"))
    )
    ...
    for md in sorted([p for p in models.iterdir() if p.is_dir()]):
        ...
        tasks = yaml.safe_load(
            (Path("benchmarks") / "suites" / f"{suite}.yaml").read_text(
                encoding="utf-8"
            )
        )["task_groups"]
