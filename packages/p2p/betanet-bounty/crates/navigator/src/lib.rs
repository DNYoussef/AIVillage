...
// Core modules
// pub mod api;  // Commented out for standalone testing
// pub mod route;  // Commented out for standalone testing
pub mod semiring;
...
// Re-export main types for convenience (commented out for standalone testing)
...
// Re-export key DTN types for integration (commented out for standalone testing)
...
mod integration_tests {
    use super::*;
    // Commented out for standalone testing
    // use betanet_dtn::{ContactGraphRouter, EndpointId, RoutingPolicy};
    // use route::QosRequirements;

    /* Commented out for standalone testing - requires DTN types
    #[tokio::test]
    ...
    */
    ...
    fn test_cost_semiring_properties() {
        ...
        // Test basic semiring properties with realistic values (lat_ms, jitter_ms, failure_prob, privacy_penalty, fee)
        let cost1 = Cost::new(100.0, 10.0, 0.1, 0.2, 15.0); // Good path
        let cost2 = Cost::new(200.0, 8.0, 0.05, 0.1, 12.0); // Different tradeoffs
        let _cost3 = Cost::new(50.0, 20.0, 0.2, 0.3, 25.0); // Fast but expensive

        // Test composition (multiplication)
        let composed = cost1 * cost2;
        assert_eq!(composed.lat_ms.0, 300.0); // Latencies add
        assert_eq!(composed.fee.0, 27.0); // Fees add

        // Test merge (addition) - should keep better cost
        let better = Cost::new(50.0, 5.0, 0.05, 0.1, 10.0);
        let worse = Cost::new(200.0, 15.0, 0.2, 0.3, 30.0);
        ...
        assert!(zero.lat_ms.is_infinite());
        assert_eq!(one.lat_ms.0, 0.0);

        // Composition with one should be identity
        let identity_test = cost1 * one;
        assert!((identity_test.lat_ms - cost1.lat_ms).abs() < 1e-10);
    }

    /* Commented out for standalone testing - requires route types
    #[test]
    fn test_pareto_frontier_capacity() {
        ...
        // Add multiple incomparable paths (lat_ms, jitter_ms, failure_prob, privacy_penalty, fee)
        let paths = vec![
            (100.0, 10.0, 0.1, 0.2, 15.0),   // Balanced
            (50.0, 20.0, 0.2, 0.3, 25.0),    // Fast but expensive/unreliable
            (200.0, 5.0, 0.05, 0.1, 8.0),    // Slow but efficient/reliable
            (150.0, 15.0, 0.15, 0.25, 20.0), // Another point
            (75.0, 12.0, 0.1, 0.15, 18.0),   // Another tradeoff
        ];

        for (lat, jitter, fail_prob, priv_penalty, fee) in paths {
            let cost = Cost::new(lat, jitter, fail_prob, priv_penalty, fee);
