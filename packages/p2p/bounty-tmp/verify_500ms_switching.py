...
class PolicySwitchBenchmark:
    ...
    async def establish_baseline_conditions(self):
        ...
        for i in range(5):
            if hasattr(self.metrics_collector, "record_message_sent"):
                seq_id = self.metrics_collector.record_message_sent(self.test_peer, f"baseline_{i}", 1024)
                # Simulate 60ms RTT
    ...
    async def simulate_condition_change(self, condition_type: str):
        ...
        if condition_type == "high_rtt":
            ...
            for i in range(3):
                if hasattr(self.metrics_collector, "record_message_sent"):
                    seq_id = self.metrics_collector.record_message_sent(self.test_peer, f"high_rtt_{i}", 1024)
            ...
            for i in range(5):
                if hasattr(self.metrics_collector, "record_message_sent"):
                    seq_id = self.metrics_collector.record_message_sent(self.test_peer, f"loss_{i}", 1024)
            ...
            for i in range(4):
                if hasattr(self.metrics_collector, "record_message_sent"):
                    seq_id = self.metrics_collector.record_message_sent(self.test_peer, f"jitter_{i}", 1024)
