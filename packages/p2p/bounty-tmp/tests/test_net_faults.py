...
try:
    from core.p2p.metrics.net_metrics import MeasurementType, NetworkMetricsCollector, NetworkSample
...
class TestNetworkFaultInjection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_control_ping_provides_measurements_under_faults(self):
        ...
        asyncio.create_task(self.metrics_collector.start_control_ping_loop(self.test_peer, self.ping_sender.send_ping))
    ...
    async def test_delivery_thresholds_under_faults(self):
        ...
        for i in range(total_messages):
            ...
            # Record send
            seq_id = self.metrics_collector.record_message_sent(self.test_peer, f"delivery_test_{i}", payload_size=1024)
