...
class TestOnionLayers:
    ...
    def test_layer_isolation(self):
        ...
        hop_keys = [
            (name, crypto.get_public_key_bytes()) for name, crypto in zip(hop_names, crypto_instances, strict=False)
        ]
        ...
        for i, (hop_name, crypto) in enumerate(zip(hop_names, crypto_instances, strict=False)):
            ...
            if i < len(hop_names) - 1:
                ...
                if i < len(hop_names) - 2:
                    ...
                    assert hop_names[i + 2].encode() not in inner, f"Hop {hop_name} should not see beyond next hop"
