...
class AdaptiveNetworkConditions:
    ...
    def needs_path_switch(
        ...
        return (
            (self.measured_rtt_ms is not None and self.measured_rtt_ms > rtt_threshold_ms)
            or (self.measured_loss_rate is not None and self.measured_loss_rate > loss_threshold)
            or (self.quality_score is not None and self.quality_score < quality_threshold)
        )
