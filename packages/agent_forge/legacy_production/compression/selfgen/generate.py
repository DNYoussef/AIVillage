...
class SelfDataGenerator:
    ...
    def generate_dataset(self, num_samples: int, max_new_tokens: int = 256, seed: int = 42) -> list[dict]:
        ...
        for i in range(num_samples):
            ...
            try:
                ...
                if self.apply_quality_filters(instruction, response):
                    ...
                    if (i + 1) % 50 == 0:
                        print(f"[SelfGen] Generated {len(samples)} valid samples ({i + 1}/{num_samples} attempted)")
...
def main():
    parser = argparse.ArgumentParser(description="Generate self-training data for BitNet")
    ...
    parser.add_argument("--max_new_tokens", type=int, default=256, help="Max tokens per response")
