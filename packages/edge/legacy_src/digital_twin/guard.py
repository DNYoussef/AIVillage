...
_SECRET_PATTERN = re.compile(r"(?i)(api[_-]?key|secret|password|private[_-]?key)")


def risk_gate(message: dict[str, Any], risk: float | None = None) -> Literal["allow", "ask", "deny"]:
    ...
    if not allow_shell and (_SHELL_PATTERN.search(content) or _NETWORK_PATTERN.search(content)):
        return "deny"
