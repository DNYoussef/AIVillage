...
class OperationalArtifactCollector:
    ...
    async def collect_ci_artifacts(
        ...
        commit_sha: str, 
        branch: str,
        artifact_sources: Dict[ArtifactType, str]
        ...
        try:
            if file_path.suffix.lower() == ".json":
                ...
                if artifact_type == ArtifactType.COVERAGE:
                    ...
                    metadata["critical_count"] = len([v for v in data.get("vulnerabilities", []) 
    ...
    async def generate_operational_report(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        if security_artifacts:
            ...
            if critical_count > 0:
                report["recommendations"].append(f"Found {critical_count} critical vulnerabilities - immediate attention required")
        ...
        if performance_artifacts:
            ...
            if avg_response_time > 1000:  # 1 second
                report["recommendations"].append("Average response time exceeds 1 second - performance optimization needed")
