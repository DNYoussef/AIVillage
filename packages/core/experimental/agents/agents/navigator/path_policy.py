...
try:
    from src.transport.scion_gateway import GatewayConfig, SCIONConnectionError, SCIONGateway, SCIONPath
...
class LinkChangeDetector:
    ...
    def update_link_state(self, new_state: dict) -> bool:
        ...
        if requires_switch:
            ...
            logger.info(f"Link change detected requiring path switch: " f"{[c['field'] for c in changes_detected]}")
...
class NavigatorAgent:
    ...
    def __init__(
        ...
        logger.info(f"Navigator initialized: {self.agent_id} " f"(priority={routing_priority.value})")
        ...
        if scion_available and "scion" in available:
            ...
            if (
                ...
                if best_scion_path and best_scion_path.rtt_us < 100000 and best_scion_path.loss_rate < 0.05:
                    logger.debug(f"SCION selected for high-performance " f"(RTT={best_scion_path.rtt_us / 1000:.1f}ms)")
    ...
    async def _check_fast_switching(self, destination: str, context: MessageContext) -> bool:
        ...
        if change_detected:
            ...
            if time_since_last_switch > self.path_switch_threshold_ms:
                ...
                logger.info(f"Fast switching triggered for {destination} after " f"{time_since_last_switch:.0f}ms")
                return True
