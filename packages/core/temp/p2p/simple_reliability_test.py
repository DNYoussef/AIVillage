...
async def test_dual_path_creation():
    ...
    try:
        ...
        transport = DualPathTransport(node_id=f"test_node_{port_base}", enable_bitchat=True, enable_betanet=True)
        ...
        return log_result("Transport Creation", success, f"Created transport {transport.node_id}")
...
async def test_mock_communication():
    ...
    try:
        ...
        transport_b = DualPathTransport(node_id=f"node_b_{port_b}", enable_bitchat=True, enable_betanet=False)
        ...
        if start_a and start_b:
            ...
            try:
                await asyncio.wait_for(
                    transport_a.send_message(transport_b.node_id, test_message.payload, priority=5),
                    timeout=2.0,
                )
...
async def run_reliability_suite():
    ...
    message_rate = (
        test_results["message_successes"] / test_results["message_tests"] if test_results["message_tests"] > 0 else 0
    )
    transport_rate = (
    ...
    print(f"  Message Tests: {message_rate:.1%} ({test_results['message_successes']}/{test_results['message_tests']})")
    print(
        f"  Transport Tests: {transport_rate:.1%} ({test_results['transport_successes']}/{test_results['transport_tests']})"
    )

    # Overall reliability score
    overall_score = pass_rate * 0.4 + connection_rate * 0.3 + message_rate * 0.2 + transport_rate * 0.1
    ...
    print(f"Target (>90% + 4 tests pass): {'ACHIEVED' if target_achieved else 'NOT MET'}")
