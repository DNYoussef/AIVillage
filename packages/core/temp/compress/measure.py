...
class CompressionMeasurement:
    ...
    def create_test_models(self) -> dict[str, nn.Module]:
        ...
        models["tiny"] = nn.Sequential(nn.Linear(32, 64), nn.ReLU(), nn.Linear(64, 32), nn.Linear(32, 10))
    ...
    def measure_model_compression(
        ...
        try:
            ...
            if compressor_name in ["SimpleQuantizer", "Unified"]:
                ...
                for param in model.parameters():
                    result = self.measure_tensor_compression(param.data, f"{name}_param", compressor_name, compressor)
    ...
    def run_all_measurements(self):
        ...
        print(f"Testing {len(models)} models and {len(tensors)} tensors with {len(self.compressors)} compressors...")
        ...
        for tensor_name, tensor in tensors.items():
            ...
            for comp_name, compressor in self.compressors.items():
                ...
                try:
                    result = self.measure_tensor_compression(tensor, tensor_name, comp_name, compressor)
        ...
        for model_name, model in models.items():
            ...
            for comp_name, compressor in self.compressors.items():
                print(f"  - {comp_name}... ", end="")
                try:
                    result = self.measure_model_compression(model, model_name, comp_name, compressor)
                    self.results.append(result)
                    if result["success"]:
                        print(f"[OK] {result['ratio']:.2f}x")
                    else:
    ...
    def generate_report(self) -> str:
        ...
        report.append("This report contains actual compression measurements to validate documented claims.\n")

        # Claims validation section
        report.append("## Claims Validation\n")
        report.append("### Documented Claims:\n")
        report.append("- **Mobile Report Claims**: tiny 2.95x, small 3.91x, medium 3.96x, large 3.98x")
        ...
        report.append(f"### Test Results: {len(successful)} successful, {len(failed)} failed\n")

        if successful:
            report.append("## Successful Compression Results\n")
            report.append("| Artifact | Compressor | Before (bytes) | After (bytes) | Ratio |\n")
            report.append("|----------|------------|----------------|---------------|-------|\n")
        ...
        if failed:
            ...
            for result in failed:
                ...
                report.append(f"| {result['artifact']} | {result['compressor']} | {error_msg} |\n")
        ...
        if successful:
            ...
            for comp, ratio in sorted(best_ratios.items(), key=lambda x: x[1], reverse=True):
                report.append(f"- **{comp}**: {ratio:.2f}x\n")

            # Claim validation
            report.append("\n### Claim Validation:\n")

            bitnet_ratios = [r["ratio"] for r in successful if r["compressor"] == "BitNet"]
            if bitnet_ratios:
                avg_bitnet = sum(bitnet_ratios) / len(bitnet_ratios)
                max_bitnet = max(bitnet_ratios)
                report.append(f"- **BitNet**: Average {avg_bitnet:.2f}x, Max {max_bitnet:.2f}x vs claimed ~16x")
                if max_bitnet >= 14:
                    report.append(" [VALIDATED] **CLAIM VALIDATED**\n")
                else:
                    report.append(" [DISPUTED] **CLAIM DISPUTED**\n")

            seedlm_ratios = [r["ratio"] for r in successful if r["compressor"] == "SeedLM"]
            if seedlm_ratios:
                avg_seedlm = sum(seedlm_ratios) / len(seedlm_ratios)
                max_seedlm = max(seedlm_ratios)
                report.append(f"- **SeedLM**: Average {avg_seedlm:.2f}x, Max {max_seedlm:.2f}x vs claimed ~5x")
                if max_seedlm >= 4:
                    report.append(" [VALIDATED] **CLAIM VALIDATED**\n")
                else:
                    report.append(" [DISPUTED] **CLAIM DISPUTED**\n")
            ...
            if vptq_ratios:
                ...
                report.append(f"- **VPTQ**: Average {avg_vptq:.2f}x, Max {max_vptq:.2f}x vs claimed 14-16x")
                if max_vptq >= 12:
                    report.append(" [VALIDATED] **CLAIM VALIDATED**\n")
                else:
                    report.append(" [DISPUTED] **CLAIM DISPUTED**\n")

            simple_ratios = [r["ratio"] for r in successful if r["compressor"] == "SimpleQuantizer"]
            if simple_ratios:
                avg_simple = sum(simple_ratios) / len(simple_ratios)
                max_simple = max(simple_ratios)
                report.append(f"- **SimpleQuantizer**: Average {avg_simple:.2f}x, Max {max_simple:.2f}x vs claimed 4x")
                if avg_simple >= 3.5:
                    report.append(" [VALIDATED] **CLAIM VALIDATED**\n")
                else:
                    report.append(" [DISPUTED] **CLAIM DISPUTED**\n")
        ...
        report.append(f"- Available compressors: {', '.join(self.compressors.keys())}\n")
