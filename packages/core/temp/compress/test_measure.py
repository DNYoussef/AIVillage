...
class TestCompressionAlgorithms:
    ...
    @pytest.mark.skipif(not COMPRESSION_AVAILABLE, reason="Compression modules not available")
    def test_bitnet_compressor(self):
        ...
        compressed_bytes = len(compressed["packed_weights"]) + 4 + 4  # scale + shape info
        assert compressed_bytes < original_bytes, "BitNet should achieve some compression"
...
class TestCompressionMeasurement:
    ...
    def test_model_size_calculation(self):
        ...
        model = nn.Sequential(nn.Linear(10, 20), nn.Linear(20, 5))  # 10*20 + 20 = 220 params  # 20*5 + 5 = 105 params
    ...
    def test_small_model_creation(self):
        ...
        # Tiny model
        tiny = nn.Sequential(nn.Linear(32, 64), nn.ReLU(), nn.Linear(64, 32), nn.Linear(32, 10))

        param_count = sum(p.numel() for p in tiny.parameters())
        assert 3000 < param_count < 15000, f"Tiny model has {param_count} params, expected ~5K"
