...
from test_mobile_policy_simple import PowerMode, SimpleBatteryThermalResourceManager, TransportPreference
...
class TestMobilePolicyWithEnv(unittest.TestCase):
    ...
    async def test_low_ram_profile(self):
        ...
        self.assertEqual(state.transport_preference, TransportPreference.BITCHAT_PREFERRED)
    ...
    async def test_cellular_network_optimization(self):
        ...
        print(f"+ Cellular network: transport={decision['primary_transport']}, " f"policies={state.active_policies}")
    ...
    async def test_memory_constrained_device(self):
        ...
        print(f"+ Memory constrained: chunk_size={tensor_rec['chunk_size']}, " f"batch_size={tensor_rec['batch_size']}")
    ...
    async def test_transport_selection_logic(self):
        ...
        for scenario in scenarios:
            ...
            print(f"+ Transport: B:{scenario['battery']}% {scenario['network']} -> {state.transport_preference.value}")
