    ...
    for query_data in TEST_QUERIES:
        ...
        try:
            ...
            if matches > 0 or (not expected_docs and len(results) >= 0):
                ...
                print(f"[OK] Query successful - {matches}/{len(expected_docs)} expected matches")
            else:
                print(f"[WARNING]  Query had limited results - {matches}/{len(expected_docs)} expected matches")
...
async def test_end_to_end_pipeline(pipeline: RAGPipeline):
    ...
    start_time = time.perf_counter()
    try:
        results, answer = await pipeline.query(query=test_query, top_k=3, synthesize=True, use_cache=True)
...
async def test_cache_functionality(pipeline: RAGPipeline):
    ...
    if second_query_time < first_query_time * 0.8:  # 20% improvement suggests caching
        print(f"[OK] Cache appears to be working (speedup: {(first_query_time / second_query_time):.1f}x)")
        cache_working = True
    else:
...
async def run_smoke_tests():
    ...
    print("\nTest Results:")
    print(f"   Pipeline Instantiation: {'PASS' if test_results['instantiation'] else 'FAIL'}")
