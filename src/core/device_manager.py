...
def get_available_device(preferred_device: str = "auto") -> str:
    ...
    if preferred_device == "auto":
        if _device_info_cache["cuda_available"]:
            return "cuda"
        elif _device_info_cache["mps_available"]:
            return "mps"
        else:
            return "cpu"
...
def configure_device_for_component(component_name: str, config_device: Optional[str] = None) -> str:
    ...
    if actual_device != device and device != "auto":
    ...
    if device.startswith("cuda") and device_info["cuda_available"]:
        ...
        if model_size == "small":  # <1B parameters
            if gpu_memory >= 8:
                ...
                return 4
        elif model_size == "medium":  # 1-7B parameters
            if gpu_memory >= 16:
                return 16
            elif gpu_memory >= 8:
                return 8
