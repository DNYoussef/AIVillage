...
class DeviceProfile:
    ...
    max_evolution_memory_mb: int | None = None
    max_evolution_cpu_percent: float | None = None
...
class DeviceProfiler:
    ...
    def _is_evolution_capable(self, device_type: DeviceType, total_memory: int) -> bool:
        ...
        if (device_type == DeviceType.EMBEDDED and memory_gb < 1) or (
            device_type in [DeviceType.PHONE, DeviceType.TABLET] and memory_gb < 2
        ):
            return False
        if memory_gb < 4:  # Minimum 4GB for other devices
            return False
        ...
        try:
            if platform.system() == "Windows":
                ...
                try:
                    ...
                    logger.debug(f"Failed to parse CPU info line: {e}")
        except Exception as e:
            logger.debug(f"Failed to read CPU info file: {e}")
    ...
    def _get_gpu_info(self) -> tuple[int | None, int | None, float | None]:
        ...
        try:
            ...
            return mem.used, mem.total, float(util.gpu)
    ...
    def _format_alert_message(self, alert_type: str, snapshot: ResourceSnapshot) -> str:
        ...
        if "temperature" in alert_type or "thermal" in alert_type:
    ...
    def get_evolution_resource_allocation(self) -> dict[str, Any]:
        ...
        # Apply constraints
        allocated_memory = min(available_memory_mb, constraints["max_memory_mb"])

        allocated_cpu = min(available_cpu_percent, constraints["max_cpu_percent"])
