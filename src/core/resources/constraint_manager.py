...
from .device_profiler import DeviceProfiler, ResourceSnapshot, DeviceType, PowerState, ThermalState
...
class ConstraintManager:
    ...
    async def start_constraint_monitoring(self):
    ...
    async def stop_constraint_monitoring(self):
    ...
    async def _monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                logger.error(f"Error in constraint monitoring: {e}")
                await asyncio.sleep(self.check_interval)
                
    async def _check_all_constraints(self):
    ...
    async def _check_task_constraints(self, task_id: str, constraints: ResourceConstraints,
        ...
        if (constraints.min_battery_percent is not None and
            ...
            elif (constraints.battery_warning_percent is not None and
                  snapshot.battery_percent < constraints.battery_warning_percent):
                violations.append(ConstraintViolation(
                    constraint_type=ConstraintType.BATTERY,
                    severity=ConstraintSeverity.MEDIUM,
                    current_value=snapshot.battery_percent,
        ...
        if (constraints.max_temperature_celsius is not None and
            ...
            elif (constraints.temperature_warning_celsius is not None and
                  snapshot.cpu_temp > constraints.temperature_warning_celsius):
                violations.append(ConstraintViolation(
                    constraint_type=ConstraintType.THERMAL,
                    severity=ConstraintSeverity.MEDIUM,
                    current_value=snapshot.cpu_temp,
    ...
    async def _handle_violations(self, task_id: str, violations: List[ConstraintViolation]):
    ...
    async def _enforce_action(self, task_id: str, action: str):
    ...
    def _check_resource_availability(self, constraints: ResourceConstraints) -> bool:
        ...
        # Check thermal state
        if current_snapshot.thermal_state in [ThermalState.HOT, ThermalState.CRITICAL, ThermalState.THROTTLING]:
            return False
    ...
    def get_constraint_template(self, evolution_type: str) -> Optional[ResourceConstraints]:
        """Get constraint template for evolution type"""
        return self.constraint_templates.get(evolution_type)
        
    def update_constraints(self, task_id: str, new_constraints: ResourceConstraints):
    ...
    def register_violation_callback(self, callback: Callable[[str, ConstraintViolation], None]):
        """Register violation callback"""
        self.violation_callbacks.append(callback)
        
    def register_enforcement_callback(self, callback: Callable[[str, str], None]):
