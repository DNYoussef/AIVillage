...
class SecureDigitalTwinDB:
    ...
    def create_learning_profile(self, profile_data: dict[str, Any]) -> str:
        ...
        with self.get_connection() as conn:
            ...
            encrypted_data = {}
            sensitive_fields = ["learning_style", "knowledge_domains", "learning_goals", "privacy_settings"]
            ...
            """, (
                profile_id, datetime.utcnow(), ttl_expires_at,
                f"TTL_{self.encryption.profile_ttl_days}_DAYS"
            ))

            # Log compliance audit
            self._log_compliance_audit(
                conn, "create", profile_id, user_id_hash,
                compliance_type="GENERAL", additional_data=json.dumps({
    ...
    def get_learning_profile(self, profile_id: str, decrypt: bool = True) -> dict[str, Any] | None:
        ...
        with self.get_connection() as conn:
                ...
                for encrypted_field in encrypted_fields:
                    if profile[encrypted_field]:
                        ...
                        try:
                            ...
                            logger.error(f"Failed to decrypt {original_field}: {e}")
            ...
            # Log access
            self._log_compliance_audit(
                conn, "read", profile_id, profile["user_id_hash"],
                field_name="profile", compliance_type="GENERAL"
            )
    ...
    def update_learning_profile(self, profile_id: str, updates: dict[str, Any]) -> bool:
        ...
        with self.get_connection() as conn:
            ...
            # Log compliance audit
            self._log_compliance_audit(
                conn, "update", profile_id, existing["user_id_hash"],
                compliance_type="GENERAL", additional_data=json.dumps({
    ...
    def delete_learning_profile(self, profile_id: str, reason: str = "user_request") -> bool:
        ...
        with self.get_connection() as conn:
            ...
            # Log compliance audit
            self._log_compliance_audit(
                conn, "delete", profile_id, user_id_hash,
                compliance_type="GDPR", additional_data=json.dumps({
    ...
    def _log_compliance_audit(self, conn, action: str, profile_id: str, user_id_hash: str,
                            field_name: str = None, compliance_type: str = "GENERAL",
        ...
        """, (
            action, profile_id, user_id_hash, field_name,
            compliance_type, "compliant", os.urandom(8).hex(), additional_data
        ))
    ...
    def export_user_data(self, user_id_hash: str) -> dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            # Log the export
            self._log_compliance_audit(
                conn, "data_exported", None, user_id_hash,
                compliance_type="GDPR", additional_data=json.dumps({
    ...
    def get_compliance_stats(self) -> dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            """)
            audit_stats = {row[0]: {"count": row[1], "last_entry": row[2]}
                          for row in cursor}
