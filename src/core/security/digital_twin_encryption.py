...
class DigitalTwinEncryption:
    ...
    def __init__(self, encryption_key: str | None = None):
        ...
        if not self.encryption_key:
            raise DigitalTwinEncryptionError(
                "DIGITAL_TWIN_ENCRYPTION_KEY environment variable is required"
            )
    ...
    def decrypt_sensitive_field(self, encrypted_data: bytes, field_name: str = "") -> Any:
        ...
        try:
            ...
            raise DigitalTwinEncryptionError(f"Decryption failed for {field_name}: {e}")
    ...
    def _validate_compliance_access(self, metadata: dict[str, Any], field_name: str):
        ...
        if compliance_flags.get("coppa") and not self.coppa_compliant:
            raise ComplianceViolationError(
                f"COPPA compliance required to access {field_name}"
            )

        if compliance_flags.get("ferpa") and not self.ferpa_compliant:
            raise ComplianceViolationError(
                f"FERPA compliance required to access {field_name}"
            )

        if compliance_flags.get("gdpr") and not self.gdpr_compliant:
            raise ComplianceViolationError(
                f"GDPR compliance required to access {field_name}"
            )
