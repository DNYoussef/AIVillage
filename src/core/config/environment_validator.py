...
class EnvironmentValidator:
                    ...
                    if (
                        ...
                        self.report.add_issue(
                            ValidationIssue(
                                ...
                                suggestion="Set permissions to 600: chmod 600 "
                                + str(key_file),
                            )
                        )
                except OSError as e:
    ...
    def _validate_paths(self, env_vars: dict[str, str]) -> None:
        ...
        for var in self.TYPE_RULES["path"]:
            if var in env_vars:
                ...
                if var.endswith("_DIR") or (
                    var.endswith("_PATH") and not var.endswith("_FILE")
                ):
                    # Directory path
    ...
    def generate_report(self) -> str:
        ...
        report_lines.extend(
            ["", "=" * 80, "Generated by AIVillage Environment Validator", "=" * 80]
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Validate AIVillage environment configuration"
    )
    parser.add_argument(
        "--profile",
