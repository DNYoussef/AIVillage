    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            self.transport, self.protocol = await loop.create_datagram_endpoint(
                lambda: UDPProtocol(self.message_queue, self.stats), local_addr=(self.host, self.port)
            )
            self.is_active = True
...
class EnhancedTransportManager:
    ...
    async def send_message(
        self, peer_id: str, message_type: str, payload: dict, preferred_transport: str | None = None
    ) -> bool:
