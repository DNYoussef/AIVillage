...
try:
    from .bitchat_transport import BitChatTransport, BitChatMessage
    from .betanet_transport import BetanetTransport, BetanetMessage
...
try:
    ...
    from navigator.path_policy import NavigatorAgent, PathProtocol, MessageContext, NetworkConditions
...
class DualPathTransport:
    ...
    def __init__(self, 
        ...
        logger.info(f"Available transports: BitChat={self.enable_bitchat}, "
    ...
    async def send_message(self,
                          ...
                          privacy_required: bool = False,
                          deadline: Optional[float] = None,
                          preferred_protocol: Optional[str] = None) -> bool:
    ...
    async def _process_store_forward_queue(self) -> None:
        ...
        for message, queued_time in self.offline_queue:
            ...
            if message.recipient:
                ...
                if (self.enable_bitchat and self.bitchat and 
                    self.bitchat.is_peer_reachable(message.recipient)):
                    can_deliver = True
                    
                elif (self.enable_betanet and self.betanet and
                      message.recipient in self.betanet.discovered_peers):
                    can_deliver = True
        ...
        if delivered > 0 or failed > 0:
            logger.info(f"Store-and-forward: delivered {delivered}, expired {failed}, "
    ...
    def get_reachable_peers(self) -> Dict[str, List[str]]:
        ...
        if self.bitchat:
            bitchat_peers = set(peer.device_id for peer in self.bitchat.discovered_peers.values())
