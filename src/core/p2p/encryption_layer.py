...
class EncryptionLayer:
    ...
    def __init__(self, node_id: str, config: Optional[SecurityConfig] = None):
    ...
    async def encrypt_message(self, message_data: str, recipient_id: Optional[str] = None) -> bytes:
    ...
    async def decrypt_message(self, encrypted_data: bytes, sender_id: Optional[str] = None) -> str:
    ...
    async def _decrypt_broadcast(self, encrypted_data: bytes) -> bytes:
        ...
        try:
            ...
            raise ValueError("Cannot decrypt broadcast message without sender information")
            
        except Exception as e:
            logger.error(f"Failed to decrypt broadcast message: {e}")
            raise
    ...
    async def _create_session_key(self, peer_id: str):
        ...
        try:
            ...
            logger.error(f"Failed to create session key for {peer_id}: {e}")
            raise
            
    async def _rotate_session_key(self, peer_id: str):
        ...
        try:
            ...
            logger.error(f"Failed to rotate session key for {peer_id}: {e}")
            
    async def exchange_public_keys(self, peer_id: str, peer_public_key_data: bytes):
        ...
        try:
            ...
            logger.error(f"Failed to exchange keys with {peer_id}: {e}")
            raise
    ...
    async def _cleanup_loop(self):
        ...
        while True:
            try:
                ...
                expired_sessions = [
                    peer_id for peer_id, timestamp in self.session_created.items()
