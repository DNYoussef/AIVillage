...
def load_gdc_registry(config_path: pathlib.Path | None = None) -> dict[str, GDCSpec]:
    ...
    if not config_path.exists():
        msg = f"GDC rules file not found: {config_path}"
    ...
    try:
        ...
        msg = f"Invalid YAML in GDC rules file: {e}"
        raise ValueError(msg)

    if not isinstance(raw_specs, list):
        msg = "GDC rules file must contain a list of specifications"
        raise ValueError(msg)
    ...
    for spec_data in raw_specs:
        try:
            ...
            if spec.id in registry:
                msg = f"Duplicate GDC ID: {spec.id}"
                raise ValueError(msg)
            ...
            logger.exception(f"Failed to load GDC spec: {e}")
            msg = f"Invalid GDC specification: {e}"
            raise ValueError(msg)
...
try:
    ...
    logger.exception(f"Failed to load GDC registry: {e}")
    GDC_REGISTRY = {}
