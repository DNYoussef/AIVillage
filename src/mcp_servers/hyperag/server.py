...
class HypeRAGMCPServer:
    ...
    async def _handle_client_session(
        ...
        async for message in websocket:
            try:
                ...
                logger.exception(
                    f"Error processing message from {connection_id}: {e!s}"
                )
                await self._send_error(
    ...
    async def get_server_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "components": {
                "permission_manager": (
                    "initialized" if self.permission_manager else "not_initialized"
                ),
                "model_registry": (
                    "initialized" if self.model_registry else "not_initialized"
                ),
                "protocol_handler": (
                    "initialized" if self.protocol_handler else "not_initialized"
                ),
            },
        }
