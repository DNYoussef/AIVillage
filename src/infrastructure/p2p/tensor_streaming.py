...
class TensorChunk:
    ...
    def __init__(self, chunk_id: int, total_chunks: int, data: bytes, 
                 checksum: str, metadata: Optional[Dict] = None):
...
class TensorStreamer:
    ...
    def _serialize_tensor(self, tensor: Any) -> bytes:
        ...
        elif TORCH_AVAILABLE and isinstance(tensor, np.ndarray):
            np_array = tensor
            metadata = {
                "shape": list(np_array.shape),
                "dtype": str(np_array.dtype),
        ...
        return pickle.dumps({"metadata": metadata, "data": np_array})
    ...
    def _deserialize_tensor(self, data: bytes) -> Any:
        ...
        if TORCH_AVAILABLE and metadata.get("is_torch_tensor", False):
            ...
            # Note: device restoration would be handled by caller
            return tensor
        else:
            return np_array
    ...
    def compress_and_chunk_tensor(self, tensor: Any, transfer_id: Optional[str] = None) -> Iterator[TensorChunk]:
        ...
        try:
            ...
            logger.info(f"Compressed tensor: {len(serialized)} -> {len(compressed)} bytes "
    ...
    async def stream_tensor_to_peer(self, p2p_node, peer_id: str, tensor: Any, 
        ...
        try:
            ...
            logger.info(f"Successfully streamed tensor to {peer_id} "
    ...
    async def stream_model_to_peers(self, p2p_node, model_state_dict: Dict[str, Any], 
                                   peer_ids: List[str], parallel: bool = True) -> Dict[str, bool]:
    ...
    async def _stream_model_to_single_peer(self, p2p_node, model_state_dict: Dict[str, Any], 
                                          peer_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully streamed {successful_layers} layers "
...
class TensorReceiver:
    ...
    async def _complete_transfer(self, transfer_id: str):
        ...
        try:
            ...
            logger.info(f"Successfully reconstructed tensor for transfer {transfer_id} "
                       f"in {duration:.2f}s")
    ...
    def get_pending_transfers(self) -> List[str]:
        ...
        return [tid for tid, info in self.pending_transfers.items() 
