...
class P2PNode:
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.error(f"Failed to parse message from {peer_addr}: {e}")
    ...
    async def _handle_ping(self, message: Dict, writer: asyncio.StreamWriter):
        """Handle ping message"""
        response = {
            'type': 'PONG',
    ...
    async def _handle_pong(self, message: Dict, writer: asyncio.StreamWriter):
    ...
    async def _handle_capability_update(self, message: Dict, writer: asyncio.StreamWriter):
    ...
    async def _handle_discovery_request(self, message: Dict, writer: asyncio.StreamWriter):
    ...
    async def _handle_discovery_response(self, message: Dict, writer: asyncio.StreamWriter):
    ...
    async def _handle_tensor_chunk(self, message: Dict, writer: asyncio.StreamWriter):
    ...
    def _get_battery_percent(self) -> Optional[int]:
    ...
    async def _try_discover_peer(self, host: str, port: int):
    ...
    async def _capability_broadcast_loop(self):
        ...
        while self.status == NodeStatus.ACTIVE:
            try:
                ...
                logger.error(f"Capability broadcast error: {e}")
