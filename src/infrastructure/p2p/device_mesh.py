...
class DeviceMesh:
    ...
    def __init__(
        ...
        *,
        ...
        enable_wifi_direct: bool = True,
    ) -> None:
        self.p2p_node = p2p_node

        self.bluetooth_enabled = (
            enable_bluetooth
            and self._check_bluetooth()
            and self._check_bluetooth_dependencies()
        )
        self.wifi_direct_enabled = (
            enable_wifi_direct
            and self._check_wifi_direct()
            and self._check_wifi_direct_dependencies()
        )
    ...
    def _check_bluetooth_dependencies(self) -> bool:
        """Return True if optional Bluetooth dependencies are available."""
        try:  # pragma: no cover - import check
            import bleak  # type: ignore

            self._bleak_client = bleak.BleakClient  # type: ignore[attr-defined]
            return True
        except Exception as e:  # pragma: no cover - logging only
            logger.debug("Bleak not available: %s", e)
            return False

    def _check_wifi_direct_dependencies(self) -> bool:
        """Return True if optional WiFi Direct dependencies are available."""
        try:  # pragma: no cover - import check
            import pywifi  # type: ignore

            self._pywifi_cls = pywifi.PyWiFi  # type: ignore[attr-defined]
            self._wifi_const = pywifi.const  # type: ignore[attr-defined]
            self._wifi_profile_cls = pywifi.Profile  # type: ignore[attr-defined]
            return True
        except Exception as e:  # pragma: no cover - logging only
            logger.debug("pywifi not available: %s", e)
            return False
    ...
    async def _try_bluetooth_connect(self, peer_addr: str) -> str | None:
        """Attempt a Bluetooth connection using bleak."""
        if not self.bluetooth_enabled:
            return None
        ...
        try:
            async with bleak_client_cls(peer_addr) as client:  # type: ignore[call-arg]
                if getattr(client, "is_connected", True):
                    return "bluetooth"
        except Exception as e:  # noqa: BLE001
            logger.debug(
                "Bluetooth connection to %s failed: %s", peer_addr, e
            )
        return None

    async def _try_wifi_direct_connect(self, peer_addr: str) -> str | None:
        """Attempt a WiFi Direct connection using pywifi."""
        if not self.wifi_direct_enabled:
            return None

        pywifi_cls = getattr(self, "_pywifi_cls", None)
        wifi_const = getattr(self, "_wifi_const", None)
        wifi_profile_cls = getattr(self, "_wifi_profile_cls", None)
        if not (pywifi_cls and wifi_const and wifi_profile_cls):
            return None

        try:
            wifi = pywifi_cls()
            iface = wifi.interfaces()[0]
            profile = wifi_profile_cls()
            profile.ssid = peer_addr
            profile.auth = wifi_const.AUTH_ALG_OPEN
            profile.akm.append(wifi_const.AKM_TYPE_NONE)
            profile.cipher = wifi_const.CIPHER_TYPE_NONE
            iface.remove_all_network_profiles()
            tmp_profile = iface.add_network_profile(profile)
            iface.connect(tmp_profile)

            for _ in range(20):  # Wait up to ~10s
                if iface.status() == wifi_const.IFACE_CONNECTED:
                    iface.disconnect()
                    return "wifi_direct"
            ...
            logger.debug(
                "WiFi Direct connection to %s failed: %s", peer_addr, e
            )
        return None
    ...
    def get_mesh_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "stored_messages": sum(
                len(msgs) for msgs in self.store_forward_buffer.values()
            ),
