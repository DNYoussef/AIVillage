    ...
    async def send_message(
        ...
        try:
            ...
            if not target_service:
                ...
                target_url = f"ws://localhost:8000/ws"  # Default fallback
            else:
    ...
    async def send_request_response(
        ...
        try:
            ...
            def response_handler(message: Message):
                if (message.metadata.get("correlation_id") == correlation_id and
                    message.metadata.get("is_response", False)):
            ...
            logger.error(f"Error in request-response to {target_agent_id}: {e}")
            return None
...
async def create_message_system(agent_id: str, port: int = None) -> MessagePassingSystem:
