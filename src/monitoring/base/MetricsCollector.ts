/**
 * Base MetricsCollector - Comprehensive system metrics aggregation and analysis
 * Provides time-series data collection, anomaly detection, and capacity planning
 */
...
export interface MetricsCollectorConfig {
  ...
  capacityPredictionEnabled?: boolean;
}

export class MetricsCollector extends EventEmitter {
  protected metrics = new Map<string, TimeSeriesMetric>();
  protected collectors = new Map<string, () => Promise<number>>();
  protected collectionInterval: number;
  protected intervalId?: NodeJS.Timeout;
  protected retentionPeriod: number;
  protected anomalyDetectors = new Map<string, AnomalyDetector>();
  protected requestMetrics = new Map<string, RequestMetrics>();

  // Configuration flags
  protected anomalyDetectionEnabled: boolean;
  protected capacityPredictionEnabled: boolean;

  constructor(config: MetricsCollectorConfig = {}) {
    super();

    this.collectionInterval = config.collectionInterval || 10000; // 10 seconds
    this.retentionPeriod = config.retentionPeriod || 7 * 24 * 60 * 60 * 1000; // 7 days default
    this.anomalyDetectionEnabled = config.anomalyDetectionEnabled !== false;
    this.capacityPredictionEnabled = config.capacityPredictionEnabled !== false;
   ...
   */
  public completeRequest(requestId: string, error?: Error): RequestMetrics | null {
    const request = this.requestMetrics.get(requestId);
    if (!request) {
      return null;
    }
   ...
   */
  public updateMetadata(requestId: string, metadata: Record<string, any>): void {
    const request = this.requestMetrics.get(requestId);
    if (request) {
   ...
   */
  public recordQueueMetrics(queueMetrics: QueueMetricsData): void {
    // Record individual queue metrics
    this.recordMetric(`queue.${queueMetrics.queueName}.depth`, queueMetrics.depth);
    this.recordMetric(`queue.${queueMetrics.queueName}.wait_time`, queueMetrics.waitTime);
    this.recordMetric(`queue.${queueMetrics.queueName}.processing_time`, queueMetrics.processingTime);
    this.recordMetric(`queue.${queueMetrics.queueName}.throughput`, queueMetrics.throughput);

    // Emit queue metrics event
    this.emit('queueMetrics', queueMetrics);
  }
  ...
  public getRequestTimingStats(): {
    ...
    const errors = completedRequests.filter(r => r.error !== undefined);
    ...
    return {
      ...
      errorRate: completedRequests.length > 0 ? (errors.length / completedRequests.length) * 100 : 0
    };
  }
  ...
  public getCapacityPredictions(): CapacityPrediction[] {
    if (!this.capacityPredictionEnabled) {
      return [];
    }
   ...
   */
  public exportMetrics(format: 'prometheus' | 'json' | 'csv'): string {
    switch (format) {
      case 'prometheus':
  ...
  protected async collectAllMetrics(): Promise<void> {
  ...
  protected initializeSystemMetrics(): void {
    // CPU metrics
    this.registerMetric(
      'system.cpu.usage',
      'gauge',
    ...
    this.registerMetric(
      ...
      'count',
      async () => {
        return this.getRequestTimingStats().totalRequests;
      }
    );
  ...
  protected async collectSystemMetrics(): Promise<MetricsSnapshot['systemMetrics']> {
  ...
  protected async collectApplicationMetrics(): Promise<MetricsSnapshot['applicationMetrics']> {
    const requestStats = this.getRequestTimingStats();

    return {
      requests: {
        total: requestStats.totalRequests,
        rate: requestStats.totalRequests / (this.collectionInterval / 1000),
  ...
  protected calculateAggregations(
    name: string,
    points: MetricPoint[],
    startTime: Date,
    endTime: Date
  ...
  protected detectAnomalies(points: MetricPoint[]): AnomalyPoint[] {
  ...
  protected predictCapacity(metric: TimeSeriesMetric): CapacityPrediction | null {
  ...
  protected calculateGrowthRate(values: number[]): number {
  ...
  protected generateCapacityRecommendations(
    metricName: string,
  ...
  protected cleanupOldPoints(metric: TimeSeriesMetric): void {
  ...
  protected exportPrometheusFormat(): string {
    let output = '';
  ...
  protected exportJsonFormat(): string {
    const data = Object.fromEntries(this.metrics);
    return JSON.stringify(data, null, 2);
  }

  protected exportCsvFormat(): string {
