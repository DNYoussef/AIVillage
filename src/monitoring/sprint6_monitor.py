...
class Sprint6Monitor:
        ...
        try:
            ...
            if alerts_file.exists():
                with open(alerts_file) as f:
                    data = json.load(f)
                    self.active_alerts = [
                        AlertInfo(**item) for item in data if not item.get("resolved", False)
                    ]
            
            logger.info(f"Loaded monitoring data: {len(self.health_history)} health records, {len(self.performance_history)} performance records, {len(self.active_alerts)} active alerts")
    ...
    async def _check_evolution_system_health(self) -> str:
        """Check evolution system health."""
        try:
            from src.production.agent_forge.evolution.infrastructure_aware_evolution import InfrastructureAwareEvolution, InfrastructureConfig
    ...
    def check_alerts(self, health: InfrastructureHealth, metrics: PerformanceMetrics):
        ...
        if metrics.avg_p2p_latency_ms > self.thresholds["p2p_latency_critical"]:
            current_alerts.append(self._create_alert(
                ...
                {"latency_ms": metrics.avg_p2p_latency_ms, "threshold": self.thresholds["p2p_latency_critical"]}
            ))
            ...
            current_alerts.append(self._create_alert(
                ...
                {"latency_ms": metrics.avg_p2p_latency_ms, "threshold": self.thresholds["p2p_latency_warning"]}
            ))
        ...
        if health.resource_utilization > self.thresholds["resource_utilization_critical"]:
            current_alerts.append(self._create_alert(
                ...
                {"utilization": health.resource_utilization, "threshold": self.thresholds["resource_utilization_critical"]}
            ))
            ...
            current_alerts.append(self._create_alert(
                ...
                {"utilization": health.resource_utilization, "threshold": self.thresholds["resource_utilization_warning"]}
            ))
        ...
        if metrics.memory_pressure > self.thresholds["memory_pressure_critical"]:
            current_alerts.append(self._create_alert(
                ...
                {"memory_pressure": metrics.memory_pressure, "threshold": self.thresholds["memory_pressure_critical"]}
            ))
            ...
            try:
                ...
                if age_seconds > self.thresholds["validation_age_critical"]:
                    current_alerts.append(self._create_alert(
                        ...
                        {"age_hours": age_seconds/3600, "threshold_hours": self.thresholds["validation_age_critical"]/3600}
                    ))
    ...
    def _create_alert(self, alert_id: str, severity: str, component: str, message: str, details: Dict[str, Any]) -> AlertInfo:
        """Create a new alert."""
        return AlertInfo(
            alert_id=alert_id,
            timestamp=datetime.now().isoformat(),
    ...
    def _update_alerts(self, current_alerts: List[AlertInfo]):
    ...
    async def run_sprint6_validation(self) -> bool:
        ...
        try:
            ...
            logger.error("Sprint 6 validation timed out")
            return False
        except Exception as e:
            logger.error(f"Sprint 6 validation failed: {e}")
            return False
    ...
    async def monitoring_loop(self):
        ...
        while self.is_monitoring:
            try:
                ...
                # Log current status
                logger.info(f"Health: {health.overall_health}, CPU: {metrics.system_load:.1f}%, Memory: {metrics.memory_pressure:.1f}%, Active alerts: {len([a for a in self.active_alerts if not a.resolved])}")
