...
from .openrouter_client import APIResponse, OpenRouterClient
...
class TaskRouter:
    ...
    def classify_task(
        ...
        if any(keyword in prompt_lower for keyword in generation_keywords):
            ...
            if context and context.domain == "mathematical_proofs":
        ...
        if any(keyword in prompt_lower for keyword in variation_keywords):
    ...
    async def generate_problem_with_variations(
        self, domain: str, difficulty: int, num_variations: int = 3
    ) -> dict[str, Any]:
