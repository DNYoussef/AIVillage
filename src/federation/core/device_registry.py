...
class DeviceProfile:
    ...
    def is_suitable_for_role(self, role: DeviceRole) -> bool:
        ...
        if role == DeviceRole.BEACON:
            return (
                DeviceCapability.ALWAYS_ON in self.capabilities and
                self.uptime_hours > 24 * 7 and  # Minimum 1 week uptime
                self.calculate_device_score() > 0.8
            )
        
        elif role == DeviceRole.WORKER:
            return (
                DeviceCapability.HIGH_COMPUTE in self.capabilities and
                self.cpu_cores >= 2
            )
        
        elif role == DeviceRole.RELAY:
            return (
                DeviceCapability.BANDWIDTH_HIGH in self.capabilities and
                len(self.protocols) >= 2  # Support multiple protocols
            )
        
        elif role == DeviceRole.STORAGE:
            return (
                DeviceCapability.STORAGE_LARGE in self.capabilities and
                self.storage_gb >= 100
            )
...
class DeviceRegistry:
    ...
    async def initialize_local_device(self, 
                                    capabilities: Set[DeviceCapability],
                                    region: str = "unknown") -> DeviceProfile:
        ...
        logger.info(f"Local device initialized: role={optimal_role.value}, "
    ...
    def get_devices_by_region(self, region: str) -> List[DeviceProfile]:
        ...
        return [
            profile for profile in self.devices.values() 
    ...
    def find_best_beacon(self, region: str = None) -> Optional[DeviceProfile]:
        ...
        beacons.sort(key=lambda x: (x.calculate_device_score(), x.uptime_hours), 
                    reverse=True)
    ...
    def get_federation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'regional_distribution': {
                region: len(devices) 
                for region, devices in self.region_devices.items()
            },
            'active_beacons': len([b for b, t in self.known_beacons.items() 
    ...
    def _determine_optimal_role(self, 
                               capabilities: Set[DeviceCapability],
                               resources: Dict[str, Any]) -> DeviceRole:
        """Determine optimal role for device based on capabilities"""
        
        # Check beacon suitability (highest priority)
        if (DeviceCapability.ALWAYS_ON in capabilities and
            resources.get('memory_gb', 0) >= 4 and
            resources.get('bandwidth_mbps', 0) >= 10):
            return DeviceRole.BEACON
        
        # Check storage node suitability
        if (DeviceCapability.STORAGE_LARGE in capabilities and
            resources.get('storage_gb', 0) >= 100):
            return DeviceRole.STORAGE
        
        # Check worker node suitability  
        if (DeviceCapability.HIGH_COMPUTE in capabilities and
            resources.get('cpu_cores', 1) >= 2):
            return DeviceRole.WORKER
        
        # Check relay node suitability
        if (DeviceCapability.BANDWIDTH_HIGH in capabilities and
            resources.get('bandwidth_mbps', 0) >= 50):
