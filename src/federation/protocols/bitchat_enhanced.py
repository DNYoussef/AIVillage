...
from core.p2p.bitchat_transport import BitChatTransport, BitChatMessage, BitChatPeer
    ...
    def compress_payload(self) -> bool:
        ...
        try:
            ...
            if len(compressed) < len(self.payload):
                ...
                logger.debug(f"Compressed message {self.id[:8]}: {original_size} -> {len(compressed)} bytes ({compression_ratio:.1f}%)")
                return True
...
class EnhancedBitChatTransport(BitChatTransport):
    ...
    async def send_enhanced_message(self,
                                  recipient: str,
                                  payload: bytes,
                                  channel: str = "",
                                  encrypt: bool = True,
