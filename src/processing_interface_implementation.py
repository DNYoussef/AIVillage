...
from typing import Any, Dict, List, Optional, Set, TypeVar, Generic, Callable, Union
    ...
    async def process(self, input_data: T, context: Optional[ProcessingContext] = None, **kwargs) -> ProcessResult[U]:
        ...
        if context is None:
            context = ProcessingContext(
                request_id=f"{self.processor_id}_{int(time.time() * 1000)}",
    ...
    async def _process_with_monitoring(
        ...
        **kwargs
        ...
        try:
            ...
            self.logger.info(
                f"Processing completed successfully for {context.request_id} in {processing_time:.3f}s"
            )
            ...
            self.logger.error(
                f"Processing failed for {context.request_id}: {e}",
                exc_info=True
            )
