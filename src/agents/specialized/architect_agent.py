...
class ArchitectAgent(AgentInterface):
    ...
    async def rerank(self, query: str, results: list[dict[str, Any]], k: int) -> list[dict[str, Any]]:
        keywords = ['architecture', 'design', 'system', 'microservices', 'database', 'api', 'scalability']
        for result in results:
            score = sum(str(result.get('content', '')).lower().count(kw) for kw in keywords)
            result['architecture_relevance_score'] = score
        return sorted(results, key=lambda x: x.get('architecture_relevance_score', 0), reverse=True)[:k]
    ...
    def _get_components_for_style(self, style: str) -> List[Dict[str, Any]]:
        ...
        if style == 'microservices':
            ...
            return [
                {'name': 'Application Layer', 'purpose': 'Business logic orchestration'},
