...
class DevOpsAgent(AgentInterface):
    ...
    async def rerank(self, query: str, results: list[dict[str, Any]], k: int) -> list[dict[str, Any]]:
        ...
        keywords = ['deploy', 'pipeline', 'infrastructure', 'docker', 'kubernetes', 'monitoring', 'ci/cd']
    ...
    async def monitor_infrastructure(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'services': {
                'api_gateway': {'status': 'healthy', 'instances': 3, 'cpu': 45, 'memory': 60},
                'user_service': {'status': 'healthy', 'instances': 5, 'cpu': 30, 'memory': 55},
                'data_service': {'status': 'warning', 'instances': 2, 'cpu': 85, 'memory': 78}
            },
            'infrastructure': {
                'kubernetes_nodes': 8,
                'pods_running': 45,
