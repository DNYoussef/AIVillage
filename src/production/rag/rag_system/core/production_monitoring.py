...
class ProductionMonitor:
    ...
    def __init__(
        ...
        pipeline,
        cache,
        metrics_dir: Path = Path("/tmp/rag_metrics")
    ):
    ...
    async def _check_latency_target(self) -> bool:
        ...
        recent_latencies = [
            p.value for p in list(self.performance_history)[-100:]
    ...
    async def graceful_degradation(
        ...
        try:
            results = await self.handle_with_circuit_breaker(
                "index",
                lambda: self.pipeline.retrieve(query, k=k, use_cache=False)
            )
    ...
    async def _monitoring_loop(self) -> None:
        ...
        while True:
            try:
                ...
                recent_requests = [
                    p for p in list(self.performance_history)[-60:]
                ...
                if self.health_status in [HealthStatus.DEGRADED, HealthStatus.UNHEALTHY]:
        ...
        try:
            ...
            metrics = {
                ...
                "performance": {
                    ...
                    "cache_hit_rate": self.cache_hit_rate._value.get() if hasattr(self.cache_hit_rate, '_value') else 0
                },
                "circuit_breakers": {
                    name: {
