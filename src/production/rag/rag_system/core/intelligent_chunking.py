...
class IntelligentChunker:
    ...
    def detect_idea_boundaries(
        ...
        for boundary in boundaries:
            ...
            elif boundary.confidence > max(b.confidence for b in filtered_boundaries 
                                         if abs(b.sentence_idx - boundary.sentence_idx) < consecutive_windows):
                # Replace lower confidence boundary
                filtered_boundaries = [
                    b for b in filtered_boundaries 
                    if abs(b.sentence_idx - boundary.sentence_idx) >= consecutive_windows
                ]
                filtered_boundaries.append(boundary)
    ...
    def handle_edge_cases(self, sentences: List[str], boundaries: List[int]) -> List[int]:
        ...
        for boundary in boundaries:
            ...
            if boundary < len(sentences):
                ...
                if self.detect_content_type(boundary_text) in [ContentType.CODE, ContentType.FORMULA]:
                    continue
            ...
            if boundary > 0 and boundary < len(sentences):
                ...
                if (self.detect_content_type(prev_text) == ContentType.LIST and
                    self.detect_content_type(curr_text) == ContentType.LIST):
                    continue
