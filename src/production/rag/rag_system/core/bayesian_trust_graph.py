...
class BayesianTrustGraph:
    ...
    def add_semantic_chunk(
        ...
        try:
            ...
            logger.error(f"Failed to add semantic chunk {chunk_id}: {e}")
            raise
    ...
    def detect_semantic_relationships(
        ...
        try:
            ...
            for other_chunk in existing_chunks:
                ...
                relationship_type, confidence, bidirectional = self._classify_relationship(
                    chunk_node, other_chunk, similarity
                )
            ...
            logger.error(f"Failed to detect relationships for {chunk_node.chunk_id}: {e}")
            return []

    def _classify_relationship(
        self,
        chunk1: SemanticChunkNode,
        chunk2: SemanticChunkNode,
        similarity: float
        ...
        if (chunk1.document_id == chunk2.document_id and
            self._is_hierarchical_elaboration(chunk1, chunk2)):
    ...
    def _has_contrasting_markers(self, text1: str, text2: str) -> bool:
        ...
        contrast_markers = [
            ...
            "in contrast", "conversely", "alternatively", "whereas",
            "unlike", "different from", "rather than"
        ]
    ...
    def _has_supporting_markers(self, text1: str, text2: str) -> bool:
        ...
        support_markers = [
            ...
            "evidence suggests", "studies show", "research indicates"
        ]
    ...
    def _has_definition_markers(self, text1: str, text2: str) -> bool:
        ...
        definition_markers = [
            "is defined as", "refers to", "means", "is the process of",
            "can be understood as", "is characterized by", "denotes",
    ...
    def _has_example_markers(self, text1: str, text2: str) -> bool:
        ...
        example_markers = [
            ...
            "specifically", "namely", "in particular", "like",
            "consider the case", "take the example"
        ]
    ...
    def propagate_trust(
        ...
        try:
            ...
            self.stats["avg_trust_propagation_time"] = (
                (self.stats["avg_trust_propagation_time"] * (self.stats["trust_propagations"] - 1) +
                 propagation_time) / self.stats["trust_propagations"]
            )
        ...
        try:
            ...
            self.stats["avg_retrieval_time"] = (
                (self.stats["avg_retrieval_time"] * (self.stats["graph_retrievals"] - 1) +
                 retrieval_time) / self.stats["graph_retrievals"]
            )
    ...
    def _expand_through_graph_traversal(
        ...
        for chunk_id, initial_score, _ in initial_results[:5]:  # Limit expansion seeds
            self._traverse_from_chunk(
                chunk_id, traversal_depth, expanded_chunks, result_map,
                query_embedding, initial_score
            )
        ...
        for neighbor_id in self.graph.neighbors(chunk_id):
            ...
            if neighbor_node.embedding is not None:
                ...
                if neighbor_score > 0.3:
                    ...
                    # Continue traversal
                    self._traverse_from_chunk(
                        neighbor_id, remaining_depth - 1, expanded_chunks,
                        result_map, query_embedding, neighbor_score
                    )
        ...
        try:
            ...
            return {
                ...
                "quality_metrics": {
                    ...
                    ]) if self.chunk_nodes else 0.0,
                    
                    "avg_coherence_score": np.mean([
