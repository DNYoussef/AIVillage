...
from contextual_tagging import (
    ContentDomain, ReadingLevel, DocumentType, ChunkType,
    DocumentContext, ChunkContext
)
from bayesian_trust_graph import RelationshipType
from codex_rag_integration import Document, Chunk, RetrievalResult
...
class EnhancedQueryProcessor:
    ...
    async def process_query(
        ...
        custom_strategy: Optional[RetrievalStrategy] = None
    ) -> SynthesizedAnswer:
        """
        ...
        try:
            ...
            logger.debug(f"Intent: {decomposition.primary_intent.value}, "
            ...
            return SynthesizedAnswer(
                answer_text=f"I encountered an error processing your query: {str(e)}",
    ...
    def _build_intent_patterns(self) -> Dict[QueryIntent, List[str]]:
        ...
        return {
            QueryIntent.FACTUAL: [
                ...
                r"\bdefine\b", r"\bdefinition\b", r"\bmeans\b", r"\brefers to\b"
            ],
            QueryIntent.EXPLANATORY: [
                r"\bhow does\b", r"\bhow to\b", r"\bwhy does\b", r"\bexplain\b",
                r"\bdescribe\b", r"\bprocess\b", r"\bmechanism\b", r"\bwork\b"
            ],
            ...
            QueryIntent.ANALYTICAL: [
                ...
                r"\bimpact\b", r"\beffect\b", r"\bimplications\b", r"\bconsequences\b"
            ],
            QueryIntent.PROCEDURAL: [
                r"\bsteps\b", r"\bprocedure\b", r"\bmethod\b", r"\bguide\b",
                r"\binstructions\b", r"\bhow to\b", r"\bprocess\b"
            ],
            QueryIntent.TEMPORAL: [
                r"\bwhen\b", r"\btimeline\b", r"\bhistory\b", r"\bevolution\b",
                r"\bchronology\b", r"\bover time\b", r"\btrend\b"
            ],
            QueryIntent.CAUSAL: [
                r"\bcause\b", r"\breason\b", r"\bwhy\b", r"\blead to\b",
                r"\bresult\b", r"\bdue to\b", r"\bbecause\b", r"\btrigger\b"
            ],
            QueryIntent.HYPOTHETICAL: [
                r"\bwhat if\b", r"\bsuppose\b", r"\bhypothetical\b", r"\bscenario\b",
                r"\bimagine\b", r"\bassume\b", r"\bwould happen\b"
            ]
        }
    ...
    def _build_context_patterns(self) -> Dict[ContextLevel, List[str]]:
        ...
        return {
            ContextLevel.OVERVIEW: [
                r"\boverview\b", r"\bsummary\b", r"\bbrief\b", r"\bgeneral\b",
                r"\bbasics\b", r"\bintroduction\b", r"\bquick\b"
            ],
            ContextLevel.DETAILED: [
                r"\bdetailed\b", r"\bspecific\b", r"\bparticular\b", r"\bexact\b",
                r"\bprecise\b", r"\bthorough\b", r"\bin depth\b"
            ],
            ContextLevel.COMPREHENSIVE: [
                r"\bcomprehensive\b", r"\bcomplete\b", r"\bexhaustive\b", r"\ball\b",
                r"\bentire\b", r"\bfull\b", r"\beverything\b"
            ],
            ContextLevel.SPECIFIC: [
                r"\bonly\b", r"\bjust\b", r"\bspecifically\b", r"\bparticular\b",
                r"\bfocus on\b", r"\blimited to\b", r"\bnarrow\b"
            ]
        }
    ...
    def _build_complexity_indicators(self) -> List[str]:
        ...
        return [
            r"\band\b", r"\bor\b", r"\bbut\b", r"\bhowever\b", r"\balthough\b",
            r"\bmultiple\b", r"\bseveral\b", r"\bvarious\b", r"\bcomplex\b",
            r"\badvanced\b", r"\bsophisticated\b", r"\bintricate\b"
        ]
    ...
    def _build_temporal_patterns(self) -> Dict[TemporalRequirement, List[str]]:
        ...
        return {
            TemporalRequirement.HISTORICAL: [
                r"\bhistory\b", r"\bhistorical\b", r"\bpast\b", r"\boriginal\b",
                r"\bearlier\b", r"\bprevious\b", r"\btraditional\b", r"\bwas\b"
            ],
            TemporalRequirement.CURRENT: [
                r"\bcurrent\b", r"\bnow\b", r"\btoday\b", r"\bpresent\b",
                r"\brecent\b", r"\blatest\b", r"\bmodern\b", r"\bcontemporary\b"
            ],
            TemporalRequirement.PREDICTIVE: [
                r"\bfuture\b", r"\bpredict\b", r"\bforecast\b", r"\btrend\b",
                r"\bexpected\b", r"\bwill\b", r"\bprojected\b", r"\bnext\b"
            ],
            TemporalRequirement.TIMELESS: [
                r"\balways\b", r"\bnever\b", r"\buniversal\b", r"\bgeneral\b",
                r"\bprinciple\b", r"\blaw\b", r"\brule\b", r"\bconstant\b"
            ]
        }
    ...
    def _assess_complexity(self, query: str, key_concepts: List[str]) -> ComplexityLevel:
        ...
        elif complexity_score >= 2 or word_count > 15 or concept_count > 3:
            return ComplexityLevel.COMPLEX
        elif complexity_score >= 1 or word_count > 10 or concept_count > 2:
    ...
    def _extract_relationships(self, query: str) -> List[str]:
        ...
        relationship_patterns = [
            r"caused by", r"leads to", r"results in", r"related to",
            r"associated with", r"similar to", r"different from"
        ]
    ...
    def _determine_target_domains(self, query: str, context: Dict[str, Any]) -> List[ContentDomain]:
        ...
        domain_keywords = {
            ContentDomain.SCIENCE: ["science", "research", "study", "experiment", "theory"],
            ContentDomain.TECHNOLOGY: ["technology", "computer", "software", "digital", "AI"],
    ...
    def _requires_multi_hop_reasoning(self, query: str) -> bool:
        ...
        multi_hop_indicators = [
            r"relationship between", r"connection", r"impact of.*on",
            r"how.*affect", r"chain", r"sequence", r"leads to"
        ]
    ...
    def _needs_synthesis(self, query: str, complexity: ComplexityLevel) -> bool:
        ...
        synthesis_indicators = [
            r"compare", r"analyze", r"evaluate", r"explain",
            r"describe", r"summarize", r"overview"
        ]
        ...
        return has_synthesis_terms or complexity in [ComplexityLevel.COMPLEX, ComplexityLevel.EXPERT]
    ...
    def _create_retrieval_strategy(self, decomposition: QueryDecomposition) -> RetrievalStrategy:
        ...
        if decomposition.complexity_level in [ComplexityLevel.COMPLEX, ComplexityLevel.EXPERT]:
            strategy.enable_graph_traversal = True
    ...
    def _is_relationship_relevant(
        ...
        relevance_map = {
            QueryIntent.EXPLANATORY: [
                RelationshipType.ELABORATES, RelationshipType.DEFINES,
                RelationshipType.EXEMPLIFIES
            ],
            QueryIntent.COMPARATIVE: [
                RelationshipType.CONTRASTS, RelationshipType.SUPPORTS
            ],
            QueryIntent.CAUSAL: [
                RelationshipType.SUPPORTS, RelationshipType.CONTINUES
            ],
            QueryIntent.ANALYTICAL: [
                RelationshipType.REFERENCES, RelationshipType.CONTEXTUALIZES
            ]
        }
    ...
    def _calculate_complexity_bonus(
        ...
        if decomposition.complexity_level == ComplexityLevel.EXPERT:
            ...
            if result.document_context and result.document_context.reading_level.value == "high_school":
        ...
        for result in results:
            if result.idea_boundary_complete:
                boundary = {
                    ...
                    "confidence": 0.9,
                    "method": "intelligent_chunking" if result.result.metadata and 
                            result.result.metadata.get("chunking_method") == "intelligent" 
    ...
    def _update_processing_stats(
        ...
        self.processing_stats["avg_decomposition_time"] = (
            (self.processing_stats["avg_decomposition_time"] * (count - 1) + decomposition_time) / count
        )
        
        self.processing_stats["avg_retrieval_time"] = (
            (self.processing_stats["avg_retrieval_time"] * (count - 1) + retrieval_time) / count
        )
        
        self.processing_stats["avg_synthesis_time"] = (
            (self.processing_stats["avg_synthesis_time"] * (count - 1) + synthesis_time) / count
        )
