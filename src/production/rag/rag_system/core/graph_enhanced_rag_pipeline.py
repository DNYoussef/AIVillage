...
from bayesian_trust_graph import BayesianTrustGraph, RelationshipType
from codex_rag_integration import Document, Chunk, RetrievalResult
from contextual_tagging import ContentDomain, ReadingLevel, DocumentType, ChunkType
...
class GraphEnhancedRAGPipeline(ContextualRAGPipeline):
    ...
    def __init__(
        ...
        similarity_thresholds: Optional[Dict] = None,
    ...
    async def retrieve_with_graph_enhanced_analysis(
        ...
        domain_filter: Optional[ContentDomain] = None,
        reading_level_filter: Optional[ReadingLevel] = None,
        ...
        if not self.enable_trust_graph or not enable_graph_traversal:
            return await self.retrieve_with_contextual_analysis(
                query=query, k=k, use_cache=use_cache,
        ...
        try:
            ...
            if any([domain_filter, reading_level_filter, document_type_filter, 
                   chunk_type_filter, min_credibility > 0, min_quality > 0, require_entities]):
            ...
            self.graph_stats["avg_graph_retrieval_time"] = (
                (self.graph_stats["avg_graph_retrieval_time"] * (self.graph_stats["graph_retrievals"] - 1) +
                 retrieval_time) / self.graph_stats["graph_retrievals"]
            )
    ...
    def analyze_graph_relationships(self, chunk_id: str) -> Dict[str, Any]:
        ...
        for (source_id, target_id), relationship in self.trust_graph.relationships.items():
        ...
        return {
            ...
            "graph_analysis": {
                ...
                "relationship_types": {
                    "outgoing_types": list(set(rel["relationship_type"] for rel in outgoing_relationships)),
                    "incoming_types": list(set(rel["relationship_type"] for rel in incoming_relationships))
                }
            }
        }
    ...
    def get_trust_propagation_paths(
        ...
        try:
            ...
            logger.error(f"Failed to find trust paths: {e}")
            return []


# Test function
async def test_graph_enhanced_rag_pipeline():
    ...
    if graph_stats and "graph_structure" in graph_stats:
        ...
        print(f"  - Trust Score Range: {trust_metrics.get('min_trust_score', 0):.3f} - {trust_metrics.get('max_trust_score', 0):.3f}")
    ...
    if (graph_enabled and chunks_in_graph and relationships_detected and 
        trust_propagation_working and avg_latency_good and context_richness_good):
