...
class EnhancedCODEXRAGPipeline(CODEXRAGPipeline):
    ...
    def chunk_document_intelligently(self, document: Document, force_traditional: bool = False) -> list[Chunk]:
        ...
        try:
            ...
            intelligent_chunks = self.intelligent_chunker.chunk_document(
                text=document.content, document_id=document.id, doc_type=doc_type
            )
    ...
    def get_enhanced_performance_metrics(self) -> dict[str, Any]:
        ...
        return {**base_metrics, **chunking_metrics}
...
async def test_enhanced_rag() -> bool:
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        results, metrics = await pipeline.retrieve_with_content_analysis(
            query=query, k=3, include_entities=True, min_coherence=0.0
        )
