...
class MathFitnessEvaluator:
    ...
    async def evaluate(
        ...
        avg_correctness = (
            statistics.mean([eval.correctness_score for eval in all_evaluations])
            if all_evaluations
            else 0.0
        )
        avg_step = (
            statistics.mean([eval.step_by_step_score for eval in all_evaluations])
            if all_evaluations
            else 0.0
        )
        avg_explanation = (
            statistics.mean([eval.explanation_quality for eval in all_evaluations])
            if all_evaluations
            else 0.0
        )
        avg_encouragement = (
            statistics.mean([eval.encouragement_score for eval in all_evaluations])
            if all_evaluations
            else 0.0
        )
        avg_cultural = (
            statistics.mean([eval.cultural_sensitivity for eval in all_evaluations])
            if all_evaluations
            else 0.0
        )
        ...
        self.kpi_scores = {
            ...
            "step_by_step": avg_step,
            "explanation_quality": avg_explanation,
            "encouragement": avg_encouragement,
