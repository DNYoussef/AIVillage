...
class BenchmarkOrchestrator:
    ...
    def benchmark_model(
        self, model_path: str, generation: int, phase: str, child_id: str
    ) -> dict[str, float]:
        ...
        try:
            ...
            wandb.log(
                {
                    "aggregated_fitness": sum(objective_scores.values())
                    / len(objective_scores)
                }
            )

            # Save detailed results
            self._save_detailed_results(
                results, model_path, generation, phase, child_id
            )
    ...
    def _map_objective_to_result_key(
        self, objective: str, results: dict[str, Any]
    ) -> str | None:
    ...
    def _save_detailed_results(
        ...
        aggregate_summary = {
            ...
            "aggregated_fitness": sum(objective_scores.values()) / len(objective_scores)
            if objective_scores
            else 0.0,
        }
...
def determine_model_suite(model_path: str, override_suite: str | None = None) -> str:
    ...
    if models_yaml.exists():
        ...
        for model in models_config.get("models", []):
            if model_name in model.get("local", "") or model_name in model.get(
                "id", ""
            ):
