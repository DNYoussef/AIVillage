...
class EvolutionaryTournament:
        ...
        for i in range(target_count):
            ...
            try:
                ...
                child_config = Configuration(
                    models=[
                        ModelReference(name=f"parent_{j}", path=parent)
                ...
                child_config.merge_settings.dfs_techniques = (
                    techniques[1:] if len(techniques) > 1 else []
                )
                ...
                logger.exception(
                    f"Failed to create child {i + 1} with techniques {techniques}: {e}"
                )
    ...
    def should_early_stop(
        self, fitness_history: list[float], delta_pct: float, window_size: int
    ) -> bool:
        ...
        logger.info(
            f"Improvement over {window_size} generations: {improvement_pct:.3f}%"
        )
        ...
        try:
            ...
            for generation in range(start_generation, max_gens):
                ...
                try:
                    ...
                    phase_pre_children = self.create_generation_children(
                        population, generation, "pre", target_count=8
                    )
                    ...
                    for i, child_path in enumerate(phase_pre_children):
                        ...
                        scores = orchestrator.benchmark_model(
                            child_path, generation + 1, "pre", child_id
                        )
                        pre_scores.append(scores)

                    # SELECT top-3 via NSGA-II
                    selected_children, selected_scores = nsga2_select(
                        phase_pre_children, pre_scores, 3
                    )
                    ...
                    if phase_both:
                        for i, child_path in enumerate(phase_post_children):
                            child_id = f"child_{i + 1:02d}"
                            scores = orchestrator.benchmark_model(
                                child_path, generation + 1, "post", child_id
                            )
                    ...
                    best_aggregated = max(
                        sum(score.values()) / len(score) for score in post_scores
                    )
                    ...
                    clean_up_models(
                        [
                            model
                            for model in phase_pre_children
                            if model not in selected_children
                        ]
                    )
                    ...
                    if (generation + 1) % 5 == 0:
                        self.save_checkpoint(
                            generation + 1, population, post_scores, base_mutation_rate
                        )

                    # Early stopping check with configurable window
                    if self.should_early_stop(
                        aggregated_fitness_history, early_delta_pct, window_gens
                    ):
                        logger.info(
                            f"Early stopping: improvement < {early_delta_pct}% "
                            f"over {window_gens} generations"
                        )
                        break
            ...
            for model in population:
                final_scores.append(
                    orchestrator.benchmark_model(
                        model, generation + 1, "final", "final"
                    )
                )
...
def main() -> None:
    ...
    parser.add_argument(
        "--max_gens", type=int, default=50, help="Maximum generations (default: 50)"
    )
    parser.add_argument(
        "--early_delta_pct",
        type=float,
