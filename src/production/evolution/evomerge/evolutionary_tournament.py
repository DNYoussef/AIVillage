...
from .visualization import plot_evolution_progress, plot_fitness_over_generations, plot_pareto_front
        ...
        try:
            ...
            for generation in range(start_generation, self.config.evolution_settings.num_generations):
                ...
                try:
                    ...
                    selected_population, selected_scores = nsga2_select(population, scores, self.population_size)
                    ...
                    # Clean up resources
                    clean_up_models([model for model in population if model not in new_population])
                    ...
                    if generation > self.config.evolution_settings.early_stopping_generations:
                        if (
                            max(self.fitness_scores[-self.config.evolution_settings.early_stopping_generations :])
                            <= self.fitness_scores[-self.config.evolution_settings.early_stopping_generations - 1]
                        ):
...
def run_evolutionary_tournament(config: Configuration, start_from_checkpoint: str | None = None) -> list[str]:
