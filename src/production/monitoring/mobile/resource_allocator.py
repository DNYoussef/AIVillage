...
class ResourceAllocator:
    ...
    def _select_base_profile(self, profile: DeviceProfile) -> dict[str, Any]:
        ...
        if (
            ...
            and profile.battery_percent <= self.battery_low_threshold
    ...
    async def apply_allocation(self, allocation: ResourceAllocation) -> bool:
        ...
        if handler:
            try:
                ...
                if success:
                    logger.info(
                        f"Applied resource allocation: {allocation.reason} "
                        f"(CPU: {allocation.cpu_limit_percent}%, "
                        f"RAM: {allocation.memory_limit_mb}MB)"
                    )
                return success
            except Exception as e:
                logger.exception(f"Failed to apply resource allocation: {e}")
                return False
        else:
    ...
    async def _apply_linux_limits(self, allocation: ResourceAllocation) -> bool:
        ...
        try:
            ...
            logger.exception(f"Linux resource limiting failed: {e}")
            return False
    ...
    async def _apply_macos_limits(self, allocation: ResourceAllocation) -> bool:
        ...
        try:
            ...
            logger.exception(f"macOS resource limiting failed: {e}")
            return False
    ...
    async def _apply_windows_limits(self, allocation: ResourceAllocation) -> bool:
        ...
        try:
            ...
            logger.exception(f"Windows resource limiting failed: {e}")
            return False
    ...
    async def _apply_android_limits(self, allocation: ResourceAllocation) -> bool:
        ...
        try:
            ...
            logger.exception(f"Android resource limiting failed: {e}")
            return False
    ...
    def predict_resource_needs(
        self, profile_history: list[DeviceProfile], upcoming_tasks: list[dict]
    ) -> ResourceAllocation:
