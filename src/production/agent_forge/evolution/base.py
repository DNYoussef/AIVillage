        ...
        if len(self.short_term) > 10000:  # Threshold for consolidation
            ...
            if LZ4_AVAILABLE:
                ...
                if self.long_term_compressed:
                    try:
                        decompressed_data = lz4.frame.decompress(self.long_term_compressed)
...
class EvolutionMetrics:
    ...
    def record_kpi(self, kpis: dict[str, float]) -> None:
    ...
    def get_trend(self, metric: str, window_hours: int = 24) -> float | None:
        ...
        for prompt_name, prompt_text in current_prompts.items():
            ...
            if effectiveness < 0.6:
                ...
                improved_prompts[prompt_name] = self._improve_prompt(
                    prompt_text, self.evolution_memory.distilled_wisdom
                )
            else:
