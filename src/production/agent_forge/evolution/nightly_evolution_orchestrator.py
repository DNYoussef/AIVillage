...
class NightlyEvolutionOrchestrator:
    ...
    async def evolve_agent(self, agent: EvolvableAgent) -> bool:
        ...
        try:
            ...
            if improvement >= self.success_threshold:
                ...
                logger.info(f"Nightly evolution successful for agent {agent_id} "
                           f"(improvement: {improvement:.1%})")
                return True
            else:
                # Insufficient improvement
                logger.warning(f"Insufficient improvement for agent {agent_id} "
            ...
            logger.error(f"Error during nightly evolution for agent {agent_id}: {e}")
            return False
    ...
    async def _select_evolution_strategy(self, agent: EvolvableAgent) -> Optional[EvolutionStrategy]:
        ...
        try:
            ...
            for param, adjustment in adjustments.items():
                if param in agent.parameters:
                    ...
                    applied_changes[param] = {"old": old_value, "new": new_value, "change": adjustment}
    ...
    async def _apply_learning_rate_adaptation(self, agent: EvolvableAgent, strategy: EvolutionStrategy) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'learning_rate' in agent.parameters:
                ...
                changes['learning_rate'] = {"old": old_lr, "new": new_lr, "factor": learning_rate_factor}
    ...
    async def _apply_confidence_threshold_tuning(self, agent: EvolvableAgent, strategy: EvolutionStrategy) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Confidence threshold tuning failed: {e}")
            return {"success": False, "error": str(e)}
            
    async def _apply_specialization_enhancement(self, agent: EvolvableAgent, strategy: EvolutionStrategy) -> Dict[str, Any]:
        ...
        try:
            ...
            best_tasks = sorted(
                [(task, info['success_rate']) for task, info in task_preferences.items()
            ...
            logger.error(f"Specialization enhancement failed: {e}")
            return {"success": False, "error": str(e)}
            
    async def _apply_context_pattern_learning(self, agent: EvolvableAgent, strategy: EvolutionStrategy) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Context pattern learning failed: {e}")
            return {"success": False, "error": str(e)}
    ...
    async def _analyze_parameter_performance(self, agent: EvolvableAgent) -> Dict[str, Any]:
        ...
        if len(agent.performance_history) > 10:
            recent_performance = [
                r.success for r in agent.performance_history[-50:]
            ]
    ...
    async def _rollback_evolution(self, agent: EvolvableAgent, pre_evolution_state: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Failed to rollback evolution for agent {agent.agent_id}: {e}")
            
    async def _record_evolution_success(self, agent_id: str, strategy: EvolutionStrategy,
                                       pre_kpis: Dict[str, float], post_kpis: Dict[str, float],
                                       improvement: float, evolution_result: Dict[str, Any]):
    ...
    def _update_strategy_effectiveness(self, strategy_name: str, success: bool, improvement: float):
