...
def test_pipeline_phases():
    ...
    try:
        from agent_forge.phases import (
            adas,
            evomerge,
            geometry,
            prompt_baking,
        ...
        for name, module, desc in phases:
            ...
            has_run = (
                hasattr(module, "run")
                or hasattr(module, "execute")
                or hasattr(module, "apply")
            )
            if has_run:
                ...
                classes = [
                    c for c in dir(module) if "Phase" in c or name.replace("-", "") in c
                ]
                if classes:
                    results.append((name, "PASS", f"{desc} - Found: {classes[0]}"))
                else:
                    results.append(
                        (name, "PARTIAL", f"{desc} - Module exists but no runner")
                    )
...
def test_compression_algorithms():
    ...
    try:
        ...
        has_compress = hasattr(compressor, "compress") or hasattr(
            compressor, "quantize"
        )
        has_decompress = hasattr(compressor, "decompress") or hasattr(
            compressor, "dequantize"
        )

        if has_compress and has_decompress:
            results.append(
                ("BitNet", "PASS", "Compressor with compress/decompress methods")
            )
    ...
    try:
        ...
        has_quantize = hasattr(compressor, "quantize") or hasattr(
            compressor, "compress"
        )
        if has_quantize:
            ...
            results.append(
                ("VPTQ", "PARTIAL", "Module exists but no quantization method")
            )
    except ImportError:
...
def test_evolution_system():
    ...
    try:
        ...
        try:
            ...
            has_track = hasattr(tracker, "track_generation") or hasattr(
                tracker, "add_generation"
            )
            has_evolve = hasattr(tracker, "evolve") or hasattr(
                tracker, "next_generation"
            )

            if has_track and has_evolve:
                results.append(
                    ("Evolution Tracker", "PASS", "Generation tracking and evolution")
                )
            elif has_track or has_evolve:
                results.append(
                    ("Evolution Tracker", "PARTIAL", "Partial evolution support")
                )
            else:
        ...
        try:
            ...
            if has_metrics:
                ...
                results.append(
                    ("KPI Monitor", "PARTIAL", "Module exists but no metrics methods")
                )
        except Exception as e:
...
def test_orchestrator():
    ...
    try:
        ...
        has_phases = hasattr(orchestrator, "phases") or hasattr(
...
def test_wandb_integration():
    ...
    try:
        ...
        if hasattr(config, "WANDB_PROJECT") or hasattr(config, "wandb_config"):
            ...
            results.append(
                ("W&B Config", "PARTIAL", "W&B installed but not configured")
            )
...
def main():
    ...
    if success_rate >= 80:
        print(
            f"VERDICT: PASS - Agent Forge claims verified ({success_rate:.1f}% success)"
        )
        verdict = "PASS"
    elif success_rate >= 50:
        print(
            f"VERDICT: PARTIAL - Some Agent Forge features working ({success_rate:.1f}% success)"
        )
        verdict = "PARTIAL"
    else:
        print(
            f"VERDICT: FAIL - Agent Forge claims not substantiated ({success_rate:.1f}% success)"
        )
        verdict = "FAIL"
