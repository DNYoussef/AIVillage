...
def scan_for_security_issues():
    ...
    if http_file.exists():
        ...
        if 'No http://' in content:
            ...
            print(f"[FAIL] Found {results['http_urls']['count']} http:// URLs in production")
...
def main():
    ...
    overall_success = (
        scan_results['http_urls']['status'] == 'PASS' and
        scan_results['pickle_loads']['status'] == 'PASS' and
        infra_results.get('pre_commit', False)
    )

    # Save results
    output_path = Path(__file__).parent.parent / 'artifacts' / 'security_gates_test.json'
    ...
    print("\n" + "=" * 60)
    print(f"Overall Security Gates Test Result: {'PASS' if overall_success else 'FAIL'}")
