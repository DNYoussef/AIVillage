...
class EdgeDeviceRegistry:
    ...
    def get_device_performance_score(self, device_id: str) -> float:
        ...
        usage_penalty = (
            device.status.cpu_usage_percent * 0.01 +
            (device.status.memory_usage_gb / device.spec.memory_gb) * 0.5
        )
...
class EdgeTaskScheduler:
    ...
    def find_suitable_device(self, task: EdgeTask) -> Optional['EdgeDevice']:
        ...
        best_device = max(suitable_devices, 
                         key=lambda d: self.registry.get_device_performance_score(d.device_id))
...
class UnifiedEdgeDeviceSystem:
    ...
    async def _discover_devices(self):
        ...
        self.stats["active_devices"] = sum(1 for d in self.device_registry.devices.values() 
                                          if d.status.is_connected)
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "integration": {
                ...
                "fog_enabled": self.fog_system is not None
            }
        }
