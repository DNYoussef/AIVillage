...
class AdaptiveLoader:
    ...
    def _initialize_preferred_strategies(
        self,
    ) -> dict[DeviceType, list[LoadingStrategy]]:
    ...
    async def _select_optimal_variant(self, model_name: str, context: LoadingContext) -> ModelVariant | None:
        ...
        if not suitable_variants:
            ...
            if context.allow_degraded_quality:
                ...
                for variant in variants:
                    if (
                        variant.memory_requirement_mb <= relaxed_memory
                        and variant.cpu_requirement_percent <= relaxed_cpu
                    ):
                        suitable_variants.append(variant)
    ...
    def _calculate_variant_score(
        self, variant: ModelVariant, context: LoadingContext, snapshot: ResourceSnapshot
    ) -> float:
    ...
    async def _load_model_variant(
        self, model_name: str, variant: ModelVariant, context: LoadingContext
        ...
        try:
            if variant.strategy == LoadingStrategy.FULL:
                ...
                msg = f"Unknown loading strategy: {variant.strategy}"
