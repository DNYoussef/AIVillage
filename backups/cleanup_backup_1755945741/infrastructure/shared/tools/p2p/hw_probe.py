...
class HardwareProbe:
    ...
    async def _probe_bluetooth_linux(self):
        ...
        try:
            ...
            # Check bluetoothctl
            result = await self._run_command_timeout(["bluetoothctl", "--version"], timeout=2)
            if result:
    ...
    async def _probe_bluetooth_windows(self):
        ...
        try:
            ...
            if service_result:
                self.results["bluetooth"]["details"]["service_status"] = service_result.strip()
    ...
    async def _probe_bluetooth_macos(self):
        ...
        try:
            ...
            result = await self._run_command_timeout(["system_profiler", "SPBluetoothDataType"], timeout=5)
    ...
    async def _probe_wifi_windows(self):
        ...
        try:
            ...
            result = await self._run_command_timeout(["netsh", "wlan", "show", "interfaces"], timeout=5)
            ...
            wd_result = await self._run_command_timeout(["netsh", "wlan", "show", "wlanreport"], timeout=3)
    ...
    async def _probe_wifi_macos(self):
        ...
        try:
            ...
            result = await self._run_command_timeout(["networksetup", "-listallhardwareports"], timeout=3)
    ...
    async def _probe_network_interfaces(self):
        ...
        try:
            ...
            if platform.system() == "Windows":
                result = await self._run_command_timeout(["ipconfig", "/all"], timeout=5)
                if result:
                ...
                if result:
                    ...
                    for line in result.split("\n"):
                        if not line.startswith(" ") and ":" in line:
                            ...
                            mac = line.split()[1] if len(line.split()) > 1 else "unknown"
    ...
    async def _run_command_timeout(self, command: list[str], timeout: int = 5) -> str | None:
        ...
        try:
            ...
            logger.debug(f"Command failed: {' '.join(command)} - {stderr.decode()[:100]}")
            return None
    ...
    def _determine_hardware_status(self):
        ...
        if bluetooth_ok:
            ...
            recommendations.append("Bluetooth adapter present but may need driver/software setup")
        else:
            recommendations.append("No Bluetooth adapter detected - consider USB Bluetooth dongle")

        if wifi_ok:
            p2p_transports.append("WiFi")
            if self.results["wifi"].get("p2p_support") or self.results["wifi"].get("wifi_direct_support"):
        ...
        if not hardware_ok:
            if not bluetooth_ok and not wifi_ok:
                ...
                self.results["summary"]["reason"] = "WiFi available but no Bluetooth for BitChat"
            elif not wifi_ok:
                self.results["summary"]["reason"] = "Bluetooth available but no WiFi for Betanet"
...
def print_detailed_report(results: dict[str, Any]):
    ...
    if bluetooth.get("available"):
        ...
        if "discovery_test" in bluetooth:
            ...
            if test.get("discovery_working"):
                ...
                print(f"   ⚠️  Discovery test failed: {test.get('error', 'Unknown error')}")
    else:
        print("   ❌ Not available")
    ...
    if summary["hardware_ok"]:
        ...
        print(f"   📡 Available transports: {', '.join(summary['available_transports'])}")
    else:
...
Examples:
  ...
  python tools/p2p/hw_probe.py || true            # Safe execution (bash)
    ...
    parser.add_argument("--verbose", "-v", action="store_true", help="Show verbose status output")

    parser.add_argument("--detailed", "-d", action="store_true", help="Show detailed hardware report")
