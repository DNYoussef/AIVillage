...
class CodingTaskGenerationTask(Task):
    async def generate_coding_tasks(self, num_tasks: int, avg_difficulty: int) -> list[CodingTask]:
        ...
        for _ in range(num_tasks):
            prompt = (
                f"Create a coding task with difficulty level {avg_difficulty}/100. Include a clear problem description."
            )
...
class SelfModelingTask(Task):
    def __init__(
        ...
        self.hidden_pred = nn.Sequential(nn.Linear(h, h), nn.ReLU(), nn.Linear(h, h)).to(self.device)
        self.beta = 0.1
    ...
    async def self_modeling_cycle(self, curriculum_level: int, num_cycles: int = 100) -> None:
        ...
        temperature_ranges = [(max(0, r[0] - delta), min(1, r[1] + delta)) for r in temperature_ranges]
        ...
        for cycle in tqdm(range(num_cycles), desc=f"Self-modeling cycle (Level {curriculum_level})"):
            for task in tasks:
                for temp_range in temperature_ranges:
                    ...
                    self_modeling_input = self.tokenizer.encode(self_modeling_prompt, return_tensors="pt").to(
                        self.device
                    )
...
if __name__ == "__main__":
    ...
    async def main() -> None:
        config = ChatAgentConfig(name="SelfModelingAgent", llm=OpenAIGPTConfig(chat_model="gpt-3.5-turbo"))
