...
from core.agent_forge.models.cogment.core.refinement_core import RefinementCore, MemoryGate
...
class TestGatedLTMIntegration:
    ...
    def test_memory_statistics(self, memory_system, config):
        ...
        memory_system.write(
            query_states=query_states,
            surprisal=torch.ones(B) * 2.0,  # High surprisal to force write
            force_write=True
        )
        ...
        expected_keys = [
            'total_slots', 'avg_usage', 'max_usage', 'min_usage',
            'unused_slots', 'overused_slots', 'update_count'
        ]
    ...
    def test_surprise_gating(self, memory_system, config):
        ...
        write_info_low = memory_system.write(
            query_states=query_states,
            surprisal=low_surprisal,
            return_gate_info=True
        )
...
if __name__ == "__main__":
    ...
    config = CogmentConfig(
        ltm_capacity=32,  # Small for testing
        ltm_dim=64
    )
