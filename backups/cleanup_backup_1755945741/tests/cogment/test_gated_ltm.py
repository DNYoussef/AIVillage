...
class TestGatedLTM:
    ...
    def test_gated_ltm_surprise_gating(self, gated_ltm, ltm_config):
        ...
        assert (
            avg_high_gate >= avg_low_gate * 0.8
    ...
    def test_gated_ltm_consolidation(self, gated_ltm, ltm_config):
        ...
        for i in range(seq_len):
            gated_ltm.write(keys[:, i : i + 1], similar_values[:, i : i + 1], surprise[:, i : i + 1])
...
class TestMemoryGates:
    ...
    def test_surprise_gate_functionality(self, surprise_gate):
        ...
        assert avg_high >= avg_low * 0.8, f"High surprise should produce higher gates: {avg_high:.3f} vs {avg_low:.3f}"
...
class TestCrossAttention:
    ...
    def cross_attention(self):
        ...
        if not MEMORY_AVAILABLE:
            pytest.skip("Memory components not available")
        return CrossAttention(query_dim=512, key_dim=256, value_dim=256, output_dim=256, n_heads=8)
...
class TestMemoryIntegration:
    ...
    def test_memory_system_integration(self):
        ...
        config = LTMConfig(mem_slots=512, ltm_capacity=256, ltm_dim=128, memory_dim=128, d_model=256)
    ...
    def test_memory_persistence_across_batches(self):
        ...
        config = LTMConfig(mem_slots=256, ltm_capacity=128, ltm_dim=64, memory_dim=64, d_model=128)
        ...
        memory_similarity = F.cosine_similarity(
            memory_after_batch1.flatten(), memory_system.memory_bank.flatten(), dim=0
        )
