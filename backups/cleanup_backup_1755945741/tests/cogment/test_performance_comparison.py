...
class TestParameterEfficiency:
    ...
    def test_cogment_parameter_count(self, cogment_model):
        ...
        assert abs(total_params - target_params) / target_params <= tolerance, \
    ...
    def test_parameter_reduction_factor(self, cogment_model, hrrm_models):
        ...
        assert reduction_factor >= min_reduction, \
...
class TestMemoryEfficiency:
    ...
    def test_cogment_memory_usage(self):
        ...
        assert memory_used <= max_expected, \
            f"Cogment memory usage {memory_used:.1f}MB exceeds target {max_expected}MB"
...
class TestModelSizeComparison:
    ...
    def test_cogment_model_size(self):
        ...
        assert size_mb <= max_expected, \
            f"Cogment model size {size_mb:.1f}MB exceeds target {max_expected}MB"
    ...
    def test_deployment_size_improvement(self):
        ...
        assert size_reduction >= min_reduction, \
            f"Size reduction {size_reduction:.1f}x below minimum {min_reduction}x"
