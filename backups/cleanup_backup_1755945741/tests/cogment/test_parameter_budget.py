...
class TestParameterCounting:
    ...
    def test_total_parameter_count(self, cogment_model, target_config):
        ...
        assert min_acceptable <= total_params <= max_acceptable, \
            f"Parameter count {total_params:,} outside target range [{min_acceptable:,}, {max_acceptable:,}]"
        
        # Should not exceed budget
        assert total_params <= budget_limit, \
            f"Parameter count {total_params:,} exceeds budget {budget_limit:,}"
    ...
    def test_component_parameter_breakdown(self, cogment_model):
        ...
        assert accounting_error < 0.01, \
            f"Parameter accounting error too high: {accounting_error:.2%}"
    ...
    def test_embedding_parameter_efficiency(self, cogment_model, target_config):
        ...
        assert min_expected <= actual_embedding_params <= max_expected, \
    ...
    def test_transformer_core_parameters(self, cogment_model, target_config):
        ...
        assert min_expected <= core_params <= max_expected, \
    ...
    def test_memory_system_parameters(self, cogment_model, target_config):
        ...
        assert min_expected <= memory_params <= max_expected, \
...
class TestBudgetAllocation:
    ...
    def test_budget_allocation_strategy(self, budget_analysis):
        ...
        config = CogmentConfig(
            ...
            n_layers=6,
            vocab_size=13000,
            target_params=budget_analysis['total_budget']
        )
    ...
    def test_efficiency_optimization(self, budget_analysis):
        ...
        configs = {
            'option_a': CogmentConfig(
                d_model=512, n_layers=6, vocab_size=13000, tie_embeddings=True
            ),
            'unoptimized': CogmentConfig(
                d_model=512, n_layers=6, vocab_size=13000, tie_embeddings=False
            )
        }
        ...
        assert option_a_efficiency < unoptimized_efficiency, \
        ...
        assert abs(option_a_efficiency - target_efficiency) <= 0.05, \
...
class TestParameterValidation:
    ...
    def test_configuration_parameter_estimation(self):
        ...
        assert estimation_error <= max_error, \
    ...
    def test_budget_constraint_enforcement(self):
        ...
        assert valid_config.validate_parameter_budget(), \
            "Valid configuration should pass budget validation"
        ...
        assert not invalid_config.validate_parameter_budget(), \
            "Invalid configuration should fail budget validation"
    ...
    def test_real_time_parameter_tracking(self):
        ...
        for i in range(1, len(parameter_tracker)):
            ...
            assert current_count >= previous_count, \
                "Parameter count should be monotonically increasing"
