...
def test_gated_ltm():
    ...
    config = CogmentConfig(
        d_model=320,
        ltm_capacity=64,  # Small for testing
        ltm_dim=128,      # Small for testing
        vocab_size=16000
    )
    ...
    print(f"Parameters: {param_info['total']:,} total ({param_info['trainable']:,} trainable + {param_info['memory_slots']:,} memory slots)")
