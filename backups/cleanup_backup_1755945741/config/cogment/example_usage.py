...
from config_loader import CogmentConfigLoader, CogmentCompleteConfig, StageConfig, TrainingConfig
    ...
    try:
        ...
        for stage_id in [0, 1, 4]:  # Show sanity, ARC, and long-context stages
            ...
            if grokfast.get('enabled', False):
                ...
                if core_settings:
                    print(f"     GrokFast: alpha={core_settings.get('alpha', 'N/A')}, lamb={core_settings.get('lamb', 'N/A')}")
            else:
        ...
        override_args = {
            "model": {
                "model": {
                    "d_model": 480  # Reduce from 512 to 480 (less drastic)
                }
            }
        }
