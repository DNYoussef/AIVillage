...
class CogmentConfigValidator:
    ...
    def validate_complete_config(self, config: CogmentCompleteConfig) -> ValidationResult:
        ...
        if not param_analysis.within_budget:
            if param_analysis.total_estimated > param_analysis.target_budget * 1.05:
                errors.append(f"Parameter budget exceeded: {param_analysis.total_estimated:,} > {param_analysis.target_budget:,}")
        ...
        logger.info(f"Configuration validation completed: valid={is_valid}, "
    ...
    def validate_parameter_budget(self, config: CogmentCompleteConfig) -> ParameterBudgetAnalysis:
        ...
        if not within_budget:
            if total_estimated > rules["max_params"]:
                ...
                memory_ratio = (component_breakdown["memory_storage"] + component_breakdown["memory_gates"]) / total_estimated
                if memory_ratio > 0.6:
                    suggestions.append(f"Reduce mem_slots from {mem_slots} to {int(mem_slots * 0.8)} (largest component)")
    ...
    def _validate_model_config(self, model_config: Dict[str, Any]) -> Tuple[List[str], List[str]]:
        ...
        if not (rules["min_vocab_size"] <= vocab_size <= rules["max_vocab_size"]):
            errors.append(f"vocab_size {vocab_size} outside valid range [{rules['min_vocab_size']}, {rules['max_vocab_size']}]")
        ...
        if not (mem_rules["min_mem_slots"] <= mem_slots <= mem_rules["max_mem_slots"]):
            errors.append(f"mem_slots {mem_slots} outside valid range [{mem_rules['min_mem_slots']}, {mem_rules['max_mem_slots']}]")
        
        if not (mem_rules["min_ltm_dim"] <= ltm_dim <= mem_rules["max_ltm_dim"]):
            errors.append(f"ltm_dim {ltm_dim} outside valid range [{mem_rules['min_ltm_dim']}, {mem_rules['max_ltm_dim']}]")
    ...
    def _validate_stage_config(self, stage_config: StageConfig, stage_id: int) -> Tuple[List[str], List[str], Dict[str, Any]]:
    ...
    def generate_validation_report(self, validation_result: ValidationResult, output_path: Optional[Path] = None) -> str:
        """
