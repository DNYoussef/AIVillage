...
async def demo_multi_model_curriculum():
    ...
    try:
        ...
        if use_mock:
            ...
            llm_client = OpenRouterLLM(api_key=api_key, model_pool=multi_model_pool, rpm_limit=60)
        ...
        async with llm_client:
            ...
            for i, solution in enumerate(test_solutions):
                result = await grader.grade_solution(problem=test_problem, model_answer=solution)

                if result.ok:
                    print(f"   Solution {i + 1}: {'✅ CORRECT' if result.correct else '❌ INCORRECT'}")
            ...
            for i, wrong_solution in enumerate(wrong_solutions):
                result = await hint_gen.generate_hint(problem=test_problem, wrong_answer=wrong_solution)
