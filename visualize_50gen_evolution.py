...
class Evolution50GenVisualizer:
    ...
    def __init__(self, results_file: str = "D:/AgentForge/results_50gen/evolution_50gen_results.json"):
    ...
    def create_comprehensive_visualization(self):
        ...
        plt.tight_layout()
        plt.suptitle('50-Generation Agent Forge Evolution Merge - Comprehensive Analysis', 
    ...
    def plot_evolutionary_progression(self, ax):
        ...
        scatter = ax.scatter(x_positions, y_positions, c=colors, s=sizes, 
        ...
        ax2 = ax.twinx()
        ax2.plot(gen_numbers, best_fitness_by_gen, 'white', linewidth=3, 
                alpha=0.8, label='Best Fitness')
        ...
        ax.set_title('Evolutionary Progression (50 Generations)', 
    ...
    def plot_fitness_evolution(self, ax):
        ...
        ax.plot(gen_numbers, best_fitness, 'o-', color='#6BCF7F', 
               linewidth=2, markersize=4, label='Best Fitness', alpha=0.9)
        ax.plot(gen_numbers, avg_fitness, 's-', color='#FFD93D', 
               linewidth=2, markersize=3, label='Average Fitness', alpha=0.7)
        
        # Add moving average
        if len(best_fitness) >= 10:
            moving_avg = pd.Series(best_fitness).rolling(window=10).mean()
            ax.plot(gen_numbers, moving_avg, '--', color='white', 
                   linewidth=2, alpha=0.6, label='10-Gen Moving Avg')
    ...
    def plot_method_evolution(self, ax):
        ...
        ax.stackplot(generations, *method_data, 
                    labels=methods,
    ...
    def plot_diversity_metrics(self, ax):
        ...
        if 'diversity_evolution' in self.data:
            ...
            ax.plot(generations, diversity_history, 'o-', color='#FF6B6B', 
            ...
            if len(diversity_history) > 5:
                ...
                p = np.poly1d(z)
                ax.plot(generations, p(generations), '--', color='white', 
            ...
            ax.text(0.5, 0.5, 'Diversity data\nnot available', 
    ...
    def plot_best_configuration(self, ax):
        ...
        ax.text(0.05, 0.95, 'Best Configuration', fontsize=12, 
               fontweight='bold', color='#6BCF7F', transform=ax.transAxes)
        
        ax.text(0.05, 0.85, f"Method: {best_config['merge_method']}", 
               fontsize=10, color='white', transform=ax.transAxes)
        
        ax.text(0.05, 0.75, f"Fitness: {best_config['fitness']:.4f}", 
               fontsize=10, color='white', transform=ax.transAxes)
        
        ax.text(0.05, 0.65, f"Generation: {best_config.get('generation', 'N/A')}", 
               fontsize=10, color='white', transform=ax.transAxes)
        ...
        if params:
            ...
            ax.text(0.05, 0.45, f"Parameters:\n{param_text}", 
                   fontsize=9, color='white', transform=ax.transAxes)
        
        # Benchmark scores
        benchmarks = best_config.get('benchmark_results', {})
        if benchmarks:
            ax.text(0.05, 0.25, "Benchmarks:", 
                   fontsize=10, fontweight='bold', color='#FFD93D', transform=ax.transAxes)
            
            y_pos = 0.15
            for metric, score in benchmarks.items():
                ax.text(0.1, y_pos, f"{metric.upper()}: {score:.3f}", 
                       fontsize=9, color='white', transform=ax.transAxes)
                y_pos -= 0.06
    ...
    def plot_benchmark_evolution(self, ax):
        ...
        for i, (metric, scores) in enumerate(benchmark_evolution.items()):
            ...
            ax.plot(gen_numbers, scores, 'o-', color=color, 
                   linewidth=2, markersize=3, label=metric.upper(), alpha=0.8)
    ...
    def plot_method_performance(self, ax):
        ...
        bp = ax.boxplot(data, labels=[m.replace('_', '\n') for m in methods], 
    ...
    def plot_generation_statistics(self, ax):
        ...
        bars = ax.bar(gen_numbers[::5], fitness_std[::5], 
                     color='#FF6B6B', alpha=0.7, width=2)
    ...
    def plot_parameter_evolution(self, ax):
        ...
        for gen_data in generations:
            ...
            ta_individuals = [ind for ind in gen_data['population'] 
                            if ind['merge_method'] == 'task_arithmetic']
            
            if ta_individuals:
                # Average scaling coefficient
                coeffs = [ind.get('parameters', {}).get('scaling_coefficient', 1.0) 
        ...
        if valid_coeffs:
            ax.plot(valid_gens, valid_coeffs, 'o-', color='#45B7D1', 
                   linewidth=2, markersize=4, alpha=0.8)
            
            ax.set_xlabel('Generation', color='white', fontweight='bold')
            ax.set_ylabel('Avg Scaling Coefficient', color='white', fontweight='bold')
            ax.set_title('Parameter Evolution\n(Task Arithmetic)', color='white', fontweight='bold')
            ax.grid(True, alpha=0.3)
        else:
            ax.text(0.5, 0.5, 'No task_arithmetic\nindividuals found', 
                   ha='center', va='center', transform=ax.transAxes,
