...
def test_contextual_tagging_features():
    ...
    for i, paragraph in enumerate(paragraphs[:4]):  # Process first 4 paragraphs
        ...
        if contextual_chunk['context_inheritance']['inherited_context']:
            print(f"  Inherited Context: {len(contextual_chunk['context_inheritance']['inherited_context'])} properties")
    ...
    print(f"[SUCCESS] Level 1 Context: Document-level metadata extracted")
    ...
    print(f"   - Content Analysis: {len(document_context.key_themes)} themes, {len(document_context.key_concepts)} concepts")
    
    print(f"\n[SUCCESS] Level 2 Context: Chunk-level metadata with inheritance")
    print(f"   - Processed Chunks: {len(contextual_chunks)}")
    print(f"   - Context Inheritance: Document -> Chunk properties preserved")
    print(f"   - Quality Metrics: Coherence, relevance, completeness scored")
    
    print(f"\n[SUCCESS] Context Chain Preservation:")
    print(f"   - Bilateral Context: Document <-> Chunk relationships maintained")
    print(f"   - Inheritance Chain: Previous chunk -> Current chunk continuity")
    print(f"   - Rich Metadata: {len(sample_chunk)} top-level properties per chunk")
    
    print(f"\n[SUCCESS] Implementation Features:")
    print(f"   - Two-level hierarchy: Global + Local context")
    print(f"   - Rhetorical structure preservation")
    print(f"   - Entity extraction framework (ready for spaCy)")
    print(f"   - Quality scoring and contextual filtering")
