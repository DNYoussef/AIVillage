...
class NetworkConditions:
    ...
    def is_low_resource_environment(self) -> bool:
        ...
        return (
            (self.battery_percent is not None and self.battery_percent < 30) or
            self.data_cost_usd_mb > 0.01 or  # >$0.01/MB is expensive
            not self.internet_available or
...
class PeerInfo:
    ...
    def is_nearby(self) -> bool:
        ...
        return (
            self.hop_distance <= 3 and
            (self.bluetooth_rssi is None or self.bluetooth_rssi > -60) and
...
class NavigatorAgent:
    ...
    async def select_path(self, 
                         ...
                         available_protocols: Optional[List[str]] = None) -> Tuple[PathProtocol, Dict[str, Any]]:
        """
        ...
        selected_protocol = await self._evaluate_routing_options(destination, message_context, available_protocols)
        ...
        logger.info(f"Selected route for {destination}: {selected_protocol.value} "
                   f"(size={message_context.size_bytes}, priority={message_context.priority})")
        
        return selected_protocol, routing_metadata
    
    async def _evaluate_routing_options(self,
                                       destination: str, 
                                       context: MessageContext,
                                       available_protocols: Optional[List[str]]) -> PathProtocol:
        ...
        if self.routing_priority == RoutingPriority.OFFLINE_FIRST or self.global_south_mode:
            ...
            if (self.conditions.bluetooth_available and 
                'bitchat' in available and
                await self._is_peer_nearby(destination)):
                
                # BitChat preferred for local/offline scenarios
                if (self.energy_mode == EnergyMode.POWERSAVE or
                    self.conditions.is_low_resource_environment()):
        ...
        if context.is_large_message():
            ...
            if (self.conditions.internet_available and 
                'betanet' in available and
                self.conditions.bandwidth_mbps > 5.0 and
                not self._is_data_expensive()):
        ...
        if context.privacy_required or self.conditions.is_privacy_sensitive():
            if (self.routing_priority == RoutingPriority.PRIVACY_FIRST and
                'betanet' in available and
        ...
        if self.routing_priority == RoutingPriority.BANDWIDTH_FIRST:
            if (self.conditions.internet_available and 
                'betanet' in available and
                self.conditions.bandwidth_mbps > 10.0):
    ...
    async def _update_network_conditions(self) -> None:
        ...
        logger.debug(f"Network conditions updated: BT={self.conditions.bluetooth_available}, "
    ...
    async def optimize_route_for_context(self,
    ...
    def cleanup_cache(self) -> None:
        ...
        expired_decisions = [
            key for key, (_, timestamp) in self.routing_decisions.items()
        ...
        expired_proximity = [
            key for key in self.peer_proximity_cache.keys()
        ...
        logger.debug(f"Cache cleanup: removed {len(expired_decisions)} decisions, "
