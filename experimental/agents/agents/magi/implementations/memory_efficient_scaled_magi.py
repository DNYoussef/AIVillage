...
class MemoryEfficientScaledMagi:
    ...
    async def execute_curriculum_training(self, curriculum, evolved_config):
        ...
        logger.info(
            "Improvement over baseline: %.3f", final_specialization_score - 0.774
        )
...
async def main():
    ...
    try:
        ...
        if results:
            ...
            logger.info("=" * 80)
            logger.info(
                "Duration: %.1f minutes (%.1f seconds)", duration / 60, duration
            )
            logger.info(
                "Questions Processed: %s", f"{results['questions_processed']:,}"
            )
            logger.info(
