...
async def example_basic_training():
    ...
    session_info = await service.start_training_session(
        task_id=task_id,
        training_parameters={"example_mode": True},
        model_names=["example_model_1", "example_model_2"]
    )
...
async def example_progress_tracking():
    ...
    await service.start_training_session(
        task_id=task_id,
        training_parameters={"detailed_progress": True},
        model_names=["progress_demo_model"]
    )
