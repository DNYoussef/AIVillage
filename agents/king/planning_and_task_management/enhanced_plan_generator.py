...
from agents.utils.exceptions import AIVillageException, PlanningException, ModelInteractionException
...
class EnhancedPlanGenerator:
    def __init__(self, king_agent: KingAgent = None, config: PlanConfig = PlanConfig(), model_name='gpt2'):
    ...
    async def _expand_node(self, node: Node, current_resources: Dict[str, Any]):
        ...
        prerequisites = await self.king_agent.generate_structured_response(prompt)
    ...
    async def _extract_node_tasks(self, node: Node):
        ...
        """
        node.tasks = await self.king_agent.generate_structured_response(prompt)
    ...
    async def _optimize_tasks(self, root_node: Node):
        ...
        optimized_tasks = await self.king_agent.generate_structured_response(optimization_prompt)
    ...
    async def _conduct_premortem(self, node: Node):
        ...
        """
        node.failure_modes = await self.king_agent.generate_structured_response(prompt)
    ...
    async def _assess_node_antifragility(self, node: Node):
        ...
        """
        result = await self.king_agent.generate_structured_response(prompt)
    ...
    async def _update_plan(self, node: Node) -> Node:
        ...
        """
        updated_node_data = await self.king_agent.generate_structured_response(update_prompt)
    ...
    async def incorporate_feedback(self, plan_file: str, feedback: Dict[str, Any]) -> Dict[str, Any]:
        ...
        updates = await self.king_agent.generate_structured_response(feedback_prompt)
    ...
    async def enhance_plan(self, initial_plan: Dict[str, Any], rag_info: Dict[str, Any]) -> Dict[str, Any]:
        context = f"Initial Plan: {json.dumps(initial_plan, indent=2)}\nRAG Info: {json.dumps(rag_info, indent=2)}"
        ...
        for line in enhanced_lines:
            if ':' in line:
                ...
                enhanced_plan[key.strip()] = value.strip()
        
        # Merge the enhanced plan with the original plan
        merged_plan = {**initial_plan, **enhanced_plan}
        
        return merged_plan

    async def update(self, task: Dict, result: Dict):
        # Update both the reverse tree planner and the language model
        await self._update_reverse_tree_planner(task, result)
        await self._fine_tune_language_model(task, result)

    async def _update_reverse_tree_planner(self, task: Dict, result: Dict):
        # Implement logic to update the reverse tree planner based on task execution results
        pass

    async def _fine_tune_language_model(self, task: Dict, result: Dict):
        # Implement logic to fine-tune the language model based on task execution results
        pass

    def save(self, path: str):
        self.model.save_pretrained(f"{path}/enhanced_plan_generator_model")
        self.tokenizer.save_pretrained(f"{path}/enhanced_plan_generator_tokenizer")

    def load(self, path: str):
        self.model = GPT2LMHeadModel.from_pretrained(f"{path}/enhanced_plan_generator_model")
        self.tokenizer = GPT2Tokenizer.from_pretrained(f"{path}/enhanced_plan_generator_tokenizer")
        self.model.to(self.device)
